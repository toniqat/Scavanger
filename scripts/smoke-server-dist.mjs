...
ok(!/^\s*await\s/m.test(code), 'top-level await 이 없다');
...
const proc = spawn(process.execPath, [bundle, `--port=${PORT}`, `--data=${dataDir}`], { stdio: ['ignore', 'pipe', 'pipe'] });
let out = '';
