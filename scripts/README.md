...
| `perf-measure.mjs` | **The performance harness** (`docs/PERF.md`, perf Phase 0 … last measurement) — plays S1-S4 in a **headful** Chrome (a headless window does not present frames, so its cadence means nothing) and prints frame time, per-system script time, draw calls and heap. Wraps every system's `update` / `lateUpdate`, the render block, the light budget, `AudioSystem.play` and the spawn paths **on the live page** — no `src/` change. `--only s1,s3a` · `--seconds` · `--label` · `--spike` (autopsy threshold) · `--headless` · `--display bloom=0,shadows=0,scale=0.75` (applies `설정 › 화면 설정` before recording — how a run splits the render block into scene · shadow pass · composer, which is what turned the 2026-09-20 ranking over; `scale` answers the **pixel** question and answered it **free** — −44 % of the pixels, two runs per side, no change in the render block). The report also carries a **draw-call census** — visible drawables and shadow casters per top-level scene group, i.e. which bodies own the render block. `--alloc` adds **allocation by owner** (finding B6) in three counters, because no single one answers it: the V8 sampling heap profiler over CDP (on-heap, but it keeps only the samples still alive when the profile is taken, so it sees almost none of the churn), a construct trap on the typed-array / `ArrayBuffer` / `AudioContext.createBuffer` paths (off-heap stores, which the profiler undercounts by design), and a `performance.memory` read around **each wrapped mark** (an upper bound, but the only one of the three that sees churn — it is what attributes the raid's 63 MB/s to a system). An `--alloc` run's `ms` columns are not comparable with a plain one. The report also prints an **AI LOD census** (how the enemy list splits into the full-rate · half · quarter bands; its two distances mirror `ENEMY_AI_LOD_*` by hand). **`--only s5a` · `s5b` · `s5` · `s5c`** (Phase D) plays the same raid with **two humans through the relay** (`s5c` recruits the cockpit bays into the squad first, so `ally state` is on the wire; it cannot share a run with the others, since androids would then be in their windows too): the host is the usual headful page, the second client a second browser (headless 960x540 on the same GPU; `--peer-headful` to watch it), and both are recorded over one window, so the results table gains a **`Replica p95 ms`** column. It needs a relay — run `npm run dev:all`, not `npm run dev` — and it is the one run that does not park the page's relay socket, so do not mix it with solo scenarios in the same invocation. The squad is made by code (`createLobby` + `joinLobby(code)` + `setReady` on both, then `startGame`): the pod is only the hub's way of setting `ready`, and a join by code cannot be hijacked by a stale public lobby the way quick match can. S5 adds three counters to the report: the **wire census** (every message that crossed the socket, by `t` + `ev`, with bytes/s, size per message and the single worst inbound frame — this is how A6's `ee spawn` burst and C3's `ally state` are judged), the **`o:wireIn`** mark (time inside `onmessage`, the one mark that is **not** inside `Engine.frame` — it is a task of its own, so it never shows in `js/frame`, only as a late rAF), and the **`SoldierPool`** counter (acquired / built / released, counted for the page's whole life as well as the window, because a remote body is built at raid entry — outside every window). Writes `scripts/logs/perf/<label>.json`; re-run it with the same options after each phase for the before/after delta — **twice per side**: on 2026-09-20 two runs of the *same* build differed by 1.6× in spike count, so a single run proves nothing about `ms` (draw calls, scene nodes and triangles do repeat). Asserts nothing, so `verify` never runs it |
...
- **`check-css-prefixes` enforces one prefix per folder**; on a collision the smaller set moves.
- **`check-comment-labels` is advisory** (exit 0, not in `verify`). Rejected: gating `verify` on it.
- **`smoke-layout-reads` guards the no-layout-in-frame rule.** Rejected: a note in this file.
- **`noUnusedLocals` over a smoke** — a build error beats a check at the end of a session.
