...
try {
  ...
  /* ══ 2. 연산 클러스터 화면 ════════════════════════════════════════════════ */
  console.log('연산 클러스터 화면');
  await H((u) => window.__game.ctx.housing.openComputeCluster(u), C1);
  await waitFor(page, () => document.querySelector('.menu.mining-cluster') && !document.querySelector('.menu.mining-cluster').hidden, 'cluster screen', 5000);
  await sleep(60);
  const dom = await H(() => {
    const r = document.querySelector('.menu.mining-cluster');
    return {
      cores: r.querySelectorAll('.mn-core[data-core]').length,
      rail: r.querySelectorAll('.hs-rail-item').length,
      coins: r.querySelectorAll('.mn-coin[data-coin]').length,
      locked: r.querySelectorAll('.mn-coin.is-locked').length,
  ...
  ok(dom.cores === 9 && dom.rail === 2 && dom.coins === 8, `코어 칸 9 · 레일 2 · 코인 8 (${JSON.stringify(dom)})`);
  ok(dom.locked === dom.expectLocked && dom.locked > 0, `잠긴 코인 딤드 ${dom.locked} (housing ${dom.expectLocked})`);
  ok(dom.grids.includes('stash') && dom.grids.includes('bag'), '함선 창고 · 가방 격자 카드');
  ...
  const dropVia = (defId, sel) => H(({ defId, sel }) => {
    const ctx = window.__game.ctx;
    const p = ctx.housing.clusterScreen;
  ...
  if (!hasCoreApi) {
    ...
    const d1 = await dropVia('mat_compute_core', '.mining-cluster .mn-core[data-core="4"]');
    await sleep(60);
    ok(d1.ok && (await coresOf(C1)) === 3 && (await countAll('mat_compute_core')) === 0, `스택 3 드롭 → 코어 3 (창고 비움) ${JSON.stringify(d1)}`);
    await giveStash('mat_compute_core', 9);
    await dropVia('mat_compute_core', '.mining-cluster .mn-core[data-core="0"]');
    await sleep(60);
    ok((await coresOf(C1)) === 9 && (await countAll('mat_compute_core')) === 3, `스택 9 드롭 → 빈 칸 6 만큼 (코어 9 · 남은 3)`);
    const lit = await H(() => ({ on: document.querySelectorAll('.mining-cluster .mn-core.is-on').length, count: document.querySelector('.mining-cluster .mn-corebox .mn-sec-count')?.textContent }));
    ...
    await dropVia('mat_scrap', '.mining-cluster .mn-core[data-core="0"]');
    await sleep(30);
    ...
    await H(() => document.querySelector('.mining-cluster .mn-core.is-on')?.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true, clientX: 10, clientY: 10 })));
    await sleep(60);
    ok((await coresOf(C1)) === 8 && (await bagQty('mat_compute_core')) === bag0 + 1, '우클릭 → 코어 1개 빼기 (가방 먼저)');
    await H(() => document.querySelector('.mining-cluster .mn-core.is-on')?.dispatchEvent(new MouseEvent('dblclick', { bubbles: true, cancelable: true })));
    await sleep(60);
  ...
  if (open.length >= 2) {
    await H((id) => document.querySelector(`.mining-cluster .mn-coin[data-coin="${id}"]`)?.click(), open[0].id);
    await sleep(60);
    ok((await coinOf(C1)) === open[0].id, `코인 클릭 → setClusterCoin (${open[0].id})`);
    const stat = await H(() => [...document.querySelectorAll('.mining-cluster .mn-stat')].filter((r) => !r.hidden).map((r) => `${r.querySelector('.k').textContent}=${r.querySelector('.v').textContent}`));
    ...
    await H((id) => document.querySelector(`.mining-cluster .mn-coin[data-coin="${id}"]`)?.click(), open[1].id);
    await sleep(80);
  ...
  if (locked.length) {
    await H((id) => document.querySelector(`.mining-cluster .mn-coin[data-coin="${id}"]`)?.click(), locked[0].id);
    await sleep(60);
    ok((await coinOf(C1)) !== locked[0].id && (await lastNotify()) === (locked[0].reason ?? '잠긴 코인입니다'), `잠긴 코인 클릭 → 거절 (${await lastNotify()})`);
  }
  await shot('02-cluster');
  // 레일 전환
  await H((u) => document.querySelector(`.mining-cluster .hs-rail-item[data-uid="${u}"]`)?.click(), C2);
  await sleep(60);
  ok(await H((u) => window.__game.ctx.housing.clusterScreen.currentUid === u && document.querySelector('.mining-cluster .hs-station-head .title')?.textContent === '연산 클러스터 2', C2), '레일 → 클러스터 2');
  // Tab 닫기
  await tap('Tab');
  await sleep(80);
  ok(await H(() => document.querySelector('.menu.mining-cluster').hidden), 'Tab → 클러스터 화면 닫힘');
  ...
  const viaInteract = await H((u) => {
    ...
    i.interact();
    return !document.querySelector('.menu.mining-cluster').hidden && window.__game.ctx.housing.clusterScreen.currentUid === u ? 'ok' : 'not opened';
  }, C1);
  ok(viaInteract === 'ok', `연산 클러스터 interact() → 클러스터 화면 (${viaInteract})`);
  await tap('Escape');
  await waitFor(page, () => document.querySelector('.menu.mining-cluster').hidden, 'Esc closes cluster', 5000).catch(() => null);
  ok(await H(() => document.querySelector('.menu.mining-cluster').hidden), 'Esc → 클러스터 화면 닫힘');
  ...
  await waitFor(page, () => document.querySelector('.menu.mining-computer') && !document.querySelector('.menu.mining-computer').hidden, 'computer screen', 5000);
  await sleep(80);
  const pc = await H(() => {
    const r = document.querySelector('.menu.mining-computer');
    return {
  ...
  // 줄 클릭 → 클러스터 화면
  await H((u) => document.querySelector(`.mining-computer .mn-crow[data-uid="${u}"]`)?.click(), C2);
  await sleep(80);
  ok(await H((u) => document.querySelector('.menu.mining-computer').hidden && !document.querySelector('.menu.mining-cluster').hidden && window.__game.ctx.housing.clusterScreen.currentUid === u, C2), '현황 줄 클릭 → 그 클러스터 화면 (컴퓨터 닫힘)');
  await tap('Tab');
  await sleep(60);
  ...
  const wallet = await H(() => {
    ...
    return { rows: r.querySelectorAll('.mn-wrow[data-coin]').length, units: row?.querySelectorAll('.mn-num')[0]?.textContent, value: row?.querySelectorAll('.mn-num')[1]?.textContent, watch: window.__cryptoStub.watch, active: r.querySelector('.hs-tab.is-active')?.dataset.tab };
  });
  ...
  // 거래소
  await H(() => document.querySelector('.mining-computer .hs-tab[data-tab="exchange"]')?.click());
  ...
  const x = await H(() => {
    ...
    const cv = document.querySelector('.mining-computer .mn-chart-canvas');
    let painted = 0;
    try {
    ...
    return { coin: m.selectedCoin, watch: window.__cryptoStub.watch, unwatch: window.__cryptoStub.unwatch, req: window.__cryptoStub.req.slice(), debug: { ...m.chart.debug }, painted,
      list: document.querySelectorAll('.mining-computer .mn-xrow[data-coin]').length, msg: !document.querySelector('.mining-computer .mn-chart-msg').hidden };
  });
  ...
  const hover = await H(() => {
    ...
    const tip = document.querySelector('.mining-computer .mn-chart-tip');
  ...
  await H(() => document.querySelector('.mining-computer .mn-chart-canvas')?.dispatchEvent(new PointerEvent('pointerleave', { bubbles: false })));
  await H(() => document.querySelector('.mining-computer .mn-seg-btn[data-range="1w"]')?.click());
  await sleep(80);
  ok(await H((c) => window.__cryptoStub.req.includes(`${c}:1w`) && document.querySelector('.mining-computer .mn-seg-btn[data-range="1w"]').classList.contains('is-active'), x.coin), '기간 1주 → 이력 요청 · 활성 표시');
  const draws0 = await H(() => window.__game.ctx.housing.miningComputer.chart.debug.draws);
  await H(() => document.querySelector('.mining-computer .mn-seg-btn[data-mode="line"]')?.click());
  ...
  await freshPrices();
  const trade = () => H(() => {
    const r = document.querySelector('.mining-computer');
  ...
  const setAmount = (v) => H((v) => { const i = document.querySelector('.mining-computer .mn-amt-input'); i.value = v; i.dispatchEvent(new Event('input', { bubbles: true })); }, v);
  await setAmount('');
  let t = await trade();
  ...
  await H(() => { const b = document.querySelector('.mining-computer .mn-hold'); b.click(); b.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, pointerId: 7 })); b.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, button: 0, pointerId: 7 })); });
  await sleep(1300);
  ok(await H(() => window.__trades.length === 0), '클릭 · 짧게 누르기로는 거래하지 않는다');
  await freshPrices();
  const subT0 = await H(() => ({ ...window.__cryptoStub }));
  await H(() => document.querySelector('.mining-computer .mn-hold').dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, pointerId: 8 })));
  await sleep(1200);
  const mid = await H(() => ({ trades: window.__trades.slice(), s: { ...window.__cryptoStub }, label: document.querySelector('.mining-computer .mn-hold-label').textContent }));
  ...
  await H(() => document.querySelector('.mining-computer .hs-tab[data-tab="clusters"]')?.click());   // 답이 오기 전에 탭을 바꾼다
  await sleep(40);
  ...
  await freshPrices();
  await H(() => document.querySelector('.mining-computer .mn-seg-btn[data-side="sell"]')?.click());
  await setAmount('0.2');
  t = await trade();
  ...
  if (locked.length) {
    await H((id) => document.querySelector(`.mining-computer .mn-xrow[data-coin="${id}"]`)?.click(), locked[0].id);
    await sleep(120);
    t = await trade();
    const lockedChart = await H((id) => ({ req: window.__cryptoStub.req.includes(`${id}:1w`), msg: !document.querySelector('.mining-computer .mn-chart-msg').hidden }), locked[0].id);
  ...
  await H(() => document.querySelector('.mining-computer .hs-tab[data-tab="clusters"]')?.click());
  await sleep(40);
  ...
  await H((id) => document.querySelector(`.mining-computer .mn-xrow[data-coin="${id}"]`)?.click(), x.coin);
  await sleep(60);
  await H(() => { const ctx = window.__game.ctx; ctx.net.crypto.available = false; ctx.bus.emit('net:cryptoPrices', { at: Date.now() }); });
  await sleep(80);
  const off = await H(() => ({ msg: document.querySelector('.mining-computer .mn-chart-msg'), t: document.querySelector('.mining-computer .mn-trade-block').textContent }));
  const offMsg = await H(() => { const m = document.querySelector('.mining-computer .mn-chart-msg'); return { shown: !m.hidden, text: m.textContent }; });
  ok(offMsg.shown && offMsg.text === '서버에 연결되어야 합니다', `오프라인 → 차트 자리 문구 (${offMsg.text})`);
  ok(off.t === '서버에 연결되어야 합니다', `오프라인 → 매매 사유 (${off.t})`);
  await H(() => document.querySelector('.mining-computer .hs-tab[data-tab="clusters"]')?.click());
  await sleep(40);
  ok(await H(() => document.querySelectorAll('.mining-computer .mn-crow[data-uid]').length === 2), '오프라인이어도 현황 탭은 그대로');
  // Tab 닫기 → 구독 해제 · 이벤트
  await H(() => { window.__game.ctx.net.crypto.available = true; document.querySelector('.mining-computer .hs-tab[data-tab="exchange"]')?.click(); });
  await sleep(40);
  await tap('Tab');
  await sleep(80);
  const closed = await H(() => ({ hidden: document.querySelector('.menu.mining-computer').hidden, s: { ...window.__cryptoStub }, ev: window.__toggles.some((t) => !t.open && t.page === 'computer') }));
  ...
  const pcInteract = await H((u) => {
    ...
    if (!i) return 'no interactable';
    i.interact();
    return !document.querySelector('.menu.mining-computer').hidden ? 'ok' : 'not opened';
  }, PC);
  ok(pcInteract === 'ok', `메인 컴퓨터 interact() → 화면 (${pcInteract})`);
  await tap('Escape');
  await waitFor(page, () => document.querySelector('.menu.mining-computer').hidden, 'Esc closes computer', 5000).catch(() => null);
  ok(await H(() => document.querySelector('.menu.mining-computer').hidden), 'Esc → 메인 컴퓨터 닫힘');
