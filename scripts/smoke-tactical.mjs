...
try {
  ...
  const cat = await page.evaluate(() => {
    ...
    const packs = defs.filter((d) => d.category === 'backpack').map((d) => loot.getBackpackDef(d.backpackId)).filter(Boolean);
    return {
      byCat,
      armorCount: armor.length,
      armorPerks: [...new Set(armor.map((a) => a.perk))].sort(),
      packCount: packs.length,
      packPerks: [...new Set(packs.map((b) => b.perk))].sort(),
  ...
  ok(cat.packCount >= 8, `backpack defs: ${cat.packCount}`);
  ok(JSON.stringify(cat.packPerks) === '["jump","none","special","tactical"]', `backpack perks ${JSON.stringify(cat.packPerks)}`);
  ok(cat.quickSlotMax === 8, `tactical backpack grants 8 quick slots`);
  ...
  const armor = await page.evaluate(() => {
    ...
    inv.tryAddItem(item);
    return { done: inv.equip('armor', item), dr: ctx.player.damageReduction };
  });
  ...
  for (let i = 0; i < placeIds.length; i++) {
    ...
    await gameSleep(page, 1.2);
    placed[placeIds[i]] = await page.evaluate((g) => window.__game.ctx.gadgets.use(g, false), placeIds[i]);
    await gameSleep(page, 1.2);
  }
