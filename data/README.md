...
### 2026-09-15 — 폭발 감쇠 2단 계단 · 수류탄 수치 이관 (`constants.csv` 5줄 신규)

**사용자 결정 둘.** ① 고폭 수류탄(G-12)의 폭발 반경을 **×1.2** 로 넓힌다. ② **모든 폭발물이 같은 감쇠 공식**을 쓴다 —
중심에서 `0.5 × 반경` 안쪽은 피해 100 %, 거기서 반경까지는 **거리와 무관한 60 %**, 반경 밖은 0. 예전에는 어디서나
`damage × (1 − d / radius)` 선형이라 반경을 넓혀도 한가운데 말고는 거의 안 아팠다 (반경 7.2 m 수류탄의 6 m 지점이
옛 식으로는 250 × 0.167 = **42**, 새 식으로는 **150**).

| key | value | 누가 읽나 |
|---|---|---|
| `GRENADE_RADIUS` | **7.2** (6 → ×1.2) | weapons `Grenade` (2026-09-15 까지 코드에 박혀 있었다) |
| `GRENADE_DAMAGE` | 250 | 같음 |
| `GRENADE_PLAYER_DAMAGE_MUL` | 0.6 | 같음 — 수류탄이 **플레이어**에게 주는 몫 (적 · 드론에는 안 곱한다) |
| `EXPLOSION_FULL_FRACTION` | 0.5 | `src/shared/explosion.ts` — 이 비율(× 반경) 안쪽은 100 % |
| `EXPLOSION_OUTER_MUL` | 0.6 | 같음 — 그 바깥 띠의 **고정** 배수 |

- 수식은 `src/shared/explosion.ts` **하나**다 (`explosionFalloff` · `explosionDamage`). weapons(수류탄 · 바주카 엄폐물) ·
  enemies(적 피해 · 적 수류탄 · 탐사 차량) · gadgets(지뢰 · C4 · 드론) · stratagems(함선 호출 착탄 · 구조물)가 같이 부른다.
- 자리마다 다른 **하한 클램프**는 없애지 않고 그 위에 얹었다: 엄폐물 0.3 · 적 0.15 · 탐사 차량 0.15 · 적 수류탄 0.1.
  그래서 `EXPLOSION_OUTER_MUL` 을 0.15 아래로 내리면 적 쪽에서는 하한이 이겨 값이 먹지 않는다.
- **안 바뀐 것**: 카메라 흔들림 · 소리 거리 곡선 · 넉백 감쇠, 그리고 **스스로 다른 곡선을 적어 둔** 적 공격 —
