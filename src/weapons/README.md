...
| `fx/ThrowArc.ts` | **투척 궤적 미리보기** (2026-09-08): a dotted arc from the hand along the throw, shown by `WeaponSystem.updateThrowArc` whenever a grenade or a `use: 'throw'` gadget is in the hand. `show(origin, velocity)` — 릴리스 지점 · 속도는 던지기가 쓸 바로 그 값이라 근력(`derived.throwRangeMul`) · 오버/언더핸드 · 플레이어 자신의 관성이 그대로 반영된다. `STEP` 1/30 × `MAX_STEPS` 75 (2.5 s of flight), one dot per step. Pooled and light-free: one `Points` cloud over a fixed buffer picked by `setDrawRange` and one `RingGeometry` marker, both `depthTest: false`. `hide()` / `isShowing` / `getImpact(out)`. **2026-09-11 — 끝까지 그리지 않는다** (사용자 요청): 끝까지 적분한 뒤 수평 비거리의 `THROW_ARC_PREVIEW_FRACTION`(0.5)까지만 점을 찍고 **착지 고리는 감춘다**. 근력 · 임플란트로 사거리가 늘면 보이는 궤적도 그만큼 길다. **2026-09-14 2차 — 튕김을 계산하지 않는다** (사용자 결정): `world.resolveCollision` · `getSurfaceY` · `getNormalAt` 을 전부 뺐고 이제 **릴리스 지점 · 속도 · 중력뿐인 순수 포물선**이다. 비행의 끝도 지형이 아니라 **던진 높이로 되돌아오는 점**(내려 던지면 최소 `MIN_STEPS` 12 = 0.4 s). 그래서 궤적이 벽을 통과해 보이는 것은 **의도**이고(「여기 맞는다」가 아니라 「이쪽으로 나간다」), `getImpact` 는 착지점이 아니라 그 포물선이 끝나는 자리다. 실제 수류탄 · 투척 가젯의 비행 · 튕김 · 신관(`Grenade.ts` / `ThrownGadget`)은 무변경. |
