...
| `WeaponModel.ts` | Procedural rifle / pistol / shotgun / energy-rifle / **SMG** (+ **Phase 6** six unique silhouettes: 「인페르노」 fuel tank under the fore-end + hose + nozzle bell with an emissive pilot ring, 「테슬라 코일」 copper coil rings around a glowing core rod + capacitor pack, 「카게」 forearm bracer + launcher rail + a holder stacked with stars, 「롱혼」 **held horizontally since 2026-09-15** — limbs spread along ±X in one plane `BOW_Y` above the grip (riser cheeks around a centre arrow window + shelf + grip into the hand in front, limbs sweep back and recurve at the tips), the string runs nock to nock **behind** the riser on the archer's side (+Z, two halves that bend into a V) and the nocked arrow's tail sits on it with the head ahead of the riser, `muzzle` at the arrow tip; `poseBow(f)` pulls string centre + arrow toward the body and flexes the limbs for `setBowDraw(0..1)` or the recoil kick, whichever is larger; `setBowNocked(false)` = out of arrows (empty string), `bowLoose(s)` = empty for one cooldown after a release; the bow never plays the mag-drop reload animation, 「해머헤드」 fat olive tube with flare / venturi / shoulder rest / flip sight + loaded warhead, 「사이클론」 six barrels around a hub that `setSpin(0..1)` rotates + motor housing + drum; `setHeat(0..1)` drives the pilot / coil glow) (compact receiver, shrouded stub barrel, skeleton stock, vertical mag) / **sniper** (long heavy barrel + brake, scope tube with emissive lens rims and turrets, folded bipod, long stock with cheek riser, bolt handle) meshes (grip at origin, barrel -Z) with `muzzle` and `ejectPort` sockets on every variant. **Attachments** (`setAttachments(WeaponAttachmentVisuals)`, rebuilt from `inst.sockets` by `WeaponSystem.attachmentsFor`): `muzzle` brake (baffled can) / comp (ported can + accent strip) / choke (flared tube) extend the `muzzle` socket forward; `grip` vertical / angled block under the fore-end; `muzzle` **barrel** (2026-09-14, `att_barrel_ext` 확장 총열: sleeve of `BARREL_EXT_LEN[kind]` + rings + crown, the `muzzle` socket moves to its end); `sight` laser (emitter beside the barrel + thin emissive beam in a lens pivot — no light, no shadow; **2026-09-14** `setLaserAim(point | null, dt)` turns / stretches it toward a world point (0.3–40 m) with a 14/s blend, 1.6 m along the barrel at rest; `hasLaser` · `laserAimBlend`) or a scope tube with lens rims on non-SR weapons (mini red-dot on pistols, nothing on the sniper which already has one); `mag` extended body parented to the animated magazine (side-saddle on shotguns); `stock` cheek riser + rubber butt pad. Per-kind anchors in `ANCHORS`; attachment geometry/materials are tracked separately and disposed on every rebuild and in `dispose()`. Animation: recoil kick (`kick`), reload (mag drops, disappears, new mag slides in; weapon cants), draw/holster swing (`setDraw`), bolt cycle (`setBolt(0..1)`: handle lifts, pulls back, returns; weapon cants), energy glow pulse. |
| `RemoteWeapons.ts` | **Multiplayer only** (every entry point returns immediately unless `ctx.isMultiplayer && ctx.net`). Owned by `WeaponSystem`. **Phase 6**: `onFireMessage(from, FireMessage)` (raw `fire` with `m` / `c`, subscribed through `ctx.net.onMessage` because the `net:remoteFired` bus event drops those fields) replays the uniques — flame cone / lightning arcs held from the replica muzzle until the next ≤ 10 Hz refresh, `BEAM_GRACE` 0.35 s or the final `c:-1`; charged bolt = thick tracer + flash; shuriken = 1 / 3 visual stars; bazooka = visual rocket (fuse on `m:1`) that pops in `onVisualProjectileHit` (explosion FX + sound + shake, never damage). **2026-09-14 bow** = a visual arrow whose speed / drop come from `c` (the shooter's draw) through `unique/Bow.bowBallistics` — the same function the shooter used. `onFired` (bus) skips those five uniques and keeps handling the minigun like an ordinary shot. (1) **Remote weapon models**: each frame walks `ctx.net.getRemotePlayers()`; when `ref.weaponId` or `ref.avatar.weaponSocket` changes it disposes the old `WeaponModel` and parents a new one (def via `ctx.loot.getWeaponDef(id)`, then the family id with `_gN` stripped, then `DEFAULT_PISTOL`/`DEFAULT_RIFLE`) into the avatar's socket with identity local transform (same convention as the local `getWeaponSocket()`); kept while `DEAD`; swept when the ref disappears, removed on `net:remotePlayerRemoved`, cleared on `game:abort` / `game:newMission` / `world:ready`. `getMuzzleWorld(id, out)` exposes the model's muzzle. (2) **Replicated fire** (`onFired` ← `net:remoteFired`): muzzle = remote model muzzle if attached else message origin; muzzle flash + tracer(s) via the shared FX pools; visual-only hit test (`enemies.raycast` → `world.raycast`) for impact sparks/dust/ichor — **no damage, no `weapon:hit`, no `weapon:fired`, no hitmarker**; shotgun fans `pellets` rays with the def's hip `spread`; weapons with a muzzle velocity spawn `ProjectilePool.fire(..., visualOnly=true)` (impacts routed to `onVisualProjectileHit`) — **2026-09-14**: speed / drop from `ctx.loot.getEffectiveStats(weaponId)` (`ballisticsOf`, cached per id, cleared with the entries; no new wire field, the sender's attachments are not folded in), streak style `bullet`, `report: false`; model `kick`, casing, SR bolt cycle animation + delayed `bolt_cycle`; shot sound via `audio:play {id: shotSoundId(kind), position: muzzle, pitch: shotPitchFor(class)}`. Rate-limited per peer with a token bucket (20 FX/s, burst 6). (3) `onReloaded` ← `net:remoteReloaded`: drives `WeaponModel.setReload` over `def.reloadTime`, `audio:play reload` at the remote's chest, `reload_done` when finished. (4) `onGrenade` ← `net:remoteGrenade {…, fuse?}`: `GrenadeManager.throw(p, v, visualOnly=true, fuse ?? GRENADE_FUSE)` — a cooked remote grenade pops early on every client, `fuse 0` (blew up in the thrower's hand) explodes on the next update. A remote's model is hidden while its flags carry `IN_HUB`, `IN_POD`, `DROPPING`, **`HOLDING_ITEM`** (stim / grenade in hand — player/ draws the held item from `PlayerSnapshot.h`) or **`DOWNED`**, or lack `HAS_WEAPON`. **Phase 7**: `syncAttachments` mirrors the snapshot's attachment ids (`RemotePlayerRef.attachments`, read duck-typed) onto the replica through `WeaponModel.setAttachments` — element-wise compare per frame, rebuilt only when the id set differs (also after a model rebuild). |
| `Attachments.ts` | **Phase 7**: attachment def ids → `WeaponAttachmentVisuals` shared by the local weapon (`attachmentVisualsFor(ctx, inst)` from `inst.sockets`) and remote replicas (`attachmentVisualsFromIds(ctx, ids)` from `PlayerSnapshot.att`); socket from `ItemDef.attachment.socket`, else the instance's socket key, else a keyword in the id. **2026-09-14**: an id containing `barrel` (`att_barrel_ext`) → `muzzle: 'barrel'` (checked before comp / choke; also a muzzle keyword for unknown defs). `attachmentIdsOf(inst, out)` (stable socket-key order) and `sameIds(a, b)` back `ctx.weapons.remoteState.attachments`. |
...
| `Projectile.ts` | **2026-09-14: every ordinary gun round flies here** (see 변경 이력). `ProjectilePool(ctx, onHit, onVisualHit, sweep)`: free list that grows (local rounds never dropped; past `SOFT_CAP` the oldest visual-only replica is evicted first), one swept segment per step through the owner's `raycastAll` (enemies · world · interceptable shells · barriers / deployables — no tunnelling at 650 m/s × 50 ms), exact constant-gravity step (`opts.gravity` m/s², else `gravityMul × GRAVITY`), breakable window panes broken and passed, a terrain-under fallback that bisects to the surface crossing, falloff carried per round (`falloffStart/End/Min` → `ProjectileHit.falloffApplied`), `report: false` for callers that report once per trigger pull, bullet streaks in **one additive `InstancedMesh`** (no lights, no per-frame allocation, muzzle `visualOffset` fading over `VIS_CONVERGE_M`), `live` / `liveCount` for smokes. Legacy: hit callback with `ProjectileHit.distance` (meters travelled from the launch point, for falloff). `fire(..., visualOnly = false, opts?)`: visual-only slugs (remote replicas) route their hit to the optional second constructor callback `onVisualHit(h, weaponId)` instead of the damage callback. **Phase 6** `ProjectileOptions`: `style` (`slug` sphere / `shuriken` spinning four-point star / `arrow` shaft + head + fletching / `rocket` tube + warhead + fins with a thruster + smoke trail — one hidden body group per slug, shared geometry), `gravityMul` (0.15 default, 0 = dead straight), `fuse` (self-detonates: synthetic hit at the current position, `ProjectileHit.fused`), `tag` (echoed in `ProjectileHit.tag`). `projectileOptsFor(def)` picks the body / drop for a unique def. **2026-09-14 flight trail** (`trail?`, default on for `shuriken` / `arrow`): one segment per integration step (`prev → pos`, plus `prev → impact` on a hit) into a `TRAIL_CAP` 1024 ring buffer drawn as camera-facing quads in **one** additive vertex-colour mesh (`ProjectileTrails`, same material setup as the core tracer pool, in the scene from the start, no lights) — white, `TRAIL_LIFE` 0.25 s, brightness (1 − age/life)² and width tapering toward the tail (× 0.4), widened with camera distance from 10 m (× up to 8, earlier than the streaks so a far ribbon does not alias into dashes); segments outlive the slug so the tail fades out after the hit; visual-only replicas get it too. `trailSegments` counts live segments (smokes). |
...
| `fx/ThrowArc.ts` | **투척 궤적 미리보기** (2026-09-08). **2026-09-15: 점선이 아니라 연한 빨강 반투명 트레일** — 손에서 0.12 m 폭으로 시작해 끝에서 한 점으로 좁아지고 옅어지는 카메라 향 리본 한 장(고정 인덱스 쿼드 스트립 + `setDrawRange`, 광원 없음; 아래 옛 설명의 `Points` · `RingGeometry` 는 없어졌다 — 변경 이력 참고). 옛 설명: a dotted arc from the hand along the throw, shown by `WeaponSystem.updateThrowArc` whenever a grenade or a `use: 'throw'` gadget is in the hand. `show(origin, velocity)` — 릴리스 지점 · 속도는 던지기가 쓸 바로 그 값이라 근력(`derived.throwRangeMul`) · 오버/언더핸드 · 플레이어 자신의 관성이 그대로 반영된다. `STEP` 1/30 × `MAX_STEPS` 75 (2.5 s of flight), one dot per step. Pooled and light-free: one `Points` cloud over a fixed buffer picked by `setDrawRange` and one `RingGeometry` marker, both `depthTest: false`. `hide()` / `isShowing` / `getImpact(out)`. **2026-09-11 — 끝까지 그리지 않는다** (사용자 요청): 끝까지 적분한 뒤 수평 비거리의 `THROW_ARC_PREVIEW_FRACTION`(0.5)까지만 점을 찍고 **착지 고리는 감춘다**. 근력 · 임플란트로 사거리가 늘면 보이는 궤적도 그만큼 길다. **2026-09-14 2차 — 튕김을 계산하지 않는다** (사용자 결정): `world.resolveCollision` · `getSurfaceY` · `getNormalAt` 을 전부 뺐고 이제 **릴리스 지점 · 속도 · 중력뿐인 순수 포물선**이다. 비행의 끝도 지형이 아니라 **던진 높이로 되돌아오는 점**(내려 던지면 최소 `MIN_STEPS` 12 = 0.4 s). 그래서 궤적이 벽을 통과해 보이는 것은 **의도**이고(「여기 맞는다」가 아니라 「이쪽으로 나간다」), `getImpact` 는 착지점이 아니라 그 포물선이 끝나는 자리다. 실제 수류탄 · 투척 가젯의 비행 · 튕김 · 신관(`Grenade.ts` / `ThrownGadget`)은 무변경. |
...
| 「테슬라 코일」 `u_shock` (`Shockgun.ts`) | arc to the `SHOCK_MAX_TARGETS` 4 nearest enemies in `SHOCK_RANGE` 14 m / `SHOCK_CONE_DEG` 50° with LOS: `SHOCK_DPS` 72 in 10 Hz ticks + `applyStatus('shocked', SHOCK_SLOW_FACTOR, SHOCK_SLOW_DURATION)` every tick (enemies emit `enemy:shocked` once); `setArc` every frame; cells `ammoPerSec` 8/s. **2026-09-14**: in a 시뮬레이션 훈련장 the pop-up targets (destructible obstacles) share the 4-target budget (`coneDestructibles` → `destructible.onDamage`); an empty cone draws `SHOCK_FIZZLE_FORKS` 3 forks within `SHOCK_FIZZLE_SPREAD_DEG` toward `aimTarget(SHOCK_RANGE)` (`shockFizzleEnds`, no sparks, no damage); `shot_energy` crackle on the press (0.35) + every `SHOCK_CRACKLE_INTERVAL` 0.2 s (0.14) | hold = charge over `chargeTime` (`SHOCK_CHARGE_TIME` 1.1 s), `weapon:chargeChanged {kind:'charge', t}` every frame; release = hitscan bolt `altDamage × lerp(SHOCK_CHARGE_MIN_RATIO, 1, t)` over `SHOCK_CHARGE_RANGE` (def falloff), `SHOCK_CHARGE_CELLS` 6 (refused as a dry fire below), thick tracer, cooldown = charge time, `t:-1`; input loss cancels the charge (`t:-1`) | arc stops while charging; `setHeat(charge)` coil glow | `weapon:beamChanged`, `weapon:altFired`, `fire {m:0, c:1}` 10 Hz / `{m:1, c:t}` per bolt |
...
| 「롱혼」 `u_bow` (`Bow.ts`) | **2026-09-14 hold = draw, release = shoot**: draw 0 → 1 over `def.chargeTime` (`BOW_DRAW_TIME` 0.8 s), `weapon:chargeChanged {kind:'draw', t}` every frame (t 0 on the press, `-1` on release / cancel / input loss / unequip / reset), `pose.charging`. Release → own `ProjectilePool` launch on the `aimShot` line (style `arrow`, `visualOffset` from the bow, falloff + ammoType from the effective stats → `Firing.onProjectileHit` → `applyHit`); `bowBallistics(t)` (exported from `Bow.ts` — the **one** mapping) lerps speed `BOW_TAP_SPEED` 45 → `BOW_PROJECTILE_SPEED` 140 m/s, drop `BOW_TAP_GRAVITY` 12 → `BOW_FULL_GRAVITY` 0.5 m/s², damage `BOW_DAMAGE` 150 × `BOW_TAP_POWER` 0.3 → 1. One arrow + one durability per release (`spend`), a dry / broken press clicks + reloads; a press+release inside one frame shoots a tap at once; cooldown `1 / fireRate` (`BOW_FIRE_RATE` 4 → 0.25 s) so taps chain; recoil / kick scale with the draw | **cancels the draw** (no arrow, no ammo; a fresh LMB press is needed). No ADS any more (`allowsAim` false → zoom 1, `altFire` pose) | `WeaponModel.setBowDraw(t)` pulls string + arrow; no casing / flash | `weapon:fired` + `fire {m:0, c: draw}`; replicas rebuild the arrow from `c` (`onFireMessage` → `bowBallistics`), `onFired` skips the bow; **2026-09-15 no reload** (`autoFeed`): `magSize` 1 = the nocked arrow, refilled at once from the quiver (`s.feed`) after each release / when arrows are picked up, R and dry presses never reload, 0 arrows = dry click without a draw; replicas ignore a `reload` for the bow |
| 「해머헤드」 `u_bazooka` (`Bazooka.ts`) | impact rocket (style `rocket`, 0.02 g, tag 0) → `onProjectileHit`: `applyExplosion(BAZOOKA_RADIUS 5.5, BAZOOKA_DAMAGE 420)` + destructible cover in the radius (`world.getObstaclesNear` → `destructible.onDamage` with falloff) | air-burst rocket (tag 1, `fuse: BAZOOKA_ALT_FUSE` 0.4 s → `ProjectileHit.fused`), `BAZOOKA_ALT_RADIUS` / `altDamage` | **no self damage** (2026-09-14): inside the radius only `applyKnockback(away, BAZOOKA_KNOCKBACK)`, and airborne with the blast below the feet → rocket jump `applyImpulse(boost, BAZOOKA_SUPER_JUMP 17, …)` + `player:blastJump` (impulse = that vector; boost = `BAZOOKA_JUMP_FORWARD` 8 × `min(1, speed / PLAYER_WALK_SPEED)` along the horizontal velocity read before the knockback); **3-rocket tube** (`magSize` 3) at `BAZOOKA_FIRE_RATE` 2.85 (≈ 0.35 s apart), auto-reloads once empty (`reloadAsked`); `heavy` pose; big flash / shake / `explosion` FX + sound | `weapon:altFired` on RMB, `fire {m}`; remote rockets pop visually via `onVisualProjectileHit` |
...
**Verified** (`node scripts/smoke-uniques.mjs`, headless Chrome, 64/64): flame cone damage / fuel ~12 per s / durability −1 per s / `enemy:incinerated` + `isIncapacitated` on a charger after ~2.5 s / RMB jet alt on-off; shock arc on two warriors + 2 × `enemy:shocked` / cells ~8 per s / charge gauge rising → bolt (`altFired`, −6 cells, `weapon:hit`, `t:-1`); shuriken mag 10 → 9 → 6 / F tap = `melee:swing` / F hold → gauge 1 → 50 % stamina, `viewWiden` true → false after the swing, `player:slashed` 1+ hit, 280 dmg / refused + 스태미나 toast when short; bow (2026-09-14 draw) zoom 1 / hold → `draw` gauge rising + `remoteState.charging` → release = one arrow ≥ 140 dmg at 8 m, t 1 → −1 / tap = weaker arrow / RMB cancel = no arrow, no ammo, no ADS; bazooka `weapon:hit` ≥ 400 / warrior + scavenger 2 m aside damaged / no self damage at 14 m / auto reload → 1; airborne + looking down + RMB → `altFired`, `player:blastJump` impulse 17, hp −22, `velocity.y` up; minigun no shot at 0.6 s / spin gauge rising / ≥ 8 shots after spin-up / gauge 1 / belt drain / `-1` after spin-down / RMB pre-spin no shots / LMB instant. 0 console errors. Regression: `smoke-weapons` 44/44, `smoke-phase2` 44/44.

**Not done / notes**: unique SFX reuse existing ids (`shot_energy` flame + arc, `melee_swing` stars / bow / slash, `shot_shotgun` rocket, `shot_rifle` minigun, `ui_click` charge / spin start) — audio can hook `weapon:beamChanged` / `weapon:chargeChanged` for loops; the flame / arc never damage the player, deployables or destructible cover (the bazooka does) — the one exception since 2026-09-14 is the shock arc on training-range targets, gated by `ctx.isTraining()` (the flamethrower still does not hit them); remote replicas pick arc targets with their own cone test (no wire list of ids); `player:slashed` and the slash arc are local-only beyond the `melee` message; the shuriken fan uses one `fire` message with `m:1` (the receiver fans 3); the bow's string follows the real draw locally since 2026-09-14 (`setBowDraw`), but a replica's string is still faked from the recoil kick (the draw is not on the wire — only the released arrow's `c`).
...
- While a **wielded implant** (배리어 — 대전차포는 2026-09-15 은퇴) holsters the gun, pressing 1 / 2 / 3 / V now calls `ctx.implants.stow()` and draws that weapon (`requestSwap` unless it is
...
- **2026-09-15 (「롱혼」 가로 파지 · 장전 없음 · 「인페르노」 화염 호흡 — 사용자 요청)**:
  **활 모델** — `WeaponModel.buildBow` 가 활을 **가로로** 쥔다: 팔(limb)이 무기 공간 ±X 로 벌어지고(한 평면 `BOW_Y` 0.07), 앞쪽(−Z)에 가운데 화살 창을 둔
  ...
  있었다 — 이고 두 토막이라 당기면 V 가 된다. 화살 꼬리는 시위에, 촉은 라이저보다 앞(`muzzle` = 화살 끝 z −0.70). `poseBow(f)` 가 당긴 정도(`setBowDraw` 와 킥 중 큰 값)로
  시위 가운데 · 화살을 몸 쪽 `BOW_PULL` 0.24 만큼 당기고 팔을 `BOW_FLEX` 0.14 rad 휜다(매 프레임 할당 없음). 손 자세는 무기 앵커에 IK 하지 않으므로 player/ 는 안 바뀌었다.
  **장전 없음** — `UniqueHandler.autoFeed`(추가만) + `UniqueServices.feed` → `parts/Slots.autoFeed`: 탄창(`data/weapons_unique.csv` u_bow `magSize` 12 → **1**)은 시위에 건 화살
  한 발이고, 쏘는 즉시 · 화살을 주우면 · 장착 뒤 첫 프레임에 지닌 화살에서 곧바로 채운다(재장전 단계 · `weapon:reload*` · 넷 `reload` 없음). `parts/Firing.tryReload` 가
  `autoFeed` 면 곧장 돌아가므로 R 도 빈 격발도 재장전을 시작하지 않고 「탄약 없음」 토스트도 없다. 화살이 하나도 없으면 좌클릭 = 빈 격발 딸깍(`weapon:dryFire`), 당기지 않는다.
  그래서 `weapon:ammoChanged` / `weapon:equipped` 의 **`ammoInMag + reserveRounds` 가 늘 지닌 화살 수**다(HUD 가 한 숫자로 보여 준다). 옛 12발 세이브는 로드할 때
  남는 화살을 가방으로 돌려주고, 가방에 안 들어간 것은 탄창에 남아 먼저 쓰인다(`onItemUpdated` 의 탄창 자르기도 `autoFeed` 는 건너뛴다). 모델은 화살이 없을 때
  (`setBowNocked`) · 쏜 직후 한 쿨다운(`bowLoose`) 동안 시위를 비워 두고, 활은 탄창 빠지는 재장전 애니메이션을 절대 틀지 않는다. 원격: `onReloaded` 가 활을 무시하고
  `onFireMessage` 가 `bowLoose` 를 건다. `reloadTime` 0 · `reserveMags` 30(인벤토리 없는 dev 폴백) · 설명에 「재장전 없이 지닌 화살을 한 발씩 바로 건다」.
  **화염 호흡** — `unique/UniqueFx`: 원뿔이 새로 나타나거나(분대원 포함 — 같은 `setFlame`) 길이가 20 % 넘게 바뀌면(넓은 분사 ↔ 제트) `FLAME_GROW_FROM` 0.12 → 1 로
  `FLAME_GROW_S` 0.26 초 ease-out 으로 피어나고, 켜져 있는 동안 길이 · 반지름이 서로 다른 주파수의 사인 셋(원뿔마다 위상 시드)으로 불규칙하게 숨쉰다(`FLAME_BREATH_LEN` 0.15 ·
  `FLAME_BREATH_RAD` 0.12, 뜨거운 속불은 따로), 깜빡임 0.8–1. 피해 원뿔 · 사거리는 무변경(순수 연출 상수라 모듈 상수로 둔다).
  `smoke-uniques` 활 절: 가로 팔 · 시위 +Z · 당기면 화살이 +Z 로 · 한 발 쏘면 탄창 1 유지 + 예비 −1 + HUD 합 = 지닌 화살 · R 무반응 · 화살 0 이면 빈 격발 · 재장전 · 토스트 없음 · 주우면 다시 걸림.
- **2026-09-15 (전설 유니크 = 사격 숙련 밖 · 로켓 점프 수직 되돌림 — 사용자 결정, 에이전트 C)**: `parts/Firing.recoilMulFor(sys, cls, unique = false)` ·
  `reloadSpeedFor(sys, cls, unique = false)`(+ `WeaponSystem` 위임)이 `unique` 면 숙련 몫이 없다 — 반동 1 · 장전은 각성제 배수만. `fire()` · `tryReload()` 가 `def.unique` 를 넘긴다.
  `WeaponSystem.applyHit` 은 손에 든 무기가 유니크면 `withLocalGunHit(null)` — 유니크 처치는 계열 처치가 아니다(NPC 퀘스트 「계열 처치」에 안 센다, 일반 처치로는 센다).
  로켓 점프 `BAZOOKA_SUPER_JUMP` 26 → **17**(수직만 — 09-14 강화는 천장 있는 훈련장에서 잰 것이었다), `BAZOOKA_JUMP_FORWARD` 8 · `PlayerController.airCarry` 는 그대로.

- **2026-09-15 (투척 궤적 = 연한 빨강 트레일, 사용자 결정)**: `fx/ThrowArc` 가 점선(`THREE.Points`)을 버리고 **끝으로 갈수록 좁아지는
  반투명 리본** 하나를 그린다. 중심선 계산(순수 포물선 · `THROW_ARC_PREVIEW_FRACTION` · `MIN_STEPS` · 스텝마다 한 점 · 경계 보간 · 착지 고리 없음)은
  **그대로**이고 모양만 바뀌었다: 점마다 정점 둘을 `접선 × (점 − 카메라)` 방향으로 벌려 **카메라를 향하게** 하고(이웃과 내적이 음수면 뒤집어 꼬임 방지,
  시선과 나란하면 이웃 방향 → 수평 폴백), 폭은 손에서 `TRAIL_WIDTH_M` 0.12 m → 끝에서 0(3D 호 길이 비례), 정점 알파는 1 → `TAIL_ALPHA` 0.3.
  재질은 `MeshBasicMaterial` `#ff8080` · `opacity 0.5` · `vertexColors`(RGBA) · `transparent` · `depthWrite/depthTest false` · `DoubleSide` · `toneMapped false`,
  `renderOrder 3`. 고정 크기 인덱스 쿼드 스트립(`MAX_PTS` = `MAX_STEPS + 2`) + `setDrawRange`, 매 프레임 할당 · 광원 없음. 카메라 위치는 `show()` 시점의
  `ctx.camera.position`(씬 루트 — 한 프레임 늦어도 폭 방향은 같다). 보이지 않던 착지 고리 메시는 지웠고 `getImpact` 는 필드에 기억한 포물선 끝을 돌려준다.
  생성자 `color` 기본값이 새 연한 빨강이다(호출부 `WeaponSystem` 은 색을 넘기지 않는다). 실제 투척 · `WeaponSystem.updateThrowArc` 무변경.
...
- **2026-09-14 (「테슬라 코일」 좌클릭이 안 나가던 것 — 버그 보고)**: 「전격이 아예 안 나간다」의 뿌리는 판정이 아니라 **표현**이었다.
  레이드에서 5 · 10 · 13 m 정면 · 4 m 옆 · 인간형 · 안드로이드는 모두 맞았다(헤드리스 진단). 그런데 ① `Shockgun.update` 는 원뿔(14 m · 50°) 안에
  적이 없으면 `ufx.release('local')` 로 **아무것도 그리지 않았고**, 로컬 전격에는 **소리가 하나도 없었다**(원격 재생만 시작음을 냈다) — 전지는 조용히
  닳았다. ② 시뮬레이션 훈련장의 표적은 `ctx.enemies` 가 아니라 **파괴 가능한 월드 장애물**(`TrainingArena` 의 hash 엔트리)이라 `coneTargets` 가 영영
  찾지 못했다 — 무기 거치대에서 유니크를 꺼내 시험하는 바로 그 자리에서 좌클릭이 죽어 보였고, 우클릭 충전탄은 히트스캔(`applyHit` → `onDamage`)이라 멀쩡했다.
  고친 것: 대상이 없으면 조준점(`aimTarget(SHOCK_RANGE)` — 벽 또는 사거리 끝)으로 갈라지는 방전(`shockFizzleEnds`, 가지 `SHOCK_FIZZLE_FORKS` · 반각
  `SHOCK_FIZZLE_SPREAD_DEG`, 피해 · 불꽃 없음 — `UniqueFx.setArc(…, sparks=false)`), 켜는 순간 + `SHOCK_CRACKLE_INTERVAL` 마다 `shot_energy` 지지직,
  훈련장(`ctx.isTraining()`)에서만 표적을 원뿔 · 시야 규칙 그대로 잡아 같은 10 Hz 틱으로 `destructible.onDamage`(`UniqueHandler.coneDestructibles` ·
  `obstacleAimPoint`, 창유리 제외 — 레이드의 엄폐물 · 창문은 여전히 건드리지 않는다). 원격 복제(`RemoteWeapons.animateBeam`)도 대상이 없으면 같은 모양의
  방전을 `visualRaycast` 끝까지 그리고 같은 간격으로 지지직한다. 광원 · 머티리얼 추가 없음(기존 호 풀). 새 상수 3개는 `data/constants.csv` → `shared/constants`
  (추가만). 멀티 비호스트의 피해 가드(E-8 `HIT_REQUEST_DPS_MAX` 5000 · 상태이상 60/s)는 72 × 4 대상에 한참 못 미쳐 원인이 아니었다.
  검증: `smoke-uniques` 90/90 — 전격 절에 「대상 없음 → beamChanged · 가지 3 · sparks false · shot_energy ×3 / 0.5 s · 히트마커 0」, 「11 m · 2.5 m 옆 워리어가 맞는다」 추가.

- **2026-09-14 (「해머헤드」 바주카 3연발 · 자해 없음 · 로켓 점프 강화 — 사용자 결정)**: ① 탄창 1 → **3**(`weapons_unique.csv`
  `magSize`) · `BAZOOKA_FIRE_RATE` 0.8 → **2.85**(발 간격 1.25 → 0.35 초) — 비면 스스로 재장전하는 `reloadAsked` 는 그대로이고
  재장전 시간(3 초)도 그대로다. ② **자해 피해 제거** — `unique/Bazooka.explode` 의 `host.takeDamage(BAZOOKA_SELF_DAMAGE, …)` 를
  걷어냈다. `BAZOOKA_SELF_DAMAGE` 는 계약 상수라 csv · `@/shared` 에 은퇴 표시로만 남는다. 넉백 · 로켓 점프 조건은 무변경.
  ③ **로켓 점프 = 더 센 임펄스만**(중력 · 부양 · 느린 낙하는 손대지 않았다, 사용자 결정): `BAZOOKA_SUPER_JUMP` 17 → **26** +
  새 상수 `BAZOOKA_JUMP_FORWARD` **8** — 폭발 순간의 수평 속도 방향으로 `8 × min(1, 속도 / PLAYER_WALK_SPEED)`(제자리 = 0).
  방향은 **넉백 전에** 읽는다(앞에서 터진 로켓의 넉백이 몸을 뒤로 민다). `player:blastJump.impulse` 는 이 합친 벡터다.
  수평 가속이 남으려면 공중 조작이 그것을 깎지 않아야 해서 `player/PlayerController` 에 `airCarry` 가 생겼다(player README).
  실측(시드 7, 달리기 7.2 m/s → 점프 → 좌클릭 발밑, 같은 자리): 정점 **+25.5 → +40.2 m** · 수평 **18.3 → 38.8 m** ·
  체공 **3.0 → 3.8 초** · 체력 100 → 78 에서 **100 → 100**. 체공 중에 나머지 두 발(0.35 초 간격)을 쏠 시간이 넉넉하다.
  ⚠ 넉백이 `KNOCKBACK_MIN_LIFT` 로 늘 몸을 띄우므로 `!host.isGrounded` 는 넉백 뒤에 사실상 늘 참이다 — 서서 발밑을 쏴도
  로켓 점프가 난다(예전부터 그랬다, 이번에 바꾸지 않았다). `scripts/smoke-uniques.mjs` 바주카 절 갱신.

- **2026-09-14 (「롱혼」 활 시위 — 사용자 결정)**: 활이 **좌클릭 홀드로 당기고 놓으면 쏘는** 무기가 됐고 **우클릭은 당기기 취소**다 (정조준 폐지).
  `unique/Bow.ts` 가 더 이상 `fireStandard` 를 타지 않고 자기 발사체를 쏜다 — 판정선은 그대로 `aimShot`(하이브리드), 보정 없음.
  당긴 정도 t(0 … 1, `def.chargeTime` = `BOW_DRAW_TIME` 0.8 초)를 `bowBallistics(t)` **한 함수**가 속도 45 → 140 m/s · 낙차 12 → 0.5 m/s² ·
  피해 × 0.3 → 1 로 바꾸고(`BOW_TAP_*` · `BOW_PROJECTILE_SPEED` · `BOW_FULL_GRAVITY` · `BOW_DAMAGE`, `data/constants.csv`) 원격 복제도 같은 함수를 부른다.
  끊어 쏘기가 연달아 되도록 발사 뒤 쿨타임은 `1 / fireRate`(`BOW_FIRE_RATE` 2.4 → 4 = 0.25 초). 당기는 동안 `weapon:chargeChanged {kind:'draw', t}`
  (계약은 리드가 추가, 크로스헤어 바는 ui 가 그린다) · `pose.charging` · `WeaponModel.setBowDraw(t)`, 놓기 · 취소 · 입력 상실 · 해제 · 리셋에 `t: -1`.
  취소 · 헛누름 뒤에는 **새로 눌러야** 다시 당긴다(우클릭으로 취소한 채 좌클릭을 계속 누르고 있어도 다시 당겨지지 않는다).
  `allowsAim` false → `zoomStatsFor` null(배율 1) · `ws.altFire` 로 RMB 가 ADS 에 들어가지 않는다. csv: `altFire` true · `adsZoom` 비움 ·
  `chargeTime =BOW_DRAW_TIME` · 설명 교체, HUD 모드 줄 `당겨 쏘기 / 당기기 취소`(`ui/hud/WeaponPanel`, `smoke-ui-p6` 동기화).
  **원격**: `onFired` 가 활을 건너뛰고 `onFireMessage` 가 `c` = 당긴 정도로 `bowBallistics` → `ProjectilePool.fire(…, visualOnly, {style:'arrow', gravity})`.
  와이어 필드 추가 없음(`fire.c` 는 원래 있던 칸). 원격 모델의 시위는 여전히 반동 킥으로만 당겨진다(당김 값은 와이어에 없다).
  `smoke-uniques` 활 절: 줌 1 · 홀드 중 `draw` t 상승 + `remoteState.charging` · 놓으면 화살 1발 ≥ 140 · t 1 → −1 · 탭 = 약한 화살 · 우클릭 취소 = 화살 · 탄약 소모 없음.

- **2026-09-14 (표창 · 화살 비행 궤적 — 사용자 요청)**: 「카게」 표창(LMB 1개 · RMB 3개 부채꼴)과 「롱혼」 화살이 **흰 꼬리**를 남긴다.
  `Projectile.ts` 만 바뀌었다 — 옛 표창의 `fx.tracers.add(prev, pos, def색, 0.03, 0.05)`(코어 트레이서 풀 96칸을 스텝마다 쓰던 것)를 걷어내고
  풀 안에 전용 **링 버퍼**(`TRAIL_CAP` 1024 선분)와 카메라를 향한 사각형 배치 메시 하나(`ProjectileTrails`, additive · 톤매핑 없음 · 정점 색 ·
  코어 트레이서와 같은 머티리얼 설정이라 같은 셰이더 프로그램, 생성자에서 씬에 넣어 선컴파일 대상, **광원 0**)를 둔다. 스텝마다 `prev → pos`
  선분 하나(명중이면 `prev → 착탄점`)를 기록하므로 느린 화살의 **낙차 곡선을 그대로 따라간다**. 선분 양 끝이 자기 나이로 밝기
  `0.85 × (1 − age/0.25)²` · 폭 `× (0.4 + 0.6 f)` 를 가져 이어진 스텝이 꼬리 쪽으로 부드럽게 사라지고, 폭은 카메라 거리 10 m 부터
  최대 ×8 넓어진다(표창 0.07 · 화살 0.05 m — 예광보다 일찍 넓힌다: 20 m 에서 2 px 짜리 리본은 계단 현상으로 **점선처럼** 보여서, 헤드리스
  스크린샷으로 확인하고 10 m ~ 80 m 에서 약 3–4 px 를 지키게 했다). 선분은 탄보다 오래 살아 **명중 뒤에도 0.25 초 동안 꼬리가 사라진다**.
  3개 × 60 fps × 0.25 s ≈ 45 선분이라 1024 칸은 넉넉하고, 넘치면 가장 오래된(이미 희미한) 칸을 덮는다 — 코어 트레이서 풀과 경쟁하지 않는다.
  프레임당 할당 없음(Float64/Float32 배열 + 스크래치 벡터), 새 선분이 `TRAIL_LIFE` 보다 오래됐으면 그리기 루프 자체를 건너뛴다.
  `ProjectileOptions.trail?`(선택, 기본 = `shuriken` · `arrow` 에서 켜짐)이 추가 계약의 전부라 호출부는 그대로이고 원격 복제 탄(`visualOnly`)도
  같은 풀을 지나 저절로 꼬리가 붙는다. 비행 · 피해 · 명중 판정은 한 줄도 안 바뀌었다. `clear()` 가 꼬리도 비우고 `dispose()` 가 지오메트리 ·
  머티리얼을 버린다. 스모크용 게터 `trailSegments`.
...
- **2026-09-14 2차 (투척 궤적에서 튕김 계산 제거 — 사용자 결정)**: `fx/ThrowArc.show` 가 더 이상 세계에 아무것도 묻지 않는다.
  ...
  비행을 끊던 `landed`, ③ (보이지 않는) 착지 고리의 `world.getNormalAt`. 남은 것은 릴리스 지점 · 속도 · 중력뿐인
  **순수 포물선**이고, 비행의 끝은 **던진 높이로 되돌아오는 점**이다(새 상수 `MIN_STEPS` 12 = 0.4 s — 발밑을 겨눠 던지면
  첫 스텝부터 내려가므로 최소 길이가 필요하다. `BODY_R` · `PROP_STEP_UP_MAX` import · `_n` 스크래치는 지웠다).
  평지에서는 옛 착지 시각과 사실상 같은 길이가 나오고, `THROW_ARC_PREVIEW_FRACTION`(0.5) · 착지 고리 감춤 · 근력 반영은
  2026-09-11 그대로다. **궤적이 벽을 통과해 보이는 것은 의도**다 — 이 점선이 답하는 질문은 *어디로 던지는가* 하나이고,
  맞는 자리를 약속하지 않는다. 실제 수류탄 · 투척 가젯의 비행 · 튕김 · 신관(`Grenade.ts` / `gadgets` 의 `ThrownGadget`)은
  **한 글자도 바뀌지 않았다**. `WeaponSystem.updateThrowArc`(릴리스 계산)도 무변경. `npx tsc --noEmit` — `src/weapons` 에러 0.

- **2026-09-14 (리드 후속 — 계열별 연사 퍼짐 회복 · 미니건 탄도)**: `WeaponSystem.update` 의 bloom 감쇠가 `BLOOM_DECAY`(2.6/s) 고정에서
  손에 든 무기의 `stats.bloomDecay`(계약 추가, `weapons.csv` 계열 열)로 바뀌었다 — 산탄 · 지정사수 · 저격은 한 발 간격 동안 다 식어 연사 퍼짐이 한 번도
  쌓이지 않았다. `BLOOM_DECAY` 는 무기가 없을 때의 폴백으로만 남는다. 유니크 미니건은 `weapons_unique.csv` 에 `projectileSpeed` 340 · `bulletGravity` 7.2 가
  생겨 일반 사격 경로에서 발사체로 나간다. `smoke-ballistics` 39 · `smoke-weapons` 147 · `smoke-uniques` 72 · `smoke-aim-sway` 25 통과.
- **2026-09-14 (모든 총알을 발사체로 · 레이저 사이트 · 확장 총열 — 에이전트 agent-ballistics, 사용자 결정)**
  - **`parts/Firing.fire`**: `stats.projectileSpeed > 0` 이면 모든 일반 총탄(돌격 · 기관단총 · 지정사수 · 저격 · 산탄 펠릿 하나하나 ·
    `fire()` 를 타는 미니건 · 활)이 `ProjectilePool` 의 발사체다. 낙차 = `stats.bulletGravity` m/s². 출발점 · 방향은 여전히 `parts/AimLine`
    의 resolver(`shot.origin` → 조준점) 이고 **낙차 보정도 이동 표적 리드도 하지 않는다** — 플레이어 몫이다. 예외 둘: resolver 가 `near`
    (총열 · 총구 앞 `WEAPON_MUZZLE_BLOCK_RANGE`)로 푼 탄은 그 프레임에 바로 맞는다(빨간 원 자리와 정확히 같다), 속도 0 은 옛 히트스캔.
    유니크(화염 · 전격 · 표창 · 바주카)는 그대로. 거리 감소는 **실효 스탯**(`damageFalloffStats`, 부착물 반영)으로 **실제로 날아간 거리**에서
    잰다. `reportShot` 은 발사체도 **트리거당 1회**(해석된 선의 끝 = 착탄 예상점) — 풀에는 `report: false` 로 넘긴다(비호스트의 `shotq`
    가 펠릿마다 두 번씩 나가지 않게). 연사 퍼짐은 `stats.bloomPerShot` · `bloomSpread`, `applyAimZoom` 의 흔들림 진폭 × `stats.swayMul`.
    히트마커는 풀 한 스텝의 명중을 모아 `flushHitmarker` 가 한 번 낸다.
  - **`Projectile.ts` 재구성**: 자유 목록 + 필요하면 성장 — 로컬 탄은 절대 버리지 않고, `SOFT_CAP`(384) 을 넘으면 가장 오래된 **복제 탄**부터
    퇴출(`HARD_CAP` 4096 은 폭주 방지뿐). 스텝마다 `prev → pos` 선분 하나를 소유자의 `raycastAll`(적 · 월드 · 요격 가능한 포탄 · 배리어/설치물)
    로 훑는다 — 650 m/s × 50 ms(32.5 m) 도 적 · 얇은 벽을 뚫지 않는다. 일정 중력 정확 적분. **창문 유리**는 로컬 탄이 깨고(`destructible.onDamage`)
    **지나간다**(복제 탄은 그냥 지나간다 — 깬 쪽이 `struct glass` 로 맞춘다). 예광은 **InstancedMesh 하나**(늘인 상자 · additive · 톤매핑 없음 ·
    인스턴스 색, 광원 0, 처음부터 씬에 있어 코어 선컴파일이 컴파일) — 거리에 따라 폭을 넓히고, 총구 → 판정선 시각 오프셋이 `VIS_CONVERGE_M`
    동안 사라진다(스코프 중엔 없음). 착탄 순간 짧은 트레이서 1개. 옛 구체 메시 48개 + 복제 머티리얼 48개는 없어졌고 유니크 몸체는 공유
    지오메트리/머티리얼로 필요할 때 늘린다. 새 옵션: `gravity` · `report` · `falloffStart/End/Min` · `light` · `ammoType` · `visualOffset` · `width`,
    `ProjectileHit` 에 `intercept` · `falloffApplied` · `light` · `ammoType` · `reported` 추가(모두 선택).
  - **`RemoteWeapons`**: 복제 탄은 받는 쪽에서 `ctx.loot.getEffectiveStats(weaponId)` 로 같은 속도 · 낙차를 구해 시각 전용 발사체로 날린다
    (와이어 필드 추가 없음 — `fire` 는 여전히 트리거당 1개, 보낸 사람의 부착물은 반영하지 않는다).
  - **레이저 사이트** (`WeaponModel.setLaserAim` · `hasLaser` · `laserAimBlend`, `WeaponSystem.updateLaser`): 빔이 렌즈의 피벗에 달린 단위 길이
    상자다. 정조준 중이거나 사격 후 `FIRING_POSE_HOLD` 동안은 이번 프레임 `aimLine.end`(크로스헤어 점, 막히면 막힌 자리)를 향하고 그 거리
    (0.3–40 m)만큼 늘어나며, 아니면 총열 방향 1.6 m 로 돌아온다(블렌드 14/s). 빔은 그림자를 드리우지 않는다. 원격 복제는 총열 방향 그대로.
  - **확장 총열** (`att_barrel_ext`): `Attachments` 가 id 의 `barrel` → `muzzle: 'barrel'`, `WeaponModel` 이 계열별 길이(`BARREL_EXT_LEN`)의
    슬리브 + 방열 링 + 크라운을 그리고 `muzzle` 소켓을 그 끝으로 옮긴다(재빌드마다 dispose).
  - 검사: 새 `scripts/smoke-ballistics.mjs` (verify 매핑 `weapons` · `items`) · `smoke-weapons` 정밀 사격 기준을 방아쇠 순간으로 · 낙차 허용 ·
    `smoke-aim-sway` 흔들림 속 사격 기준을 방아쇠 순간으로.
...
- **2026-09-15 (결과 창 개편 — 플레이어 피해 출처)** — `Grenade.explode` 가 로컬 플레이어에게 준 폭발 피해에 출처를 싣는다:
  내 수류탄(파편 · 소이 폭발, 오래 쥐고 있다 손 가까이서 터진 것 포함) = `{ kind: 'self' }`, 분대원 수류탄의 복제(`visualOnly`) = `{ kind: 'ally' }`
  (모듈 상수 둘 — 할당 없음). 소이 수류탄이 남기는 불은 gadgets 화염 지대 소유라 거기서 `self` / `ally` 가 붙는다. 바주카 자해는 2026-09-14 에 없어졌다.
