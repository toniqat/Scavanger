...
/**
 * **식사 화면** (주방 A-3c, 2026-09-11 — `openDiningTable(uid)` ← E on a 식탁; `uid` null = 공유 함선의 고정 식탁).
 *
 * **좌 = 지금 실린 식사 + 그 버프 한 줄, 우 = 가진 요리 목록 + 가방 · 함선 창고 격자.** `ui/Analyzer.ts` ·
 * `ui/GrowStation.ts` 와 같은 틀이고, 왼쪽 접시가 곧 드롭 대상이라 요리를 끌어다 놓아도 먹는다.
 *
...
export class DiningTable extends HousingPanel {
  ...
  constructor(ctx: GameContext, private readonly housing: HousingSystem) {
    super(ctx, 'dining', 'dining-table');
    const f = this.frame;
    const head = el('div', { cls: 'hs-head', parent: f });
    const hl = el('div', { cls: 'hl', parent: head });
    this.title = el('div', { cls: 'title', text: '식탁', parent: hl });
    this.subtitle = el('div', { cls: 'subtitle', text: '', parent: hl });

    const body = el('div', { cls: 'dt-body', parent: f });
    const left = el('div', { cls: 'dt-left', parent: body });
    ...
    this.activeNote = el('div', { cls: 'hint', text: '', parent: left });

    const right = el('div', { cls: 'dt-right', parent: body });
    el('div', { cls: 'ui-label', text: '가진 요리', parent: right });
    this.listEl = el('div', { cls: 'dt-list', parent: right });
    el('div', { cls: 'ui-label', text: '가방 · 함선 창고', parent: right });
    this.invNote = el('div', { cls: 'hint', text: '요리를 왼쪽 접시로 끌어다 놓으면 먹습니다.', parent: right });
    this.invHost = el('div', { cls: 'dt-inv', parent: right });
  ...
  /** Built lazily for the same reason the other panels do it: housing/ is registered before inventory/. */
  private mountGrids(): void {
    ...
    setText(this.invNote, '요리를 왼쪽 접시로 끌어다 놓으면 먹습니다.');
    this.grids = inv.createTradeGrids(this.invHost, {
      grids: ['bag', 'stash'],
      dropSelector: '.dt-plate',
  ...
  refresh(): void {
    ...
    const shared = h.isSharedTable();
    setText(this.title, shared ? '공유 함선 식탁' : '식탁');
    setText(this.subtitle, shared
      ? '먹으면 나에게, 차리면 분대 전원에게 — 차릴 때도 요리는 하나만 씁니다.'
      : '요리를 먹으면 다음 레이드 1회분 버프가 실립니다.');
  ...
  private paintPlate(): void {
    ...
    setText(this.plateNote, def
      ...
      : '오른쪽 목록에서 「먹기」를 누르거나 요리를 여기로 끌어다 놓으세요');
