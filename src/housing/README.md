...
| `ui/GrowStation.ts` | **재배 화면** (온실 개편 2026-09-11, `openGrowStation(uid)` ← E on a 재배 스테이션; 옛 `ui/GrowMenu.ts` 는 가구와 함께 삭제). **좌 = 재배층, 우 = 가방 + 함선 창고.** 한 층은 가로로 긴 **하얀 바** 하나이고 그 위에 **위가 살짝 잘린 원**(`clip-path`)이 3개 — 흙을 부으면 원 안이 `SOIL_TAG_COLOR` 로 **80 % 높이까지** 차고, 씨앗을 심으면 그 위에 아이템 글리프 + 진행바 + 남은 실시간 + 수확 버튼이 산다. 오른쪽은 `ctx.inventory.createTradeGrids(host, {grids:['bag','stash'], dropSelector:'.gs-pot:not(.is-locked)', onTake})` 그대로이고 **칸 크기는 제작 창과 같은 기본값 54 px** 다 (`target.dataset.tier/slot` 으로 어느 칸인지 읽는다 — 토양이면 `fillSoil`, 씨앗이면 `plantSeedAt`, 그 외에는 한국어 사유). 잠긴 층은 흐리게 + 「Lv.N 강화로 열립니다」이고 드롭 대상이 아니다. 수확하면 우측 격자를 `refresh()` 해 창고로 들어간 것이 곧바로 보인다. 1초 틱은 글자 · 바 · 버튼만 다시 칠하고(타이머는 `ShipState.grows` 에 있다), 닫으면 틱과 격자 뷰를 모두 정리한다. Emits `ui:growToggled {open, uid}`; `ui:housingToggled` 에는 `page: null` (계약의 page union 은 동결). **강화 줄 `.gs-up`** (2026-09-11): 재배층 열 위에 `Lv.n / 3` · 다음 레벨이 여는 층(`growTiersForLevel` 에서 유도) · 비용 칩(`renderCost`) · 「강화」 버튼 — `furnitureUpgradeBlock(uid)` 이 버튼을 잠그고 사유를 적으며, 누르면 `upgradeFurniture(uid)` 뒤 `refresh()` 가 새 층 + 우측 격자를 함께 다시 그린다. **이 화면이 게임에서 `upgradeFurniture` 를 부르는 유일한 곳이다.** 1초 홀드 없음(되돌릴 수 있는 확정). |
| `ui/Analyzer.ts` | **분석 화면** (A-12, 2026-09-11, `openAnalyzer(uid)` ← E on a 분석기). `ui/GrowStation.ts` 가 그대로 본보기다 — **좌 = 강화 줄(`.az-up`) + 해석 칸 `ANALYZER_MAX_SLOTS` 개, 우 = 가방 · 함선 창고 격자 + 해석 도감.** 칸은 언제나 전부 그리고 잠긴 칸은 딤드 + 「Lv.N 강화로 열립니다 · 위 「강화」」이며 드롭 대상이 아니다(`dropSelector` 가 `.az-cell:not(.is-locked)`). 오른쪽 격자는 `ctx.inventory.createTradeGrids({grids:['bag','stash'], …})` 그대로이고 `target.dataset.slot` 으로 어느 칸인지 읽어 `startAnalysis` 를 부른다 (표본이 아니면 한국어 사유). 1초 틱은 글리프 · 진행바 · 남은 시간 · 버튼만 다시 칠한다(재구축 없음). 강화 줄은 `.gs-up` 과 같은 판단이라 **1초 홀드가 없다**(되돌릴 수 있는 확정). `ui:housingToggled` 에는 `page: null` (계약의 page union 은 동결) 이고 전용 toggled 이벤트는 계약에 없다 — 소비자는 `housing:analysisChanged` 를 본다. |
...
| `ui/CultureTank.ts` | **배양 화면** (A-14, 2026-09-11, `openCultureTank(uid)` ← E on a 배양조). `ui/Analyzer.ts` 가 그대로 본보기다 — **좌 = 강화 줄(`.ct-up`) + 배양 칸 `CULTURE_MAX_SLOTS` 개, 우 = 가방 · 함선 창고 격자.** 잠긴 칸은 딤드 + 「Lv.N 강화로 열립니다」이고 드롭 대상이 아니다(`dropSelector` 가 `.ct-cell:not(.is-locked)`). 칸의 몸짓만 온실에서 왔다: 배양관 안이 **배지 색(아이템 `color`)으로 78 % 높이까지** 차오르고(`.ct-fluid`), 그 위에 세포주 글리프 · 진행바 · 남은 시간 · 수확 버튼이 산다. 떨어뜨린 것이 `def.medium` 이면 `fillMedium`, `def.strain` 이면 `insertStrain`, 그 외에는 한국어 사유. 1초 틱은 글리프 · 바 · 버튼만 다시 칠한다(재구축 없음). 강화 줄은 `.gs-up` · `.az-up` 과 같은 판단이라 **1초 홀드가 없다**. `ui:housingToggled` 에는 `page: null` 이고 소비자는 `housing:cultureChanged` 를 본다. |
| `ui/DiningTable.ts` | **식사 화면** (A-3c, 2026-09-11, `openDiningTable(uid \| null)` ← E on a 식탁; **null = 공유 함선의 고정 식탁**). **좌 = 접시**(지금 실린 식사 칩 + `mealBuffText` 한 줄 + 안내, `.dt-plate` 자체가 드롭 대상이라 요리를 끌어다 놓아도 먹는다), **우 = 가진 요리 목록**(칩 + 이름 · 일반/특선 · 버프 + `먹기`, 공유 함선이면 `분대에 차리기`) **+ 가방 · 함선 창고 격자.** 규칙은 하나도 여기 없다 — `parts/Dining` 이 돌려주는 한국어 사유를 메시지 줄에 옮길 뿐이다. 버프 표기의 원본은 계약의 `MEAL_BUFF_LABEL_KO` · `MEAL_BUFF_UNIT` 한 쌍이고(`mealBuffText` 가 이 파일에서 export 된다), 단위가 `'%'` 인 줄만 `amount × 100`, 부호는 값이 정한다 — `durabilityLossMul` 은 음수라 「장비 손상 −20 %」로 읽힌다. 1초 틱이 **없다**(현실 시간 타이머가 없는 화면이다). |
