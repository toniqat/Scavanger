...
| `PowerRules.ts` | **발전기 전력 순수 판정** (2026-09-13, docs/plans/power-crypto.md). ctx · DOM 없음 — `computePower(state)` 한 번이 공급(`generatorPowerSupply`) · 시설(지을 수 있는 용도가 붙은 방, 조종석 · 빈 방 제외)별 `base` / `required`(= 기본 + **활성** 가구 demand) / `allocated` / `powered` · 가구별 사유 `blocks`(비활성 → `FURNITURE_DISABLED_REASON_KO`, 할당 < 요구 → `POWER_SHORT_REASON_KO`, 가동 중인 메인 컴퓨터가 채굴 시설에 없으면 클러스터 → `MINING_COMPUTER_REQUIRED_REASON_KO`)를 낸다. demand = `FurnitureDef.power` (+ 연산 클러스터면 `state.clusters[].cores` × `COMPUTE_CLUSTER_POWER_PER_CORE`), 조종석 가구는 0. `operationalBenchLevel` · `benchBlockIn`(인벤토리 제작 목록), 세이브 정리 `sanitizePowerFields`(시설 방만 · 합이 공급을 넘으면 **높은 방 번호부터** 깎는다 · 배치된 uid 만) · v12 이관 `autoAllocateRequirements`(방 순서대로 요구량 통째로, 못 채우는 시설은 건너뛴다). `Rules.ts` 와 섞지 않는다(배치 규칙 에이전트가 그 파일을 고친다). |
| `parts/Power.ts` | **발전기 전력 — 할당 · 비활성화 · 멈춤** (2026-09-13). `bindPower(sys)`(init 에서 한 번, 첫 계산 포함) 가 가구 배치 · 이동 · 회수 · 강화 · 발전기 · 용도 · `housing:changed` · `housing:loaded` 마다 `recompute(sys, reason)` 를 건다: ① 시설 아닌 방의 할당 · 없는 uid 의 비활성 정리 ② **자동 보충**(`POWER_AUTO_TOPUP`, `data/tuning.csv`) — 가동 중이던(또는 새로 증축한) 시설의 요구가 늘면 부족분을 **남는 전력에서만** 채운다(다른 시설 할당은 안 가져오고, 모자라면 채우지 않는다; 할당 직접 변경 · 비활성 · 불러오기는 보충 없음) ③ 멈춤 기록 `state.pausedAt[uid]` = 「지금 멈춰 있다」의 원본 — 멈추는 순간 `now`, 다시 돌면 지우고 `housing:operationalChanged {operational, pausedMs}` 를 **동기로** ④ 서명이 바뀌면 `housing:powerChanged {reason}` ⑤ 가동 중이던 시설이 멈추면 토스트 `<시설> 전력 부족 — 발전기에서 전력을 할당하세요` ⑥ 저장 예약(`sys.saveSoon()` — `housing:changed` 를 내지 않는다). 같은 스택에서 다시 불리면 끝난 뒤 한 번 더 돈다. 재배 · 배양 · 해석의 시각 밀기(`shiftPaused*`)를 이 파일이 묶는다(채굴은 `parts/Mining`). 계약 9종: `getPowerOverview` · `getFacilityPower` · `setPowerAllocation`(정수 ≥ 0, 합 ≤ 공급 — 넘치면 `남는 전력이 부족합니다 (최대 n)`) · `isFurnitureDisabled` · `setFurnitureDisabled`(전력을 쓰는 가구만) · `furnitureOperationalBlock` · `stationNow`(= `min(pausedAt, now)` ?? now) · `getOperationalBenchLevel` · `benchOperationalBlock`. |
...
| `ui/mining/ClusterScreen.ts` | **연산 클러스터 화면** (2026-09-13 암호화폐 채굴, `openComputeCluster(uid)` ← E on a 연산 클러스터 · 메인 컴퓨터 현황 줄). `HousingPanel` page `'cluster'` (`.mining-cluster`), `StationShell` 카드 배치 (`upgrade: false`, `power` = 비활성화 버튼 + 멈춤 배너). **레일** = 함선의 연산 클러스터 전부(`클러스터 n` + 3×3 점 = 꽂힌 코어, 채굴 중 초록 / 멈춤 회색, 코인 · 코어가 있는데 멈추면 레드닷). **코어 칸 3×3** `.mn-core[data-core]` = `mountStationGrids` 의 드롭 대상 — 연산 코어 스택을 떨어뜨리면 `insertClusterCores(uid, min(스택, 빈 칸))`, 코어가 아니면 `연산 코어만 꽂을 수 있습니다`, 가득이면 `코어 칸이 가득 찼습니다`. 꽂힌 칸 = 켜진 코어(채굴 중이면 발광 · 초록 LED). 빼기 = 우클릭(가방 먼저) · 더블클릭(`ProductDrag`, 함선 창고 먼저) · 끌어서 격자에 놓기(그 격자) → `removeClusterCores(uid, 1, dest)`. **상태 줄** = 채굴 코인 · 채굴 주기(`fmtDuration`) · 코어 +1 이면 · 주기당 채굴 · 시간당 예상 코인 · 시간당 크레딧(시세가 있을 때만) · 요구 전력 · 시설 전력(`getFacilityPower`). **이번 주기** 막대(`scaleX`) + `HH:MM:SS`(멈추면 `정지됨`). **채굴 코인** 8종 `.mn-coin[data-coin]` — 잠긴 코인 딤드 + 호버 카드(`StationTip`: 코어 n개 주기 · 시세 · 잠긴 사유 · 설명), 클릭 = `setClusterCoin`, 진행도가 있으면 **1초 홀드 경고** `mn-coin-change`(진행도가 0 이 된다), 지금 코인 우클릭 = 해제(`mn-coin-clear`). 막는 사유 배너 `.mn-banner` = `ComputeClusterInfo.block` (전력 · 비활성은 틀의 멈춤 배너가 이미 말하면 숨는다). 1초 틱 · `housing:clusterChanged` · `cryptoMined` · `powerChanged` · `operationalChanged` · `net:cryptoPrices` → 합쳐진 refresh. `ui:miningToggled {page:'cluster'}`. `openComputeClusterScreen(sys, uid)` = 계약 메서드의 몸통(연산 클러스터가 아니면 토스트). |
| `ui/mining/MiningComputer.ts` | **메인 컴퓨터 화면** (2026-09-13, `openMiningComputer(uid \| null, tab?)` ← E on a 메인 컴퓨터; null = `getMiningComputerUid()`, 없으면 토스트 `메인 컴퓨터가 없습니다`). page `'computer'` (`.mining-computer`), `StationShell` 카드 하나(`inventory: false` · 업그레이드 없음 · `power`). **레일 탭** 「클러스터 현황」 · 「지갑」 · 「거래소」 (`MiningComputerTab`). 현황 = 요약(채굴 중 n / m · 시간당 예상 크레딧 · 코인별 시간당 채굴) + 줄 `.mn-crow[data-uid]`(클러스터 n · 방 · 코인 글리프 + 티커 · 코어 n/9 핍 · 막대 + 시계 · 채굴 중 / 사유 · ⚡ 전력) — 누르면 `openComputeCluster`. 지갑 = 줄 `.mn-wrow[data-coin]`(보유 `formatCoinUnits` · 평가액 · 24시간 · 누적 채굴 `state.cryptoMined`) + 총 평가액 · 보유 크레딧, 줄을 누르면 그 코인의 거래소. 거래소 = 코인 목록 `.mn-xrow` + 머리(시세 · 변동) + 기간 `.mn-seg-btn[data-range]` · 봉/선 `[data-mode]` + `CryptoChart` + 매매(`[data-side]` 매수/매도 · 수량 입력 `coinToUnits` + 25/50/100 %(매도 = 지갑, 매수 = `affordableUnits`) · 견적 `cryptoQuote` 시세 · 수수료 · 내는/받는 크레딧 · 사유 줄 · **1초 홀드 확정** `.mn-hold` → `await tradeCrypto` → 토스트). UI 가 먼저 막는 사유: 잠긴 코인 · 오프라인(`서버에 연결되어야 합니다`) · **오래된 시세**(`pricesAt` 이 `CRYPTO_QUOTE_WINDOW_S − CRYPTO_TICK_S/2` 초보다 오래됨) · 수량 없음 → 그다음 housing 견적의 `block`. **시세 구독**: 지갑 · 거래소 탭이 열린 동안 `ctx.net.crypto.watch()` 하나(탭 전환 · 닫기에 해제) + **거래 한 건마다 답이 올 때까지 하나 더**(탭을 바꿔도 쥔다 — 릴레이가 구독 소켓의 시세 창으로 검증한다). 이력은 코인 · 기간을 고를 때마다 `requestHistory`, 서버에 막 붙으면 한 번 더; `net:cryptoHistory`(맞는 것만) · `net:cryptoPrices` 에서 다시 그린다. `debug {watches, unwatches, requests, trades}`. |
...
- **2026-09-13 (암호화폐 채굴 화면, docs/plans/power-crypto.md, 에이전트 ④)**
...
- **2026-09-13 (발전기 전력 — 할당 · 비활성화 · 멈춤 · v12, docs/plans/power-crypto.md, 전력 에이전트)**
...
- **2026-09-13 (배치 규칙 — 접근 면, docs/plans/power-crypto.md, 배치 에이전트 · 사용자 결정)**
...
- **2026-09-13 (암호화폐 채굴 규칙 · 지갑 · 거래소 — 에이전트 ③, docs/plans/power-crypto.md)**
