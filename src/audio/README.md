...
- An explicit `audio:play` and an auto-hook of the same id within `DEDUPE_WINDOW` play once — route racing auto-hooks
  through `playRequested(…, auto)`. `RATE_MAX_SAME` limits frequency only; overlap is capped by `VOICE_CAP`. — `AudioSystem.play`
...
- 2026-09-13 — Rover SFX (8 ids) + `rover:*` auto-hooks; engine as repeated clips.
