...
| `Input.ts` | Keyboard / mouse state with per-frame pressed / released sets (mouse buttons mirrored as `Mouse0..4` key codes), pointer lock (deferred re-lock, bounce filter, fullscreen keyboard lock, click-to-relock), `setCursorMode`, `onUserUnlock`; `endFrame()` called by Engine (`holdEdges()` / `releaseEdges()` hand edges to the last smoke-clock sub-step, and the earlier sub-steps see the held keys as they were before the frame) |
...
| `nav.ts` | Shared pathfinding contract (2026-09-21, A-18): `NavRef` (`WorldRef.nav` — `findPath` · `walkable` · `createPath` · `ready` · `revision`), `NavPath` / `NavWaypoint` / `NavPathStatus`, `NavLinkKind`, the `NAV_CAN` body mask and `NAV_CAN_PERSON`. Owner: `world/nav`; authority-side movers only, no wire |
...
| `explosion.ts` | Explosion falloff (`explosionFalloff` · `explosionDamage` · `explosionDamageRange` for tooltips): two-step curve from `EXPLOSION_FULL_FRACTION` / `EXPLOSION_OUTER_MUL` — all explosives. Occlusion: `blastReachesBody` (blast centre ↔ body feet · chest · head, any clear ray = hit), `meleeReachesBody` (attacker head ↔ the same 3 points), `lineClear` — all over `WorldRef.raycastBlast` (2026-09-18: the glass-blocking ray, so a broken window no longer lets a blast into a room), numbers `BLAST_LOS_*` / `MELEE_LOS_SLACK_M`. Destructibles: `blastDestructibles` (2026-09-21, B-98 — the one loop every player-side explosive hands its blast to, so dropped cover · window glass · the 탐사 차량 all break to the same explosion; no occlusion ray, the thing damaged **is** the collider) |
...
- 2026-09-22 — `Input.holdEdges` winds the held keys / buttons back with the edges (E-12): an earlier smoke-clock sub-step
  no longer sees a button already up with no release edge (`ui/hud/Pings` cancelled the ping on it).
- 2026-09-21 — `nav.ts` added (add-only, TODO A-18): the shared pathfinding contract — `WorldRef.nav` (appended in `types.ts`), path buffers that allocate nothing on a replan, the `NAV_CAN` mask (ladder · wall climb · window · indoor) so one graph serves an android, a humanoid and a small bug. `ALLY_FLAGS.CLIMB` (an android on a ladder its path took) and the `NAV_*` / `ALLY_NAV_*` constants (`data/constants.csv`) came with it.
- 2026-09-21 — `explosion.blastDestructibles` added (add-only, B-98, user's decision 「공용 경로로 전부」): the bazooka's private walk over `Obstacle.destructible` moved here and grenade · mine · ship call · fire zone now call it too. Until then only the bazooka broke a destructible, which is why the 탐사 차량 — damageable since the same day through exactly that hook — could be shot but not blown up. `net.ts`: `struct turret` added (B-100, the ceiling turret's chosen body) and `lobby:look.shipModel` re-documented as a **nudge** — the relay fills `LobbyPlayer.shipModel` from the profile store now (B-101).
