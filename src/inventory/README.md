...
| `parts/SocketDetach.ts` | **2026-09-14 — 소켓 하나 꺼내기** (고정 툴팁에서 부착물 끌어내기, 사용자 결정). `attachFromImpl` 의 거울: `detachSocket(sys, weaponUid, socket, target)` · `previewDetach`(끄는 동안의 칸 강조, 아무것도 안 바꾼다) · `canDetachSockets`(무기가 `canSocketAt` 자리 — 가방 · 장비칸 · 함선 창고 — 에 있나; 상자 · 시체 무기는 false) · `worldDetachAllowed`(레이드에서만). `target` = `DetachTarget`(`model.ts` — 가방 · 창고 · 주머니의 **가리킨 칸 그대로**, 막히면 거절하고 부착물은 소켓에 남는다 · 창고는 함선에서만 · 주머니는 `pouchAccepts` · 또는 레이드의 바닥). 순서는 부착과 같다: 소켓 비움 → 칸에 놓기(`emitTransfer` — 창고 ↔ 가방이면 획득/제거 이벤트) 또는 `throwToWorld` → 무기가 든 격자 `version++` → `inventory:socketChanged {attachment: null}` → `afterSocketChange`(줄어든 탄창의 넘치는 탄은 가방으로, `inventory:itemUpdated`) → `afterChange`. 시스템 위임 `detachSocket` · `previewDetach` · `canDetachSockets` |
| `parts/SocketRules.ts` | **2026-09-14 (총기 밸런스 — 계열별 소켓, 사용자 결정 "로드할 때 떼어서 돌려주기")**: `returnForbiddenAttachments(sys, 'stash' \| 'bag')` — 장비칸 · 가방 · 주머니의 무기에서 **더는 맞지 않는 부착물**(`LootRef.canAttach` 거절 · 자기 칸이 아닌 칸)을 `Serialize.detachForbiddenSockets` 로 떼어 돌려준다. `'stash'`(로컬 로드아웃 `restoreLoadoutSave` · 서버 문서 `applyProfileDocs`) = 함선 창고 → 가방, `'bag'`(레이드 blob `applyRaidState`) = 가방만 — 레이드 중 창고로 보내면 죽어도 안 잃는 보관이 되므로. 둘 다 자리가 없으면 **무기에 그대로 남긴다**(효과 없음 — `items/WeaponStats.fittingAttachments`, 다음 로드에서 다시 시도) — 아이템은 절대 버리지 않는다. 창고로 옮겼으면 창고를 dirty 로, 호출부가 로드아웃도 `markDirty('sockets')` 해 **둘이 한 번에**(`scheduleSaves` → `setMany`) 올라간다 — 창고만 저장되면 다음 로드에서 같은 부착물이 한 번 더 창고로 간다. 창고 문서 속 무기는 `Stash.load` 가 같은 함수로 창고 안에서 처리한다 |
...
| `ui/parts/Screens.ts` | **Tab 창의 화면 탭과 부속 열.** **2026-09-13**: `setCatalog(open)` 이 `.inv-root` · `.inv-layout` 에 `.is-catalog` 를 걸고 끈다 (무한 상자 배치 — `InventoryUI.show` 도 이 함수를 지난다), `updateCatalogTarget` 은 그동안 장비칸을 대상으로 보지 않는다. 함선에서 Tab 은 인벤토리 / 캐릭터 / 기업 / 함선 네 화면을 한 창 안에서 전환한다 (각 화면은 그 폴더가 준 `EmbeddedView` 라서 이 파일은 붙였다 뗐다만 한다). 제작 열과 무한 상자 카탈로그의 열고 닫기도 여기 있다. **2026-09-09**: `setTab` 이 탭마다 `sys.emitGuide()` 로 키 가이드 줄을 갱신하고, `setCraftOpen` 이 `'inventory.craft'` owner (`1초 홀드 제작`) 를 열고 닫는다. |
...
| `ui/InventoryUI.ts` | **2026-09-15 3차 (사용자 결정)**: `.inv-col-right` 가 `section.inv-panel.inv-panel-grids` — **창고 + 가방이 한 장의 패널**(창고 왼쪽 · 가방 오른쪽, 칸마다 자기 스크롤 · 자기 정렬 · 자기 필터 드롭다운)이고 장비 열은 그 카드 왼쪽이다. 창고 머리의 `STASH` eyebrow + `함선 창고` 제목은 **삭제**(머리 = 개수 · 정렬 · 필터). 가방의 `.inv-bag-tools`(`모두 수리` · 정렬 · 필터)가 격자 위 줄에서 **가방 머리** 안으로 옮겨 갔다. `filterChips: FilterControl[]` 는 두 드롭다운을 같은 `filterGroup` 으로 맞춘다. DOM layout under `ctx.uiRoot` (**Phase 7**: `.inv-search-status` readout in the container header, `setSearchProgress(uid, p, active)` / `refreshSearchStatus()` / `shakeItem(uid, loc)`; an unsearched tile (`sys.isItemLocked`) gets no tooltip, press / drag, context menu, double-click or middle-click request; `result()` treats `'pending'` as silent): container panel (left), bag (center, width follows the grid columns; **quick-slot rose** under the grid), equipment column (right: 4 slots), world-drop zone (**2026-09-14**: the 중앙 하단 힌트 알약 바 `.inv-hints` is gone — it duplicated five of the bottom-right 키 가이드 entries; the two that were only there, `Shift + 드래그` / `Ctrl + 드래그`, moved into `guideKeys()`). Drag & drop with live ghost + valid/invalid/swap/merge highlight, **attachment → weapon tile drag** (socket target lit green/red via `previewAttach`), **stim/grenade → wheel cell drag**, cell → cell / cell → out drags, Shift/Ctrl partial drags, R to rotate, right-click quick action / context menu, middle-click request, X drop, double-click equip/move/register, "모두 가져가기", tooltip, shake on refusal, `audio:play` sfx. **Phase 6**: hosts the `CatalogView` (leftmost panel) and drives **catalog drags** (`DragState.catalog`: a fresh instance per press, targets = equipment slots + active grids only, `previewCatalog` / `dropFromCatalog`, no world drop, no wheel; double press within 400 ms = `takeFromCatalog`), `setCatalog(open)`, `setCraftOpen(open)` for bench mode. **Phase 8**: screen tabs 인벤토리 / 캐릭터 / 기업 / 함선 (`setTab` / `screenTab`, `.scr-tabs` hidden outside hub mode) swap `.inv-layout` for the `.inv-screen` host and build `ctx.progression.createSheetView` / `ctx.meta.createCorpView` / `ctx.housing.createShipView` into it (`refresh()` on show, `dispose()` on leave); the `.inv-modeless-layer` holds the modeless popups, `closeCraft()`, `openDisassemble(uid)`, `canDisassemble`, `disassemblePanel`; the 수리 context entry renders `renderItemCost` chips. **2026-09-07**: 제작 left the modeless layer and is a column of `.inv-layout` again (`.is-craft`, `.inv-col-right`), and `showScreenTab(tab)` backs `InventorySystem.openScreen`. **2026-09-07 (안정화)**: the world-drop zone sits in a fixed-height `.inv-footer` so it appearing at drag start cannot re-centre the whole window (2026-09-14: it no longer shares that slot with anything), and `resolveGridTarget()` resolves a drag's grid **strictly first** — a grid that actually contains the pointer beats one that only sits inside its half-cell tolerance, so 가방 and 함선 창고 (stacked with a gap) stop stealing each other's edge rows. An **equipment slot → grid** drop whose exact cell is blocked now retargets to `sys.nearestFreeSpot` (highlight included) instead of snapping the weapon back into its slot with a shake; grid → grid keeps the strict Diablo rule. **2026-09-07 (2)**: `positionGhost` writes the ghost's 1.04 size lift into its own `transform` — as the standalone `scale:` property it was applied *before* the `transform` (CSS order: translate → rotate → scale → transform), which multiplied the translate and drew the item 42 px right of the cursor at x = 1080, further out the wider the window; the picture and the in-game cursor visibly came apart mid-drag |
| `ui/GridTools.ts` | **2026-09-15 3차 (사용자 결정)**: 필터 칩 줄이 **정렬 버튼 오른쪽의 드롭다운 하나**가 됐다 — `buildFilterChips` → **`buildFilterSelect`**(타입 `FilterChips` → `FilterControl`, 모양 `{ el, set(id) }` 은 같다), DOM 은 `.inv-filter-sel > select.inv-filter-select`(네이티브 `<select>` — 스크롤 상자에 안 잘리고 Escape 가 인벤토리 창까지 안 내려간다, `color-scheme: dark`). 항목의 원본은 그대로 `model.FILTER_GROUPS`(글리프 + 한국어 이름을 함께 적는다), 「전체」가 아니면 컨트롤에 `.is-on`. 옛 `.inv-filters` · `.inv-filter-chip` · `--inv-filter-n` 은 **삭제**. **2026-09-12** — 가방 · 창고 머리의 `정렬` 버튼(`buildSortButton`, `.inv-sort-btn`). Tab 인벤토리와 `TradeGrids` 가 같은 DOM · 같은 클래스를 쓴다. 상태 없음 |
| `ui/TradeGrids.ts` | **2026-09-15 3차 (사용자 결정)**: 창고 + 가방은 **한 패널의 두 칸**이다 — `layout` 의 두 값(`'wrap'`/`'split'`)이 같은 것을 그리고(옛 `'split'` 하나만 남았다), 칸마다 `.tg-gridwrap` 이 자기 스크롤러(`scrollbar-gutter: stable`)이며 **머리에 자기 정렬 버튼 · 자기 필터 드롭다운**이 선다. **창고 쪽 `함선 창고` 라벨은 숨는다**(`.tg-label` — 가방 쪽 `내 가방` 은 남는다), 블록 아래 따로 서던 칩 줄(`.tg-block > .tg-tools`)은 없어졌다. 바깥 틀(배경 · 테두리)은 여전히 **호출자의 카드**가 갖는다 — 그래서 창고 · 가방을 한 카드로 보이게 하려면 호출자가 `createTradeGrids(host, { grids: ['stash','bag'] })` 를 **한 번** 불러야 한다(카드마다 한 격자씩 두 번 부르면 카드가 둘이다). 다른 폴더 화면(기업 거래 · 재배 스테이션 · 분석기 · 배양조 · 식탁)에 끼우는 **진짜 가방 / 함선 창고 격자** — 읽기 + 끌어내기 전용(`onTake`). **2026-09-12 (2차, 사용자 결정)**: 기본 순서가 `['stash','bag']` 이고 `.tg-scroll` 이 **가로 2열**이다 — **창고 왼쪽 · 가방 오른쪽**, 폭이 모자라면 `flex-wrap` 이 가방을 아랫줄로 내린다(끼어드는 화면마다 열 폭이 달라 뷰포트 미디어 쿼리로 추측하지 않는다 — 다만 **세로가 되면 한 스크롤 안에서 둘 중 하나가 화면 밖으로 나가므로**, 그걸 감당 못 하는 화면은 스스로 해결한다: 가구 화면은 `.hs-inv` 에서 블록마다 자기 스크롤을 준다). 격자마다 **감싸는 상자 `.tg-gridwrap`** 이 하나 있다 — 기본값에서는 아무 일도 하지 않고, 세로 스크롤을 걸 화면이 그것을 쓴다(격자 자신은 폭이 px 로 못박혀 있어 스크롤바가 마지막 열을 갉아먹는다). `GridView.setClip` 도 그 상자를 가리킨다. 탄약 사선 띠도 여기서 갱신한다(`setNeededAmmoFrom`). **2026-09-12 (1차)**: 가방과 창고가 **한 스크롤**(`.tg-scroll`) 안에 붙는다(가방 5칸 · 틀 `BAG_FRAME_ROWS` / 창고 10칸), 칩 줄(`.tg-tools`)은 스크롤 밖 맨 위, 블록 머리마다 `정렬`(`sortGrid(id, isStaged)` — 이 뷰가 인벤토리를 바꾸는 유일한 동작), 블록에 `data-tg-grid="bag" \| "stash"`. 드래그는 pointermove 를 **rAF 한 번**으로 합쳐 고스트를 `transform` 으로 옮기고(크기는 집을 때 계산 — 이동 중 `offsetWidth` 를 읽지 않는다) `elementFromPoint` 도 프레임당 한 번, 고스트에 `filter` 없음. 버스 이벤트는 rAF 하나로 합쳐 갱신하고 `refresh()` 는 버전 게이트(바뀐 타일만 다시 그린다) + 타일 플래그(`is-staged` · `data-item-tip`)만 훑는다 |
...
| `ui/TipPin.ts` | **2026-09-14 — 툴팁 고정** (사용자 결정). `TipPin` 한 인스턴스가 누르고 있기 · 고정 카드 · 그 카드의 소켓 호버/끌어내기를 전부 갖고, 격자 모양 · 드롭 대상 · 고스트는 호스트가 `TipPinHost` 로 답한다(Tab 창 `InventoryUI` · `TradeGrids`). `beginHold(uid, x, y, onFire)` — 확정은 `UI_HOLD_CONFIRM_S` 타이머, 링은 rAF 로 `ui:cursorHold {owner, progress, x, y}`(**2026-09-15: 진행도 `RING_SHOW_AT` 0.25 에 닿은 뒤에만**) · 문턱을 넘게 움직이거나 떼면 `cancelHold`(링 `null`) · 다 차면 `onFire`(호스트가 아직 드래그가 아닌 누름을 버린다) 뒤 `pinAt`. 고정 카드 = 자기 `Tooltip` + `.is-pinned`(포인터를 받는다) + 마름모 `.inv-tt-pin`, 자리는 떠 있던 호버 카드의 `transform`(`pinAnchor`)이고 커서를 따라가지 않는다. 해제 = window capture `pointerdown` 이 카드 밖 · 마름모 · `ctx.escape`(`inventory.tipPin:<owner>`) · `unpin()`(호스트의 창 닫기 · 탭 전환) · `validate()`(아이템을 못 찾으면 — 서명이 바뀌면 같은 자리에 다시 그린다) · 다른 owner 의 `ui:tipPinned`. 소켓: `.inv-tt-sock.is-filled` 호버 = `host.hoverTip` 에 부착물 카드, `.can-detach` 누르고 문턱을 넘으면 `host.buildGhost` 고스트(`.inv-sock-ghost`, 초록/빨강) · `host.aimDetach` · 놓으면 `host.detach` → 효과음. 카드 위에서 놓으면 제자리. `inventoryTooltipLookups(sys, ctx)` = 두 호스트가 쓰는 툴팁 조회. 게터 `isHolding` · `isPinned` · `pinnedUid` · `isSocketDragging` |
| `ui/Tooltip.ts` | **2026-09-15 (가젯 개편)**: 실드 충전기 · 전투 소모품 · 회복 줄이 `@/items` 의 **`itemSpecRows`** 로 접혔고(칩 카드 `ui/hud/ItemTip` 과 같은 함수 · 맨 위는 언제나 `사용 시간`), 스펙 행 값이 `string | SpecSeg[]`(숫자만 본문 색 · 나머지 흐림), 설명이 **`parseItemText`** 인라인 마크업(`{em}` · `{dim}` · `{br}`), 내구도 게이지가 **가젯도** 받는다(돔 실드 · 바리케이드). `TEXT.shieldChargeStats` · `TEXT.boostStats` 는 읽는 곳이 없어졌다(`@deprecated`). **2026-09-15 3차**: 이 카드를 **제작 상세 패널이 그대로 붙여 쓴다** — `CraftPanel` 이 생성 직후 `className` 을 `.inv-tt-card is-embedded` 로 갈아 끼운다(`show()` 는 `className` 을 안 건드린다). **2026-09-12 (사용자 결정)**: ① **내구도는 게이지 한 줄**(`buildDurabilityBar` → `.inv-tt-durbar`, 무기 2×2 게이지와 같은 `.track` / `.fill` 을 전체 폭으로) — 무기 · 가방 · 방탄복 · 회복 스프레이가 같은 함수를 부르고, 그 아래 있던 `구간` 줄(C-37)과 `TooltipLookups.getDurabilityBucket` / `canSalvage` · `TEXT.durability.tooltip*` · `.is-bucket` 은 **전부 사라졌다**(구간 안내는 수리 · 분해 팝업이 계속 적는다). ② **가방**에 「소지 한계 +N kg」 한 줄(`Gear.bagCapacityBonus` — 무게 계산과 같은 식, 0 이면 생략). ③ **방탄복**의 `특성` 행은 설명 문단과 글자가 같으면 서지 않는다(유니크 description 이 곧 퍽 문장이라 같은 줄이 두 번 나왔다). Hover card (name, category · rarity, description; attachments: socket, 호환, effects; bags: grid + 퀵슬롯 + **내구도 (2026-09-11)**; **서적 (Phase 9)**: 스킬 (한국어 name via the new `TooltipLookups.getSkillName` → `ctx.progression.getSkillDef`, the raw id as fallback) + 용도 `서재 책장에 꽂으면 해당 스킬 XP 증가`; qty). **2026-09-09 무기 카드 재설계**: 대미지 · 연사 · 반동 · 사거리 are a **2×2 게이지 격자** (`.inv-tt-gauges`, bar = value ÷ the **catalog maximum** of that stat, computed lazily once from `TooltipLookups.allWeaponItemDefs()` × `getBaseStats(defId)` = `LootRef.getEffectiveStats(defId)`; damage = `damage × pellets`, range = `effectiveRange(weapon)`; the raw number stays small at the right). Two layers per bar: the bare def value in **white**, a socket surplus as a **green `.bonus`** segment, a socket reduction (muzzle brake on 반동) as a **hollow green `.reduced` outline** over the removed span. The head's right corner holds the **탄종 썸네일** (`.inv-tt-ammo`, `findAmmoDef(type)` → the `ammo` item's glyph in its colour + the calibre name as a caption inside; no ammo def → dashed square with the label alone). The five sockets are a **row of 34 px squares** (`.inv-tt-sock`: attachment glyph + rarity border, or dashed + `socketAbbr`; `title` = `조준경: 없음` / `총구: 소음기`). Kept rows: 장전 `n / max` (that is where the mag size lives now), 발사 모드, 배율, 내구도 (`is-low` / `is-broken`). **Gone for every item**: 크기 and 무게 rows — the bottom bar (`.inv-tt-value`) is two-ended, **무게 left** (stack total, only with `def.weight`) and **가치 right** |
| `ui/labels.ts` | **칸 크기의 단일 원본 (2026-09-14)** — `CELL` · `STEP` 은 `let` 이고 `gridCellForHeight(창 높이)` 가 정한다 (수치는 `data/tuning.csv` 의 `INV_CELL_*`: ≤ 800 px → 40 · ≤ 900 px → 46 · 그 위 54). `syncGridCell()` 이 옮기고 `applyGridCellVar(el)` 이 `--inv-cell` 을 인라인으로 적는다 — CSS 에는 미디어 쿼리가 없다. 자세한 것은 위 「격자 칸 크기의 원본은 `ui/labels.ts` 하나다」 절. Cell metrics (`GAP=2`), `SLOT_LABEL` / `SLOT_KEY`, `QUICK_DIR_GLYPH` (▲ ◥ ► ◢ ▼ ◣ ◄ ◤), `QUICK_ROSE_ORDER` (3×3 DOM order), Korean UI strings (hints, menu, quick panel, split dialog, stat labels, **`search`**: `?` / `???` / `감정 중 · n개 남음` / `감정 완료` / denied toast), formatters (`fmtDeg`, `fmtMul`, `gradeLabel`), `DURABILITY_LOW` (0.3). **Phase 8**: `TEXT.tabs` gained `ship` / the per-tab hints / `unavailable`, `TEXT.disassemble` (분해 dialog), `TEXT.modelessClose`, `TEXT.catalog.tabs.seed`, and **`TEXT.credits.value` returns the bare number** so the pill reads `CREDITS 500`. **Phase 9**: `TEXT.bookStats` (스킬 / 용도 / the 서재 책장 line) and `TEXT.catalog.tabs.book` (`서적`). **Phase 10**: `fmtValue` = `formatCredits` from `@/shared` (`1,200 C`, no `₩`) and `TEXT.credits.value` = `formatCreditAmount`. **2026-09-09**: `socketAbbr(slot)` (two-letter caption of an empty tooltip socket square) · `socketTip(slot, name?)` (`조준경: 없음` / `총구: 소음기`) · `TEXT.socketNone`. **2026-09-10**: `TEXT.durability`(내구도 구간 문구 — `repair(label, mul)` / `salvage(label, mul)` / 안내 두 줄; 배수는 `ctx.loot.durabilityBucketInfo` 에서 오고 여기에 숫자는 없다) 와 `TEXT.craftTabs`(작업대 탭의 `전체` · `빠른제작` 라벨 + 글리프. 작업대 **이름**은 `WORKBENCH_LABEL_KO` 가 원본이다) |
| `inventory.css` | **2026-09-15 3차 (사용자 결정)**: `.inv-panel-grids`(= 옛 `.inv-col-right`)가 **창고 + 가방을 담는 한 장의 카드** — 안쪽 두 패널은 테두리 · 배경 · 여백을 버리고 칸이 되며 사이에 세로선 하나, 장비 열은 그 카드 왼쪽에 이어 붙는다. 새 규칙 묶음: `.inv-filter-sel` / `.inv-filter-select`(필터 드롭다운), `.inv-craft-body` / `.inv-craft-list`(4칸 격자) / `.inv-craft-cell` / `.inv-craft-detail` / `.inv-craft-dhead` / `.inv-craft-dkind` / `.inv-craft-owned` / `.inv-craft-spec` / `.inv-tt-card`(붙인 툴팁 카드 틀 — `position`/`transform` 을 `!important` 로 눌러야 한다, `Tooltip.move()` 가 인라인 `transform` 을 쓴다). 삭제: `.inv-filters` · `.inv-filter-chip` · `.tg-block.is-narrow .inv-filters`. `.inv-panel-craft` 폭 564 → 700 px(≤ 1560 px 474 → 610 px). **2026-09-13**: 함선 창고 패널 = 가방 패널 높이 (인벤토리 탭은 창고 · 장비 · 가방 `align-self: stretch` + `.inv-stash-scroll { flex: 1 1 0px }`, 제작 4열은 `.inv-col-right { align-items: stretch }` + 옛 창고 높이를 `min-height` 로), `.is-catalog` 배치 (장비 열 · 퀵슬롯 · 주머니 숨김, 무한 상자 패널이 가방 높이로), 무한 상자 폭 528 px / ≤ 1560 px 3열 412 px + 간격 16 px / ≤ 1360 px 2열 296 px. Styles (imported by `InventoryUI.ts`); scoped under `.inv-*` (`.inv-quick*` for the wheel panel, `.inv-tile-quick` badge, `.inv-cat-*` catalog, `.inv-repair-*` bench repair list, **Phase 7** `.is-hidden-item` footprint mask, `.inv-tile-scan` bottom-to-top gauge (`::before` height = `--p`) under the moving sheen, `.is-scanning`, `.inv-search-status` (`is-done` / `is-paused`; **2026-09-07** a fixed `min-width` so 감정 중 ↔ 감정 완료 cannot resize the container panel), `.inv-footer` (fixed-height slot for the drop zone; collapsed in `.is-hub`), `.is-pending` pulse; **Phase 8** `.inv-screen` / `.inv-screen-note` embedded-screen host, `.inv-modeless-layer` / `.inv-modeless[-implant|-craft|-disassemble]` / `.inv-modeless-head` / `.inv-modeless-body` / `.inv-modeless-close` / `.is-centred`, `.inv-dis-*` dialog, `.inv-craft-costs`, `.inv-menu-line` / `.inv-menu-item.has-costs` / `.inv-menu-costs`), no dependency on `src/ui/styles` — **except** the `.item-chip*` rules of `buildItemChip` / `renderItemCost`, which `src/ui/styles/base.css` owns (Phase 8 contract). **2026-09-11 (C-60)** `.inv-cont-scroll` (+ `.is-scroll`): the container grid's vertical scroll viewport. **2026-09-14**: `.inv-root` 는 `justify-content: safe center` + **줄어드는 아래 균형 칸** `.inv-root::after`(`flex: 0 1000 <padding-top>`, `margin-top: -12px` 로 gap 상쇄) — 남는 높이가 있으면 창이 화면 세로 가운데에 서고, 꽉 차면 예전 `flex-start` 와 완전히 같다. 균형 여백을 `padding-bottom` 으로 주면 안 된다(줄어들지 않아 `.inv-layout` 이 대신 눌린다). 새 플래시 키프레임 `inv-slot-flash` · `inv-quick-flash` · `inv-implant-flash` (`.is-flash`); `.inv-hints` / `.inv-hint` 삭제 |
...
칸 한 변(px)은 **`ui/labels.CELL`** 이고, `STEP = CELL + GAP` 이 격자 상자의 px 폭 · 높이(`GridView.syncDims`) ·
드래그 히트테스트(`hitTest` · `cellForGhost`) · 하이라이트 자리 · 고스트 크기(`tileSize`)를 전부 정한다. CSS 의
`--inv-cell` 은 **같은 값의 복사본**이지 원본이 아니다 — 한쪽만 줄이면 놓은 자리가 커서 밑 칸이 아니게 된다.
그래서 `--inv-cell` 을 그리는 쪽마다 `applyGridCellVar(el)` 로 **인라인으로 덮어쓴다**: `GridView`(자기 격자, 크기가
기본값이어도 늘) · `InventoryUI.mount`(`.inv-root`) · `TradeGrids`(루트 + 고스트) · `CrewLoadoutView`(루트).
`inventory.css` · `meta.css` 에 남은 `--inv-cell: 54px` 선언은 JS 가 덮기 전의 기본값이고, **CSS 에는 이 값의
미디어 쿼리가 없다.**

`CELL` 은 **창 높이**를 탄다 (`gridCellForHeight`) — 사다리의 수치는 `data/tuning.csv` 의 `INV_CELL_*` 다:
높이 ≤ `INV_CELL_TINY_MAX_VH`(800) → `INV_CELL_TINY_PX`(40), ≤ `INV_CELL_SHORT_MAX_VH`(900) →
`INV_CELL_SHORT_PX`(46), 그보다 크면 `INV_CELL_PX`(54). **가로가 아니라 세로가 기준이다**: 1280×760 · 1440×900 에서는
창이 화면을 세로로 꽉 채워 `.inv-root` 의 `safe center` 가 `flex-start` 로 떨어졌고(= 위로 붙었다), 칸을 줄여 창을
짧게 만드는 것이 남는 높이를 만드는 유일한 길이었다. 1080 px 이상은 예전과 **바이트 단위로 같다**.

`syncGridCell()` 이 `CELL` · `STEP` 을 옮기고(ESM 라이브 바인딩이라 `import` 한 쪽도 새 값을 본다) 그것을 부르는 곳은
`InventoryUI` 둘뿐이다 — `mount()`(격자를 만들기 전에) 와 `onViewportResize`(창 크기가 계단을 넘으면 `.inv-root` 의
변수와 살아 있는 격자 넷에게 `GridView.setCell(CELL)` 로 나눠 주고, 보폭이 바뀌므로 끌던 것은 취소한다).
⚠ **`.inv-bag-scroll` · `.inv-cont-scroll` 의 `max-height` 예산은 한 줄도 안 건드렸다.** 그 식들은 창이 화면 안에
서는지를 정하는 상한이고, 칸이 작아지면 그 안에서 격자가 **덜 잘릴** 뿐이다 (1280×760 함선 가방: 숨은 406 → 238 px).
예산을 칸 크기만큼 늘려 주면 패널이 다시 화면을 꽉 채워 **균형이 도로 사라진다** — 그래서 일부러 그대로 뒀다.

⚠ 스모크에서 칸 중심을 계산할 때 `56` · `27` 을 적지 않는다 — 격자 요소의 `--inv-cell` 을 읽는다
(`smoke-inventory-p6` · `smoke-quickslots` · `smoke-tip-pin` 이 그 패턴이다).
...
규칙은 **`parts/AutoQuick.ts` 한 벌**이고, 거는 자리는 「밖에서 내 손에 들어왔다」 **세 지점**뿐이다:
...
2026-09-10 의 **「상자에서 찾은 것은 무조건 가방이 먼저다」** 와 부딪히지 않는다: 그 결정이 막으려던 것은 *주우면서
지금 든 총 · 방탄복이 조용히 바뀌는 것*(장비 칸)이고, 자동 장착은 **비어 있는 휠 칸만** 채운다. 자동으로 올라간
스택도 평소처럼 빼고(우클릭 `빠른 슬롯 해제` · 휠 칸 더블클릭) 옮길 수 있다 — 다른 규칙이 하나도 붙지 않는다.
...
- **2026-09-14 (계열별 소켓)**: `canAttach` now also refuses a socket the weapon class does not have (SG = muzzle · mag · sight, SMG + grip, DMR / SR muzzle · mag · sight, AR all). Saves from before are fixed **at load**, never mid-session: `Serialize.detachForbiddenSockets` + `Stash.load` (창고 문서 — the attachment goes into the same 창고) and `parts/SocketRules.returnForbiddenAttachments` (로드아웃 → 창고 → 가방, 레이드 blob → 가방). No room anywhere → the attachment stays socketed with no effect. No `inventory:socketChanged` is emitted (it runs before / inside the announce of a load).
...
### 2026-09-13 — 카드마다 격자 하나 (`layout` · `chips` · `setCell` · `mountFilterChips`)

기업 화면이 함선 창고와 가방을 **서로 다른 카드**로 세우면서 생긴 API 다 (가구 화면도 같은 방식으로 쓰면 된다).
옛 호출(`createTradeGrids(host, { grids: ['stash','bag'], dropSelector, onTake })`)은 **한 픽셀도 바뀌지 않는다** —
새 옵션을 안 넘기면 `layout: 'wrap'` · `chips: 'shared'` · `cell: 54` 이다.

```ts
import type { TradeGridsView, TradeGridsViewOptions } from '@/shared';

// 카드 하나에 격자 하나. 카드(= host)는 호출자의 것이다 — 배경 · 테두리 · 안여백 · 높이.
for (const id of ['stash', 'bag'] as const) {
  const card = el('div', { cls: `my-card ${id}`, parent: row });   // flex column, 높이가 정해진 상자
  const opts: TradeGridsViewOptions = {
    grids: [id],          // 이 뷰가 그리는 격자 (기본 ['stash', 'bag'] — 창고 왼쪽 · 가방 오른쪽)
    layout: 'split',      // 블록마다 자기 스크롤 (아래)
    chips: 'block',       // split 의 기본값 — 블록 머리 아래 자기 필터 줄
    cell: 40,             // 칸 한 변 px (기본 54). 살아 있는 뷰는 setCell 로 바꾼다
    dropSelector: '.my-drop', onTake: (item, gridId, target) => { /* … */ },
    isStaged: (uid) => false, takeLabel: '빠른 이동', className: 'my-tg',
  };
  const view = ctx.inventory!.createTradeGrids(card, opts);
  // ref 시그니처는 여전히 `EmbeddedView` 다 (계약은 추가만) — 좁혀서 쓴다
  if (typeof (view as Partial<TradeGridsView>).setCell === 'function') (view as TradeGridsView).setCell(34);
}
```
```css
.my-card { display: flex; flex-direction: column; min-height: 0; }      /* 행 안에서 stretch 로 높이를 받는다 */
.my-card > .trade-grids { flex: 1 1 auto; min-height: 0; }
```

- **`layout: 'split'`** — DOM 은 `.trade-grids.is-split > .tg-scroll > .tg-block[data-tg-grid] > .tg-head + .tg-tools? + .tg-gridwrap > .inv-grid`
  (`'wrap'` 과 같은 이름들이라 `[data-tg-grid]` · `.tg-gridwrap` 선택자가 그대로 통한다). `.tg-scroll` 은 틀도 스크롤도 없는
  `nowrap` 한 줄이 되고, 블록은 뷰 높이까지 늘어나며 **`.tg-gridwrap` 이 세로로 스크롤한다**(`scrollbar-gutter: stable` —
  격자는 폭이 px 로 못박혀 스크롤바가 마지막 열을 갉아먹으므로 감싸는 상자에 건다). 머리줄 · 칩 줄은 `width: 0; min-width: 100%`
  라 **블록 폭 = 격자 폭 + 스크롤바 자리**다 — 카드를 `flex: none` 으로 두면 카드 폭이 곧 격자 폭이다. 그래서 뷰에 **높이가 정해진
  부모**가 필요하다(위 CSS). 격자 둘을 한 split 뷰에 넣어도 되지만(나란히, 각자 스크롤), 카드를 나누려면 뷰 둘이다.
- **`chips`** — `'shared'`: 모든 블록 위 한 줄(모든 블록에 같은 필터, `'wrap'` 기본). `'block'`: 블록 머리 아래 자기 줄 · 자기 필터
  (`'split'` 기본). 격자 폭이 260 px 보다 좁은 블록(가방 5칸 · 40 px)은 `.is-narrow` 로 **고른 두 줄**이다(한 줄 칸 수 = 칩 수의 절반 올림, `--tg-chip-cols`). `'none'`: 뷰 안에 칩이
  없고, `view.mountFilterChips(host)` 가 모든 블록을 거르는 한 줄을 **호출자가 고른 자리**에 붙인다(다시 부르면 옮긴다, 뷰와 함께 지워진다).
- **`setCell(px)`** — 살아 있는 뷰의 칸 크기를 바꾼다. `GridView` 의 칸은 생성 때 고정이므로 블록마다 `GridView` 를 새로 짓고,
  **블록의 필터와 스크롤된 행**을 지킨다. 끄는 중이면 드래그를 취소하고 열린 메뉴를 닫는다. 같은 값이면 아무것도 안 한다. `view.cell` 이 지금 값.
- **`.tg-compact`** — 칸이 54 px 보다 작으면 루트에 붙는다. 타일 글리프 · 이름 · 수량 배지 · 소켓 핍이 `--inv-cell` 을 따라 줄어든다
  (`inventory.css` 의 TradeGrids 블록). 드래그 고스트는 `<body>` 에 붙으므로 자기 `--inv-cell` · `.tg-compact` 를 달고 다닌다.
- 검사: `scripts/smoke-meta.mjs`(기업 화면 — 카드마다 격자 하나 · 칸 크기 = `data-cv-cell`), `scripts/smoke-stations.mjs`(`'wrap'` 그대로).
...
- **스택 열쇠** — `Grid.stackKeyOf` 가 꽂힌 규칙(회수 계약 `'rf'`)의 열쇠에 **품질**(`|q<n>`, 0 = 덧붙이지 않음)을 늘 덧붙인다 — 규칙을 갈아 끼우지
  ...
  부분 드래그 · 카탈로그 · 상자 / 시체 격자 · `mergeCapacity(defId, incoming)`)가 품질이 다른 요리를 합치지 않는다. 타입은 `StackItem`
  (`RaidFoundItem` + `quality`). 합치기는 같은 품질끼리만 일어나므로 `snapshot` / `restore` 에 품질 칸은 필요 없다.
- **나누기 · 복사** — `splitItem` · `dropItemImpl`(일부 버리기) · `dropPartialImpl`(부분 드래그) · `dropQuickPartial`(휠 부분 드롭)이
  `copyMealQuality` 로 품질을 물려준다. `emitTransfer({ ...item })` · `Peek` · `fitCorpseGrid` 의 사본은 스프레드라 그대로 간다.
  **레이드가 끝나도 지우지 않는다** (`stripRaidMarks` 는 `raidFound` 만).
- **저장** — `Serialize.serializeExtras` 가 품질 > 0 이면 `SavedExtras.q` 를 직접 싣는다 → 창고 문서 · 로드아웃 문서 · 레이드 blob · 크루 카드
  모두. `reviveItem` 이 `normalizeMealQuality` 로 되살린다. 세이브 버전은 그대로다(없던 선택 필드).
- **와이어** — 픽업 `PickupWire.q`(`pickups/PickupSystem.wireOf` / `itemFromWire`) · 플레이어 시체 `CorpseItemWire.q`(`game/Corpses.itemsToWire` →
  `game/parts/CorpseNet.itemsFromWire` · 이 폴더의 `CorpseLoot.corpseItemsFromWire`). 생략 = 품질 0.
- **타일** — `buildTileContent` 가 `def.meal` 이고 품질 > 0 이면 **좌하단 `★n`**(`.inv-tile-quality[data-quality]`, 타일에 `.has-quality`,
  `title` = `★★★☆☆`). 수량(우하단) · 휠 방향(좌상단) · 사선 띠(우상단)와 모서리가 다르다. `tileSignature` 에 품질이 들어간다.
- **정렬** — `compareForSort` 가 이름 다음에 품질 높은 순 (같은 요리면 ★ 많은 스택이 앞).
- **우클릭 `먹기`** — `useMealItem` 이 그 스택의 품질을 `ProgressionRef.useMeal(defId, quality)` 에 넘긴다.
- **조리대 레시피는 일반 제작 경로에 없다** — `getRecipes` 는 `bench 'cook'` 레시피를 **조리대를 이름으로 물을 때만**
  (`getRecipes('ship', 'cook', lv)` — housing 조리대 화면의 요리 목록) 돌려주고, `canCraft` 는 언제나 false → `craft` 는 `craft:failed missing`.
  `openBenchCraft('cook', …)` 은 창을 열지 않고 토스트 `조리대에서 요리하세요`, `switchBench('cook')` 은 무시, 제작 열의 작업대 리스트에 조리대가 없다
  (주방 묶음이 비므로 주방 머리줄도 더는 나오지 않는다).
- **`cookBlock(recipeId, benchLevel)`** — 순서: `조리대 레시피가 아닙니다` → `조리대는 함선에서만 사용할 수 있습니다` → `조리대 Lv.n 이 필요합니다` →
  `제작 숙련 n 이 필요합니다` → 튜토리얼 `blockReason('craft', id)` → `재료가 부족합니다`(`craftCost`, **가방 + 창고** `countDefAll`) →
  `넣을 자리가 없습니다`(창고 → 가방, 시작 전에는 품질을 모르므로 **기존 스택에 합치지 않는다고 보고** 새 칸을 요구한다).
- **`completeCook(recipeId, benchLevel, quality)`** — `cookBlock` 을 다시 보고, 그 품질로 자리를 한 번 더 dry-run 한 뒤에야 뺀다. 재료 = 재료마다
  `consumeDefAll`(가방 작은 스택부터 → 주머니 → 휠 → 창고). 산출 = `outputQty` 1회분(품질) + `extraOutputs`(품질 없음), 덩어리마다 **창고
  `autoPlace` → 가방 `autoPlace`**. 돌려주는 `item` 은 그 덩어리를 실제로 들고 있는 스택(합쳐졌으면 같은 열쇠 스택)이고 `landed` 는 첫 덩어리의 자리.
  `inventory:itemAdded` · `craft:completed {recipeId, item, count: 1}` · `afterChange`. `craft_done` 소리는 내지 않는다(housing 이 `cook_finish`).
  실패면 `{item: null, landed: null, reason}` 이고 아무것도 빠지지 않는다.

## 변경 이력

- **2026-09-15 (전설 유니크의 종류 표기, 사용자 결정)** — `ui/labels.weaponClassLabel(w)` 가 유니크면 `UNIQUE_WEAPON_LABEL_KO[w.unique]`
  (화염방사기 · 전격총 · 표창 · 컴포짓 보우 · 바주카 · 미니건)를, 아니면 예전 계열 라벨을 돌려준다 — csv `class`(AR / DMR / SMG / SR)는 사격 숙련만 고르고
  화면에 나오지 않는다. `ui/Tooltip` 의 부제가 유니크 무기에서 `무기 · 전설` 대신 **`컴포짓 보우 · 전설`** 이다(이름이 별명 한 단어가 되므로 종류는 여기서 읽힌다).
  일반 등급 무기 · 다른 아이템 부제와 부착물 `호환` 줄(계열 라벨)은 그대로. 같은 규칙의 HUD 쪽은 `ui/hud/WeaponPanel.weaponTypeLabel` · `hud/ItemTip`.

- **2026-09-15 (키 가이드 · 우클릭 메뉴의 마우스 그림, 사용자 결정)** — `ui/InventoryUI.guideKeys()` 의 `우클릭 메뉴` · `휠클릭 요청` 이
  글자 대신 `keyLabel('Mouse2')` · `keyLabel('Mouse1')`(= `RMB` · `MMB`)을 넘겨 키 가이드가 공용 마우스 그림으로 그린다(둘 다 리바인딩과
  무관한 실제 버튼이라 `Keys` 가 아니다). `Shift + 드래그 절반` · `Ctrl + 드래그 하나` 에서 `combo: ['드래그']` 를 빼 `Shift 절반` · `Ctrl 하나`.
  `ui/ContextMenu` 의 `hint` 가 `LMB` · `MMB` · `RMB` 면 `shared/keycap.mouseGlyphSvg` 를 `kbd.inv-menu-kmouse` 안에 그리고(`<title>` 이
  라벨을 들고 있어 `textContent` 는 `MMB`), `ui/parts/ContextMenu` 의 요청 항목 두 곳이 `'휠클릭'` 대신 `keyLabel('Mouse1')` 을 넘긴다.
  CSS `inventory.css` `.inv-menu-item kbd.inv-menu-kmouse`.
- **2026-09-15 (빠른 이동 뒤 남는 툴팁 · 고정 홀드 링 문턱, 사용자 결정)**
  - **버그 「시체 창에서 더블클릭으로 옮긴 아이템의 툴팁이 마우스를 움직일 때까지 남는다」.** 원인: 호버 카드는 타일의 `pointerleave`
    로만 내려가는데, 빠른 이동(더블클릭 · 우클릭 메뉴 · 자동 배치)이 아이템을 옮기면 `refresh()` 가 **커서 아래의 타일을 DOM 에서 떼어
    낸다**(`GridView` `el.remove()` · 장비칸 / 휠 칸 `innerHTML = ''`). 떼어 낸 요소에는 브라우저가 `pointerleave` 를 보내지 않으므로
    `hovered` 가 없는 타일을 가리킨 채 카드가 떠 있었다(시체 창에서의 더블클릭은 거의 늘 아이템을 다른 칸으로 보내서 도드라졌다).
    고침: `InventoryUI.validateHover()` — `refresh()` 끝과 `result()` 다음 프레임에, 호버 중인 아이템이 그 자리에 아직 있고 그 타일 요소가
    연결돼 있고 사라지는 중이 아니며 **마지막 포인터 위치(창 루트 capture `pointermove`) 아래에 그 타일이 있는지** 확인하고, 아니면
    `hoverLeave()`. 격자 · 장비칸 · 휠 · 주머니 · 상자/시체 창 전부가 같은 경로다. `Tooltip.isShowing` 게터 추가.
    `TradeGrids`(기업 · 스테이션 격자)의 카드는 `ui/hud/ItemTip` 이라 거기서 같은 확인을 한다(ui README).
  - **`ui/TipPin` — 고정 홀드 링은 진행도 25 % 부터.** `RING_SHOW_AT` 0.25 미만에서는 `ui:cursorHold` 를 내지 않고(더블클릭 · 드래그
    시작마다 링이 번쩍이지 않게), 닿은 뒤로 실제 진행도를 그린다. 링을 한 번도 안 올린 홀드의 `cancelHold` 는 `null` 도 보내지 않는다.
    확정 시각(`UI_HOLD_CONFIRM_S`) · 시설 관리 이동 홀드(`housing:moveHold`)는 무변경.
...
- **2026-09-14 (작은 화면에서 격자 칸 축소 — 마무리 G, 사용자 결정)** — 위 「격자 칸 크기의 원본은 `ui/labels.ts` 하나다」 절 전부.
  ...
  `applyGridCellVar()`), 창 높이별 사다리를 `data/tuning.csv` 의 `INV_CELL_PX` · `INV_CELL_SHORT_PX`(46) ·
  `INV_CELL_TINY_PX`(40) · `INV_CELL_SHORT_MAX_VH`(900) · `INV_CELL_TINY_MAX_VH`(800) 로 뺐다.
  바뀐 파일: `ui/labels.ts`(위 다섯 줄 + `gridCellForHeight` · `syncGridCell` · `applyGridCellVar`, `CELL` · `STEP` 이
  `const` → `let`) · `ui/GridView.ts`(`cell` · `step` 이 `readonly` 가 아니게 되고 새 `setCell(px)`, 생성자가 `--inv-cell` 을
  **늘** 적는다) · `ui/InventoryUI.ts`(`mount` 의 `syncGridCell()` + `applyGridCellVar(root)` + `resize` 리스너
  `onViewportResize`, `dispose` 에서 뗀다) · `ui/CrewLoadoutView.ts`(루트에 변수) · `inventory.css`(`.inv-root` 주석만) ·
  `scripts/data-owners.mjs`(`labels.ts` 가 tuning.csv 를 읽는 모듈이 됐다 + `tuning.csv → inventory`).
  **효과(헤드리스 실측, 함선 Tab 창 위/아래 여백)**: 1280×760 76/2 → **76/44**(가방 격자에 숨은 줄 406 → 238 px,
  무한 상자 가로 스크롤 39 → 0), 1440×900 76/2 → **76/26**(숨은 줄 266 → 170, 무한 상자 70 → 0, 레이드 창 10/3 → 20/18,
  상자 루팅 10/3 → 20/18), **1920×1080 은 한 픽셀도 안 바뀌었다**(95/93 · 제작 192/190 · 레이드 62/60).
  1280×760 의 위 76 px 은 `.scr-tabs` 자리라 더 줄일 수 없다 — 남는 높이는 전부 아래로 간다.
  **드래그 검증**: 세 해상도 × (1×1 · 마지막 열 · 3×2 무기) 드롭이 전부 커서 밑 칸에 떨어진다(18/18), 창을 연 채
  1080 → 760 으로 줄였다 늘려도 CSS 변수 · JS 칸 · 상자 px 이 함께 움직이고 드래그도 맞는다.
  **스모크**: `smoke-inventory-p6`(가방 틀 높이 · 제작 썸네일) · `smoke-quickslots`(C-60 시체 스크롤 — 10행짜리 시체가
  작은 칸에서는 더 이상 넘치지 않아 **196개 = 20행**으로 키웠다) · `smoke-tip-pin` 의 칸 중심 계산에서 `56` · `27` 을 걷어내고
  격자의 `--inv-cell` 을 읽게 했다. 상자 패널은 이제 격자보다 머리줄(제목 + `모두 가져가기`)이 넓을 수 있어
  C-60 의 「viewport 가 격자 상자와 정확히 같다」 는 **높이만** 본다.
...
- **2026-09-14 (HUD · 인벤토리 소소 개선, 에이전트 B · 사용자 결정, docs/DECISIONS.md 「2026-09-14 — 정보상」)** — 셋 다 국소 수정이고 새 파일은 없다.
  ...
  두 번 한다). DOM 생성 · `buildHints()` · 마운트 · CSS 규칙(`.inv-hints` · `.inv-hint` + `.is-dragging` ·
  `.is-quick-source` · `.is-catalog-drag` 세 파생)을 전부 지웠다. 겹치지 않던 항목 중 **눈에 보이지 않는 수식 키**
  `Shift + 드래그 절반` · `Ctrl + 드래그 하나` 만 `guideKeys()` 로 옮겼다(`KeyGuideEntry.combo`).
  옛 줄의 `드래그→무기`(부착) · `드래그→퀵슬롯`(등록)은 **일부러 뺐다** — 끌고 가면 그 칸이 초록으로 켜져 스스로
  알려 주는 조작이고, 아홉 항목이면 한 줄(`white-space: nowrap`)이 1280 px 화면 가로를 넘긴다.
  `.inv-footer` 는 남는다 (드래그 시작에 창이 튀지 않게 버리기 영역 높이를 예약하는 자리다).
  ⚠ `scripts/smoke-controls-hub.mjs` 의 `hints` 검사를 **「요소가 없다」**로 고쳤다 (`querySelector('.inv-hints').hidden` 은 이제 TypeError).

  ② **창의 세로 자리** (`inventory.css` — 「가방이 너무 위로 가 있다」). 2026-09-12 의 `justify-content: flex-start` +
  `padding-top` 은 남는 높이를 **전부 아래로** 보낸다 — 1920×1080 함선 창은 위 76 / 아래 112 px, 작업대 제작 창은
  위 76 / 아래 285 px 였다. 그렇다고 예전 `center` 로 되돌리면 창이 화면보다 길 때 **위쪽도** 밀려 `.scr-tabs` 밑으로
  들어간다(그것이 2026-09-12 에 `flex-start` 로 간 이유다). 그래서 **`safe center`** — 자리가 있으면 가운데,
  넘치면 `flex-start` 와 완전히 같다. 위 여백만 있으면 가운데가 그 절반만큼 내려가므로 아래에도 같은 높이를 두는데,
  **`padding-bottom` 으로 주면 안 된다**: padding 은 줄어들지 않아 flex 가 `.inv-layout` 을 대신 찌그러뜨린다
  (실측 — 1280×760 함선 패널 669 → 596 px, 장비 열이 눌리고 창고 줄이 사라졌다). 대신 **줄어드는 칸**
  `.inv-root::after { flex: 0 1000 <위 여백>; margin-top: -12px }` 을 둔다 — `flex-shrink: 1000` 이라 모자란 높이를
  (거의) 전부 이 칸이 받고 `.inv-layout` 몫은 1 px 미만이며, 음수 `margin-top` 이 `.inv-root` 의 `gap: 12px` 을 상쇄한다.
  **높이 예산(`.inv-bag-scroll` 의 `max-height`)은 한 줄도 안 고쳤다** — 그 식들은 상한이고, 넘치는 순간 `safe` 가
  `flex-start` 로 떨어지므로 2026-09-12 에 재 둔 1280×760 수치가 그대로 산다.
  **헤드리스 실측** (1280×760 · 1440×900 · 1920×1080 × 함선 인벤토리 · 캐릭터 탭 · 무한 상자 · 작업대 제작 ·
  레이드 인벤토리 · 상자 루팅): 패널 높이는 **전부 변경 전과 같고**(bag 668 · equip 620 · stash 808 …) 화면 밖으로
  나가는 패널이 하나도 없다. 위/아래 여백은 1920×1080 함선 76/112 → **95/93**, 제작 76/285 → **182/180**,
  레이드 10/112 → **62/60**. 창이 화면을 꽉 채우는 배치(1280×760 · 1440×900)는 예전과 **완전히 동일**하다.

  ③ **상자 · 시체 폴백 배치는 토스트 대신 플래시** (`parts/DropResolver.ts` · `ui/InventoryUI.ts` · `inventory.css`).
  `tryAutoPlace` 가 문구(`WENT_TO` / `SENT_TO`) 대신 **「놓였다」 콜백**(`OnPlaced` — `PlacedAt` 자리 + 라벨)을 받는다:
  창고 더블클릭은 `notifySentTo`(예전 `<이름> → <어디>` 토스트 그대로 — **의도한 이동이다**), 가방이 꽉 차 밀려난
  폴백은 `flashPlaced` → `InventoryUI.flashSlot(slot)` / `flashQuick(index)` / `flashImplant()`. 새 클래스 `.is-flash` 와
  키프레임 `inv-slot-flash` · `inv-quick-flash` · `inv-implant-flash`(0.62 s) 이고 `animationend` 에 스스로 뗀다
  (+ 900 ms 보험 타이머). 드래그 프리뷰의 `is-target-ok` 를 **재사용하지 않는다** — `Drag.updateDragTarget` 이 매 프레임
  그 클래스를 지우므로 플래시가 첫 프레임에 사라진다. 임플란트는 `equipImplant` 가 어느 칸인지 안 알려 주므로
  블록(`.inv-impitems`) 전체가 번쩍이고, 그 칸만 **배경 · 글로우**로 그린다(테두리를 건드리면 `border-top` 이 사라진다).

- **2026-09-14 (준비 중 로드아웃 읽기 전용 — 발사 슬롯 UI 대개편, 에이전트 A · 사용자 결정)** —
  `InventorySystem.readOnlyReason(): string | null` 한 곳이 **「왜 지금 장비를 바꿀 수 없는가」** 를 답한다(null = 바꿀 수 있다).
  지금 그 사유는 하나 — 발사 슬롯에 앉아 **준비까지 마친** 동안(`HubSystem.launchReady` = `boardedSlot >= 0 && readyLocal`).
  Tab 창은 그대로 열리고(분대원 장비를 들여다보는 것은 계속 된다) **변경만** 거절된다: 드래그(`drop` · `dropPartial` ·
  `quickMove` · `activate` · `rotateItem` · `attachFrom` · `takeAll` · `takeOne` · `changeBag`), 장착/해제(`equip` ·
  `unequipToStorage` · `equipFromStorage` · `applyLoadout`), 퀵슬롯(`setQuickSlot` · `registerQuick` · `unregisterQuick` ·
  `mergeIntoQuickSlot` · `dropQuickPartial`), 나누기 · 버리기 · 정렬 · 창고 이동, 제작 · 분해 · 수리(`craft` · `canCraft` ·
  `openDisassemble` · `repair` · `repairWeapon` · `benchRepairAll`), 소켓(`attachToWeapon` · `detachSocket` ·
  `detachAllSockets` · `unloadWeapon`), 준비물 · 식사 사용, 개발 카탈로그. 미리보기(`previewDrop` · `previewPartial` ·
  `previewAttach` · `previewQuickPartial` · `previewDetach` · `previewCatalog`)는 토스트 없이 `'bad'` 라 드래그 고스트가
  빨갛다. 거절 토스트는 `READY_LOCK_TOAST_S`(1.5 s)마다 한 번 — 드래그 중 격자가 초당 수십 번 묻는다.
  ⚠ **`isItemLocked` 는 건드리지 않았다** — 그것을 켜면 툴팁 · 우클릭 메뉴까지 죽는데, 읽는 것은 계속 돼야 한다.
  상태는 `ctx.hub` 의 **선택 필드**(`launchReady`)로 읽는다 — `HubRef` 에 아직 그 필드가 없어서다(계약 추가는 리드 몫).

- **2026-09-14 (툴팁 고정 · 고정 툴팁에서 소켓 꺼내기 · 내구도 게이지, agent-invtip — 사용자 결정)** — 네 가지.
  ① **툴팁 고정** — 새 `ui/TipPin.ts`(위 표). 가방 · 창고 · 장비칸 · 휠 · 주머니 · 상자/시체 창 타일(`parts/Drag.beginPress` 가 `pin.beginHold`)과 `TradeGrids` 타일에서
  움직이지 않고 `UI_HOLD_CONFIRM_S` 누르면 커서 링(`ui:cursorHold` → `ui/hud/CursorHoldGauge`)이 차고, 다 차면 카드가 제자리에 고정된다(`.inv-tooltip.is-pinned` + 우상단 마름모
  `.inv-tt-pin`). 문턱(`DRAG_THRESHOLD`)을 넘게 움직이면 평소 드래그(`startDrag` 가 `cancelHold`), 먼저 떼면 평소 클릭. 해제 = 카드 바깥 누르기(누름은 그대로 아래로 간다) · 마름모 ·
  Escape(`ctx.escape` 맨 위) · 창 닫기(`InventoryUI.hide`) · 탭 전환(`setTab`) · 아이템 사라짐(`validate` — `refresh` · 인벤토리 이벤트) · 다른 곳의 새 고정(`ui:tipPinned`).
  결정: 고정한 채 **다른** 아이템에 올리면 평소 호버 카드가 뜬다(비교), 고정한 그 타일에서는 복사본을 띄우지 않는다(`hoverEnter`). `TradeGrids` 는 이제 누르는 순간 고스트를 들지 않고
  문턱을 넘을 때 든다(`press` → `liftGhost`), 고정 카드는 인스턴스 카드(`Tooltip`)를 `ctx.uiRoot` 에 `.tg-tip` 으로 띄우고 그 타일에 `data-tip-pinned` 를 달아 `ItemTip` 을 뺀다.
  ② **고정 무기 카드의 소켓** — 썸네일 호버 = 그 부착물의 카드, 끌어서 가방 · 창고 · 주머니 칸이나 (레이드의) 버리기에 놓으면 새 `parts/SocketDetach.detachSocket`(위 표).
  함선 바탕에 놓으면 아무 일도 없다(바닥이 없다 — 창고 칸을 가리키면 된다). 상자 · 시체 안 무기는 호버만(`canDetachSockets` false → `.can-detach` 없음). 칸 판정은 `parts/Drag.aimDetach`
  (Tab 창 — 상자 격자 제외) · `TradeGrids.aimDetach`. ③ **받는 소켓만 · 가운데** — `ui/GridView.shownSockets(item, stats)`(= `EffectiveWeaponStats.sockets` + 받지 않는 칸에 아직 든
  부착물)가 타일 핍 · 장비칸 핍 · 툴팁 소켓 줄(`justify-content: center`, 빈 칸은 `SOCKET_LABEL_KO` 전체 이름)을 정한다. ④ **내구도 게이지** — `buildTileContent` 가 무기뿐 아니라
  `durabilityMax` 가 있는 **모든** 아이템(방탄복 · 가방 · 스프레이 …)에 바닥 게이지를 단다(`durabilityMaxOf`), 채움은 `i.inv-dur-fill`, 색은 `setDurabilityColorVars` 가 비율로
  초록 → 노랑 → 주황 → 빨강(`inventory.css :root --dur-c-*`) 사이를 고른다. 장비칸 카드의 `.inv-slot-durnum`(`120/300`)은 없어졌다 — 숫자는 툴팁만. 툴팁 내구도 막대도 같은 색.
  결정: **가방은 0 이어도 `is-broken` 을 달지 않는다**(빈 게이지만 — C-36 「0 이어도 효과 없음」, 툴팁이 `파손` 이라 적지 않는 것과 같은 이유; 무기 · 방탄복 · 스프레이는 예전처럼 빨간 파손 모습).
  드래그 고스트가 무기 스탯을 받아 핍 · 게이지를 유지한다. 옛 두 번째 `.inv-tile-dur` 규칙 묶음(`i` 자식 변형)은 하나로 합쳤다. `CraftPanel` 산출 썸네일은 stats 없이 그리므로
  무기 산출물에는 게이지가 없고 방탄복 · 가방 산출물에는 새것 게이지가 선다. 검사 `scripts/smoke-tip-pin.mjs`(새), `smoke-controls-hub` 는 숫자 대신 게이지 `data-ratio` 를 본다.

- **2026-09-14 (총기 밸런스 — 로드할 때 규칙 밖 부착물 떼기, agent-gunbal)** — 사용자 결정 "로드할 때 떼어서 돌려주기". `Serialize.ts`: `detachForbiddenSockets(weapon, getDef, loot)`
  (규칙 밖 · 제 칸이 아닌 부착물을 떼어 `{socket, item}` 목록으로). `Stash.ts` `load`: 창고 속 무기에서 뗀 부착물을 **저장된 아이템이 전부 자리를 잡은 뒤** 같은 창고에 넣고,
  자리가 없으면 무기에 되돌린다(효과 없음), 하나라도 옮겼으면 창고를 dirty 로 남긴다. 새 `parts/SocketRules.ts` `returnForbiddenAttachments` — 호출 세 곳:
  `Lifecycle.restoreLoadoutSave`(창고 → 가방, + `loadoutStore.markDirty('sockets')`) · `ProfileDocs.applyProfileDocs`(같음, `saveNow('profile')` **뒤에** 표시 — 그 저장이 대기 중인
...
- **2026-09-13 (요리 미니게임 §6-2 — 요리 품질 스택 · 저장 · 와이어 · 조리 API)** — 위 절 전부. 새 파일 `parts/MealQuality.ts`.
  ...
  `parts/Crafting.ts`(조리대 제외 · `cookBlock` · `completeCook`) · `ui/CraftPanel.ts`(작업대 리스트에서 조리대 제외) · `ui/GridView.ts`(★ 배지 · 서명) ·
  `inventory.css`(`.inv-tile-quality`) · `scripts/smoke-inventory-p6.mjs`(주방 묶음 검사 → 토스트 · 목록 제외 검사로 교체, 새 9절 「요리 품질」).
  **결정**: 조리 재료는 함선 작업대 제작 코드(가방만)가 아니라 `parts/Crafting.ts` 머리의 「함선에서는 가방 + 함선 창고」 를 따른다 — 조리대 화면이
  창고 · 가방 카드를 나란히 보여 주고 수확물이 창고로 들어오기 때문. **알려진 것**: 산출물 자리 부족(`넣을 자리가 없습니다`)은 스모크가 만들지 않는다.
  **후속 (리드 요청)**: 격자 툴팁 `ui/Tooltip.ts` 가 `ui/hud/ItemTip` 과 같은 요리 줄을 갖는다 — `구분`(티어) · `품질 ★★★☆☆ +15 %`(인스턴스 품질 > 0,
  `.v.is-quality`) · `사용` · 능력치 줄(× `1 + mealQualityBonus`) · `조리 ① 썰기 → ② 젓기`(`cookStepsOf`, 없으면 줄 없음). 포맷은 `hud/mealText` 를
  import 할 수 없어 파일 안에 작게 한 벌(`mealAmountText` · `mealEffectsFor` · `cookStepsLine`) — 규칙이 바뀌면 둘을 같이 본다. 스모크 9절에 툴팁 3줄.
  `meta/parts/Trade.restoreSold`(판매 거절 되돌리기)는 확인만 했다 — `snap` 이 인스턴스 전체의 JSON 사본이고 새 uid · qty 만 덮어써 품질 · 내구도 ·
  `raidFound` 가 이미 산다 (고칠 것 없음).

- **2026-09-13 (창고 높이 · 시설별 작업대 리스트 · 무한 상자 배치 — 사용자 결정)** — 세 가지.
  ① **창고 패널 = 가방 패널 높이.** 함선 인벤토리 탭에서 창고 · 장비 · 가방이 `align-self: stretch` 로 한 줄 높이를 받고
  `.inv-stash-scroll` 이 `flex: 1 1 0px` 이라 창고 24행이 줄을 밀어 올리지 않은 채 남는 높이를 채운다 (레이드 무관, `.is-hub` 한정).
  제작 4열에서도 `.inv-col-right` 가 `stretch` 이고 창고 격자는 옛 높이(`min(560px, 100vh − 330px)`)를 바닥으로 두고 늘어난다 —
  1920 / 1600 / 1440 / 1280 모두 두 패널 높이가 같다(실측).
  ② **제작 열의 작업대 리스트는 같은 시설의 작업대만.** `ui/CraftPanel.benchFacility(kind)` 가 `@/shared` 의 `FURNITURE_DEFS` 에서
  `benchKindOf(interaction) === kind` 인 가구의 `room` 을 읽는다 (원본 = `data/furniture.csv`, 인벤토리에 두 번째 표 없음, 은퇴 · `any` 제외,
  모르는 kind 는 작업실). 작업실 = 총기 · 장비 · 가젯 · 의학 · 가공 + 맨 위 `빠른제작`, 연구실 = 추출기 · 조합대 · 3D 프린터, 주방 = 조리대.
  작업대 없이 연 제작 열(가방의 `제작`)은 작업실 묶음이고, 연구실 · 주방 묶음에는 빠른제작이 없다. 항목이 하나뿐인 시설도 리스트가 보이고
  (`benchesEl.hidden` = 레이드일 때만), 머리의 영문 줄이 시설을 말한다 — `TEXT.bench.eyebrow(facility)` → `WORKSHOP / LAB / KITCHEN BENCH`.
  쓰이지 않던 `TEXT.craftTabs.all` · `icon.all` 삭제.
  ③ **무한 상자가 열린 동안에는 무한 상자 · 함선 창고(함선) · 가방만.** `parts/Screens.setCatalog` 가 `.inv-root` · `.inv-layout` 에
  `.is-catalog` 를 걸고(`InventoryUI.show` 도 같은 함수를 지난다) CSS 가 장비 열 · 퀵슬롯 로즈 · 주머니 격자를 감춘다. 감춘 칸은 드롭 대상이
  아니다 — `Drag.updateDragTarget` · `Screens.updateCatalogTarget` 이 장비칸 · 퀵슬롯을 건너뛰고 `activeViews()` 가 주머니를 뺀다.
  무한 상자 패널도 가방 높이로 늘고(`.inv-cat-scroll` 이 채운다) 가방 격자는 로즈가 없으니 넓은 배치 높이 예산을 쓴다. 가로 스크롤을 없애려고
  패널 폭을 격자에 맞췄다 — 기본 528 px(옛 486 px 는 4열의 마지막 열을 잘랐다), ≤ 1560 px 3열 412 px + 패널 간격 16 px, ≤ 1360 px 2열 296 px.
  하단 안내 줄(`TEXT.catalog.hint` + `.inv-stash-hint` 요소)은 없앴다. 훈련장 무기 거치대도 같은 `openCatalog` 라 같은 배치다.
  바뀐 파일: `ui/CraftPanel.ts` · `ui/labels.ts` · `ui/CatalogView.ts` · `ui/parts/Screens.ts` · `ui/parts/Drag.ts` · `ui/InventoryUI.ts` ·
  `inventory.css` · `scripts/smoke-inventory-p6.mjs`(작업실 묶음 6개 · 연구실 3개 · 주방 1개 · 머리 줄 · 무한 상자 배치 · 복원).
  **알려진 것**: 제작 4열은 1440 급에서 여전히 약 70 px 가로 스크롤이다(이번 범위 밖, 기존 동작).
...
- **2026-09-10 (상자 더블클릭 · 퀵슬롯 교체 · 제작 정렬)** — 플레이 피드백 3건. 전부 국소 수정이고 새 파일은
  ...
  `ui:notify` 가 말한다 (`가방이 가득 찼습니다 — <이름> → 주무기 II`). — **2026-09-14 부터는 토스트가 아니라
  그 칸의 플래시다** (사용자 결정, 아래 2026-09-14 줄). 자세한 규칙은 위 `Equipment slots` 절.
...
## 변경 이력 — 2026-09-15 3차 (사용자 결정): 창고+가방 통일 · 필터 드롭다운 · 제작 UI 전면 개편

### 1. 창고 + 가방 = **한 장의 패널** (어느 화면에서든)

- **Tab 인벤토리**: 옛 `display: contents` 껍데기 `.inv-col-right` 가 **진짜 카드**(`section.inv-panel.inv-panel-grids`)가
  됐다. 안에서 `.inv-panel-stash`(order 0) · `.inv-panel-bag`(order 2)이 가로로 서고 — **창고 왼쪽 · 가방 오른쪽**
  (2026-09-12 결정 그대로) — 안쪽 두 패널은 자기 테두리 · 배경 · 여백을 버리고 칸 역할만 한다
  (`.inv-panel-grids > .inv-panel { background:none; border:0; padding:0 }`, 사이에 가는 세로선 하나).
  **장비 열은 그 카드의 왼쪽**에 붙는다 — 예전에는 창고와 가방 **사이**에 끼어 있었다.
- **끼워 넣는 격자(`ui/TradeGrids`)**: `layout` 의 두 값이 같은 것을 그린다 — 옛 `'split'` 하나만 남았다.
  블록마다 `.tg-gridwrap` 이 자기 스크롤러이고(`scrollbar-gutter: stable`) 머리에 **자기 정렬 버튼 · 자기 필터**가 선다.
  ⚠ 2026-09-12 의 경고는 그대로 유효하다 — **두 격자를 한 스크롤에 세로로 이어 붙이지 않는다** (창고 24행 ·
  가방 틀 12행이 600 px 스크롤 창을 나눠 쓰면 언제나 한 쪽이 화면 밖이고, 끄는 중에는 스크롤할 수 없다).
- **창고 쪽 라벨 제거**: Tab 창의 `STASH` eyebrow + `함선 창고` 제목이 없어졌고(`머리 = 개수 · 정렬 · 필터`),
  `TradeGrids` 의 `함선 창고` 라벨도 숨는다. 가방 쪽 `내 가방` 라벨은 남는다.
- **스크롤 · 정렬은 칸마다 따로** 작동한다 (필터는 Tab 창에서 두 칸이 같은 `filterGroup` 을 공유한다 — 옛 규약 그대로).

### 2. 필터 = **정렬 버튼 오른쪽의 드롭다운**

- `ui/GridTools.buildFilterChips` → **`buildFilterSelect`** (`FilterChips` → `FilterControl`, 모양 `{ el, set(id) }` 은 같다).
  항목의 원본은 그대로 `model.FILTER_GROUPS` 이고 글리프 + 한국어 이름을 함께 적는다.
- **네이티브 `<select>`** 다 (`.inv-filter-sel > .inv-filter-select`). 직접 그린 목록은 ① 스크롤 상자에 잘리지 않으려면
  `position: fixed` 로 띄워야 하고 ② 바깥 클릭 · Escape 를 스스로 먹어야 하는데 그 Escape 는 인벤토리 창을 닫는 키와
  같은 키다. 네이티브 목록은 셋 다 브라우저가 하고 `color-scheme: dark` 로 어둡게 그려진다.
  「전체」가 아닌 항목이 걸리면 컨트롤에 `.is-on` 이 붙어 **어느 격자가 걸러져 있는지**가 보인다.
- 옛 칩 줄(`.inv-filters` · `.inv-filter-chip` · `--inv-filter-n` · `--tg-chip-cols`)과 그 CSS 는 **삭제**됐다.
- 가방 쪽 `모두 수리`(`.inv-repair-open-btn`)는 `.inv-bag-tools` 안에 그대로 있고, 그 줄이 격자 위에서 **가방 머리**로 옮겨 갔다.

### 3. 제작 UI 전면 개편 — **왼쪽 조합 목록 + 오른쪽 상세**

- `.inv-craft-body` 안에 **`.inv-craft-list`**(산출물 썸네일만의 격자, 가로 `CraftPanel.CRAFT_LIST_COLS` = 4칸,
  칸은 `.inv-craft-cell[data-recipe]`)와 **`.inv-craft-detail`**(고른 것 하나의 상세)이 나란히 선다.
  줄마다 썸네일 · 이름 · 재료 칩 · 스테퍼 · 버튼을 늘어놓던 옛 `.inv-craft-row` **줄 목록은 없어졌다**.
- 상세는 위에서 아래로 ① 산출물 썸네일 + `이름 ×n` · **종류 · 등급**, ② **설명 · 스펙**, ③ **현재 보유 수**
  (`TEXT.craftOwned` ← `craftCountDef`), ④ **재료 썸네일**(`renderItemCost`; 「재료 부족: …」 같은 글자 줄은 **없다** —
  모자란 것은 칩이 스스로 빨갛게 말한다), ⑤ 가로 수량 조절 `지금 목표 / 최대`, ⑥ 길게 눌러 제작(1초 홀드 + 키캡).
- **②는 다시 그리지 않는다**: `ui/Tooltip` 의 아이템 카드를 그 자리에 붙여 쓴다. 그래서 무기 2×2 게이지 · 내구도 게이지 ·
  소켓 줄 · 무게 · 가치 · A 의 `items/ItemSpec.itemSpecRows` · `items/ItemText.parseItemText` 개편이 **전부 그대로** 온다.
  ⚠ 붙인 카드의 클래스는 `.inv-tooltip` 이 **아니라** `.inv-tt-card is-embedded` 다 — 이 카드는 `.inv-layout` 안,
  즉 떠다니는 호버 카드보다 **DOM 앞**에 살아서 이름을 그대로 두면 `document.querySelector('.inv-tooltip')` 를 가로챈다
  (「새 오버레이는 떠다니는 카드 뒤에 붙인다」 규약이 지키려던 바로 그것). `Tooltip.show()` 는 `className` 을 건드리지
  않으므로 생성 직후 한 번 갈아 끼우면 끝이고, 카드 **안쪽** 클래스는 한 벌 그대로다.
- **고를 것이 하나도 없는 작업대**는 목록 · 상세 대신 가운데 한 줄 `제작할 수 있는 레시피가 없습니다.`(`TEXT.craftNone`).
- **산출물이 가는 곳이 뒤집혔다** (`parts/Crafting`): **함선이면 창고 먼저 · 차면 가방**, 레이드 현장은 가방만.
  `roomForOutputs`(누르기 전 검사)와 새 `addCraftOutputs`(실제 지급)가 **같은 순서**를 본다. 레이드 경로는
  `InventorySystem.addUnits` 그대로라 퀵슬롯의 부분 스택을 먼저 채우는 2026-09-09 규약이 살아 있다.
  보유 수 · 재료 세는 범위는 `craftCountDef` 하나(함선 = 가방 + 창고, 현장 = 가방)로 그대로다.
- **왼쪽 세로 작업대 리스트**(`.inv-craft-benches` · `.inv-craft-bench[data-bench]`) · 홀드 1초 · 홀드 중 게이지만
  다시 그리기(`frozen`) · 튜토리얼 `hides('craft', id)` · `craftHasRoom` 게이트는 **그대로**다.
- ⚠ **튜토리얼 스포트라이트**: `src/tutorial/Steps.ts` 가 `.inv-craft-row[data-recipe="…"] .inv-craft-btn` 으로 홀드
  버튼을 찾으므로 **상세 패널이 `.inv-craft-row` 라는 이름을 함께 갖는다**. 그리고 목록이 다시 만들어질 때 고른 것이
  없으면 **맨 앞 칸을 자동으로 고른다** — 튜토리얼은 그 단계의 레시피 하나만 남기므로 그 하나가 곧 상세가 된다.
- 패널 폭 564 → **700 px** (≤ 1560 px 에서 474 → **610 px**) — 열이 [작업대][목록 4칸][상세] 셋이 됐다.

### 4. 그 밖

- `ui/CatalogView.setOpen(true)` 가 검색창을 **focus + select** 한다 (`/items` 로 여는 것이 곧 "찾겠다"). 클래스
  이름 `.inv-cat-search` 는 **그대로**다 — 콘솔의 임시 배선이 그 이름으로 찾고 있었다.
- `ui/labels.TEXT` — `craftNone` 문구 교체, `craftOwned(n)` 추가. `shieldChargeStats` · `boostStats` 는 읽는 곳이
  없어졌지만(2026-09-15, A 의 `itemSpecRows`) 문구의 옛 원본으로 `@deprecated` 표시만 달아 남긴다.

### 5. 후속 (같은 날, 스모크 주행으로 드러난 것)

- **분해 산출도 같은 규칙이다** — 함선이면 창고 먼저 · 차면 가방. `break_*` 는 `updateCraft` 를 그대로 타므로
  갈래를 새로 만들지 않았고(그러면 `roomForOutputs` 도 둘이 된다), 나오는 것이 **재료**라 제작이 가방 + 창고에서
  재료를 꺼내 쓰는 지금 창고가 제자리다. 분해 팝업의 거절 문구(`가방과 함선 창고에 공간이 없습니다`)도 이미 둘을 말한다.
- ⚠ **스모크가 산출물을 셀 때는 `countDefAll`(가방 + 창고)이다.** `countWhere` · `countDef` 는 가방 쪽이라 0 이 나온다.
  비울 때도 창고까지 비워야 한다 — 창고에 남은 **부분 스택이 산출물을 흡수**하면 「자리가 없다」를 만들 수 없다
  (`roomForOutputs` 의 `dryMerge`). `scripts/smoke-inventory-p6.mjs` 의 `window.__purgeAll(pred)` 가 그 일을 한다.
  「가방 · 창고가 둘 다 가득이면 분해 버튼이 잠긴다」가 빨갛던 것은 **코드가 아니라 그 준비 부족**이었다.
- **수량 스테퍼는 `지금 목표 / 최대` 두 수**를 읽는다 — 앞의 수만 떼어 검사한다(최댓값은 가진 재료가 정한다).
- **튜토리얼**(`src/tutorial`): 만든 소총이 창고로 가면서 `equipGun` 의 안내 · 포커싱만 창고까지 넓혔다
  (완료 판정 `onLoadout` 은 그대로). `stowAmmo`(창고 → 가방)는 원래 있던 단계라 손대지 않았고, 오히려 이제 늘 제 일을 한다.
  ⚠ `InventoryRef.takeItem(uid)` 은 「가방으로 옮기기」가 **아니다** — 거래 화면이 쓰는 「인벤토리 밖으로 넘겨주기」라
  스택이 사라진다. 옮기는 것은 `quickMove(uid, from)` 이다 (스모크가 이 둘을 헷갈려 탄약을 지우고 있었다).
