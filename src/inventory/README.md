...
| `ui/parts/Screens.ts` | **Tab 창의 화면 탭과 부속 열.** 함선에서 Tab 은 인벤토리 / 캐릭터 / 기업 / 함선 네 화면을 한 창 안에서 전환한다 (각 화면은 그 폴더가 준 `EmbeddedView` 라서 이 파일은 붙였다 뗐다만 한다). 제작 열과 무한 상자 카탈로그의 열고 닫기도 여기 있다. |
