...
| `Stash.ts` | **함선 창고** (2026-09-06): fixed `STASH_COLS × STASH_ROWS` (10×24) `Grid` persisted in localStorage `scav.stash` (positions + `durability` / `ammoInMag` / nested `sockets`; uids are re-minted on load, unknown defs dropped with a warning, overlapping cells auto-placed). `markDirty()` debounces a save 350 ms after a change, `flush()` on pagehide / dispose. Survives missions, deaths and reloads — the bag / loadout keep their own reset policy. |
