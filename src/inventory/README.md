...
| `model.ts` | 폴더 공용 어휘 — `GridId` / `ItemLocation` / `DropTarget` / `OpResult` / `slotAccepts` 등 타입 · 상수 · 순수 술어. 상태도 DOM 도 없다. `InventorySystem.ts` 가 `export *` 로 재수출하므로 기존 import 경로는 그대로다 |
...
- **2026-09-09 (제작 UI 2차 · 제작 수량 · 창고 숨김 · 스크롤바)** —
  ...
  `◀ n ▶` 스테퍼, 휠로도 조절(리스트가 같이 스크롤되지 않도록 non-passive + `preventDefault`), 상한은
