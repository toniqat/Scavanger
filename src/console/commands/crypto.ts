...
function clusterLine(c: ComputeClusterInfo): string {
  ...
  return `${c.uid} (방 ${c.room + 1}) · ${coinName(c.coinId)} · 프로세서 ${c.cores}/${c.maxCores} · ${cycle} · ${state}`;
}
...
export const cryptoCmd: CommandFactory = () => ({
  ...
  description: '암호화폐 채굴 상태를 보거나, 지갑 잔고 · 클러스터 프로세서 · 채굴 시간을 조작합니다',
  run(args, ctx) {
    ...
    if (sub === 'cores') {
      ...
      if (!Number.isInteger(n) || n < 0) return err(`프로세서 수가 올바르지 않습니다: ${args[2]}`);
      ...
      for (const uid of targets) {
        ...
        out.push(info ? clusterLine(info) : `${uid} 프로세서 ${n}`);
      }
      return out.join('\n');
    }
