...
| 5 | `equipGun` | `{INVENTORY} 인벤토리 열기` (only `inventory:opened` or the next-frame poll — never the step-entry `isOpen`, which is still true while the bench's close is closing the window) → `돌격소총 장착` (union focus, **no dim**) | rifle equipped → waits silently for `inventory:closed` (no objective row); focus off once equipped. **Skipped silently on entry** (`equipGunMoot`) when a primary slot already holds an assault rifle (family `TUTORIAL_GUN_FAMILY` or class `AR`, any grade, not unique) or both primary slots are occupied |
