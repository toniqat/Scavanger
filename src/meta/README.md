...
| `ui/TileGrid.ts` | **(2026-09-13)** `setCell(cell)` — 칸 크기를 바꿔 셀 층 · 상자 크기를 새 피치로 다시 깔고 지금 타일을 옮긴다(타일 크기는 `buildItemTile` 이 구워 넣으므로 호출부가 곧바로 다시 그린다). **(2026-09-12)** 기업 화면의 **채움식 아이템 격자**. 호출부가 만든 타일(`InventoryRef.buildItemTile`)과 발자국을 받아 인벤토리의 `.inv-grid > .inv-cells + .inv-tiles` 마크업 위에 줄 우선 first-fit(`packFootprints`)으로 `transform` 배치한다. 열 수는 고정(트레이 5) 또는 스크롤 상자 폭에 맞춘다(`minCols`), 빈 줄은 상자 높이까지 채우고, 비었을 때 사유를 가운데 띄운다. `ResizeObserver`(한 프레임 미룸)로 다시 배치. 리스너는 호출부 몫. |
...
- **Contracts**: one active (`CONTRACT_MAX_ACTIVE`), accepted in the ship at `minRepLevel` → `meta:contractAccepted`; `abandonContract` drops it
  (progress lost). Progress from bus events **only during a gameplay phase**: `enemy:killed` (인간형 적 전부 — `data/enemies.csv` 의 `faction` 이 `bug` 가 아닌 타입, 스캔 드론 제외 → `kill_rogues`, else `kill_bugs`; 2026-09-13),
...
- **NPC 퀘스트 (2026-09-14 — 아래 기업 퀘스트 규칙을 대신한다)**: 절 「메신저 NPC 퀘스트」 참고. 기업 퀘스트 API(`getQuests` · `acceptQuest` · `completeQuest`)는 계약이라 남았고 빈 목록 · false 다. `getQuestState(id)` 는 NPC 퀘스트로 답한다.
- **Quests (2026-09-14 폐지, 옛 기록)**: `locked / available` recomputed from `QuestDef.requires` (rep level + prerequisite quests complete); `accepted / complete` in the save.
...
- 2026-09-13: the desk no longer folds (the 1500 px / 1240 px rules are gone). Its cells shrink from 40 to 32 px to keep every
  card in one row (`CorpView.fitLayout`); only when 32 px does not fit are the 함선 창고 / 가방 cards hidden — around 1100 px on
  거래 · 퀘스트, narrower on 임플란트 — so selling / delivering by drag needs a wider window there. The fitted cell is **per page**
  (거래 has 25 grid columns, 임플란트 15), so switching pages at the same window width can change the tile size.
- 2026-09-13: the 기업 카드 stretches to the page height (the 2026-09-12 2차 vertical centring is gone) and scrolls itself when the
  corp tree is longer than the window. Below 900 px it still folds on top of the page.
...
- **거래가 전부 아이템 그리드다.** 구매 / 판매 트레이는 위아래로 쌓여 각각 **가로 5칸**(아이템 최대 폭)이고,
  기업 재고 · 가방 · 함선 창고가 같은 칸 크기(당시 `--ct-cell` = `CT_CELL` 40 px, `createTradeGrids({cell})` — ⚠ 옛 이름이다:
  2026-09-08 에 54 px 로 올랐고 2026-09-12 에 접두사가 `.cv-` 가 되어 `--cv-cell` / `CV_CELL` 이 됐으며, 2026-09-13 부터는 40 px 에서
  창 폭에 맞춰 32 px 까지 줄어드는 값이다 — `CorpView.fitLayout`)를 쓴다.
...
- **2026-09-15 (`readAtOf` — 메신저의 「확인해야 다음 메시지가 온다」, 사용자 결정)** — `parts/NpcQuests.ts` 에 **질의 한 개**뿐이다:
  `readAtOf(npcId)` = `save.contacts[npcId].readAt`(없거나 숫자가 아니면 0). 계약은 `NpcQuestRef.readAtOf?`(`shared/npc.ts`, 추가만).
  **기록 · 저장 · 이벤트는 한 줄도 안 바뀐다** — `markRead` · `unreadTotal` · `npc:unreadChanged` · `NpcSave` 그대로다.
  읽는 곳은 메신저 하나(`ui/menus/messenger/ChatTab`): 대화를 처음 그릴 때 `at <= readAt` 인 말풍선까지만 즉시 그리고
  그 뒤는 `...` 타이핑으로 하나씩 푼다. `NpcContactInfo.unread`(**사건** 수)로는 말풍선 수를 셀 수 없어 시각을 연 것이다 —
  `resolve()` 가 `intro` 사건 **하나**를 `NpcDef.intro` **전부**로 펴고, 그렇게 나온 말풍선들은 `at` 이 같다.

- **2026-09-14 (레이븐의 튜토리얼 첫 연락 · 대사 선택지 콘텐츠, 에이전트 F, docs/DECISIONS.md 「2026-09-14 — 튜토리얼 개편」)** — 코드는 한 줄도 안 바뀌었다:
  `parts/NpcQuests` 의 `getPendingChoices` · `chooseIntro` · `resolve()` 의 `choice` 갈래는 리드의 계약 커밋에 이미 들어와 있고, 이번에 채운 것은 **표**다.
  ① `data/npcs.csv` 의 `npc_raven` — `intro` 를 **전 함선 주인의 연락책**으로 다시 썼다(통성명 없이 통신 코드로만 거래했으므로 주인이 바뀐 것을 눈치채지 못한다)
  + `introChoices` · `introChoiceReplies` **2개씩**(부정 / 수긍). 부정을 골라도 레이븐은 **믿지 않고** 통신 보안 걱정으로 흘려 넘기므로 **분기가 남지 않는다** —
  `choice` 사건 하나만 기록되고 그 뒤의 대화는 어느 쪽이든 같다(계약 그대로). 이 표를 쓰는 NPC 는 지금 레이븐 하나다.
  ② `data/npc_quests.csv` 에 레이븐의 **첫 의뢰 `q_rv_0`**(「첫 물건」 — 크레딧 200 · XP 250 · `heal_syringe:2` · `npcTrust` 80)을 `q_rv_1` **앞**에 끼워
  튜토리얼 트랙 ② 의 「레이븐의 의뢰를 받는다」 가 가리킬 대상으로 삼았고, 줄 순서 제안 규약을 따라 `q_rv_1` 에 `reqQuests=q_rv_0` 을 달았다.
  목표는 레벨 2 가 첫 실전 레이드에서 끝낼 수 있는 둘뿐이다 — `kill 3 bug` + `recover 2 mat_scrap`(행성 조건 없음 · `chain` 없음).
  `rewardCredits > 0` 이라 **`server/economy.gen.json` 을 다시 구웠다**(`npm run data:check -- --write` — 안 하면 릴레이가 `quest:q_rv_0` 지급을 거절한다).
- **2026-09-14 (NPC 개인 신뢰도, 에이전트 C, docs/DECISIONS.md 「2026-09-14 — 정보상」)** — 모든 NPC 에 기업과 **별개**인 개인 신뢰도가 생겼다.
  누적 점수는 `NpcSave.trust`(npcId → 점수), 레벨은 기업과 **같은** `REP_TABLE`(0–5) — 표를 하나 더 만들 이유가 없다는 사용자 결정.
  `parts/NpcQuests` 의 `trustOf` · `trustLevelOf` · `addTrust` 가 유일한 소유자이고 `MetaSystem.npcTrust` / `npcTrustLevel` / `addNpcTrust` 는
  한 줄 위임이다. **지급 자리는 기업 신뢰도 바로 옆** — `report()` 가 아이템 → `quest:<id>` 크레딧 → `addRep` → `addTrust(def.npc, rewards.npcTrust)` → XP
  순으로 준다(`data/npc_quests.csv` 의 새 열 `npcTrust`, 41개 퀘스트 전부). **크레딧이 아니므로 서버 검증과 무관하다** — `credits:tx` 는 한 줄도 안 바뀌었다.
  조건 쪽은 `NpcRules.requirementMet` 이 `NpcRequirement.npcRep` 를 판정하게 됐고(`NpcReqContext.npcTrustLevel`) **실제 게이트는 하나도 걸지 않았다**
  (사용자 결정: 신뢰도별 해금 요소는 아직 정하지 않는다). 저장은 `sanitizeNpcSave` 가 `trust` 를 실어 나르고 `freshNpcSave()` 가 빈 표를 들고 나온다 —
  `MetaStorage.snapshot()` → localStorage · `profile:set meta` → `sanitizeMetaSave` 가 유일한 왕복 경로이고, 여기서 빠지면 새로고침 한 번에 사라진다
  (2026-09-09 `accent` 사고와 같은 자리). `data/npcs.csv` 의 레이븐 `reqLevel` 4 → **1** (정보상을 1레벨부터 — 무소속이라 기업 조건도 없다).
- **2026-09-13 (서재 시리즈 — 신뢰도 책, 에이전트 C, docs/DECISIONS.md 「2026-09-13 — 서재 시리즈 · 비디오게임」)** — `parts/Contracts.settleMission` 이 성공한 **계약**의 신뢰도를
...
- **2026-09-13 (암호화폐 매매용 `creditsTx` — 에이전트 ③, docs/DECISIONS.md 「2026-09-13 — 가구 접근 면 · 발전기 · 암호화폐 채굴」)** — `parts/Credits.creditsTx` + `MetaSystem.creditsTx` 한 줄 위임 (위 표).
  사유 문자열은 부르는 쪽(housing)이 `formatCreditReason({kind:'crypto-buy'|'crypto-sell', id, qty})` 로 만든다. `addCredits` · `serverTx` 는 그대로다.
- **2026-09-13 (킬 계약이 인간형 적 전부를 센다 — 행성별 적 난이도, faction-presentation, docs/DECISIONS.md 「2026-09-13 — 행성별 적 팩션」)** — `Rules.killGoalOf`
  ...
  네임드 — 를 `kill_rogues` 로 센다. 스캔 드론(`rogue_scan_drone`)은 팩션이 레이더여도 사람이 아니라 뺀다(`NON_HUMANOID_KILL_TYPES`).
  목표 id `kill_rogues` 는 계약이라 그대로다. `data/contracts.csv` 의 바스티온 소탕 4줄 설명을 「로그 n명을 처치한다」 →
  「인간형 적 n명을 처치한다」 로 (id · 이름 · 수치 그대로). ⚠ 목표 라벨 `kill_rogues: '로그 처치'` 는 `shared/meta.ts` 에 있어 손대지 않았다.

- **2026-09-13 (기업 화면 — 한 줄로 늘어선 독립 카드 · 칸 40 → 32 px 맞춤 · 창고 / 가방 분리, 사용자 결정)** — `ui/CorpView.ts` ·
  `ui/TileGrid.ts` · `meta.css`, 그리고 inventory 의 `ui/TradeGrids.ts`(이 작업의 소유 — `layout: 'split'` · `chips` · `setCell` ·
  `mountFilterChips`, 사용법은 `inventory/README.md` 의 `2026-09-13 — 카드마다 격자 하나`) · `shared/types.ts`(`TradeGridsViewOptions`
  에 `takeLabel` · `layout` · `chips` 추가, `TradeGridsView` 추가).
  1. **기업 목록 = 맨 왼쪽 독립 카드.** 호스트의 틀(배경 · 테두리 · 안여백)을 걷고 카드마다 `.inv-panel` 결의 틀을 줬다. 기업 카드도
     세로를 다 쓴다(예전 세로 중앙 고정은 카드 줄과 높이가 달라 걷었다).
  2. **거래** = [판매 물품] [거래 테이블] [함선 창고] [가방], **퀘스트** = [목록] [상세 · 납품] [함선 창고] [가방], **임플란트** =
     [데스크] [함선 창고] [가방], **계약** = 기업 목록만 분리. 창고 · 가방은 **서로 다른 카드**이고 카드마다 머리(이름 · 개수 · 정렬) ·
     필터 줄 · 세로 스크롤 — 예전에는 한 스크롤에 두 격자가 이어 붙어 좁은 창에서 가방이 창고 밑으로 내려갔다(루트 CLAUDE.md
     2026-09-12 TradeGrids 규칙 그대로).
  3. **칸 54 → 40 px**(판매 물품 · 트레이 · 창고 · 가방 · 임플란트 데스크 · 드래그 고스트). 타일 글리프 · 이름 · 수량 · 소켓 · 가격 배지를
     칸 크기에 맞춰 줄였다(`meta.css` · inventory 의 `.tg-compact`).
  4. **좁은 창**: 칸이 32 px 까지 스스로 줄어 다섯 카드를 한 줄에 지킨다(`fitLayout` — 카드 실측 선형 모델, 호스트 `ResizeObserver`
     120 ms 디바운스 · 페이지 전환 · 첫 그리기). 32 px 로도 안 들어갈 때만 창고 · 가방 카드를 숨긴다. **옛 1500 px 두 줄 접기 · 1240 px
     열 숨김 삭제.**
  5. 실측(헤드리스, 2026-09-13): 1920×1080 거래 40 px · 1600×900 40 · 1440×900 39 · 1366×768 35 · 1280×760 33 (퀘스트 33 · 임플란트 40) ·
     1100×700 거래 · 퀘스트는 창고 · 가방 숨김 / 임플란트 32. 모든 크기에서 카드 줄 `scrollWidth == clientWidth`, 화면 밖 요소 0, 잘린
     격자 열 0. 드래그: 가방 → 판매칸 · 창고 → 판매칸 · 판매 물품 → 구매칸 · 가방 → 퀘스트 납품 · 더블클릭 모두 담긴다, 콘솔 오류 0.
  - `scripts/smoke-meta.mjs`: 칸 크기 단언 54 → **호스트 `data-cv-cell`(32–40) = 재고 칸 = 창고 · 가방 `--inv-cell`**, 창고 · 가방 카드가
    따로(`stash:1,bag:1`), 재고 타일 크기 `w × (칸 + 2) − 2`.
  - 알려진 한계: 칸 크기는 **페이지마다** 맞춘다 — 같은 창 폭에서도 거래(25칸)는 33 px, 임플란트(15칸)는 40 px 일 수 있어 페이지를
    바꾸면 창고 · 가방 타일 크기가 달라진다. 헤드리스는 오버레이 스크롤바(자리 0)라 실제 창의 스크롤바 자리는 실측으로만 따라온다
    (선형 모델이 실측이므로 틀리지는 않지만 위 숫자는 1–2 px 다를 수 있다).
...
> [첫 연락 3단 · 진행 플래그 (2026-09-14 3차)](#첫-연락-3단--진행-플래그-2026-09-14-3차) 절이 덮어쓴다.
...
- **목표**: `deliver`(함선 · 나눠 납품) · `recover`(그 레이드의 raidFound 를 몸에 지니고 **탈출** — `settleMission` 이 계약 정산 전에 확정) · `interact` · `kill`(막타, `weapon` = 총기 계열) · `discover`(분대 공유) · `search`.
  레이드 목표는 **채우는 순간 확정**, 확정 전 진행은 레이드 끝에 0, `chain` 은 한 레이드에서 함께. 분대원의 킬 · 상호작용은 세지 않는다.
- **보상**: 아이템 먼저(공간 없으면 `보상을 받을 공간이 없습니다` · 아무것도 안 바뀜) → 크레딧 `quest:<id>`(서버 원장 퀘스트 id 당 1회 — 경제 표는 `NPC_QUEST_DEFS`) → `rewardRep` 마다 `addRep`(서재 신뢰도 배율 없음 — 계약 전용) → XP.
  메타 · 창고 · 로드아웃 문서는 `commitQuestTx` 한 트랜잭션.
- **이벤트**: `npc:message`(사건 하나) · `npc:unreadChanged` · `npc:questChanged {prev}` · `npc:objectiveProgress {raid, done, delta}`(확정 순간 `done: true` 한 번, 되돌림은 음수 delta) · `npc:questReady`. 옛 `meta:questChanged` 는 더 이상 나지 않는다.
- **읽음**: NPC 마다 `readAt` — 그 뒤 사건 수가 읽지 않음이다(내 행동으로 생긴 답장 포함, 메신저가 그 대화를 보면 `markRead`).
- **기업 화면**: 퀘스트 탭 · 페이지 · `.cq-*` CSS 삭제. 잠긴 탭만 있을 때 여는 페이지는 계약.
- **콘솔**: `npc list|contact <npc>|offer <quest>|accept|defer|deliver <quest> <i>|report|progress <quest> <i> <n>|reset` (옛 `quest` 대신).
- **검증**: `scripts/smoke-npc-quests.mjs` **66 / 0 / 0** (2026-09-14, private vite 5312 — 첫 연락 · 제안 · 풀이 · 읽음 · 보류/수락 brief · 나눠 납품 · 보고 보상 · 다음 제안 · 문구 ·
  추적 목록 행성 · 막타 계열 · 남의 킬 · 실제 `withLocalGunHit` 킬 · 실제 스캐너 `world:interacted` · 실제 구조물 컨테이너 `crate:open.zoneKind` · chain · 탈출 회수 · 되돌림 · 채굴 인가 해금 · 저장 v2 · 정리).
  `smoke-meta` 178 / 0 (퀘스트 부분을 폐지 확인 · 계약 탭 재화 칩 · v2 로 바꿈), `smoke-mining` 64 / 0, `smoke-favorite-chips` 52 / 0, `smoke-ui-p5` 141 / 0.

- **2026-09-14 (메신저 · NPC 퀘스트, 에이전트 B)** — 기업 퀘스트 폐지 · `ctx.meta.npc` 엔진(`NpcRules` · `parts/NpcQuests` · `parts/NpcObjectives`) · `MetaSave` v2(`npc`) · 기업 화면 퀘스트 탭 삭제 · 콘솔 `npc` · `getQuestState` = NPC 퀘스트 · `settleMission` 이 NPC 회수 목표를 먼저 정산.

---

## 정보상 (2026-09-14)

`ctx.meta.intel` (`IntelRef`, 구현 `parts/Intel.ts`). 계약 · 가격 식 · 위생은 전부 `src/shared/intel.ts` 이고
값은 `data/intel_options.csv` · `data/tables.csv` · `data/tuning.csv` 다 — **이 폴더에는 숫자가 없다.**

- **한 번에 하나.** 이미 있는데 또 사면 **환불 없이** 덮어쓴다 (사용자 결정).
- **멀티는 분대장만** 사고 버린다 (탐사 차량 요금의 「결제자 한 명」 규약). 분대원은 `lobby.intel` 을 읽기만 한다.
- **크레딧은 릴레이의 답을 기다린다** (`creditsTx`). 거절되면 보유 정보를 **바꾸지 않는다** — 산 것처럼 보였다가
  서버에서 튕기면 그 시드로 출격해 기믹만 없는 맵이 된다. 사유는 `intel:<planetId>:<code>` (`shared/credits`).
- **행성이 바뀌어도 버리지 않는다.** `get().planet` 을 그대로 노출해 화면이 「다른 행성의 정보」라고 적을 수 있게 하고,
  출격에는 **행성이 일치할 때만** 쓴다 (`hub/parts/Pods.usableIntel`). 다시 그 행성으로 가면 유효하다.
- **저장**: `MetaSave.intel` — `Storage.sanitizeMetaSave` 가 `sanitizeIntelSpec` 로 읽는다. 여기서 빠뜨리면
  산 정보가 한 번의 새로고침에 사라진다(2026-09-09 `accent` 사고와 같은 자리).
...
- `ui/CorpView` — 거래 성사 `.cv-confirm` 안, 라벨(`.cv-confirm-label`) 왼쪽에 좌클릭 홀드 키캡
  (`shared/keycap.createHoldButtonCap` → `.keycap.kc-btn`). ⚠ 그래서 `.cv-confirm` 의 `textContent` 는 캡의 SVG
  `<title>` 때문에 **`LMB거래 성사`** 다 — 라벨을 글자로 읽는 곳은 `.cv-confirm-label` 을 읽는다
  (`scripts/smoke-meta.mjs` 를 그렇게 옮겼다). 짧게 톡 눌렀을 때의 교정 토스트(`거래 성사 버튼을 1초간 꾹 누르세요`)는
  상시 안내가 아니라 **오조작 피드백**이라 남겼다.
- `ui/HoldAsk` — 확인 버튼 `.cv-ask-ok` 안에 같은 캡(이 팝업은 늘 홀드다). 안내 줄 `.cv-ask-hint`(와 일찍 떼면
  번쩍이던 `is-flash`)는 요소째 없어졌고 `meta.css` 의 그 두 규칙도 지웠다.

## 변경 이력 — 2026-09-15 3차 (사용자 결정): 창고 + 가방은 **한 카드**

인벤토리가 2026-09-15 2차에 `TradeGrids` 를 **한 패널**(왼쪽 함선 창고 · 오른쪽 내 가방 — 칸마다 자기 스크롤 ·
자기 정렬 버튼 · 자기 필터 드롭다운)로 바꾸면서, 바깥 틀(배경 · 테두리)을 갖는 쪽이 **호출자의 카드**가 됐다.
기업 화면은 여전히 `createTradeGrids` 를 **카드마다 한 번씩 두 번** 부르고 있어 카드가 둘로 갈려 있었다.

- `ui/CorpView.makeInvCards` 가 카드 **하나**(`.cv-card.cv-inv`)를 만들고 `createInv(card, drop)` 가
  `grids: ['stash','bag']` 로 **한 번**만 부른다 (`id` 인자는 없어졌다 — `InvCard` 에서도 뺐다).
  돌려주는 배열은 길이 1 이라 `applyCell` · `refreshInv` 의 모양이 안 바뀐다.
- 카드에 옛 클래스 `stash` · `bag` 을 **둘 다** 단다 — 이 한 카드가 실제로 둘 다 들고 있으므로 거짓이 아니고,
  `.cv-inv.stash` · `.cv-inv.bag` 으로 칸 크기를 읽던 `scripts/smoke-meta.mjs` 가 그대로 산다
  (어떤 CSS 규칙도 그 두 이름을 읽지 않는다).
- ⚠ `fitLayout` 이 이 카드의 격자 칸 수를 **합**으로 잰다 — `widestGridCols` 가
  `gridColsIn(card, step, 'max' | 'sum')` 이 됐고 `.cv-inv` 만 `'sum'` 이다. 두 격자가 **나란히** 서므로 카드 폭이
  둘 다와 함께 자라는데, 예전처럼 넓은 쪽 하나만 재면 나머지 격자 폭이 `chrome`(= 칸 크기와 무관한 고정 몫)으로
  잡혀 칸 크기를 바꿀 때마다 계산이 어긋난다. 거래 테이블(트레이가 **위아래로** 쌓인다)은 그대로 `'max'` 다.
- `meta.css` 는 주석만 바뀌었다 (`.cv-inv` 규칙은 카드 하나에도 그대로 맞는다).
