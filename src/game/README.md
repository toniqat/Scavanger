...
| **lost-lock watchdog** (`checkLockLost`, every frame, 2026-09-07) | the handler above bails while a UI blocker is up, which left the worst case open: a cursor-mode screen keeps the lock by design (Phase 10), but Chrome drops it on any Escape and sometimes refuses to hand it back — `shared/cursor.ts` then silently *mirrors* the real OS cursor, so the game looks playable while the Windows cursor is free to wander onto another monitor. So: gameplay phase **or the ship**, not paused, player alive, no lock for `LOCK_LOST_GRACE_S` (0.5 s) and > 300 ms since the last request → 일시정지 메뉴, **regardless of blockers**. It is armed only once a real lock was ever held (`sawPointerLock`), so a browser (or a headless smoke) where the lock never works never gets an unclosable menu. **2026-09-07 (2)**: it also holds off while `ctx.input.awaitingLockGesture` — a *denied* request is not a lost lock, `shared/Input` is holding the intent until the player's next real gesture |
| unpause (Esc or 게임으로 돌아가기) | after emitting `game:paused false`, re-requests pointer lock in a microtask when still in a gameplay phase **or the ship**. **2026-09-07 (2)**: the guard is `uiBlockers.has('menu')`, not `size > 0` — every other surface *keeps* the lock since Phase 10, so the old test silently skipped the re-lock whenever the menu had been opened over the inventory / the terminal / 함선 관리 (the watchdog path) and the watchdog then re-opened it 0.5 s later, forever. Chrome grants no activation for Escape, so the request that follows an Esc-resume is usually denied — `Input` retries it from the next click or key |
