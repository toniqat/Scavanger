 ...
 * 표현 헬퍼다 — **식사 배지(`hud/MealBadge`)와 아이템 툴팁(`hud/ItemTip`)이 같은 문장**을 써야 하는데
 * 그 둘이 각자 포맷하면 같은 요리가 화면마다 다르게 읽힌다 (CLAUDE.md 의 「같은 것을 두 곳이 쓰면 하나로
 * 뽑는다」를 폴더 안에서 적용한 것 — 계약인 `shared` 를 건드리지 않고 끝난다).
 *
