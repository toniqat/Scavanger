 ...
 *   - `개인 대화` (2026-09-14, was `귓속말하기`) — the messenger switches to its 대화 tab on that 아이디.
 *   - `친구 삭제` — friends only, and only through the confirm card (mutual removal).
 *   - `친구 추가` — non-friends only (a 최근 플레이어 row).
 *   - (2026-09-11, B-4) `차단` behind its own confirm card. A blocked row (the column's 차단 목록) offers only `차단 해제`
 *     (2026-09-14: the `대화 기록` entry went with its page — the messenger conversation replaces it).
 *
...
export class SocialMenu {
  ...
  private openBlockConfirm(p: SocialPlayer): void {
    this.askConfirm(
      '차단',
      `${p.name || '이름 없음'} (${formatPlayerCode(p.code)}) 을(를) 차단합니다. 개인 대화 · 친구 요청 · 분대 초대가 오지 않고 분대 채팅도 가려집니다. 친구 · 요청 · 최근 목록에서도 사라지며, 상대에게는 알리지 않습니다.`,
      '차단',
