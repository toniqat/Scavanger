...
| `index.ts` | Barrel. |
| `styles/base.css` | **(2026-09-14)** `.screen-fade` — 화면 전체 검은 페이드 판 (`position: absolute; inset: 0; z-index: 82; background: #000; opacity: 0; pointer-events: none`). 불투명도 · 전이 시간은 `HudSystem` 이 인라인으로 몬다. z 근거는 규칙 주석에 있다. Global reset, `#app` / `#game-canvas` / `#ui-root` layout (`pointer-events:none`; children opt in with `.interactive`), palette CSS variables (`--c-accent #ffb347`, `--c-danger #ff4d4d`, `--c-success #4fd17e`, rarity colors), font stack, utilities (`.ui-panel`, `.ui-label`, `.ui-btn`, `.ui-input`, `.keycap`) and all HUD/menu component styles. Ends with the `── multiplayer ──` and `── ship hub / chat ──` sections (see below). |
| `styles/social.css` | **소셜 추가분 스타일시트** (2026-09-11 B-3 · B-4, `menus/social/SocialColumn` 과 `hud/ChatLog` 가 import). `.sc-inv`(카드의 `초대 중 · n초` 배지) · `.cp-blocked`(커뮤니티 머리의 `차단 목록 n`) · `.sc-page`(열 위에 겹치는 페이지 — `.cp-frame .social-col` 을 `position:relative` 로) > `.sc-page-head`(`.sc-back` · `.sc-page-title` · `.sc-page-sub`) · `.sc-page-body`(대화 기록은 줄 목록, `.is-blocked` 는 2열 격자) · `.sc-logline(.out/.in/.pending/.stored/.failed/.backlog)` · `.sc-log-input-row` · `.sc-unblock`, 채팅 줄의 `.wst` 와 `.chat-line.whisper.pending/.stored/.failed/.backlog`. 기존 소셜 열 · 채팅 규칙은 base.css 에 그대로다. |
| `styles/title.css` | **(2026-09-14 — 시작 임플란트 제거)** `.cc-*` 는 이제 **두 열**이다: `.cc-body` = `[.cc-main] [.cc-side]`, `.cc-main` 은 이름 카드(`.cc-panel`) + 능력치 카드(`.cc-stats-wrap`, `flex: 1` 로 남는 높이를 먹는다) 를 세로로 쌓는다 (`max-width` 1120 → 1000 px). **`.cc-implant-sec` · `.cc-implants` · `.cc-imp-tile*` · `.cc-imp-menu*` 블록은 통째로 삭제됐다** — 새 캐릭터의 임플란트는 고정값이라 고를 화면이 없다. 아래는 옛 설명. **타이틀 흐름 전용 스타일시트** (2026-09-09, `menus/TitleMenu.ts` 가 import). `.menu.title.home` (워드마크 위쪽 가운데 · 액자 없는 프레임 · `.title-actions` 버튼 열 · `.stacked`), `.title-screen` (`.char-select` / `.char-create` 공용 전체화면 껍데기, z-index 84 · `.ts-head` · `.ts-foot` · `.form-msg`), `.cs-card*` (슬롯 카드 · `--ac` 악센트 · `.cc-mini` 능력치 축소판 · 빈 칸), `.cc-*` (생성창의 세 열 `.cc-body` · 패널 · 임플란트 타일 `.cc-imp-tile` + 컨텍스트 메뉴 `.cc-imp-menu`(`.sc-menu` 껍데기) · 세로 능력치 `.cc-stat-list > .cc-stat` · 미리보기 열 `.cc-side` + 스와치 `.cc-accent`), `.tm-ask*` (경고 팝업 + 홀드 채움), **`.kb-notice`** (2026-09-11 옛 키 설정 알림 카드 — `.kbn-head` · `.kbn-lines > .kbn-line` · `.kbn-sub` · `.kbn-foot`, 가운데 정렬은 `menuIn` 의 transform 과 싸우지 않게 `margin: 0 auto`, `.stacked` 에서 같이 흐려진다). 색 · 여백 · 트랜지션은 전부 `base.css` 의 변수를 쓰고 여기서 새로 정의하지 않는다. **`base.css` 는 건드리지 않는다** — `.menu.title .frame` 의 900 px 상자는 `.home` 을 하나 더 얹은 선택자로 덮어쓴다. |
...
| `styles/buffs.css` | **캐릭터 버프 썸네일 줄 스타일시트** (2026-09-12, `hud/BuffStrip` 이 import). `.bfs(.has-items .is-mini)`(`--bfs` 한 변 22 px · 미니 14 px, 비면 `display:none`) > `.bfs-cell(.is-pending .is-debuff .is-timed)[data-key][data-kind]`(`--bc` 버프 색 · `--r` 0…1 남은 비율) > 흐린 `.bfs-face` + `.bfs-reveal > .bfs-face.lit`(밑에서부터 `--r` 만큼 밝다 — `implant.css` 의 두 얼굴 기법) · `.bfs-t` 우하단 `23h` / `42m` / `35s`(미니에서는 숨김). `pending` = opacity 0.42 + 채도 감소, 디버프 = `--c-danger` 테두리 + 글로우. 자리 규칙 둘: `.hud .vitals .bfs`(체력바 아래) + 줄이 있으면 **`.hud-bl` 을 151 px 로 올린다**(`.hud.social:not(.spectating):has(.vitals .bfs.has-items) .hud-bl:not(.whispering)` — 관전 · 귓속말 자리는 그대로), `.squad .srow:has(.bfs.has-items)` 는 행 높이 30 px 고정을 풀어 준다. 접두사 `bfs-`(`rg "\.bfs-" src` 로 비어 있음 확인). **2026-09-13:** 요리 품질 별 배지 `.bfs-cell[data-q]::after`(좌상단 8 px 금색 `★n`, `.bfs.is-mini` 는 6 px 별 하나). |
...
| `styles/rover.css` | **탐사 차량 · 지도 범례 견본 스타일시트** (2026-09-13, `map/MapScreen` · `hud/RoverHud` 가 import). `.map-legend-row .sw-cv`(28×20 범례 견본 캔버스) · `[hidden]` 이 flex 규칙에 지지 않게 하는 한 줄 · `.map-canvas.pick`(정류장 위 포인터) · `.map-rover`(목적지 선택 패널 — `.map-rover-row .k/.v` · `.map-rover-reason` · `.map-rover-go(.is-blocked/.is-holding) > .fill + .lbl` 1초 홀드 버튼 · `.map-rover-note`) · `.hud.rover-view` 전투 위젯 숨김 목록(`drone.css` 의 `drone-view` 와 같은 목록) · `.rv-hud(.show/.low)` 탑승 HUD. |
...
| `styles/results.css` | **결과 창 몸통 스타일시트** (2026-09-15, 결과 창 개편 — `menus/ResultReport` 만 import, 접두사 `.rs-`, `rg "\.rs-" src` 로 비어 있음을 확인했다). 제목 줄 `.rs-titlerow` · 임무 시간 `.rs-time(-v)`, `.stats.rs-stats`(base.css 의 2열 격자를 세로 줄로), 전리품 줄 `.rs-loot(.lost) > .rs-loot-v`, 사망 원인 줄 `.rs-cause > .rs-cause-thumb(img · svg) · .rs-cause-cap · .rs-cause-text > .rs-cause-name · .rs-cause-dmg > .rs-cause-num`, 사망 모드 탈출 창 `.menu.complete.rs-dead` 틀 색, `.menu.death .frame` 최소 폭, 사망 창의 `획득 경험치` 숫자를 호박색으로(base.css 의 흐린 색을 덮는다). 전이 · 애니메이션 없음. |
| `styles/fall.css` | **낙하 붉은 비네트 스타일시트** (2026-09-15, TODO B-14, `hud/FallVignette` 만 import — 접두사 `.fall-`, `rg "\.fall-" src` 로 비어 있음을 확인했다). `.fall-vignette` 한 규칙: `#ui-root` 직계 전면 판 · `pointer-events: none` · 가장자리로 짙어지는 `radial-gradient` + 아래쪽 `linear-gradient` 두 겹 · **전이 없음**(불투명도는 컴포넌트가 인라인으로 쓴다). **z 25 의 근거**(HUD 레이어 위 · 재개 게이트 30 · 지도 40 · 인벤토리 50 · 페이드 82 · 메뉴 83 + 아래)는 파일 머리 주석이 갖는다. |
...
| `styles/music.css` | **음악 재생 창 스타일시트** (2026-09-14, `hud/MusicPlayer` 만 import — 접두사 `.mus-`, `rg "\.mus-" src` 로 빈 이름을 확인하고 골랐다). 자리(좌상단 24 / 20) · **z 81** 의 협상 근거(인벤토리 창 50 과 딸림 층 ≤ 80 위 · `.screen-fade` 82 · `.net-badge` 83 · `.key-guide` 84 아래) · 창은 `pointer-events: none` 이고 **버튼에만 `auto`**(`.nb-actions` 와 같은 규칙) · `.is-empty` · 진행 막대 `transition: width 140ms linear`(프레임마다 갱신하되 0.2 % 미만 변화는 건너뛰므로 그 사이를 트랜지션이 메운다)를 **파일 머리 주석이 갖는다**. |
| **hud/** | |
| `hud/Reticle.ts` | Center 4-tick crosshair + dot. **2026-09-15 제세동기 모드**: 손에 든 소모품이 제세동기(`ItemDef.gadgetId === 'defib'`)면 `.reticle.defibmode` — 네 틱이 숨고 가운데에 **작은 하얀 원**(`.rdf-inner`)과 **큰 반투명 원**(`.rdf-outer`)이 선다(둘의 실제 지름은 같고 작은 쪽만 `transform: scale()`, `DEFIB_MIN_SCALE` 0.25 → 1 — 배율이 테두리도 키우므로 「커지면서 두께가 굵어진다」가 전이 없이 나온다 = reduced-motion 에 안 잘린다). 좌클릭 충전이 끝나면 `.defib-armed`(겹침 + 글로우), 쓰러진 아군을 겨누면 `.defib-target`(둘 다 강조색 주황) — 그때 떼면 일으킨다. **판정은 흉내내지 않는다**: weapons/ 의 `parts/Defib` 이 보내는 `gadget:defibAim {armed, charge, target}` 하나가 근거이고, 모드 자체는 `quick:equipped` 의 def 가 정한다(이벤트가 한 번도 안 와도 원은 서 있다). 충전값은 활 시위와 같은 `damp`. 디버그 getter `defibMode` · `defibGauge` · `defibOnTarget`. **2026-09-15 활 크로스헤어 가로 두 배**(`.rbow-tier` 28 / 40 px · `.rbow-draw` 52 px, 끝 조각 20 px — CSS 폭만, 위치 수학 무변경). **2026-09-12 총구 막힘**: `weapon:aimBlocked {blocked}` 이면 점과 틱이 `--c-danger` 빨강(`.reticle.blocked`, `styles/base.css`) — 벽의 빨간 원(weapons/fx/AimBlockMarker)과 실제 사격이 쓰는 같은 판정이고 여기서는 레이캐스트를 쏘지 않는다. `game:newMission` · `game:abort` 에서 해제, 디버그 getter `isBlocked`. Base gap by stance × aim (`ctx.player.stance`: stand 14/7, crouch 10/5, prone 8/4 px hip/ADS), sprinting 18, +2 while moving (`ctx.player.velocity`); blooms on `weapon:fired` (decays). Hitmarker X on `ui:hitmarker` (white = hit, red = kill; **2026-09-09** a `headshot` draws the same X at **1.6배**(24 → 38.4 px) via `.hitmarker.head`, colour unchanged — 처치의 빨강만 따로다). Fades out while any UI blocker is active, is fully hidden while the scope overlay shows (`weapon:scopeChanged.scope && aiming`) and dims to 25 % while the quick-use or ship-call wheel is open (`quick:wheelChanged.open` / `stratagem:wheelChanged.open`); `.hud.targeting .reticle` hides it via CSS while a call is being aimed.  **2026-09-10 (임플란트 HUD 이전):** 크로스헤어에서 임플란트 쿨타임 · 충전 수 표기가 **전부 사라지고**(중앙 하단 `hud/ImplantWidget` 썸네일로 이사) 대신 **갈고리 칩**(`.rgrap`, 크로스헤어 **좌측**)이 붙었다 — 갈고리를 장착했을 때만 존재하고, 지금 조준한 방향에 걸 수 있으면 아이콘이 밝고 `Keys.IMPLANT` 키캡이 보이며, 걸 수 없으면 **키캡은 `hidden`, 아이콘만 딤드**다. 판정은 흉내내지 않는다: implants/ 의 `updateGrapple` 이 자기 조준 광선으로 계산해 보내는 `implant:grappleTargetChanged {valid}` 하나가 근거이고(= `castGrapple` 이 보는 `grappleTargetValid`) 이 파일은 레이캐스트를 쏘지 않는다. 기존 `.hook` 괄호(거리 표시)는 그대로다. 스타일은 `styles/implant.css`, 디버그 getter `grappleChip` (`off` / `dim` / `ready`). **2026-09-14 활 모드**: 손에 든 무기가 활(`WeaponDef.unique === 'bow'`, 「롱혼」 `u_bow`)이면 `.reticle.bowmode` — 네 틱이 숨고 점은 남으며, 점 아래 가로 안내선 두 단(`.rbow-tier` · 아래 단 `.rbow-low`, 오프셋 원본 = 파일의 `BOW_TIER_Y` 12 / 24 px)과 움직이는 가로 바(`.rbow-draw`, 가운데 6 px 가 비어 점이 가려지지 않는다)가 선다. 바는 평소 아래 단에 흐리게 서 있고 `weapon:chargeChanged {kind:'draw', t}` 를 따라 damp 로 올라가 `t = 1`(완전히 당김 = 크로스헤어대로 날아간다)에 정확히 점 위에서 흰 글로우(`.full`)를 낸다 · `t = −1` 이면 아래 단으로 돌아간다. 활 판정 = `weapon:equipped` 의 `ctx.loot.getWeaponDef(id).unique` + 놓친 장착에 대비해 `t ≥ 0` 인 `draw` 이벤트 자체(다른 kind 는 활이 아니다). 소모품 모드가 이기고(`quick:equipped {item}` 동안 꺼짐), `.blocked` 빨강 · blocker/스코프 숨김 · 휠 25 % 딤은 그대로 적용된다. 디버그 getter `bowMode` · `bowDraw` · `bowFull`, `HudSystem.bowReticle`. |
...
| `hud/BuffStrip.ts` | **캐릭터 버프 썸네일 줄** (2026-09-12, 사용자 결정 — `docs/DECISIONS.md` 「2026-09-12 — 캐릭터 버프」). 한 캐릭터의 `CharBuff[]`(`src/shared/charBuffs`)를 **받은 순서 그대로** 작은 정사각 썸네일로 그린다 — PC 체력바 아래(`hud/Vitals`, 22 px)와 분대원 행 체력바 아래(`hud/Squad`, `{ mini: true }` 14 px)가 같은 컴포넌트다. 글리프 · 색은 `meal` · `prep` = 아이템 def 의 `icon` · `color`(`ctx.loot.getItemDef` → `ctx.inventory.getDef`), `env_exposed` = `ENV_ICON` · `ENV_COLOR`, 그 밖 · 못 찾으면 `CHAR_BUFF_GLYPH` · `CHAR_BUFF_COLOR`. `pending` = 흐림, `debuff` = 빨간 테두리, `startedAt` + `endsAt` 가 있으면 **시간 게이지**(`charBuffRemainingRatio` → `--r`, 밝은 얼굴이 위에서 아래로 빠진다) + 글자(`formatRemaining`: `23h` · `42m` · `35s` — 시 · 분은 내림, 초는 올림). `title` = `charBuffTitle`. `set(list, ctx)` 은 **참조 비교**(같은 배열 = 아무것도 안 함) · 썸네일 DOM 은 `key` 로 재사용 · 순서가 어긋난 노드만 옮기고 사라진 키만 뗀다. `update(ctx)` 는 매 프레임 불러도 되고 **타이머가 있는 썸네일이 있을 때만** 1초에 한 번 게이지 · 글자를 쓴다. 시각은 `ctx.net.serverNow() ?? Date.now()`. 디버그 `state`(`BuffCellState[]` — key · kind · glyph · color · dim · debuff · ratio · time · title) · `count`. **2026-09-13 (요리 재료 티어):** 요리 썸네일의 DOM `title` 은 `charBuffTitle` 아래에 능력치 줄 전부(`hud/mealText.mealEffectLines`)가 한 줄씩 붙는다 — `state[].title` 은 `charBuffTitle` 그대로다(`smoke-buffs` 가 그 동일성을 본다). **2026-09-13 (요리 미니게임):** `meal` 의 `quality` 가 있으면 좌상단 금색 별 배지(`data-q` = `★3` → `styles/buffs.css` 의 `::after`, 미니는 별 하나)이고 DOM `title` 의 능력치 줄이 보너스 반영 수치(`mealEffectLines(meal, quality)`). `cooking`(조리 중)은 계약의 글리프 · 색 그대로, `title` 아래에 그 요리의 단계 줄(`cookStepsText`). `BuffCellState` 에 `quality`(추가만). |
| ~~`hud/EnvBadge.ts`~~ | **2026-09-12 삭제** (버프 썸네일 줄로 대체 — 노출은 `env_exposed` 디버프 썸네일이다; 「차단됨」 상태는 HUD 에서 사라졌다). 아래는 옛 설명: **행성 상시 환경 배지** (A-13, 2026-09-11). `player:envChanged {env, protected}` 의 **유일한** 소비자 — 판정(행성에 환경이 있나 · 준비물이 실려 있나)은 전부 `player/` 가 하고 이 위젯은 한 줄을 그린다. 이벤트는 **상태가 바뀔 때만** 오므로 `update()` 도 타이머도 없다. `env: null`(환경 없는 행성 · 훈련장 · 함선)이면 뜨지 않고, `protected` 면 **차분한 색 + 「차단됨」**, 아니면 **`ENV_COLOR` + 글로우 + 맥박 + 「노출」**(지금 체력이 깎이고 있다는 뜻). 게임플레이 레이어 좌측 상단(임무 시계 아래 · 계약 패널 위)이라 **레이드에서만** 보인다 — 레이어가 함선 · 타이틀에서 내려가므로 자기 게이트를 따로 두지 않고, 함선에서 오는 `env: null` 과 `.hud.hub .env-badge{display:none}` 이 이중으로 막는다. 이름 · 색 · 글리프 · 설명은 전부 `shared/labels` 의 `ENV_*` 표 (행성 터미널 브리핑 · 준비물 툴팁과 같은 원본). 스타일은 `styles/env.css`. `shownEnv` / `isProtected` getters. |
| ~~`hud/MealBadge.ts`~~ | **2026-09-12 삭제** (버프 썸네일 줄의 `meal` 로 대체, `styles/meal.css` 와 `.hud-badges` 행도 함께 삭제). 아래는 옛 설명: **식사 배지** (A-3c, 2026-09-11). `progress:mealChanged {meal, active}` 의 **유일한** 소비자 — 무엇을 먹었나 · 언제 실리고 언제 비는가는 전부 `progression/` 이 정하고 이 위젯은 **`active`**(이번 레이드에 실려 있는 요리) 한 줄을 그린다. 대기분(`meal`)은 여기서 안 그린다: 함선에서 차려 둔 요리는 식탁 화면이 말하고 이 배지는 「지금 내 몸에 무엇이 걸려 있나」다. 요리 글리프 + 이름 + `MEAL_BUFF_LABEL_KO` + 부호 있는 값 한 줄이고, `tier === 2` 면 `.is-special`. 이벤트는 상태가 바뀔 때만 오므로 `update()` 도 타이머도 없다(`EnvBadge` 와 같은 결). 레이드 경계(`game:newMission` · `game:abort`)에서는 **비우지 않고 `ctx.progression.getActiveMeal()` 에 다시 묻는다** — 출격이 `armPreps()` 로 식사를 싣는 순간과 이벤트 순서를 이 폴더가 보증할 수 없어 지웠다가는 실린 식사를 놓친다(구현이 아직 없으면 `typeof … === 'function'` 으로 조용히 빈다). 자리는 `EnvBadge` 와 **한 줄에 나란히**(`.hud-badges`) — 게임플레이 레이어 안이라 **레이드에서만** 보인다. 문장은 `hud/mealText` 하나에서 온다. 스타일은 `styles/meal.css`. `shownMeal` getter (`HudSystem.mealBadgeDefId`). |
| ~~`hud/GymFatigueBadge.ts`~~ | **2026-09-12 삭제** (버프 썸네일 줄의 `gym_fatigue` 로 대체, `styles/gymFatigue.css` 도 삭제). 아래는 옛 설명: **운동 디버프 배지** (A-3a, 2026-09-12) + `styles/gymFatigue.css`. **함선 전용** — 헬스장 운동 뒤 걸리는 현실 시간 디버프(근력 = 근육통 · 지구력 = 심폐 피로, `GYM_FATIGUE_LABEL_KO`)마다 `⚖ 근육통 23:12:05` 칩. 레이드 배지와 반대로 **소셜 레이어**에 살며 `HubDot` 과 같은 조건(`phase === 'hub'` · blocker 없음 · 도킹 / 워프 컷씬 아님)으로 스스로 게이트한다. 끝 시각은 `ctx.progression.getGymFatigueUntil(stat)`(optional)에 이벤트(`progress:gymFatigue` · `progress:trainedChanged` · `hub:entered` · `net:profileLoaded` · `game:phaseChanged`)가 올 때만 다시 묻고, 글자는 디버프가 있고 보일 때만 1초마다 쓰며 0 초에 칩을 스스로 내린다. 시각 `ctx.net.serverNow() ?? Date.now()`. 자리는 좌측 하단(함선에서 빈 체력 자리 — 좌측 상단은 튜토리얼 패널). 접두사 `gfat-`. `shownChips` / `isShowing` getters (`HudSystem.gymFatigueChips` · `isGymFatigueOn`). |
| `hud/mealText.ts` | **요리 버프 문장 헬퍼** (A-3c, 2026-09-11) — `hud/stratagemGlyphs` 와 같은 성격의 폴더 공용 표현 모듈. `mealBuffAmountText(buff, amount)` 는 `+15 %` / `+6 kg` / `−20 %` 한 조각, `mealBuffText` 는 라벨까지 붙인 한 줄. **식사 배지와 아이템 툴팁이 같은 문장을 써야 하는데** 둘이 각자 포맷하면 같은 요리가 화면마다 다르게 읽힌다(「같은 것을 두 곳이 쓰면 하나로 뽑는다」를 계약을 건드리지 않고 폴더 안에서 적용한 것). 단위를 정하는 표는 **`shared/labels` 의 `MEAL_BUFF_UNIT` 하나**다 — `'%'` 인 줄만 `amount × 100`(배수 가산이라 0.15 = +15 %), `'kg'` · `'m'` 은 그대로, `''` 는 숫자만. 여기서 새 단위를 판단하지 않는다. `durabilityLossMul` 만 `amount` 가 음수인데 **부호를 뒤집지 않는다** — 「장비 손상 −20 %」가 그대로 이득으로 읽힌다. **2026-09-13 (요리 재료 티어):** 요리 하나가 능력치 줄을 여러 개 갖는다(`MealDef.effects`) — `mealEffects(meal)`(없으면 `[{buff, amount}]`; `progression/derive.mealEffectsOf` 와 같은 규칙인데 폴더끼리 import 하지 않으므로 한 벌 더 있다) · `mealEffectLines(meal)`(라벨까지 붙인 줄 목록) · `mealEffectsText(meal, sep = ' · ')` · `mealTierLabel(meal)`(`MEAL_TIER_LABEL_KO`). 요리를 보여 주는 곳(툴팁 · 버프 썸네일 title)은 이것을 쓴다. **2026-09-13 (요리 미니게임):** `mealEffects(meal, quality = 0)` · `mealEffectLines(meal, quality)` · `mealEffectsText(meal, sep, quality)` 가 품질을 받아 줄마다 `amount × (1 + mealQualityBonus(q))` 로 곱한다(progression `applyMealBuff` 와 같은 식, 품질 0 이면 원래 배열). `mealQualityText(q)` = `★★★☆☆ +15 %`(0 이면 빈 문자열), `cookStepsText(defId)` = `① 썰기 → ② 젓기`(`cookStepsOf`, 조리대 요리가 아니면 빈 문자열). |
| `hud/RoverHud.ts` | **탐사 차량 탑승 HUD** (2026-09-13). `ctx.world.rover.localAboard` 동안 게임플레이 레이어 하단 중앙에 `.rv-hud` — `탐사 차량` 태그 · 체력 `n / 2,000` · 체력 바(25 % 이하 붉게 `.low`) · 상태 한 줄(`정차 중 · M 목적지 선택` / `출발까지 N초 → 정류장 X` / `이동 중 → 정류장 X · 하차 불가`). 게임플레이 루트에 `rover-view` 를 걸어 전투 위젯을 숨긴다(`drone-view` 와 같은 방식). 정차 중에만 `ui:keyGuide {owner:'rover'}` 로 `M 목적지 선택` (이 owner 에는 가이드가 닫기를 붙이지 않는다). 하차 E 홀드 프롬프트는 player 의 상호작용 프롬프트 몫이라 여기 없다. `HudSystem.update` 가 레이어 가시성과 무관하게 부른다. 스모크 훅 `isShowing` (`HudSystem.isRoverHudShowing`). |
| `hud/RaidAlerts.ts` | **(2026-09-13 추가)** `fog:discovered {kind:'rover'}` → `탐사 차량 정류장 발견`, `rover:stationsRevealed` · `rover:tripStarted`(내가 냈거나 탔거나 차 곁 60 m 안일 때만) · `rover:arrived {trip}`(내가 탔을 때 `정류장 X 도착 — 하차`) · `rover:destroyed` · `rover:refused`(사유 그대로 경고) · `rover:damaged`(내가 탄 차 체력 50 % / 25 % — 탑승 한 번에 문턱마다 한 번) 토스트. 아래는 옛 설명. | **레이드 알림** (2026-09-09) — DOM 이 하나도 없는 이벤트 → 토스트 변환기다(그리는 일은 `hud/Notifications` 가 한다). ① **새 랜드마크 발견**: `fog:discovered` 의 `structure` · `rail` · `grove`. 구조물은 `ctx.world.getStructures()` (실패하면 `structureAt`) 에서 `StructureKind` 를 찾아 `STRUCTURE_LABEL_KO` 로, 선로는 `getRailLines()` 에서 `RAIL_LABEL_KO` 로 이름을 붙인다. **`world/Fog` 의 `TOAST` 표에 이 셋을 넣으면 토스트가 두 번 뜬다** — 넣지 않기로 한 자리다 (신호소 · 둥지는 계속 Fog 가 띄운다). ② **레이더 강하 예고**(옛 로그 강하): `rogueDrop:incoming` → `레이더 n명 강하 감지 — n초` danger 토스트 (2026-09-13 — 강하에 분대장이 없어 `(분대장 포함)` 갈래 삭제), `dropId` 로 한 번만. **2026-09-10: 소리는 여기서 내지 않는다** — `wave_alarm`(벌레 웨이브와 같은 소리였다)을 걷어냈고 `audio/AudioSystem` 이 같은 이벤트로 `rogue_drop_alarm` 을 울린다 (거리 감쇠가 붙은 소리라 크기를 정할 수 있는 곳이 거기뿐이고, 한 사건에 두 폴더가 경보를 울리면 겹쳐 난다). 화면 표시는 `hud/DangerIndicators` 가 그린다. |
...
| `hud/Vitals.ts` | Bottom-left. **2026-09-10 (레이드 HUD 개편, 사용자 결정):** 위에서부터 **이름**(`.vt-name`) · **실드 게이지**(`.sh-bar`) · **체력 게이지**(`.hp-bar`) 세 줄이고 **체력 수치(`.hp-num`)와 `생명력` 라벨은 없어졌다**. 두 게이지는 폭 · 정렬 · 눈금이 같다 — 한 칸 = `ARMOR_SHIELD_PER_SEGMENT`(20, `data/constants.csv`) 라 체력 100 은 **5등분**이고 실드 칸 수는 `maxShield / 20` (일반 1 · 고급 2 · 희귀 3 · 서사 4 · 전설 5). 칸 색은 방탄복 등급색(`rarityColor(ctx.player.shieldRarity)` → `--shc`), 값은 `ctx.player.shield / maxShield` 폴링 + `player:shieldChanged` (둘 다 본다). **방탄복이 없으면(`maxShield` 0) 실드 줄은 통째로 `hidden` 으로 접혀** 이름 + 체력만 남는다 (전투불능 동안에도 접힌다). 이름은 `ctx.net.playerName`(= 캐릭터 이름) → `ctx.progression.profile.name` → `스캐빈저` 순으로 고르므로 싱글 플레이에서도 나온다. 칸 DOM 은 **칸 수가 바뀔 때만** 다시 만든다(`setCells`). 게이지 자체는 예전 그대로 — smooth drain + 지연된 **연한 빨강 "damage ghost"**(2026-09-14, 사용자 결정 — **실드에도 같은 잔상이 생겼다**: `setCells` 가 만들어 두기만 하던 `.sh-seg .ghost` 에 `updateShield` 가 체력과 같은 식으로 transform 을 준다. `player:shieldChanged` 에 `delta` 가 없어 직전 목표값과 비교해 「깎였다」를 판정한다. 색 원본은 `base.css` 의 `.vitals { --vt-ghost }` 하나이고 전투불능은 더 진한 `--vt-ghost-down`), thin stamina bar (`.stamina`, polls `ctx.player.stamina/maxStamina`, damped; `.full` dims to 40 %, `.low` < 25 % amber, `.depleted` amber pulse on `player:staminaDepleted`), and nothing else — **2026-09-07** the 회복약 / 수류탄 pills under the bar are gone (both counts read off the right-hand column: the `QuickStrip` thumbnail and `WeaponPanel`'s consumable block), so this corner is health + stamina only. Reads `ctx.player.hp` each frame. **Downed mode** (`.vitals.downed`, `player:downed` → `player:revived` / `player:spawned` / `player:died` / `game:newMission` / `game:abort`): the bar snaps to and shows `downHp / PLAYER_DOWN_HP` in red (`player:downHpChanged`; polled from `ctx.player.downHp` while `ctx.player.isDowned`), label `전투불능 — 아군의 구조 대기 중`, `.down-sub` `Space 길게: 포기`, stamina bar hidden; `player:reviveProgress {t, byName}` → `.revive.show` `부활 중 <byName> … n%` + green progress bar (hidden on `t = -1`). **Phase 9 → 2026-09-08**: the Space give-up hold shows a red `포기` **caption** (`.giveup.show`, `player:giveUpProgress {t}` from player/, `t < 0` hides it) — and leaving the downed state hides it too, so a revive / respawn never leaves a stale one. The progress bar that used to sit under it moved to the crosshair ring (`hud/HoldGauge`, `.hold.is-giveup`). Debug: `HudSystem.isGiveUpBarOn`, `shieldSegments` / `displayName` getters. 스타일은 `styles/raidHud.css`. **2026-09-12 (캐릭터 버프, 사용자 결정):** 블록 루트가 **소셜 레이어**로 옮겨 **함선에서도 보인다** — `new Vitals(staminaParent, vitalsParent)` 이고 HudSystem 은 `new Vitals(hudRoot, null)` 로 스태미나 바만 게임플레이 레이어(= 레이드 전용, DOM 자리 그대로)에 만들고 루트를 `.hud-bl` 뒤에 붙인다. 함선(`hub` · `docking`)에서는 체력 · 실드를 **늘 가득**으로 그린다. 레이드 규칙은 그대로다: 게임플레이 페이즈의 소셜 레이어는 게임플레이 레이어와 같은 조건으로 뜨고, `.hud.spectating .vitals` 도 이 레이어에 걸리며, 드론 시점 축소는 `drone.css` 의 형제 선택자 `.hud.gameplay.drone-view ~ .hud.social .vitals` 로 닿는다. 체력바 **바로 아래** `readonly buffs: BuffStrip` — `ctx.player.buffs` 참조 폴링 + `player:buffsChanged`, `setDebugBuffs(list | null)` 스모크 덮어쓰기. |
...
| `hud/TypingBubbles.ts` | **입력 중 말풍선** (`.typing-bubbles` / `.tbubble`, social layer, 2026-09-09). For every **remote** player whose snapshot carries `PlayerFlags.TYPING` (net raises it while the chat input is open; never for the local player) a small dark rounded bubble with three dots (`<i>` × 3, CSS `tbubbleDot` scale 0.6→1.2, `animation-delay` 0 / 0.15 / 0.3 s) is projected from `avatar.getHeadPosition()` + **0.7 m** (Nameplates' 0.35 + 0.35 so it rides just above the plate). Same gating as `Nameplates`: missions and the hub / docking phases, hidden when `avatar` is null, `!connected`, `stale`, `DROPPING` / `IN_POD`, behind the camera, off-screen or > 150 m (fades from 110 m). Elements are created lazily per peer the first time they type and pooled; removed on `net:remotePlayerRemoved`, cleared on `net:lobbyLeft` / `game:abort` / `game:newMission`. `HudSystem` constructs it right after `Nameplates`, binds / disposes it with the core widgets and runs `typing.lateUpdate(ctx)` next to `nameplates.lateUpdate` while the social HUD is visible. **2026-09-11 (B-11):** a squad-mate whose 아이디 is on my **차단 목록** gets no bubble — the gate is `menus/social/socialSource.isPeerBlocked(ctx, id)`, the very function `hud/ChatLog` drops their typed lines with, and it is read **before** an element is created (a blocked peer costs no DOM; the lookup only runs for someone who is actually typing). 이름표 · 핑 · 월드 마커 are deliberately untouched — where a squad-mate stands is gameplay information. `setDebugRefs(refs)` = the `Nameplates` smoke hook (extra refs from `remotePlayers.debugSpawn`, fed by `HudSystem.debugRemotes`). `visibleCount` / `visibleIds` getters (`HudSystem.typingBubbleIds`). |
...
| `hud/Notifications.ts` | Right-center stack (max 6, social layer → also visible in the hub) with kind-colored left border and slide/fade dismiss. Feeds: `ui:notify`, `inventory:itemAdded` (rarity-colored name), `inventory:full`, `inventory:bagChanged` with `dropped.length > 0` → `가방이 작아져 아이템 n개를 떨어뜨렸습니다` (warning; `weapon:broken` toasts come from weapons), `enemy:waveStarted` ("적 증원 감지!"), `crate:looted`, `player:stimUsed`, all `extraction:*` milestones. **Down / revive / rescue:** `player:revived {hp}` → `부활 — 체력 <hp>` (success); **2026-09-09** `rescue:called` → `<이름> 구조선 호출됨` (success, label `구조`) 와 `leader:deviceDropped` → `분대장 기기가 떨어졌습니다` (warning, label `분대장`) 가 사라진 `game:respawnAvailable` 자리를 대신한다 (`분대장이 되었습니다` 토스트는 `game/parts/Leader` 가 `ui:notify` 로 띄운다). **Squad feed** (label `분대`): `net:remoteDied` → `{name} 전사`, `net:remoteDowned` → `{name} 전투불능 — 구조 필요` (danger), `net:remoteRevived` → `{name} 부활` (success), `net:peerJoined` → `{name} 합류`, `net:peerLeft` → `{name} 이탈` (**2026-09-11 B-12: the only owner of those two lines** — `hub/HubSystem` 의 `{name} 함선 합류 / 이탈` 두 줄을 지웠다; 게이트가 없는 것은 일부러다 — 레이드 · 훈련 중에도 알아야 한다, 그리고 이 줄이 초대 수락(`social:inviteResult {accepted}`)의 **유일한** 알림이다), `net:lobbyLeft {hostLeft}` → `호스트가 나갔습니다`, `pickup:taken` (remote) → `{byName} 획득: {item}`. **Network / hub feed:** ~~`net:reconnecting` / `net:resumed`~~ (**2026-09-11 B-1: removed** — they doubled `game/GameFlowSystem`'s in-mission lines and `hud/NetBadge` + `hub/parts/Transitions.onResumed` in the ship), `net:matched` → `공유 함선 신호 포착` / `신호 송출 시작 — 대원 대기 중` (created), `hub:launchCountdown` → `발사 n초 전` (once per second, 1.1 s toasts). **Ship calls (Phase 3, label `호출`):** `stratagem:called` with a remote `caller` → `<이름> 함선 호출: <name>` (warning; name via `ctx.net.getLobbyPlayer/getRemotePlayer`, own calls are silent), `stratagem:landed` for `orbital_laser` / `airstrike` → `착탄 — <name>` (danger), `structure:destroyed` → `엄폐물 파괴` (warning, deduped 1 s), `stratagem:cooldown {remaining:0}` after a running cooldown → `함선 호출 준비 완료` (success). Names are `escapeHtml`'d. Cleared on `game:abort` / `game:newMission`. **2026-09-14 (NPC 퀘스트, label `퀘스트`):** 옛 `meta:questChanged {state:'complete'}` → `퀘스트 완료 · name`(`QUEST_DEFS`) 토스트는 기업 퀘스트와 함께 **없어졌다**. 대신 `npc:objectiveProgress {done:true, raid:true}` → `퀘스트 목표 달성 — <퀘스트>: <목표 라벨>` (success, 라벨은 `ctx.meta.npc.getQuest` 의 `objectives[].label`; `done:false` · 함선 목표는 조용), `npc:questReady` → `<퀘스트> — 함선에서 메신저로 완료 보고` (info 5 s), `npc:questChanged {state:'complete'}` → `퀘스트 완료 · <퀘스트> — 크레딧 +n · XP +n · <기업> 신뢰도 +n · <아이템> ×n` (success 5 s, 이름은 `getQuest` → `NPC_QUEST_MAP`). 셋 다 **훈련장에서는 띄우지 않는다**. 새 NPC 메시지(`npc:message`) 토스트는 메신저(`menus/messenger`)의 몫이다. **Phase 5:** `meta:contractAccepted` / `meta:contractAbandoned` → `계약 수락 / 포기 · name` (`CONTRACT_DEFS`), `meta:purchase` → `구매: name · 크레딧 −price C (창고)`, `meta:sale` → `판매: name ×qty · 크레딧 +credits C` (Phase 10: amounts through the shared `formatCredits`) (item names / rarity colours via `ctx.loot.getItemDef`). **Phase 7:** `net:hostChanged` → `호스트 변경: <name>` (+ `(나)` when `isLocalHost`; label `네트워크`, warning), `net:peerSuspended` → `<name> 연결 끊김 — 자리 유지 중` (warning) / `<name> 재연결` (success), `game:newMission {mode:'training'}` → `시뮬레이션 훈련장 입장 — 탄약 · 내구도 미소모, 출구 콘솔로 종료` (after the clear), `training:exitRequested` → `시뮬레이션 훈련장 퇴장 — 장비 복원`. `meta:purchase` lines are unchanged (meta's async buy raises the event when the item lands). **Phase 11** (label `소셜`, since this folder is the one toast owner and the social panels never toast themselves): `social:error` → the Korean `SOCIAL_ERROR_MESSAGE_KO` line (warning), `social:play` → `<이름> 분대에 합류합니다` (`joined`, success) / `<이름>에게 분대 초대를 보냈습니다` (`invited`), `social:invited` → `<이름> 분대 초대 — <keyLabel(Keys.INVITE)> 홀드로 참여`. **2026-09-11 (A-3c 공유 식탁, label `식탁`):** `housing:mealServed {defId, by}` → `<이름> 님이 <요리 이름> 을(를) 차렸습니다` (success, 이름은 `rarityColor` 로 물든다). 「한 명이 차리면 분대 전원이 받는다」(사용자 결정)의 알림이고, **토스트를 띄우는 것은 여기 하나**다 — 합류 · 이탈 토스트와 같은 규약(B-12)이라 housing · net 은 이벤트만 낸다. **2026-09-13 (요리 재료 티어, label `분석`):** `housing:analysisFound {defId}` → `분석 도감 — <산출물> 발견` (success 4 s, 이름은 `rarityColor`), `housing:analysisLevelUp {family, level}` → `<계열> 분석 Lv.n — 해석 시간 ×<analysisTimeMul(level)> · 새 결과: <이름들>` (success 5 s; 이름은 `ANALYSIS_RESULTS` 중 같은 계열 · `minLevel === level` · 가중치 > 0 · 은퇴 아닌 def, 중복 제거 — 없으면 `새 결과` 부분을 생략). 옛 `housing:sampleDexAdded` 는 더 나지 않고 ui 에 소비자도 원래 없었다. **2026-09-13 (요리 미니게임):** `housing:cookResult` 성공 = `<요리> ★★★★☆ → 함선 창고|가방`(label `요리`), 실패 = `<요리> — <result.reason>` 경고. `housing:mealServed` 토스트에 차린 요리의 별(0 이면 생략). `craft:completed` 는 산출물에 `cookStepsOf` 단계가 있으면(= 조리대 요리) 토스트를 내지 않는다 — 결과 토스트가 대신한다. |
...
| `hud/FallVignette.ts` | **낙하 붉은 비네트** (`.fall-vignette`, `#ui-root` 직계, 2026-09-15, TODO B-14). `player:fell {damage}`(로컬 · 실제로 깎였을 때만) → 세기 `min(1, damage / FALL_VIGNETTE_FULL_DAMAGE)`, 이미 더 진하면 무시(최댓값) · 더 세면 다시 시작, `FALL_VIGNETTE_S` 동안 `k²` 곡선으로 사라진다. **사라짐은 CSS 전이가 아니라 `update(dt)`** 가 인라인 `opacity` 로 쓴다 — 시뮬레이션 시간을 따르고 `prefers-reduced-motion` PC 에서도 똑같다. 0 이면 `hidden`. `game:abort` · `game:newMission` · `hub:entered` 에서 즉시 꺼진다. 분대원 낙하(`player:remoteFell`)는 소리 전용이라 듣지 않는다. 스타일 `styles/fall.css`, getter `opacity`(HudSystem `fallVignetteOpacity`). |
| `hud/DeployOverlay.ts` | Bottom-center "행성 표면 강하 중…" sweep bar, visible during phase `deploying`. |
| **hud/ — tactical kit** | |
| `hud/ImplantWidget.ts` | **전술 임플란트 썸네일 — 화면 중앙 하단, 스태미나 바 아래** (`.imp-hud`, 2026-09-10 재작업; 2026-09-06 의 크로스헤어 좌측 세로 게이지 `.implant-gauge` 를 대신한다). 조준점 주위에서 숫자를 읽게 하지 않는다는 사용자 결정이라 쿨타임 · 충전 수 · 게이지가 전부 이리로 내려왔고 **이름은 적지 않는다** — 가로로 긴 썸네일(글리프) + 그 아래 `Keys.IMPLANT` 키캡뿐이다. 표시 유형 셋은 `ctx.implants` 가 주는 값으로 갈린다(코드에 수치를 적지 않는다): **쿨타임형**(갈고리 · 정찰 — `maxCharges === 1`) = 딤드된 썸네일이 `--fill` 로 **아래에서 위로 밝아지고** 중앙에 남은 초, **충전형**(대시 — `maxCharges > 1`) = 우측 하단에 충전 수, 0 이면 쿨타임형과 같은 딤드 + 밝아짐, 1 개 이상이면 딤드 없이 **강조색이 위로 차오르며** 다음 충전을, 최대면 정상 표기, **게이지형**(배리어 내구도 · 오버차지 에너지) = 썸네일 안 **중앙 하단의 가로 게이지**. 배리어 붕괴 잠금(`barrierLockout`) 중에는 쿨타임형과 같은 딤드 + 밝아짐 + 남은 초다 — 내구도가 잠금 시간에 맞춰 0 → 만충으로 차오르므로 게이지가 그대로 진행도다. 밝아짐은 같은 얼굴(`.ib-face`) 두 장 — 딤드된 바탕 위에 밑에서부터 `--fill` 만큼만 드러나는 밝은 사본 — 으로 만든다. 값은 매 프레임 `ctx.implants` 에서 읽고 `implant:*` 는 늦은 등록과 연출(`pulse` / `hit`)에만 쓴다. 장착 없음 · **전투불능**(2026-09-08 규칙 유지) 이면 숨고, blocker 는 `.blocked`, `.hud.targeting` / `.spectating` 은 CSS. 짧은 화면(≤760 / ≤620 px)에서는 썸네일이 줄어 스태미나 바와 겹치지 않는다. 스타일 `styles/implant.css`, getters `shownId` / `displayKind` / `fillAmount` / `isDimmed`. |
| `hud/DangerIndicators.ts` | **위험 인디케이터** (`.dgr`, 게임플레이 레이어, 2026-09-10). "지금 날아오고 있는 것" 하나마다 **화면 안이면 머리 인디케이터(`.dgr-head`), 밖이면 방향 호(`.dgr-arc`)** 를 그린다 — 한 목표는 정확히 한 요소를 쓰므로 **둘이 같이 뜨지 않는다**. 호는 피격 방향 호(`hud/DamageOverlay` 의 `.dmg-arc`)와 같은 언어(크로스헤어를 감싸는 240 px 링의 한 쐐기, 각도 = 카메라 기준 상대 방위, 0° = 정면 · 시계방향)이고 색만 위험물별(`--dc`)이다. 대상 셋(사용자 확정): **적 곡사포탄**(`enemy:shellFired` → `@/shared/ballistics` 의 `shellLaunchVelocity` / `shellPositionAt` 로 실제 포탄과 **같은 포물선**을 적분한다 — 수식을 베끼지 않는다; `shellLanded` / `shellIntercepted` 에 지우고 `flightTime + 2.5 s` 안전망), **수류탄**(`ctx.weapons.getGrenades()` — 내 것 + 원격 복제본, 라벨은 남은 신관), **함선 호출 낙하물**(`stratagem:called` → `landed` / `ended`). **2026-09-10 추가 — 넷째 `레이더 강하 포드`**(2026-09-13 전 이름 로그 강하): `ctx.enemies.getRogueDrops()` 를 매 프레임 직접 묻는다(예고에서 착지까지 살아 있는 목표라 캐시한 이벤트보다 매니저의 목록이 정확하다). ⬇ **빨강**(적의 것), 라벨은 착지까지 `n초` → 착지 순간 `레이더 n` (2026-09-13 — 분대장 갈래 · 진한 빨강 삭제). 2026-09-09 에 `hud/OffscreenIndicators` 가 그리던 `.oarrow.drop` 화살표를 여기로 옮겨 온 것이라 **한 목표가 두 언어로 그려지지 않는다.** **인지력 반경 게이트**: 포탄에만 걸리고(`derived.enemyDetectRadius`), **착탄 지점이 `DANGER_NEAR_RADIUS` 안이면 인지력과 무관하게** 보여 준다 — 인디케이터의 목적이 "날아오는 줄도 모르는 것" 을 알리는 것이기 때문이다. 수류탄 · 낙하물에는 게이트가 없고, **로그 강하는 인지력을 아예 보지 않고 전용 반경 `ROGUE_DROP_ALERT_RADIUS`(260 m = 인지력의 10배) 하나를 본다** — 대기를 찢고 떨어지는 굉음이라 인지력이 좁아도 알아야 하고, `audio/AudioSystem` 의 강하음이 쓰는 반경과 **같은 값**이라 "들리는데 안 보인다" 가 없다. **전장의 안개 게이트는 걸지 않는다**(`OffscreenIndicators` 의 2026-09-09 근거 그대로 — 발견된 오브젝트가 아니라 지금 벌어지는 사건이다). DOM 은 풀링(머리 10 + 호 10)이고 반올림한 key 가 바뀔 때만 쓴다, 후보도 풀이라 프레임당 할당 0, 가까운 것부터 상한. 메뉴 / 지도 / 사망에는 전부 숨고 `game:abort` · `game:newMission` · `hub:entered` 에서 비운다. 2026-09-09 의 `hud/ShellMarkers`(+ `styles/shellMarkers.css`)를 흡수하고 그 파일은 삭제했다. **2026-09-15 (B-16) — 여섯째 갈래 `fire`(`화염 지대` · ▲)**: `ctx.enemies.getFireZones?()`(적 소이, `hostile` → 빨강) + `ctx.gadgets.getFireZones?()`(플레이어 화염 · 소이 수류탄 → 호박), 수류탄과 같은 색 규칙(`hot` 이면 짙게). 게이트는 **가장자리 거리** `FIRE_ZONE_DANGER_RANGE` 하나(인지력 · 안개 없음), 머리 마커는 지대 중심 + 남은 초, 안에 있거나 가장자리 1.5 m 안이면 `hot`. 지대는 **머무는** 위험이라 후보를 맨 마지막에 모으고 정렬 키에 `FIRE_RANK_BIAS` 를 더해 칸이 모자라면 늘 날아오는 위험 뒤로 밀린다. 슬롯 요소에 `data-cat`(갈래 이름)을 단다. 스타일 `styles/danger.css`, getters `visibleCount` / `trackedCount`. |
...
| `hud/WeaponChargeGauge.ts` | **Unique-weapon wind-up arc** (`.wcharge`, gameplay layer). **2026-09-15: 종류 클래스는 `wc-charge` · `wc-spinup` · `wc-slash`** (맨 `.charge` 가 함선 호출 링 `.charge` 규칙과 겹쳐 호가 위로 돌고 가득 차면 빨개졌다) · 전격총 라벨은 **퍼센트만**(`37%` … `100%`) · 가득 = 더 밝은 파랑 `#b8ecff` + 맥동 · 종류가 바뀌면 라벨 캐시를 비운다: 120° SVG arc (radius 48 px, `pathLength=1`, `stroke-dasharray = t 1`) bulging right of the reticle like the cook gauge, driven by `weapon:chargeChanged {kind, t}`. Kind class + colour via `--wc`: `.charge` electric blue (전격총 충전 볼트), `.spinup` amber (미니건 예열), `.slash` red (표창 용검 hold); label right of the arc `충전 n%` / `예열 n%` / `용검 n%`, at 1 `.ready` (pulse) with `충전 완료` / `사격` / `용검 준비`. Hidden on `t = −1` (cancel / release), `weapon:equipped`, `quick:equipped {item}`, death / down, mission reset. `activeKind` getter (`HudSystem.weaponChargeKind`). |
...
| `hud/KeyGuide.ts` | **(2026-09-13 추가)** owner `'map'` 의 닫기는 `Tab 또는 Esc 또는 M` (M = `keyLabel(Keys.MAP)`, Tab 이 여전히 첫 keycap). `NO_CLOSE_OWNERS`(`'rover'` — 화면이 아니라 Tab/Esc 로 닫히지 않는 탑승 안내)에는 닫기 항목을 붙이지 않고 `kg-close` 도 없다. 아래는 옛 설명. | **키 가이드** (2026-09-09; replaces `hud/HousingHint`, which is deleted — its bottom-centre key bar and `.housing-exit` chip are superseded). One line in the bottom-right corner, `R 회전 · X 버리기 · Tab 닫기`, for the topmost open screen / mode. Consumes **`ui:keyGuide {owner, keys}`**: an owner with keys is pushed onto a stack in open order (a re-emit — e.g. on `input:bindingsChanged` — updates it in place), `keys: null` pops it, the **top** owner is rendered (popups over a screen win). **The guide appends the close entry itself** (`keyLabel(Keys.INVENTORY)` + `닫기`, always last; re-rendered on `input:bindingsChanged`) — owners never list it, and that last item carries the extra class **`kg-close`** (2026-09-09) so something can point at just the close key (the tutorial's 함선 관리 닫기 step spotlights `.key-guide .kg-close`). DOM: `.key-guide(.show)` > `.kg-item` (`.keycap` + `.kg-label`) separated by `.kg-sep` `·`; a **direct child of `ctx.uiRoot`** at **z 84** (over the inventory window 50–80, the hub terminal, the 시설 관리 panel, the map 40, the tutorial spotlight 78 / popups 79–80; under the 일시정지 85). Hidden while the stack is empty, while the `'menu'` blocker is up (polled in `update()`) and while the chat input is open (`ui:chatToggled`). Stack cleared on `game:newMission` / `game:abort`. Same corner as `.ship-hint`, but they never coexist (the hint needs no blocker, the guide an open screen). Getters `owner` / `owners` / `entries` / `isShowing`. Emitters in this folder: `map/MapScreen` (`'map'`), `hud/Community` (`'community'`); hub/HousingMode emits `'housing'`, the hub terminal / 작업대 / inventory their own. |
| `hud/NetBadge.ts` | **서버 연결 배지** (2026-09-11, B-1) + `styles/netBadge.css`. Draws `ctx.net.link` (fed by `net:linkChanged`) as a small badge in the **top-right of the ship and the title** — hidden on the raid / training HUD (user decision), during a docking / warp cutscene and while any blocker is up in the ship. Lines: `서버 연결 중…` · `연결됨` (3 s after a connection lands, then gone) · `서버 재연결 중… (n)` · `오프라인 · 서버 찾는 중 (12초 뒤)` (live countdown; from the 3rd scheduled probe a second line `서버 주소를 확인하세요 — <url>`) · `서버 발견 — 함선에서 연결` (`found`) · `오프라인` + `이 PC 의 내장 서버를 시작하지 못했습니다` (`embedded`) · `추방됨` / `서버 인원 초과` / `다른 곳에서 접속됨` (+ reason line). `idle` draws nothing. **Title only** (the cursor is free there): `다시 시도` (`ensureConnected`) and `서버 설정` (HudSystem opens the 설정 overlay on its `network` section) buttons for `unreachable` / `refused`. **The one owner of the link transition toasts**, ship only: connected → reconnecting / unreachable `서버 연결이 끊겼습니다 — 다시 찾는 중`; down → connected `서버에 다시 연결되었습니다` (`서버에 연결되었습니다` when this page never had a connection) — swallowed when the same welcome emitted `net:resumed` (hub's own `함선에 재접속했습니다` line wins; decided in a microtask). A refusal gets no generic toast (`net:error` carries the reason — hub/HubMenu forwards it, C-59). DOM: `#ui-root` direct child `.net-badge(.show)(.nb-ship)(.nb-info/ok/warn/bad)` > `.nb-body` (`.nb-dot`, `.nb-main`, `.nb-sub`) + `.nb-actions` (`.ui-btn.nb-btn` ×2); z **83** (over the title `.menu`, under 캐릭터 선택 84 / 일시정지 85 / 설정 86); in the ship `.nb-ship` moves it left of the 커뮤니티 thumbnail. Every class is `nb-` prefixed (no clash with HUD widget names — the `kc-hold` lesson). Getters `isShowing` / `mainText` / `subText` / `hasActions`; `HudSystem.netBadgeState` for smokes. |
| `hud/MusicPlayer.ts` | **음악 재생 창** (2026-09-14) — `#ui-root` 직계 `.mus-player(.show)(.is-empty)`, **좌측 상단**(24 / 20, `.community` · `.net-badge` 가 쓰는 우상단을 피했다). 곡 제목 · 아티스트 · 음량 · 진행 막대 · `m:ss / m:ss` · 재생 방식(`MUSIC_MODE_LABEL_KO`) + **조작 넷** `◀`(이전) · `▶`(다음) · 재생 방식 전환(`.mus-btn-mode` — **누르면 될 값**을 적는다, 지금 값은 머리줄에 있다) · `■`(정지). 조작은 `HousingRef` 의 옵셔널 계약(`musicPrev` · `musicNext` · `setMusicMode` · `musicStop`)으로 `?.()` 호출이고 **메서드가 없는 빌드에서는 그 버튼을 숨긴다**. 표시 입력은 `housing:musicChanged {state}` **하나**이며 bind 때 `ctx.housing?.getMusicState?.()` 로 초기 상태를 한 번 읽는다(첫 이벤트를 기다리지 않는다). 볼륨은 `ctx.audio.settings.bgm` + `audio:volumeChanged {channel:'bgm'}` 다 — **소리는 나지 않는다**(사용자 결정, 위 `AudioChannel 'bgm'` 규약). 한 번도 이벤트를 못 받았으면 `MUSIC_PLAYER_OFF` 라 **창이 안 뜬다**. 재생 목록이 비면 창은 **뜨고** `꽂힌 레코드가 없습니다` 한 줄만 그린다(`.is-empty`, 곡 넘김 버튼은 `disabled` — 「켰는데 아무 반응이 없다」를 만들지 않기 위한 선택). **함선에서만** 보이고(`ctx.phase === 'hub'`) 메뉴 blocker(`'menu'`)가 서면 숨는다 — `hud/KeyGuide.apply()` 와 **같은 규칙**이라 인벤토리 · 지도 위에는 그대로 떠 있다. `game:newMission` · `game:abort` · `dispose()` 에서 상태와 DOM 을 비운다. 스모크 훅 `HudSystem.musicPlayerView`. |
| **hud/ — Phase 8 (함선 관리)** | |
...
| `hud/ShipManage.ts` | **시설 관리 screen** (`.ship-manage`, in the **`.hud.housing`** layer so the gameplay / social gating never hides it; the panels are `.interactive` because that layer is `pointer-events:none`). **Left `.sm-rooms`**: `SHIP_ROOM_COUNT` `.sm-room` buttons — `방 n` (1-based), `ROOM_PURPOSE_LABEL_KO[purpose]` (`.empty` when unassigned) and `n개` from `ctx.housing.getPlaced(i).length`; click → `ctx.housing.setManageRoom(i)` (the `.is-on` highlight follows the housing event, not the click). **Right `.sm-side`** (Phase 8 UI pass — it used to be a horizontal `.sm-bar` along the bottom): a vertical panel whose head reads `가구 · 방 n — 용도` (or `방 n — 용도 지정`) with a `빈 방으로` button beside it (hidden while `purposeBlock(room, 'empty')` refuses, so never on the built-in 작업실), showing **one of two lists**, both vertical scrollers whose wheel is captured locally so it does not also cycle the housing selection: `.sm-purposes` for an **empty** room — one `.sm-purpose` per assignable `RoomPurpose` with the **materials its 시설 증축 costs** (`ctx.housing.purposeCost` through `renderItemCost`; the prose description was dropped in the Phase 9 UI pass), a `다음 업데이트` badge for the inactive ones and `.is-blocked` + `disabled` + the reason from `ctx.housing.purposeBlock` — and `.sm-cards` otherwise, one `.fcard` per `ctx.housing.getFurnitureFor(purpose)` def (`.fcard-thumb`: def colour ring + a `MODEL_GLYPH` character per `FurnitureModelKind` + the `cols×rows` footprint; `.fcard-body`: name, `renderItemCost(def.craft, ctx.loot.getItemDef, ctx.inventory.countDefAll)` `.item-chip`s and `보유 n` from `getStored()`, `.is-empty` when none is in storage); click → `ctx.housing.selectFurniture(defId)` (clicking the selected card clears it). Cards are rebuilt only when the room / def list / storage / material counts change (`cardsKey`), purpose buttons only when a block reason changes (`purposeKey`). **Phase 9 UI pass:** the furniture side is behind two tabs (`.sm-tabs`, hidden while the 용도 지정 picker is up) — **가구 제작** (`.sm-cards`, the list above, each row gaining a `제작` button that calls `ctx.housing.craftFurniture`; this is the only place furniture is crafted) and **가구 창고** (`.sm-store`, one row per `getStored()` def — pieces this room accepts first, the rest under them dimmed + `disabled`; **2026-09-09: a store card click only selects (`.is-sel`), and each accepted card carries a `배치` button (`.fcard-place`) that drops the piece on the room's first free cell — `findFreeSpot` delegates to `ctx.housing.findFreeSpot(room, defId)` (2026-09-10: the scan itself moved into `housing/Rules.autoPlaceSpot`, x outer ascending × y inner descending = 화면 기준 좌측 상단부터 가로줄 우선, `AUTO_PLACE_YAWS = [1, 0]` so a bench faces **down** instead of the right wall, and the door mouth is kept clear); disabled with `자리 없음` in `.fcard-note(.is-full)` when nothing fits, `배치 가능` otherwise, `<용도> 전용` on blocked cards; the fit result is in the list's memo key**). Driven by `housing:shipManageChanged` (open / close / room), `housing:changed` / `inventory:changed` / `inventory:stashChanged` (refresh), `housing:furniturePlaced` / `furnitureMoved` / `furnitureRecovered` / `facilityUpgraded` / `roomPurposeChanged` (2026-09-09, re-scan the 배치 fit) and `housing:selectionChanged` (`.is-sel` only); hidden on `game:newMission` / `game:abort`. No blocker token. `isShowing` / `activeRoom` / `cardCount` / `purposeCount` / `furnitureTab` getters. **2026-09-08 (빈 방으로 = 100 % 환급)**: the header `빈 방으로` button opens the same confirm popup (title `방 n — <용도> 제거`, the refund chips from `HousingRef.facilityRefund`) and then calls `HousingRef.removeRoomFacility`. It used to call `setRoomPurpose(i,'empty')` straight — the *free* path housing takes **after** it has worked out a refund — so clearing a room here burned the 시설 증축 price and the player could not rebuild what they had just torn down by mistake. **2026-09-11 (온실 개편)**: `MODEL_GLYPH` gained **`grow_station: '✿'`** (`Record<FurnitureModelKind, …>` must be total — the missing key was a typecheck error, not a cosmetic one); the retired `grow_rack: '❀'` stays because a `Record` demands it and an old save's card must not fall through to the `?? '▨'` default. The screen needed nothing else: it draws `getFurnitureFor(purpose)` / `getStored()` straight, so housing/'s `FurnitureDef.retired` filter removes the 옛 재배층 here for free. ⚠ **A 가구 카드 shows no level and no upgrade cost** — the cards are per **def**, not per placed piece, and `HousingRef.upgradeFurniture` has **no caller anywhere in `src/`** (`git grep` 2026-09-11), so 작업대 Lv.2–3 and the new 재배 스테이션 Lv.2–3 are unreachable from any screen. The 재배 스테이션 gets exactly the benches' treatment here; opening that gate belongs to whoever owns the 강화 UI (the 재배 화면 `housing/ui/GrowStation` is the natural home). **2026-09-11 (연구실 · B-13)**: `MODEL_GLYPH` 에 `analyzer: '⌬'` · `bench_extract` · `bench_mixer` — 작업대 둘은 **`shared/WORKBENCH_ICON` 을 참조한다**(제작 탭과 같은 글자여야 하고 그 표가 원본이다), 분석기는 작업대가 아니라 스테이션이라 자기 글자를 갖는다. **클릭 인스펙터(`.sm-inspect`, 사용자 결정)**: hub/ 의 시설 관리 레이캐스트가 내는 `housing:furnitureSelected {uid}` 를 받아 **모달리스** 카드를 띄운다 — 글리프 · 이름 · `Lv.n / max` · 다음 강화 비용 칩(`HousingRef.furnitureUpgradeCost`) · 사유(`furnitureUpgradeBlock`) · `강화`(`upgradeFurniture`). `{uid: null}`(빈 곳) · ✕ · 방 바꾸기 · 화면 닫기가 내린다. **1초 홀드도 확인 팝업도 없다** — 강화는 되돌릴 수 없는 확정이 아니다(`housing/ui/GrowStation` 의 강화 줄과 같은 판단). blocker 도 escape 토큰도 잡지 않으므로 ESC 는 그대로 시설 관리를 나간다. 카드는 memo 키 없이 `refresh()` 마다 다시 그린다(요소가 열 개도 안 된다). 이것이 위 ⚠ 의 답이다 — `upgradeFurniture` 의 첫 호출자이고 작업대 · 재배 스테이션 Lv.2–3 이 이제 플레이로 도달한다. **제작 카드 잠금(사용자 결정)**: `refreshCards` 가 `HousingRef.furnitureCraftBlock(def.id)` 를 묻고 사유가 있으면 `.fcard.is-locked`(딤드 + dashed) + `title` 에 사유 + 인쇄된 `.fcard-note.is-locked` + `제작` 비활성으로 그리고 **목록 맨 아래**로 내린다(안정 정렬이라 같은 등급 안에서는 카탈로그 순서 그대로). 사유는 `cardsKey` 의 일부다 — 재료가 들어오면 카드가 딤드를 벗고 올라와야 한다. 세 질의는 구현이 늦게 붙을 수 있어 `typeof … === 'function'` 으로 감싸고 없으면 예전 질의(`canCraftFurniture` · `def.upgradeCost`)로 같은 답을 만든다. `isInspectOpen` / `inspectedUid` / `canUpgradeInspected` getters. **2026-09-11 (A-3c · A-14 · A-15):** `MODEL_GLYPH` 에 `bench_cook` · `bench_print`(원본은 `WORKBENCH_ICON` — 제작 탭과 같은 글자여야 한다) · `dining_table: '⊞'` · `culture_tank: '⚗'`(작업대가 아니라 스테이션이라 자기 글자). `Record<FurnitureModelKind, string>` 이 전부를 요구하므로 빠진 키는 typecheck 에러였고, 화면 로직은 한 줄도 안 바뀌었다. **2026-09-12 (하우징 모드 UI 개선, 사용자 결정):** ① 용도 지정 목록은 **이미 지은 용도를 그리지 않는다**(`builtElsewhere` — 모든 용도가 함선당 하나), `다음 업데이트` 용도는 잠긴 채 남는다. ② **발전기 행이 용도 목록에서 빠져 좌측 `.sm-rooms` 아래 `.sm-gen-host`** 로 옮겼다(`refreshGenerator`, 자기 memo 키 `genKey`; 클래스 `.sm-gen` / `.sm-gen-btn` 은 그대로라 튜토리얼 포커싱이 산다; `is-hint` = 발전기 레벨 < `ROOM_PURPOSE_BUILD_GENERATOR_LEVEL`). ③ 가구 제작 탭 안에 **시설 가구 / 꾸밈용 가구** 하위 탭(`.sm-subtabs` · `.sm-subtab[data-kind]`, `isUtilityFurniture` 로 가른다, 증축 직후엔 시설 가구). 이미 가진 시설 가구(배치 어디든 + 창고)는 버튼 글자가 **`이미 보유 중`**(비활성, `.fcard.is-owned`)이고 그 사유 줄은 인쇄하지 않는다 — 다른 사유(재료 부족 · 튜토리얼)는 그대로 인쇄한다. 시설 가구 카드에는 `보유 n` 이 없다. ④ 인스펙터 좌측 하단 **`위치 이동`**(`.sm-ins-move` → `housing:moveRequested {uid}`), `housing:moveStateChanged` 로 옮기는 동안 `이동 중`(비활성). ⑤ 인스펙터와 그 위 토스트가 하단 중앙 **`.sm-dock`** 한 줄기에 쌓인다 — `housing:placeRefused {reason}` → `.sm-toast`(2.2 s, 같은 문장이면 다시 번쩍임). 디버그 `craftKind` · `toastText`. **2026-09-13:** 자동 조리 가구 4종 글리프 확정 — 푸드 프로세서 `⌽` · 자동 그릴 `≋` · 자동 교반기 `⚲` · 계량 디스펜서 `⛛` (리드 임시 `⊗` `▦` `⊚` `⩡` 는 레코드랙 · 단계 칩 글리프와 겹쳤다). **2026-09-14 5차**: ① 여러 대 만들 수 있는 가구(`FurnitureDef.multi` — 책장 · 의자 · 쇼파 · 디스크 전시대 · 레코드랙 · 게임 디스크 전시대 · 연산 클러스터)의 **`이미 보유 중` 이 사라졌다** — 규칙 쪽 `furnitureCraftBlock` 은 2026-09-13 부터 `!def.multi` 를 보고 있었는데 **이 화면만 안 봤다**(`is-owned` 딤드와 목록 맨 아래 정렬도 같이 풀린다). ② 발전기 행의 **레벨별 해금 표**(`.sm-gen-unlocks` · `.sm-gen-unlock`)를 마크업 · CSS 와 함께 **삭제** — 같은 정보를 용도 지정 카드의 발전기 레벨 칩이 그 자리에서 말한다. ③ `.sm-cost` 두 곳도 `renderCostRow` 를 지나 `99+` 캡과 좌상단 앵커를 함께 받는다. ④ 가구 카드 썸네일이 **정사각형**이다 — `.fcard-thumb` 54 × 54(옛 54 × 44) · 인스펙터 머리 46 × 46(옛 46 × 40), 두 행에 걸치는 제작 카드에서는 `.fcard > .fcard-thumb` 가 `align-self: stretch`(옛 `start`)로 두 행을 정확히 채워 **썸네일 아래 여백이 사라졌다**(row-gap 6 → 4 라 이름 14 + 재료 36 이 54 와 맞아떨어진다). 가구 제작 탭과 가구 창고 탭이 같은 CSS 한 곳을 쓴다. ⑤ 시설 레벨 칩(`buildFacilityChip`)은 `nowrap` 인 `.fcard-cost` · `.inv-craft-costs` **안에서만** 정사각형이 된다(이름 숨김 + 글리프 가운데, 시설 이름은 `title`) — 이중 테두리와 `현재/필요` 는 그대로다. 넓게 감싸는 `.sm-cost` · `.sm-ins-cost` · 확인 팝업은 예전의 가로 2배 칩 그대로다. ⚠ 칩이 **늘 서는** 근거는 `housing/Rules.generatorRequirement` 가 2026-09-14 부터 **채워진 요구도 돌려주기** 때문이다 — 빈 배열이 「문제 없음」이 아니다. **2026-09-15 3차 (사용자 결정)**: ① 시설 레벨 칩이 **아이콘만**이고 시설 이름 · 현재 · 필요 레벨은 **호버 툴팁**(`facilityChip()` 이 `data-fc-tip` 을 적고, 모양 · 툴팁 규칙은 `housing/housing.css` 에 산다 — 칩을 쓰는 네 화면이 함께 쓰므로). ② 용도 지정 줄 오른쪽에 **`시설 증축` 버튼**(`.sm-purpose-build`) — 새 경로가 아니라 줄을 누르는 것과 **똑같이** `pickPurpose` → 확인 팝업 → `setRoomPurpose` 로 간다(막혀 있어도 눌리고 사유 토스트가 답한다). ③ **모자란 재료를 글로 적지 않는다** — `craftBlock` 폴백이 `재료 부족` 한 줄이고 나머지 사유의 원본은 housing 의 `Rules.MISSING_MATERIALS_REASON` 이다(재료 칩의 `.is-short` 가 무엇이 모자란지 말한다). ④ 확인 팝업 `.cost` · 인스펙터 `.sm-ins-cost` 에 `TIP_ANCHOR_ATTR` 을 **생성 때** 박아, 재료 칩이 없는 프레임(시설 칩만)에도 호버 카드가 좌상단이다. ⑤ 우측 탭(가구 제작 / 가구 창고)을 슬롯별 localStorage `scav.shipManage.tab` 에 기억한다 — **방** 쪽 기억은 housing 의 몫이다(`scav.housing.manageRoom`, `openShipManage` 가 그 방을 먼저 연다). |
| `hud/ItemTip.ts` | **2026-09-15 (가젯 개편, 사용자 결정)**: ① **스펙 줄은 `@/items` 의 `itemSpecRows(def)` 가 만든다** — 회복약 · 실드 충전기 · 전투 소모품 · 가젯 · 수류탄의 수치(맨 위는 언제나 `사용 시간`)이고, `inventory/ui/Tooltip` 이 **같은 함수**를 부르므로 두 카드가 같은 문장을 말한다. `보유` 줄 바로 아래에 들어가고 옛 `회복 +N HP` 줄은 그 안으로 접혔다. `SpecRow.tone` → 인라인 `--c-success` / `--c-danger`. ② **값이 조각 목록일 수 있다** (`TipRow` 의 두 번째 칸 `string | SpecSeg[]`) — 「5초간 매 초 HP 4 회복, 총 20 회복」에서 **숫자만 본문 색이고 나머지는 흐리다**(`--c-text-dim` 인라인). ③ **설명 문단에 인라인 마크업** — `renderDesc` 가 `items/ItemText.parseItemText` 로 `{em}` · `{dim}` · `{br}` 을 풀어 `<span>` · `<br>` 로 그린다(강조는 `--c-accent`). 토큰이 없는 설명은 예전과 똑같은 한 줄이다. ④ **내구도 줄이 가방 전용이 아니다** — 내구도를 들고 다니는 가젯(돔 실드 · 바리케이드)도 같은 `cur / max` · `최대 max` 줄을 얻는다. **재료 요구 칩 hover card** (`.item-tip`, Phase 8 UI pass). **2026-09-15: 전설 유니크 무기의 부제는 `무기` 대신 자기 종류**(`컴포짓 보우 · 전설`, `uniqueKindOf` ← `ctx.loot.getWeaponDef`). A **direct child of `ctx.uiRoot`** (z-index above every window and menu, `pointer-events: none`) with one delegated `pointerover` / `pointermove` / `pointerout` on `ctx.uiRoot`: any `.item-chip[data-def-id]` — the hook `src/shared/itemChip.ts` stamps on every chip it builds — or any `[data-item-tip][data-def-id]` element that opted in (Phase 9 UI pass: `inventory/ui/TradeGrids` stamps its 기업 거래 tiles) — shows the item's name, 분류 · 등급, description and a small stat table (보유 from `ctx.inventory.countDefAll`, 재배 시간 / 회복 / 가방 / 최대 묶음 from the `ItemDef`; hidden when no row is left). **Phase 10:** 가치 left that table for the card's **bottom bar** (`.it-value`, the card's last child, faint amber ground) rendered with the one credit formatter, `formatCredits(itemCreditValue(def))` → `1,200 C`; the old ` cr` suffix is gone. **2026-09-09:** the **`크기 (w × h)` row is gone** from every card (the bag grid already shows the footprint) and **무게 moved into the bottom bar** — `.it-value .wt` (`무게 1.2 kg`, `—` when the def has no weight) on the **left**, `.it-value .val` (`가치 1,200 C`, accent) on the **right**; the 재화 card (`.is-currency`) still hides the bar and the table. 희귀도 stays in the sub line. **2026-09-08:** an `implant` def also lists 장착칸 · 퍽 · 능력치 (· 상태 when broken) — the inventory's 임플란트 칸 is a row of square thumbnails with no words on them, so this card is where an equipped implant is read. Def data only: a chip has no `ItemInstance`, so there is no durability / socket section like `inventory/ui/Tooltip`. Hides itself on `game:phaseChanged`, `inventory:closed` and `housing:shipManageChanged` so a chip that vanishes under the cursor never strands the card. **2026-09-15:** while the card is up a rAF `watch` checks the described chip is still connected (and re-hit-tests the last pointer position for two frames after an inventory event) — a quick move that removes the tile under a still cursor re-points or hides the card. `isShowing` / `shownDefId` getters. **2026-09-11 (C-36 후속):** 가방 def(`durabilityMax`)면 `가방` 줄 아래 **`내구도`** 줄 — 호버한 요소가 `data-uid` 도 달고 `ctx.inventory.findItemAnywhere` 로 같은 def 인스턴스를 찾으면 `cur / max`(0 이어도 `파손` 이라 적지 않는다, `inventory/ui/Tooltip` 과 같은 규칙), 칩뿐이면 `최대 max`. 카드 캐시 키에 uid 가 들어간다. **2026-09-11 (온실 개편):** 토양(`def.soil`)은 **`속성`**(`SOIL_TAG_LABEL_KO[tag]`) · **`수확 n 회`**(`soil.uses`) 두 줄을, 씨앗(`def.seed`)은 `재배 시간` 아래 **`맞는 토양`**(`SOIL_TAG_LABEL_KO[seed.soilTag]`) 한 줄을 얻는다 — 어떤 흙에 심어야 `SOIL_MATCH_SPEEDUP` 를 받는지가 씨앗 카드에서 끝난다. 두 속성 줄의 **값 글자만** `SOIL_TAG_COLOR` 로 물드는데, 칠하는 방법은 `rows` 튜플의 세 번째 칸 → `vEl.style.color` 인라인이다: `.it-stats .v` 에 modifier 클래스를 새로 만들면 HUD 위젯 클래스와 이름이 겹칠 위험이 있다 (2026-09-10 `.hold` 사고). 기존 규약은 그대로 — 크기 줄 없음 · 무게 좌하단 · 가치 우하단. `soilTag` 가 비어 오면(옛 csv) 그 줄만 빠진다. **2026-09-11 (연구실 A-12 · A-13):** 표본(`def.sample`)은 **`분석기 해석 n 시간`** · **`산출물 <이름> ×n`** (· `firstDefId` 가 있으면 `최초 해석` 보너스 한 줄), 준비물(`def.prep`)은 **`사용 — 다음 레이드 1회분`** · **`차단 — <환경> 환경`** 을 얻는다. 환경 이름 · 색은 `ENV_LABEL_KO` · `ENV_COLOR` 하나에서 오고(HUD 배지 · 행성 브리핑과 같은 원본), 색은 토양 줄과 같은 이유로 **인라인 `style.color`** 다. 해석 시간은 **표의 기준 시간**이다 — 도감 진척으로 줄어드는 실제 시간은 분석 화면이 말한다(툴팁은 도감을 모른다). **2026-09-11 (주방 · 배양조 · 프린터 A-3c · A-14 · A-15):** 요리(`def.meal`)는 **`사용 — 다음 레이드 1회분`** · 버프 이름을 **행 이름으로** 쓴 **`<버프> +n`**(값은 `hud/mealText`, 인라인 `CATEGORY_COLOR.meal`) (· `tier === 2` 면 `구분 — 특선 요리`), 주머니(`def.pouch`)는 **`주머니 c × r`** · **`수납`**(`accepts` 를 `CATEGORY_LABEL_KO` 로), 세포주(`def.strain`)는 **`배양조 n 시간`** · **`산출물 <이름> ×n`**, 배지(`def.medium`)는 **`배양 n 회`** · **`배양 속도 +n %`**(`speedMul` 0.7 = +30 %, 1 보다 크면 부호가 뒤집힌다). **2026-09-13 (요리 재료 티어):** 요리 = `구분 — <티어 이름>`(`MEAL_TIER_LABEL_KO`, 은퇴 요리는 생략) · `사용` · **능력치 줄 전부**(`hud/mealText.mealEffects` — 옛 `구분 — 특선 요리` 삭제), 토양 = `속성` · **`내구도 최대 n`** · **`소켓 칸 n 칸`**(`growSocketSlotsFor(등급)`; `수확 n 회` 삭제 — 가방의 토양은 늘 새것이라 `최대` 다), 배지 = `내구도 최대 n` · `배양 속도` · `소켓 칸`(`배양 n 회` 삭제), 세포주 = 스캐폴드 산출이 있으면 **`스캐폴드 배양 n 시간`** · **`스캐폴드 산출 <이름> ×n`** 두 줄 더, 배양 스캐폴드(`def.scaffold`) = `사용` 한 줄 설명, 소켓(`def.growSocket`) = `종류`(`GROW_SOCKET_TARGET_LABEL_KO` · 끼우는 곳) · `<효과 이름>`(`GROW_SOCKET_EFFECT_LABEL_KO` — speed `시간 −n %` · wear `마모 −n %` · yield `n % 확률로 +1 개`, 값은 인라인 `CATEGORY_COLOR.socket`) · `장착`(영구 · 교체하면 파괴, speed · yield 는 내구도 비율만큼), 표본 = **`계열`**(`SAMPLE_FAMILY_ICON` + `SAMPLE_FAMILY_LABEL_KO`, 인라인 `SAMPLE_FAMILY_COLOR`) · `분석기 해석 n 시간`(표의 기준 시간) · `결과 — <계열> 결과표 — 분석 도감`(무엇이 얼마의 확률로 나오는지는 분석 도감이 말한다; `family` 가 없는 옛 def 만 옛 `산출물` · `최초 해석`), 은퇴 아이템(`def.retired`) = 맨 위 **`상태 — 더 이상 쓰이지 않는 아이템`**(흐린 글자 `var(--c-text-dim)`). 행 튜플 타입은 파일 머리의 `TipRow`, CSS 는 그대로다. **2026-09-13 (요리 미니게임):** 요리 = `구분` 아래 **`품질 — ★★★☆☆ +15 %`**(호버한 요소의 `data-uid` 인스턴스 품질 > 0 일 때만, 금색 인라인) · 능력치 줄은 보너스 반영 수치 · 마지막에 **`조리 — ① 썰기 → ② 젓기`**(단계가 없으면 줄 없음). **2026-09-14 5차 (재료 칩 카드는 커서 좌상단, 사용자 결정)**: `move(x, y, chip)` 으로 바뀌고 새 **옵트인 속성 `data-tip-anchor="left"`**(원본 상수 `TIP_ANCHOR_ATTR`)를 `closest` 로 읽는다 — 칩 자신에 찍어도 되고 줄 전체(`.fcard-cost` · `.sm-cost` · `.sm-ins-cost` · `.inv-craft-costs`)에 한 번 찍어도 된다. 기본(가방 · 창고 격자 타일)은 예전 그대로 **커서 우하단**이고, 어느 쪽이든 화면 밖으로 나가면 반대편으로 뒤집는다. 시설 관리 · 작업대 제작의 「필요 아이템」 줄은 화면 아래쪽이라 기본 자리에서 카드가 **위로만** 뒤집혀 결과가 「커서 우상단」이었고 **눌러야 할 칩을 카드가 덮었다**. |
| `hud/ItemFavoriteMenu.ts` | **즐겨찾기 우클릭 메뉴** (2026-09-12, E2 — 접두사 `.icm-`, 스타일 `styles/base.css`). `ItemTip` 과 같은 방식의 **`ctx.uiRoot` 위임 `contextmenu` 하나**: `.item-chip[data-def-id]` 전부 + `data-def-id` 옆에 `ITEM_FAVORITE_MENU_ATTR`(`data-fav-menu`)를 달아 옵트인한 요소(기업 상점 · 거래칸 타일) → 아이템 이름 머리 + `즐겨찾기 켜기 / 끄기` 한 줄 → `InventoryRef.toggleFavorite`(E1). 가지고 있지 않은 아이템(퀘스트 납품 · 보상 칩, `renderItemCost` 의 재료 칩, 계약 아이템 칩, 상점 물품)도 켤 수 있는 자리가 전부 여기로 온다. **버블 단계**라 안쪽에서 `stopPropagation` 하는 메뉴(인벤토리 타일 · 재배 스테이션 · 배양조 슬롯)가 이긴다. `toggleFavorite` 가 없는 inventory 에서는 뜨지 않는다. 닫기: 선택 · 바깥 pointerdown(window capture) · 휠 · Escape(`ctx.escape` 토큰 `ui:itemFavoriteMenu` — 창보다 먼저 닫힌다) · `inventory:closed` · `game:phaseChanged` · `housing:shipManageChanged`. `bind` 에서 `setItemChipFavoriteSource` 를 등록해 새로 만드는 칩이 `.is-favorite`(파란 사선 띠)를 달고 나오고, `inventory:favoritesChanged` 에서 DOM 에 이미 있는 같은 def 의 칩 전부의 클래스를 고친다. z 210(호버 카드 200 위). `isOpen` / `shownDefId` getters. 검사 `scripts/smoke-favorite-chips.mjs`. |
...
| `menus/messenger/textInput.ts` | 입력칸 배선 `wireTextInput(input, {onEnter, onEscape, onTab})` — 키를 게임에서 끊고(P 를 쳐도 패널이 안 닫힌다), 한국어 IME 조합 Enter 는 `compositionend` 뒤 한 번(옛 `SocialPages` 규약). 메신저 입력칸의 Escape · Tab 은 패널을 닫는다. |
| `menus/messenger/sources.ts` | `npcOf(ctx)` = 디버그 ref 또는 `ctx.meta.npc`, `roomsOf(ctx)` = 디버그 ref 또는 `ctx.net.rooms` (둘 다 없으면 null — 메신저는 없는 경우부터 그린다). `setDebugNpc` · `setDebugRooms` (`HudSystem.debugNpc` · `debugRooms`). |
| `menus/messenger/Trust.ts` | **(2026-09-14, NPC 개인 신뢰도 — docs/DECISIONS.md 「2026-09-14 — 정보상」)** 기업 신뢰도와 **별개**인 NPC 개인 신뢰도의 읽기 · 그리기. `npcTrustOf(ctx, npcId)` = `ctx.meta.npcTrust` 하나에서 읽어 `{trust, level, next, frac}`(레벨 · 구간은 공용 `REP_TABLE` · `repLevelOf`) — `ctx.meta` 가 없으면 **null** 이라 호출부가 그 자리를 비운다(meta/ 를 import 하지 않는다). `buildNpcTrust(ctx, npcId, name, {compact, color})` = `Lv.n` + `.ms-bar` 게이지 (+ `1,650 / 3,000`, `compact` 면 숫자 없이 짧은 게이지 — 2026-09-14 3차부터 목록 줄에서 빠져 부르는 곳이 없다) — 대화창 머리(`.in-right`) · 퀘스트 상세 머리(`.in-head`)가 같은 조각을 쓴다. **`buildNpcAvatar(ctx, npcId, name, {glyph, color})`** (2026-09-14 3차) = 신뢰도 **radial 고리**(conic-gradient 한 줄 · `--frac` = 지금 레벨 구간의 진행률)를 두른 `.ms-av.big` + 우하단 레벨 배지 `.ms-avlv` — 값을 못 읽으면 `.no-trust` 빈 테두리만이고 초상 자체는 늘 그린다. `buildTrustChip(name, amount, color)` = 퀘스트 카드 보상 칩: 재화 칩(`buildCurrencyChip`)과 **같은 틀**(`.item-chip.currency-chip` + 글리프 `◈`)이되 `data-currency-id` 를 안 붙인다 — NPC 신뢰도는 `data/currencies.csv` 의 재화가 아니고 가짜 재화 · 가짜 아이템 정의를 만들지 않는다(호버 카드 대신 네이티브 `title`). CSS 는 `styles/messenger.css` 의 `.ms-trust*`. |
| `menus/messenger/format.ts` | DOM 없는 글자 도우미 — `clockText`(말풍선 시각) · `clip` · `initialOf` · `roomSystemText`(공용 `roomSystemTextKo` 에 아이디 대체 이름만 채워 넘긴다). **2026-09-14 3차**: 목록 줄의 `방금 · n분`(`agoText`)은 줄에서 시각이 빠지면서 함께 지웠다. |
| `styles/messenger.css` | **메신저 스타일시트** (`Messenger` 가 import, 접두사 `ms-`). 틀은 `min(1180px, 100vw − 48)` × `min(780px, 100vh − 110)` 을 **위쪽에 붙여** 놓는다 — 가운데 두면 1280×720 에서 우하단 키 가이드가 입력칸의 `보내기` 를 가렸다. 탭 · 배지 · 초상(`.ms-av` + `pres-*` 점) · 목록 행 · 말풍선 · 입력칸 · 멤버 서랍 · 퀘스트 카드(`--qc` = NPC 색) · 퀘스트 목록 · 팝오버, 1320 px 아래 좁은 열. **2026-09-14 3차**: `.ms-avwrap`(conic-gradient 신뢰도 고리) · `.ms-avlv`(우하단 레벨 배지) · `.ms-trust.in-right`(대화창 머리줄 중앙 우측) · `.ms-bubble.ms-typing` + `@keyframes ms-type-dot`(타이핑 점 셋) 이 들어왔고, `.ms-row-sub` · `.ms-row-time` · `.ms-thead-bio` · `.ms-thread[data-kind='npc'] .ms-note` 규칙은 그리는 것이 없어져 지웠다 (`.ms-row-title` 은 `max-width: 60%` → `flex: 1`). **2026-09-14**: `.ms-trust` / `.ms-trust-lv` / `.ms-trust-bar` / `.ms-trust-num` / `.ms-trust-chip` — NPC 개인 신뢰도 게이지(기업 화면 `.corp-rep` 과 같은 계열, 색만 `--qc` = NPC 색), `.compact` = 목록 한 줄 · `.in-head` = 대화창 · 퀘스트 상세 머리. 썸네일 숫자 배지 `.cm-dot.has-num`, 친구 탭의 받은 요청 격자는 높이 제한을 푼다(수락 / 거절이 잘렸다). |
| **menus/social/ — Phase 11** | |
| `menus/social/socialSource.ts` | **(2026-09-14)** 합성 ref 도 `whisperUnread` · `markWhisperRead` · `whisperUnreadTotal` 을 갖는다 (받은 줄 중 상대별 readAt 보다 새 것). The **one** place `src/ui/` reads the social mirror from. `socialOf(ctx)` = the installed debug ref, else `ctx.net?.social`, else null; `socialReady(ctx)` folds in `available`; `SOCIAL_UNAVAILABLE_KO` is the single `소셜 기능을 사용할 수 없습니다` string. `setDebugSocial(snapshot, invites, mySquad)` builds a **synthetic `SocialRef`** from a plain snapshot for the headless smoke (the `debugRemotes` pattern): its getters are live, its mutators are real enough to test the UI (`respondFriend(code, true)` moves the row into `friends`, `acceptInvite` / `dismissInvite` drop the invite, `removeFriend` removes it) and every call is appended to the exported `debugSocialCalls` (`HudSystem.debugSocialLog`). `setDebugSocial(null)` hands the UI back to the relay. **`isPeerBlocked(ctx, peerId)`** (2026-09-11, B-11) is the one 차단 question the HUD asks: the 아이디 lives on the **lobby** row and not on `RemotePlayerRef`, so `getLobbyPlayer(id).code` → `SocialRef.isBlocked` always go together — `hud/ChatLog` (typed squad lines) and `hud/TypingBubbles` (the `…` bubble) both call it instead of each keeping their own copy. |
| `menus/social/SocialColumn.ts` | **(2026-09-14)** 메신저의 **친구 탭**이 이 열을 호스트한다 (`onWhisper` = 대화 탭으로 이동, 패널은 열린 채). `openHistory` 삭제. The **소셜 열** (`.social-col`) — one component, two hosts: the ESC screen's right column (`menus/PauseMenu`, ship only) and the 커뮤니티 panel (`hud/Community`). Sections top to bottom: **분대원** (from `ctx.net.lobby.players`, hidden with no lobby) — **2026-09-08 a 1×`NET_MAX_PLAYERS` grid**, one cell per lobby slot (slot-coloured bar, 아이디 + 이름 + `Lv. n` stacked left, the **voice slider + mute toggle** right — `SQUAD_VOICE_DEFAULT`, **UI only**, kept in a local `Map` keyed by PeerId, titled `보이스 채팅 준비 중`); unheld slots are drawn as `빈 자리` so the row never changes width, and the section header carries **파티 떠나기** (`.sc-leave` → `net.leaveLobby()` behind `SocialMenu.askConfirm`) —, **받은 친구 요청** (hidden when empty; each card gains 수락 / 거절 → `respondFriend`), **친구** (`친구 n · 접속 m` head, `SOCIAL_FRIEND_ROWS` visible then scroll) and **최근 플레이어** (`SOCIAL_RECENT_ROWS`, capped at `SOCIAL_RECENT_MAX`). Grids are `SOCIAL_CARDS_PER_ROW` wide via `--cols` and `--rows` custom properties, and capture their own `wheel`; 친구 / 최근 are `.sc-grid.fixed` — an exact `rows × 56 px` box rather than a `max-height`, so the panel never resizes around them (받은 친구 요청 stays elastic: taller cards, and it only exists while it has something in it). Everything comes from `socialOf(ctx)`; while that is missing or `available === false` the column collapses to the single `.sc-off` line (its start state, so an unrefreshed column never shows empty grids). Repaints only when a **change key** (snapshot rows + the lobby roster) moved, or on `refresh(true)` after a local mutation; re-keyed by `social:updated` / `social:play` / `social:error` / `net:lobbyUpdated` / `net:lobbyLeft`. `contextMenu` / `isAvailable` getters. |
...
| `menus/social/SocialPages.ts` | **(2026-09-14)** **차단 목록 페이지만 남았다** — 대화 기록 페이지 · `openHistory` · 입력칸은 메신저 대화 탭(`menus/messenger/ChatTab`)으로 대체돼 지웠다. 아래는 옛 설명. **2026-09-11 (B-4).** 커뮤니티 패널의 소셜 열 **위에 겹치는** 두 페이지(`.sc-page`, 패널 크기 불변). **대화 기록** `openHistory(code, name)` — `SocialRef.whisperHistory` 줄(오래된 것부터, `whisperStateText` 상태) + 아래 입력칸(`SocialRef.whisper`, Enter · 보내기, 한글 IME 조합 Enter 는 `compositionend` 뒤 전송), `social:whisper` · `social:whisperUpdated` 에 그 아이디면 다시 그린다. **차단 목록** `openBlocked()` — `SocialRef.blocked` 카드마다 `차단 해제`, 우클릭 = 열의 메뉴(차단된 형태). `← 뒤로` 또는 Escape 로 페이지만 닫는다 — 열 때 `ctx.escape.push('community:page')`. 입력칸이 키를 삼키므로 Escape(페이지 닫기) · Tab(호스트가 패널 전체 닫기)은 입력칸이 직접 처리한다. |
| `menus/social/whisperText.ts` | **(2026-09-14)** 귓속말의 공식 명칭은 **개인 대화**(`PRIVATE_CHAT_LABEL_KO`) — 문구를 쓰는 곳은 채팅 로그와 메신저 대화 탭. **2026-09-11 (B-4).** 귓속말 전송 상태 문구 하나 — `whisperStateText(line)`(`전송 중…` · `오프라인 보관 — 접속하면 전달` · `전송 실패 — 오프라인` · 받은 backlog 줄 `접속 전에 받음`)와 `whisperStateClass(line)`. 채팅 로그와 대화 기록 화면이 같은 말을 쓰게 여기 둔다. |
| `menus/social/SocialMenu.ts` | **(2026-09-14)** 항목은 `같이 하기` · **`개인 대화`**(옛 `귓속말하기` — 메신저가 대화 탭의 그 사람으로 옮긴다) · `친구 삭제` / `친구 추가` · `차단`, 차단된 행은 `차단 해제` 하나 (`대화 기록` 항목 · `onHistory` 삭제). The card's right-click menu (`.sc-menu`) and the column's shared **경고 팝업** (`.sc-confirm` — 친구 삭제 since Phase 11, and 분대 → **파티 떠나기** since 2026-09-08 through the general `askConfirm(title, body, ok, run)`), both **direct children of `ctx.uiRoot`** and `.interactive` so they float over the ESC frame and the community panel alike. Entries: `같이 하기` (enabled only while `SocialRef.playBlock(code)` is null, else disabled with the `PLAY_BLOCK_LABELS` reason beside it), `귓속말하기` (the host closes itself, then emits `chat:whisperTo`), `친구 삭제` (friends only, and only through the confirm card — its title / body / 확인 label are per-call now) / `친구 추가` (non-friends only). Positioned `fixed` at the click point, clamped into the viewport. Takes **no** blocker token — it only opens over a surface that already owns one — and a capture-phase Escape closes just the menu (or just the confirm), so the first Escape never also closes the screen underneath. Closes on an outside `mousedown`. `isOpen` / `isConfirmOpen` / `targetCode` getters. |
...
**Phase 5:** `meta:creditsChanged`, `meta:repChanged`, `meta:contractProgress`, `meta:contractSettled`, `meta:contractAccepted`, `meta:contractAbandoned`, ~~`meta:questChanged`~~ (2026-09-14 → `npc:objectiveProgress` · `npc:questReady` · `npc:questChanged`), `meta:purchase`, `meta:sale`,
...
## Events emitted
`hub:enter {ship}` (title / pause / death / complete), `game:respawn {}` (death screen button / Space, spectate Space), `game:paused {paused:false}`, `ui:objective`, `ui:notify` (title invite feedback),
...
(the MissionComplete 다시 배치 → `game:newMission {seed, planet}` emit was removed 2026-09-15). Social refs called:
`social.refresh / requestFriend / respondFriend / removeFriend / playWith / acceptInvite / dismissInvite / whisper`.
**2026-09-14 (메신저):** `ui:messengerToggled {open}` (Community, with `ui:communityToggled`). Listens `ui:openMessenger`, `npc:*`, `room:*`,
`social:unreadChanged`. Refs called: `ctx.meta.npc.getContacts / getMessages / markRead / getQuests / getQuest / accept / defer / deliver / report`,
`ctx.net.rooms.find / history / requestHistory / hasMore / create / invite / respond / leave / kick / rename / say / unread / markRead`,
`ctx.net.social.whisperUnread / markWhisperRead`. Escape tokens: `messenger:pop` (Popover), `holdAsk:*` (room leave / kick).
**2026-09-14 (NPC 개인 신뢰도):** listens `meta:npcTrustChanged {npc, trust, level, delta, levelUp}` (Messenger 토스트 · QuestsTab 다시 그리기),
refs called: `ctx.meta.npcTrust`. 쓰기(`addNpcTrust`)는 meta/ 안에서만 일어난다 — ui 는 읽고 그릴 뿐이다.
...
**`═══ Phase 6 ═══` section (end of file):** `.wcharge(.show/.ready/.wc-charge/.wc-spinup/.wc-slash) .track .fill .lbl` (2026-09-15: kind classes renamed from `.charge/.spinup/.slash`) (`--wc` per kind, `cookPulse` when ready), `.status-markers`, `.smarker(.show/.burn/.shock)` (`@keyframes sparkPop`),
...
- **행성** on the result screens: a `.planet-line` on `menus/MissionComplete` / `menus/DeathScreen`, and 다시 배치 now
  emits `game:newMission {seed, planet: ctx.missionPlanet}` (2026-09-15: 다시 배치 버튼과 기능은 삭제됐다).
...
- **CSS** (`styles/base.css`, "Phase 12" block at the end): `.compass .eticks / .etick(.scanned)`, `.detect-mark`,
  ...
  **2026-09-11 (B-13)** 그 블록 바로 뒤에: `.sm-inspect` + `.sm-ins-*` (모달리스 클릭 인스펙터, z 77 = `.sm-confirm` 79 **아래**)
  와 `.fcard.is-locked` · `.fcard-note.is-locked` (제작 카드 잠금). 인스펙터 썸네일은 `.fcard-thumb` · `.fcard-glyph` 를
  그대로 재사용하고 강화 버튼은 `.sm-gen-btn` 을 재사용한다 — 같은 화면에 같은 칩 두 벌을 만들지 않는다.
...
- **2026-09-15 (제세동기 크로스헤어 — 가젯 개편, 사용자 결정, 에이전트 B)** — `hud/Reticle` + `styles/base.css` 의
  `.reticle.defibmode` 블록만 바뀌었다 (위 표). 「작은 하얀 원이 커져 큰 반투명 원과 겹치고 두께가 굵어진다 →
  대상을 겨누면 주황 → 떼면 일으킨다」. 그리는 근거는 `gadget:defibAim {armed, charge, target}` 하나이고
  weapons/ 의 `parts/Defib` 이 그것을 낸다 — 사거리(`GADGET_DEFIB_RANGE`) · 반각(`DEFIB_AIM_CONE_DEG`) 판정을
  UI 가 다시 하지 않는다 (갈고리 칩이 `implant:grappleTargetChanged` 를 믿는 것과 같은 규약).
  제세동기는 이제 `heal:holdChanged` 를 내지 않으므로 `hud/HealGauge` 의 링과 겹치지 않는다.
  ⚠ 남은 것: `hud/WeaponPanel` 의 소모품 힌트가 아직 `좌클릭 N초 홀드 · 이동 50 %` 라 새 조작(충전 → 겨눔 → 떼기)과
  다르다 — 이 폴더의 다음 손질 거리다.
...
- **2026-09-15 (전설 총기 HUD — 사용자 결정)**
  ...
  - **전격총 우클릭 충전 게이지가 위에 뜨고 가득 차면 빨개지던 것**: 게이지 루트가 `wcharge charge` 라 함선 호출 링의 `.charge svg { rotate(-90deg) }` ·
    `.charge.ready .fill/.lbl`(빨강)이 같이 걸렸다 — 2026-09-10 `kc-hold` 와 같은 뿌리. 종류 클래스를 `wc-charge` · `wc-spinup` · `wc-slash` 로 바꿨고
    (`hud/WeaponChargeGauge`), 전격총은 크로스헤어 **오른쪽**에 원래 파랑 `#5fc8ff`, 가득 차면 **더 밝은 파랑** `#b8ecff` + 글로우 · 맥동. 라벨은 퍼센트만
    (`충전` · `충전 완료` 삭제 — 미니건 `예열 n% / 사격` · 표창 `용검 n% / 용검 준비` 는 그대로). 곁들여 고친 버그: 라벨 캐시(`lastPct`)가 종류를 몰라
    `100%` 다음 미니건 t 1 이 `사격` 대신 `100%` 로 남았다 → 종류가 바뀌면 캐시를 비운다. 함선 호출 링(`.charge`)은 무변경(회전 · 빨강 확인).
  - **전설 유니크는 자기 종류로 보인다** (csv `class` AR / DMR / SMG / SR 은 사격 숙련만 고른다): `hud/WeaponPanel` 분류 태그(`weaponTypeLabel`) ·
    `hud/ItemTip` 부제(`컴포짓 보우 · 전설`). 인벤토리 툴팁 쪽은 `src/inventory/README.md`.
  - **활 잔탄 = 한 숫자** (`hud/WeaponPanel` `.single-ammo`): 탄창 개념이 없으므로(재장전 없음 — weapons/) 장전 + 예비를 합쳐 하나로, `/ 예비` 숨김.
    `hud/ReloadGauge` 는 활의 `weapon:reloadStarted` 를 무시한다(링 없음). 재장전 키 안내는 원래 UI 에 없다.
  - `scripts/smoke-ui-p6.mjs`: 게이지 선택자 `wc-*` · 라벨 `50%` / `100%` · 호가 오른쪽(svg 회전 없음) · 가득 = 밝은 파랑 · 유니크 6종 태그 · 활 한 숫자 단언 (104 checks).

- **2026-09-15 (결과 창 개편 — 사용자 결정)**
  - **탈출 결과 창(`menus/MissionComplete`)**: `다시 배치 (같은 시드)` 버튼과 기능 전부 삭제(시드 보관 · 싱글 전용 숨김 · `game:newMission` 재발행 — 계약 `game:newMission` 자체는 그대로). 제목 줄 오른쪽에 **임무 시간만**, 그 아래 `전리품 가치`(왼쪽 라벨 · 오른쪽 `1,234 C` · 호박색 · 카운트업) → 보상(`RewardsBlock`) → `함선으로 귀환`. 처치 · 개봉한 상자 · 받은 피해 칸 삭제.
  - **사망 결과 창(`menus/DeathScreen` — 솔로 사망 · 분대 전멸 `레이드 실패` · 자발적 귀환)**: 같은 제목 줄, `잃은 전리품 가치` = `stats.peakLootValue`(그 레이드의 최고 소지품 가치, 빨강 — 예전에는 사망 **뒤** `ctx.inventory.getTotalValue()` 를 읽어 시체로 비워진 0 을 보여 줄 수 있었다), **사망 원인** 줄(`stats.death` — 적이면 `ctx.enemies.renderPortrait` 얼굴 썸네일 + 적 이름 + 그 개체에게서 받은 피해, 아니면 절차 SVG 원인 아이콘 + 원인 이름 + 그 원인의 피해, 원인을 모르면 줄이 숨는다), 그 아래 획득 경험치.
  - **분대가 탈출할 때 쓰러져 있던 사람**은 `game:complete` 로 탈출 결과 창을 받는데(`stats.extracted === false`), 그 사람에게도 사망 모습(`.rs-dead`)을 보여 준다 — 「죽은 사람이 보는 결과는 늘 사망 레이아웃」.
  - 몸통은 새 `menus/ResultReport`(두 화면 공용) + 새 `styles/results.css`(접두사 `.rs-`, base.css 무변경). `RewardsBlock` 라벨 `획득 XP` → `획득 경험치`, 사망 창에서도 숫자를 호박색으로.
  - 스크립트: `scripts/smoke-social.mjs` 의 `다시 배치` 클릭 · `game:newMission` 재발행 검사를 「버튼은 `함선으로 귀환` 하나」 검사로 바꿨다.

- **2026-09-15 (키캡 다듬기 — 꾹 누르기 chevron 을 키캡 안으로 · 마우스 버튼 그림, 사용자 결정)**
  - **모든 키캡이 `shared/keycap` 한 경로로 그려진다** (`createKeycap` / `paintKeycap` / 문장 안은 `renderKeyText`). 손으로
    `el('span', { cls: 'keycap', text: keyLabel(...) })` · `className = 'keycap kc-hold'` 를 쓰던 곳을 전부 옮겼다:
    `hud/KeyGuide` · `InteractionPrompt`(hold 를 기억해 리바인드 때 같이 칠한다) · `GadgetHandHint`(줄이 라벨이 아니라 **키 코드**를 들고,
    옛 `좌클릭` · `우클릭` 글자 → 그림; `lines` 는 `keyLabel` 로 적는다) · `DroneHud`(`KeyRow.hold`) · `QuickStrip` · `StratagemPanel` ·
    `ImplantWidget` · `Reticle`(갈고리 칩) · `ChatLog` · `ShipManageHint` · `menus/ControlsPanel`(기능 목록 + 마우스 도표의 버튼 이름 칸 `.mcap .k`
    도 키캡) · `menus/KeybindMenu`(버튼 안에 `mouseGlyphSvg` 만 — 버튼 자체가 테두리 칸이라 `.keycap` 을 겹치지 않는다, `.kb-key-mouse`).
    키 가이드 라벨로 한국어 `우클릭` 을 넘기던 `hud/Community` · `hud/RescuePicker`(실제 취소 버튼 `Keys.AIM`) 와 `map/MapScreen` 의
    `LMB 드래그` 키캡은 `keyLabel('Mouse2')` · `keyLabel(Keys.AIM)` · `LMB` + 동작 `드래그 이동` 으로 바꿔 그림이 되게 했다.
  - **꾹 누르기** (`styles/base.css` `.keycap.kc-hold`): 키캡의 강조색 테두리 · 안쪽 글로우 삭제 — **chevron 만** 강조색. 위아래로
    흔들리던 `holdChevron` 애니메이션 삭제. chevron 을 키캡 위 10 px 허공에서 **키캡 안 윗변 가운데**로 옮기고 글자는 `padding-top: 5px`
    로 내렸다(`:not(.kc-mouse)` 로 (0,3,0) — 위젯별 `padding: 0 5px` 을 이긴다). 키캡 높이는 위젯 규칙 그대로라 줄이 안 들썩이고, 떠 있는
    chevron 자리를 비우던 `:has(.kc-hold)` 여백 넷(키 가이드 · E 프롬프트 · `gadgetHint.css` · `drone.css` 의 `.dr-key.back`)을 걷어냈다.
  - **마우스 버튼 그림** (`.keycap.kc-mouse` · `.kcm-glyph` · `.kc-inline` — `rg` 로 빈 이름 확인): 좌 · 휠 · 우는 위가 둥근 마우스 윗부분,
    누를 칸 흰색 / 꾹 누르기면 강조색 + 그 칸 안의 아래 chevron(SVG 안이라 `.kc-mouse.kc-hold::before` 는 없다). 그림 크기는 키캡
    글자 크기의 `1.45em` + `max-height: 100%` 라 위젯마다 정한 크기를 따른다. `M4` · `M5` 는 글자 그대로.
  - **글자로 남긴 곳**: 키 가이드의 `휠`(굴리기 — 누르는 버튼이 아니다) · 토스트 / 상태 줄 / 버튼 문구 안의 키 이름(`Notifications` 초대
    토스트 · `CommsWheel` 부제 · `RoverHud` 상태 · `Messenger` 닫기 버튼 · `KeybindMenu` 안내 줄) · 무기 패널 `.slot` 태그 — 키캡이 아니라
    문장이다. 스모크가 `.keycap` · `.kb-key` 의 `textContent` 로 읽던 자리(`LMB` …)는 그림의 `<title>` 이 같은 글자를 들고 있어 그대로 맞는다.

- **2026-09-15 (빠른 이동 뒤 남는 툴팁 — `hud/ItemTip` 쪽)**
  - 카드는 `pointerout` 으로 내려가는데, 더블클릭 · 우클릭 메뉴로 아이템을 옮기면 격자(`inventory/ui/TradeGrids` → `GridView`)가 **커서 아래의
    칩을 DOM 에서 떼어 내고** 떼어 낸 요소에는 `pointerout` 이 오지 않아, 카드가 이미 없는 칩을 설명한 채 마우스를 움직일 때까지 떠 있었다.
    이제 카드가 떠 있는 동안 rAF 루프(`watch`)가 설명 중인 칩이 연결돼 있는지만 싸게 보고(`isConnected` · `.is-vanishing`), 떨어졌거나
    인벤토리 이벤트(`inventory:changed` · `stashChanged` · `bagChanged` · `itemUpdated` · `loadout:changed`) 뒤 두 프레임 동안은
    **마지막 포인터 위치를 다시 짚는다**(`rehit` — `elementFromPoint` → 칩이 있으면 그 칩, 없으면 숨김). 루프는 카드가 내려가면 멈춘다.
    Tab 창 격자(`inventory/ui/InventoryUI`)의 떠다니는 카드는 그 폴더의 `validateHover` 가 같은 규칙으로 맡는다.
  - `hud/CursorHoldGauge` 무변경 — 툴팁 고정 링의 25 % 문턱은 보내는 쪽(`inventory/ui/TipPin`)에 있다.

- **2026-09-15 (TODO B-16 화염 지대 위험 표시 · B-14 낙하 비네트)**
  - **`hud/DangerIndicators` 에 갈래 `fire`(`화염 지대` · ▲).** 계약 `FireZoneInfo` 를 두 폴더가 같은 모양으로 답한다 —
    `ctx.enemies.getFireZones?()`(적 소이 수류탄, 전부 `hostile`) · `ctx.gadgets.getFireZones?()`(플레이어의 화염 · 소이 수류탄).
    둘 다 옵셔널 호출이라 한쪽이 아직 없어도 갈래가 비어 있을 뿐이다. 색은 수류탄과 **같은 규칙**(누구 것인가 — 적 빨강 · 아군 호박,
    `hot` 이면 짙게)이고, 게이트는 인지력이 아니라 **가장자리 거리** `FIRE_ZONE_DANGER_RANGE`(35 m) 하나다 — 반지름 6 m 지대가
    중심 38 m 에 있으면 가장자리는 32 m 이므로 뜬다. 전장의 안개 게이트는 걸지 않는다(지금 타고 있는 사건). 화면 안이면 **지대 중심**에
    머리 마커 + 꺼질 때까지 남은 초, 밖이면 방향 호. 안에 서 있거나 가장자리 1.5 m(`FIRE_HOT_EDGE_M`) 안이면 `hot`.
    ⚠ **우선순위**: 풀은 후보 28 → 가까운 10 인데, 발밑의 불은 거리 0 이라 그대로 두면 3 m 옆 수류탄 마커를 밀어낸다.
    지대는 **머무는** 위험이고 나머지는 **날아오는** 위험이라 ① 후보를 맨 마지막에 모으고 ② 정렬 키에 `FIRE_RANK_BIAS` 를 더해
    칸이 모자라면 늘 뒤로 밀린다(지대끼리는 가장자리 거리 순). 슬롯 요소에 `data-cat` 를 단다(key 가 바뀔 때만 쓴다).
  - **새 파일 `hud/FallVignette.ts` + `styles/fall.css`(`.fall-vignette`).** `player:fell {damage}` 에 세기 `min(1, damage / 80)` 로
    화면 가장자리 · 아래쪽이 붉게 번지고 `FALL_VIGNETTE_S`(0.6 s) 동안 `k²` 로 사라진다 — 더 진하게 떠 있으면 새 낙하는 최댓값만 본다.
    **사라짐을 CSS 전이로 두지 않았다**: 이 PC 처럼 `prefers-reduced-motion` 이 켜진 곳에서 base.css 가 전이를 0.01 ms 로 잘라 번쩍 사라지고,
    일시정지 · 셰이더 hold 중에도 시뮬레이션 시간을 따라야 한다. 그래서 `HudSystem.update` 가 레이어 가시성과 무관하게 `update(dt)` 를 부른다.
    **z 25** = HUD 레이어(z 없음) 위 · 재개 게이트(30) · 지도(40) · 커뮤니티(46) · 인벤토리(50) · 페이드(82) · 메뉴(83 +) 아래.
    `#ui-root` 직계, 포인터를 먹지 않고 blocker · escape 스택에 올라가지 않는다. 착지 흔들림(player `camera:shake`)과 착지음(audio)은 다른 폴더다.

- **2026-09-14 (캐릭터 확정 팝업 · 튜토리얼 오프닝 HUD, 사용자 결정)**
  - **`menus/askPopup`** — `AskSpec` 에 추가만: `hold`(붉지 않은 확인도 `UI_HOLD_CONFIRM_S` 홀드로만 실행, 채움은 악센트) ·
    `content`(본문 아래에 붙일 노드, 닫을 때 뗀다) · `cardCls`(카드 modifier). `body` 가 빈 문자열이면 본문 줄을 숨긴다. 기존 호출부 무변경.
  - **`menus/CharacterCreate.buildSummary`** — 확정 팝업이 요약 **카드**가 됐다: 이름 한 줄 + 능력치 다섯 칸 **가로** 게이지(값 /
    `CREATE_STAT_MAX` — 1 이면 20 % 찬다, 생성 화면 바의 (값 − 최소) / (최대 − 최소) 와 **일부러 다르다**, 사용자 결정) + 오른쪽 얼굴 정지
    썸네일. 「정말로 만들겠습니까?」 삭제 · `만들기` 1초 홀드. CSS `.cc-confirm` · `.cc-cf-*` · `.tm-ask-card.cc-confirm-card`(`styles/title.css`,
    `rg "\.cc-cf-" src` 로 빈 이름 확인).
  - **`menus/SoldierPreview.snapshotFace()`** — 미리보기와 **같은 렌더러**로 얼굴 한 장(yaw π − 0.7 = 카메라 쪽 왼쪽 사선 · 짧은 변이
    0.62 m 를 담는 거리)을 그려 `toDataURL` 로 뽑고, 같은 태스크에서 턴테이블을 다시 그려 캔버스를 돌려놓는다 — 세 번째 GL 컨텍스트 없음.
    이미지는 캔버스 비율이고 `object-fit: cover` 가 정사각형으로 자른다. `fit()` 을 `render` 에서 떼어 냈다.
  - **`HudSystem` 검은 페이드 = 코드 보간** (`stepScreenFade` · `paintScreenFade`, 시뮬레이션 dt). CSS `transition` 을 걷어냈다 — 이 개발 PC 가
    `prefers-reduced-motion: reduce` 라 `styles/base.css` 가 모든 전이를 0.01 ms 로 잘랐고, 페이드가 한 프레임에 끝나 「페이드 인이 안 된다」가
    됐다(헤드리스 크롬도 같은 값을 보고한다). 스모크 훅 `screenFadeShown`(칠해진 값).
  - **`hud/Compass`** — `PlayerRef.introWaking` 인 동안 띠 전체 opacity 0 · 갱신 생략, 끝나면 `TUTORIAL_COMPASS_FADE_S`(1.2 s, csv)에 걸쳐
    코드로 나타난다(같은 reduced-motion 이유). 연출이 없는 레이드는 늘 1 이라 인라인 스타일을 안 건드린다. `update(ctx, dt)` · `revealAmount`.
  - **`hud/Objective`** — 튜토리얼 레이드(`missionMode === 'tutorial'` 또는 옛 게이트 `extractionTimer`)에서는 좌측 상단 **시계**와 상단 중앙
    **탈출 타이머 전체**(「함선 도착까지」 · 「함선 출발까지」 포함)가 `.hud-tut-hidden` 이고 핸들러가 모드에 들어가지도 않는다. 2차 결정은
    `countdown` · `departing` 을 남겼는데, 튜토리얼 함선은 이미 서 있어(`beginPreLanded` 가 `extraction:activated` 를 내 「00:20」 이 멈춘 채 떠 있었다)
    3차부터 스위치를 누르면 곧장 뜨므로 남길 사실이 없다. 훅 `hiddenForTutorial`.
  - 회귀 `scripts/smoke-intro-wake.mjs` (30 검사 — 진짜 생성 → 홀드 → 새로고침 → 기상 연출).

- **2026-09-14 (크로스헤어 활 모드 — 오버워치 한조식, 사용자 결정)** — `hud/Reticle` 이 활(「롱혼」, `unique === 'bow'`)을 든 동안
  네 틱 대신 **점 + 점 아래 가로 안내선 두 단 + 움직이는 가로 바**를 그린다(`.reticle.bowmode` · `.rbow-tier` · `.rbow-low` ·
  `.rbow-draw(.drawing/.full)`, `styles/base.css` 의 `.reticle` 규칙 옆 — `rg "rbow" src` 로 빈 이름을 확인했다).
  근거는 weapons/ 의 `weapon:chargeChanged {kind:'draw', t}` 하나다: 바는 아래 단에서 출발해 `t` 를 따라 올라가고 `t = 1` 에서
  **점 위에 정확히 닿는 것**이 곧 「이제 크로스헤어대로 날아간다」는 말이다(약하게 당긴 화살은 떨어진다). `WeaponChargeGauge`
  는 `draw` 를 무시하므로 호 게이지와 겹치지 않는다. 안내선 위치의 원본은 JS 상수 `BOW_TIER_Y` 하나(CSS 는 폭만) ·
  바는 매 프레임 `damp`(당기는 중 30 · 돌아갈 때 12)하되 transform 은 0.05 px 이상 바뀔 때만 쓴다. `t = −1` 은 활 판정의
  근거로 쓰지 않는다 — 무기 교체 뒤에 늦게 올 수 있다.

- **2026-09-14 5차 (UI 2차 개편 — 음악 재생 창 · 제작/수리 UI 정리, 사용자 결정)**
  - **새 파일 `hud/MusicPlayer.ts` + `styles/music.css`(`.mus-`).** **음악은 소리가 아니라 상태**라는 결정이
    이 화면의 모든 모양을 정한다 — 재생 상태의 주인은 `housing/parts/Music`(레코드랙에 꽂힌 것이 목록이므로
    서재의 일이다)이고, 여기서는 `housing:musicChanged {state}` **하나**만 보고 그린다. 볼륨은
    `ctx.audio.settings.bgm` 이지만 **그 채널에는 아무 소리도 걸려 있지 않다**(외부 에셋 금지 · 절차 음악 미구현) —
    그래서 이 창은 「무엇이 흐르고 있는가」를 말하는 표시판이지 플레이어가 아니다. 조작 넷은 `HousingRef` 의
    **옵셔널** 계약이라 `?.()` 이고, 메서드가 없는 빌드에서는 그 버튼을 **숨긴다**(비활성이 아니라 없음 —
    할 수 없는 일을 보여 주지 않는다). 목록이 비어도 창은 **뜬다**: 「켰는데 아무 반응이 없다」를 만들지 않으려고
    `꽂힌 레코드가 없습니다` 한 줄을 그리고 곡 넘김만 `disabled` 다. 함선에서만 보이고 메뉴 blocker 에서 숨는
    규칙은 **`hud/KeyGuide.apply()` 와 같은 것**을 쓴다 — 인벤토리 · 지도 위에는 그대로 떠 있다. z 81 은
    인벤토리(50)와 딸림 층(≤ 80) 위, `.screen-fade`(82) · `.net-badge`(83) · `.key-guide`(84) 아래이고,
    창은 `pointer-events: none` · 버튼만 `auto` 라 그 아래 입력을 가리지 않는다(`.nb-actions` 와 같은 규칙).
  - **재료 칩 호버 카드는 커서 좌상단이다**(`hud/ItemTip`). 시설 관리 · 작업대 제작의 「필요 아이템」 줄이
    화면 아래쪽이라 기본 자리(우하단)에서 카드가 **위로만** 뒤집혔고, 결과가 「커서 우상단」이라 **눌러야 할 칩을
    카드가 덮었다.** 고친 방법은 전역 규칙을 바꾸는 것이 아니라 **옵트인 속성**(`data-tip-anchor="left"`,
    원본 상수 `TIP_ANCHOR_ATTR`)이다 — `closest` 로 읽으므로 줄 하나에 한 번만 찍으면 되고, 격자 타일의 카드는
    **예전 자리 그대로**다. 화면 밖으로 나가면 어느 쪽이든 반대편으로 뒤집는 것도 그대로.
  - **`hud/ShipManage` 정리** — ① `multi` 가구의 `이미 보유 중` 삭제(규칙은 2026-09-13 부터 `!def.multi` 를 보고
    있었는데 **이 화면만 안 봤다**), ② 발전기 행의 레벨별 해금 표(`.sm-gen-unlocks`) 삭제(같은 정보를 용도 지정
    카드의 발전기 레벨 칩이 그 자리에서 말한다 — 두 곳이 같은 사실을 적으면 하나는 반드시 낡는다),
    ③ 가구 카드 · 가구 창고 카드 · 인스펙터 머리의 썸네일을 **정사각형**으로(두 행 카드는 `align-self: stretch`
    라 아래 여백이 0), ④ 시설 레벨 칩은 `nowrap` 인 줄 안에서만 정사각형(넓게 감싸는 곳은 옛 칩 그대로).
    ⚠ 그 칩이 **늘 서게 된** 근거는 ui 가 아니라 `housing/Rules.generatorRequirement` 가 **채워진 요구도
    돌려주도록** 바뀐 것이다 — 재료처럼 `현재/필요` 를 늘 보여 주고 모자랄 때만 `.is-short` 로 말한다.
  - **`menus/SettingsMenu` 의 `오디오` 채널이 셋**(`전체` · `효과음` · `음악`). `CHANNELS` 에 한 값이 는 것이
    전부이고 줄 · 슬라이더 · 저장은 한 줄도 안 바뀌었다 — 원래 그 배열의 주석이 「음악이 생기면 여기 한 줄
    더하면 된다」였다.

- **2026-09-14 4차 (함선에서의 `타이틀로` · `게임 종료`, 레인 E, docs/DECISIONS.md 「2026-09-14 — 튜토리얼 개편」)** — 사용자 결정 하나.
  - **`menus/PauseMenu.ts`** — 두 팝업의 **글과 확정 방식이 「지금 임무 중인가」로 갈린다**. 판정은 새로 만들지 않고
    `함선으로 귀환` 을 숨길 때 이미 쓰던 그 값(`this.inHub` ← `game:paused` 때의 `ctx.isHubPhase()`)을 그대로 본다 —
    새 상태도 새 이벤트도 없다.
    - **글**: 함선에서 「진행 중인 임무를 포기하고 … 회수하지 못한 전리품은 사라집니다」는 **거짓말**이었다(포기할 임무도
      잃을 전리품도 없다). 함선용으로 「타이틀 화면으로 돌아갑니다. 함선의 진행 상황은 저장되어 있습니다.」 ·
      「게임을 종료합니다. 함선의 진행 상황은 저장되어 있습니다.」 로 갈아 끼웠다. **임무 중에는 옛 문구 그대로**다.
    - **확정**: 함선에서는 **탭 한 번**이다. 「되돌릴 수 없는 확정은 1초 홀드」(2026-09-09) 규약의 근거는 *잃는 것*인데
      함선에는 잃을 것이 없다. `Ask` 에 선택 필드 `tap?: boolean` 을 더해(생략 = 예전 그대로 홀드) `confirm()` 이
      안내 줄 `.pause-ask-hint` 를 감추고, `startHold` 는 그때 **아무것도 무장하지 않으며**(`preventDefault` 도 하지
      않는다 — 그래야 뒤따르는 click 이 그대로 온다), 원래 click 을 삼키기만 하던 핸들러가 그 한 번을 확정으로 읽는다.
      게이지 · `is-holding` · rAF 는 손대지 않았으므로 홀드 팝업의 동작은 한 프레임도 바뀌지 않는다.
    - **`파티 떠나기` 는 함선에서도 홀드다** (사용자 결정) — 잃는 것은 없어도 분대 다른 사람에게 영향을 준다.
      `함선으로 귀환` · `튜토리얼 건너뛰기` 도 그대로 홀드.
    - 새 getter `isAskTap`(스모크 / 디버그). CSS 는 한 줄도 안 바뀌었다 — `.pause-ask-hint` 에 `display` 규칙이 없어
      `hidden` 속성만으로 사라진다.
  - 검증: `npm run typecheck` — `src/ui` 0건 (같은 워크트리의 다른 레인이 `src/tutorial/Steps.ts` 를 고치는 중이라
    그 파일 한 건이 남아 있었다). 병렬 작업이라 스모크는 리드가 마지막에 한 번 돌린다.
- **2026-09-14 3차 (메신저 UI — 목록 · 머리 · 타이핑 연출, 레인 A2, docs/DECISIONS.md 「2026-09-14 — NPC 첫 연락 3단」)** — 사용자 결정 넷.
  - **`menus/messenger/ChatTab.ts` · `format.ts`** — **대화 목록 한 줄은 초상 + 이름 + 마지막 대사뿐**이다. 소속 라벨 ·
    `n분` · 신뢰도 `Lv.n` + 게이지가 빠지면서 `ConvRow.sub` · `agoText` · 목록 키의 분 단위 눈금(`Math.floor(now/60_000)`)이
    함께 없어졌다 — **줄이 1분마다 다시 그려지던 이유가 그 시각 하나였다**. NPC 줄의 미리보기는 이제 엔진의
    `NpcContactInfo.preview`(제안이면 `[퀘스트] 이름`) 대신 `getMessages` 의 **마지막 줄**에서 짓는다: 퀘스트 제안이면
    그 퀘스트의 `summary` 를 잘라 쓴다(「레이븐이 새 거래 상대의 솜씨를…」). **`src/meta/` 는 한 줄도 고치지 않았다** —
    다른 레인의 파일이고, 어차피 목록이 원하는 문장은 화면 쪽 사정이다.
  - **대화창 머리** — `threadHead` 를 `threadHeadWith(초상, 제목, 부제)` 로 갈라 NPC 만 다른 초상을 넣는다:
    `Trust.buildNpcAvatar` 의 **신뢰도 radial 고리 + 우하단 레벨 배지**. 레벨 · 신뢰도 현황(`.ms-trust`)은 초상 아래가 아니라
    **머리줄 중앙 우측**(`.in-right`)으로 옮겼고, 소속 아래 `bio` 한 줄과 하단 안내 `NPC 에게는 퀘스트 카드로 답합니다` 는
    지웠다. 고리는 **conic-gradient 한 줄**이다 — 외부 에셋 금지 규약에 SVG 파일도 캔버스도 필요 없고, `--frac` 이
    **지금 레벨 구간 안의** 진행률이라 레벨이 오르면 한 바퀴 돌고 처음부터 다시 찬다 (레벨 수치는 배지가 말한다).
  - **타이핑 `...` 연출** — **새로 도착하는** NPC 말풍선 · 퀘스트 카드만 앞에 `.ms-bubble.ms-typing` 을
    `clamp(글자수 × TYPE_S_PER_CHAR, TYPE_MIN_S, TYPE_MAX_S)` 동안 세웠다 지운다 (세 상수는 **게임 밸런스가 아니라 UI
  ...
  - **`QuestCard.ts`** — 말풍선 카드의 `[생각해보지]` 버튼과 `QuestCardActions.defer` 제거 (`[수락]` 만 남는다).
    엔진의 `NpcQuestRef.defer` 는 계약에 은퇴 표시로 남아 있고 **이제 부르는 곳이 없다**. `deferred` 를 그리는 분기
    (상태 글자 · 상세의 [수락] · 퀘스트 탭의 `보류` 묶음)는 옛 세이브를 위해 **조용히 남겨 뒀다** — 엔진이 그 상태를
    돌려주지 않으므로 평소에는 한 줄도 나오지 않는다.
  - **`QuestsTab.ts`** — 상세 머리의 NPC 초상도 같은 `buildNpcAvatar` 다. 목록 필터는 **엔진이 이미 거른다**(A1 레인)는
    전제로 손대지 않았다.
  - **`styles/messenger.css`** — 새 클래스 `.ms-avwrap` · `.ms-avlv` · `.ms-trust.in-right` · `.ms-bubble.ms-typing` +
    `@keyframes ms-type-dot` (전부 `rg` 로 비어 있는 것을 먼저 확인했다 — `ms-` 접두사 안이고 HUD 의 `.typing-bubbles` ·
    `.tbubble` 과는 이름이 겹치지 않는다). 그리는 것이 없어진 `.ms-row-sub` · `.ms-row-time` · `.ms-thead-bio` ·
    `.ms-thread[data-kind='npc'] .ms-note` 규칙은 지웠다.
  - ⚠ 튜토리얼(`ravenQuest` 단계)이 쓰는 선택자 **`.ms-qcard` · `.ms-page.chats` · `.ms-frame` 은 이름을 그대로 두었다**.
  - 검증: `npm run typecheck` — `src/ui` 0건 (같은 워크트리의 다른 레인이 `enemies/models/BugParams.ts` 를 고치는 중이라
    그 파일 한 건은 남아 있었다). 병렬 작업이라 `verify` · 스모크는 리드가 마지막에 한 번 돌린다.
    `scripts/smoke-messenger.mjs` 는 옛 화면을 검사한다 — 제안 카드의 `defer|accept`, `.ms-note` 문구, `생각해보지` 클릭
    갈래, `보류` 묶음이 이번 결정으로 사라졌으므로 **그 스모크는 리드가 함께 고쳐야 한다** (에이전트 소유 밖이라 두었다).
- **2026-09-14 2차 (튜토리얼 레이드의 화면 · ESC · 캐릭터 생성, 에이전트 ③, docs/DECISIONS.md 「2026-09-14 — 튜토리얼 개편」)** — 다섯 덩어리.
  - **`HudSystem.ts` · `styles/base.css` (신규 `.screen-fade`)** — **화면 전체 검은 페이드** `ui:screenFade {opacity, durationS}`.
    `#ui-root` 직계 판 하나이고 `setScreenFade` 가 전이 시간과 불투명도를 인라인으로 몬다 (`durationS` 0 = `transition: none`
    으로 끄고 즉시 — 남은 전이가 이어지지 않는다). **연출이지 blocker 가 아니다**: `pointer-events: none` 이고
    `ctx.escape` · `uiBlockers` 에 올라가지 않으므로 페이드가 1.0 이어도 입력 · ESC 는 평소 그대로 흐른다 (입력 잠금은
    부르는 쪽인 `player/` 의 기상 연출이 자기 폴더에서 건다). **z-index 82 의 근거**: 게임 화면은 전부 덮어야 하니
    HUD · 지도(40) · 커뮤니티(46) · 인벤토리 창(50) · 허브 팝업(72) **위**, 그러나 서버 배지(83) · 타이틀 · 키 가이드(84) ·
    일시정지(85) · 설정(86) · 키 설정(88) · 콘솔(90) · 떠다니는 카드(200 +) **아래** — 불투명도 1 인 동안 ESC 메뉴까지
    삼키면 플레이어는 검은 화면에 갇힌 것으로 읽는다. 스스로 0 으로 돌아가는 자리는 `game:abort` 와 `applyVisibility` 의
    `!inGame` 가드 둘이다 (`ui:cinematic` 이 쓰던 바로 그 자리 — 기준만 `isGameplayPhase()` 가 아니라 `inGame`
    (게임플레이 + `deploying`)이다: 오프닝 페이드가 강하 · 월드 준비 구간에 걸쳐 있어 그 하나로 자르면 켜자마자
    다음 프레임에 지워진다). **`game:newMission` 은 일부러 구독하지 않았다** — `world:ready` 가 그 이벤트 **안에서 동기로**
    발행되므로(`hud/Compass` 주석이 원본), 월드가 뜨자마자 켜는 페이드를 우리 핸들러가 도로 지운다. 스모크 훅
    `HudSystem.screenFadeOpacity`.
  - **`menus/PauseMenu.ts`** — 튜토리얼 중에는 `함선으로 귀환` 자리가 **`튜토리얼 건너뛰기`** 다 (`ctx.tutorial?.track !== null`,
    어느 트랙이든). `refreshReturnButton()` 이 `game:paused` 때마다 라벨(`setText`)과 `display` 를 정한다 — **함선에서도 보인다**:
    `ship` · `build` 트랙은 함선 안에서 도는데 `함선으로 귀환` 의 hub 숨김 규칙을 물려받으면 그 두 트랙은 건너뛸 길이 없다.
    확정은 「되돌릴 수 없는 확정은 1초 홀드」 규약 그대로 `함선으로 귀환` 이 쓰던 **같은 경고 팝업 + 같은 홀드 게이지**를 지나고
    (Enter 는 먹기만, Escape 가 취소, 초기 포커스 `취소`), `resume()` 으로 메뉴를 먼저 닫은 뒤 `ctx.tutorial.skipTrack(track)`
    한 줄을 부른다 — 레이드 트랙의 「즉시 탈출 → 함선 획득」 흐름은 tutorial · extraction 의 몫이고 ui 는 부르기만 한다.
    getter `skipTrackTarget`. 튜토리얼이 아닐 때의 동작은 한 글자도 바뀌지 않았다.
  - **`map/MapScreen.ts` · `hud/WorldMarkers.ts` · `hud/Compass.ts`** — 튜토리얼 레이드의 **탈출 함선 표시**를 감춘다.
    이름 둘의 뜻은 `shared/tutorial.ts` 의 `TutorialHudPart` 가 정한 그대로다: `shipMarker` = **지도 마커 + 월드(3D) 마커**
    (마지막 `extract` 단계에 풀린다 — 그때는 「함선을 찾아가라」가 곧 목표라 표시가 안내 역할을 한다), `shipScreenMarker` =
    **화면 마커**(레이드 내내 닫혀 있다). 그래서 지도는 `hidesShip()` 하나로 `drawShip` + 「함선」 라벨과 **범례의 `ship` 줄**을
    함께 가르고(단계가 넘어가면 캔버스는 프레임마다 스스로 보고, 범례는 새 구독 `tutorial:changed` → `refreshLegend()` 로
    다시 그린다), `WorldMarkers` 는 `lateUpdate` 에서 `__ship` 마름모를 **발견 게이트와 같은 자리 · 같은 `hide(m)`** 로 넘기며,
    `Compass` 는 `__ship` 눈금을 `gated` 조건에 얹어 그리지 않는다(가장자리에 붙는 `clamped` 방향 표시도 이 띠의 것이라 함께 사라진다).
    **함선을 가리키는 표시는 `src/ui` 에 이 셋뿐이다** — `hud/OffscreenIndicators` 는 핑만 그리고(2026-09-10 에 함선 호출 ·
    수류탄 갈래가 `DangerIndicators` 로 떠났다), `hud/pillar.pillarAllowed` 는 이미 시체만 허락하므로 함선 빛기둥은 원래 없다.
    튜토리얼이 아니면 세 질의가 언제나 false 라 평소 화면은 한 픽셀도 바뀌지 않는다.
  - **`hud/Objective.ts`** — 상단 중앙 큰 타이머의 `arrived`(「도착」) · `waiting`(「자동 출발까지」) 두 모드를
    `hidesTimer(ctx)`(= `hides('hud','extractionTimer')`) 면 **아예 띄우지 않는다**: 튜토리얼 함선은
    `beginPreLanded(..., {autoDepart:false})` 라 자동 출발이 없고, 착륙한 적 없는 함선의 「도착」도 사실이 아니다.
    **`departing`(스위치 뒤 10초 유예)은 남겼다** — 그것은 튜토리얼에서도 진짜로 흐르고 취소할 수 없는 카운트다운이라,
    숨기면 설명이 아니라 **사실**을 지우는 것이 되어 함선이 말없이 떠난다. `hud/Notifications` 의 「함선 착륙」 토스트는
    이미 `ctx.extraction.idleRemaining` 으로 문장을 가르고 있어 손대지 않았다.
  - **`menus/CharacterCreate.ts` · `styles/title.css`** — **시작 임플란트 선택지 제거**(사용자 결정). 모든 새 캐릭터가
    `CREATE_IMPLANT_IDS[0]`(갈고리) 고정이다. 라벨 · 큰 타일 · 컨텍스트 메뉴 · 그 다섯 메서드 · capture 핸들러 둘 ·
    getter `isImplantMenuOpen` · CSS 블록이 전부 빠졌고 이 화면은 `ctx.implants` 를 더 이상 읽지 않는다. `implant` 는
    읽기 전용 필드로 남아 그대로 `createCharacterInSlot` 에 실린다 — **`shared/character.ts` 의 `implant` 필드 · `sanitize` 는
    계약이라 한 글자도 고치지 않았다**(값만 언제나 같다). 레이아웃은 왼쪽 설정 열에 이름만 남았으므로 **세 열 → 두 열**로
    합쳤다: `.cc-body` = `[.cc-main: 이름 카드 + 능력치 카드] [.cc-side: 미리보기 + 악센트]`, 능력치 카드가 `flex: 1` 로
    남는 높이를 먹어 옛 가운데 열이 grid stretch 로 그리던 모습 그대로다 (`max-width` 1120 → 1000 px).
  - 검증: `npm run typecheck` 클린 (`src/ui` 0건). 병렬 작업이라 `verify` · `dev` · e2e 는 돌리지 않았다.
- **2026-09-14 (메신저 대사 선택지, 에이전트 F, docs/DECISIONS.md 「2026-09-14 — 튜토리얼 개편」)** — NPC 첫 연락의 **내 대답 버튼 줄**.
  - **`menus/messenger/ChatTab.ts`** — `renderNpc` 가 말풍선을 다 그린 뒤 `ctx.meta.npc.getPendingChoices(id)` 를 묻고,
    비어 있지 않으면 `.ms-msg.out.choices > .ms-choices > .ms-btn.ms-choice[data-act="choice"][data-choice="n"]` 을 세운다.
    누르면 `chooseIntro(id, n)` — 실패하면(고를 것이 없다) 평소와 같은 `deny` 토스트다. **상태를 들고 있지 않다** — 고른 결과는
    `choice` 사건 하나로 대화에 남고(`npc:message` → 다시 그리기) 그 다음 판정에서 줄이 저절로 사라진다. 고르기 전에 대화를
    닫고 나가도 되고, 다시 열면 버튼이 그대로 있다. `threadDataKey` 지문에 선택지 개수를 한 칸 더했다.
  - **`styles/messenger.css`** — `.ms-msg.choices` · `.ms-choices` · `.ms-btn.ms-choice`. **새 스타일 체계가 아니다** —
    퀘스트 카드의 [수락] [생각해보지] 와 **같은 `.ms-btn`** 이고, 대답이 한 줄보다 길 수 있어 고정 높이 · `nowrap` 만 풀었다
    (내 말풍선과 같은 강조색 · 같은 오른쪽 정렬).
  - 표는 `data/npcs.csv` 의 `npc_raven` 한 줄뿐이다 (`introChoices` · `introChoiceReplies` 2개씩 — `src/meta/README.md` 참조).
- **2026-09-14 (피격 잔상 · 인벤토리 키 가이드, 에이전트 B, docs/DECISIONS.md 「2026-09-14 — 정보상」)** — 사용자 결정 둘.
  - **`hud/Vitals.ts` · `styles/base.css`** — **체력 · 실드 잔상이 둘 다 연한 빨강**이다. 체력 잔상은 색만 바뀌었고
    (`rgba(255,255,255,0.55)` 는 흰 `.fill` 과 구분이 안 돼 얼마나 깎였는지 안 보였다), **실드 잔상은 새로 만들었다** —
    `setCells` 가 칸마다 `.ghost` div 를 만들어 두고도 `.sh-seg .ghost` 규칙도, transform 을 주는 코드도 없었다.
    `updateShield` 가 체력과 **같은 식**으로 민다(`GHOST_HOLD` 0.55 s 정지 후 damp 4). `player:shieldChanged` 에는
    `delta` 가 없으므로 **직전 목표값과 비교**해 「깎였다」를 판정한다 (폴링이 원본이라 이벤트만 봐서는 놓친다).
    색 원본은 `base.css` 의 `.vitals { --vt-ghost · --vt-ghost-down }` 두 변수 하나뿐이다 — 전투불능 잔상
    (`--vt-ghost-down`)이 더 진한 빨강이라 두 상태가 색으로 갈린다. `raidHud.css` 는 예전처럼 위치 · 폭만 건드린다.
    ⚠ `.sh-seg` 에는 `overflow: hidden` 을 걸지 않았다 — `.sh-seg .fill` 의 바깥 글로우가 잘린다.
  - **`hud/KeyGuide.ts`** — 손대지 않았다. 인벤토리가 `Shift + 드래그` · `Ctrl + 드래그` 두 항목을 `combo` 로 올릴 뿐이고
    (`inventory/ui/InventoryUI.guideKeys`), 그 렌더링(`kg-plus`)은 2026-09-12 부터 있던 것이다.
- **2026-09-14 (NPC 개인 신뢰도, 에이전트 C, docs/DECISIONS.md 「2026-09-14 — 정보상」)** — 메신저가 NPC 개인 신뢰도를 **읽고 그린다**(적립은 meta/).
  - **`menus/messenger/Trust.ts` (새)** — `npcTrustOf` · `buildNpcTrust` · `buildTrustChip` · `npcTrustTitle`. 값은 `ctx.meta.npcTrust` 하나에서만 오고
    레벨 · 구간은 공용 `REP_TABLE` · `repLevelOf` 로 푼다(meta/ 를 import 하지 않는다). `ctx.meta` 가 없으면 전부 null → 아무것도 그리지 않는다.
  - **`ChatTab.ts`** — NPC 목록 줄에 `Lv.n` + 짧은 게이지(`compact`), NPC 대화창 머리에 넓은 게이지. 둘 다 다시 그리기 지문(`rows()` 키 · `threadDataKey`)에 들어간다.
  - **`QuestsTab.ts`** — 상세 머리에 같은 게이지, `meta:npcTrustChanged` 구독, 지문에 고른 퀘스트 NPC 의 신뢰도.
  - **`QuestCard.ts`** — 보상 줄에 NPC 신뢰도 칩(기업 신뢰도 칩 바로 뒤). 재화 칩과 **같은 틀**이지만 `data-currency-id` 는 없다 —
    가짜 재화 정의를 만들지 않는다(CLAUDE.md 「보상 단위는 아이템이 아니라 재화다」의 반대편 규약). 보상이 0 이면 칩도 없다.
  - **`Messenger.ts`** — `levelUp` 토스트 `<NPC 이름> 신뢰도 Lv.n`. 패널이 닫혀 있어도 떠야 하므로 `bind` 에서 걸고 `dispose` 에서 푼다.
  - **`styles/messenger.css`** — `.ms-trust*` (접두사 `ms-` 안, `rg "\.ms-trust" src` 로 비어 있는 것을 먼저 확인했다).
- **2026-09-14 (메신저 — 커뮤니티 패널 대체, 에이전트 C, docs/DECISIONS.md 「2026-09-14 — 메신저 · NPC 퀘스트 · 단체방」)** —
  - **`menus/messenger/` (새)**: `Messenger` · `ChatTab` · `QuestsTab` · `QuestCard` · `Popover` · `textInput` · `sources` · `format`, **`styles/messenger.css` (새, `ms-`)**.
    P 패널이 대화(NPC · 개인 대화 · 단체방) · 친구(옛 커뮤니티 열) · 퀘스트 세 탭이 됐다. 함선 전용 · Tab / Esc 닫기 · 분대 초대 스택 그대로.
  - **`hud/Community.ts`** — 패널 본체를 `Messenger` 로 넘기고 썸네일을 `✉ 메신저` + 읽지 않음 숫자 배지로, `ui:messengerToggled` · `ui:openMessenger` · NPC 메시지 토스트.
  - **`menus/social/SocialPages.ts`** — 대화 기록 페이지 삭제(차단 목록만). **`SocialMenu.ts`** — `귓속말하기` → `개인 대화`, `대화 기록` 항목 삭제. **`SocialColumn.ts`** — 친구 탭 호스트.
  - **귓속말 → 개인 대화** (`hud/ChatLog.ts` · `whisperText.ts` · `SocialMenu` 차단 확인 문구 · `HudSystem` 주석).
  - **`HudSystem.ts`** — 스모크 훅 `debugNpc` · `debugRooms` · getters `messenger` · `messengerUnreadBadge`. **`socialSource.ts`** — 합성 ref 의 개인 대화 읽지 않음.
  - 스모크 **`scripts/smoke-messenger.mjs`** (새, `ui` · `meta` · `net` — 스텁 NPC · 단체방 · 합성 소셜로 탭 전부 + 두 해상도 스크린샷 `scripts/shots/messenger/`, 마지막에 진짜 ref 로 한 번 연다) 66/66,
    **`scripts/smoke-social.mjs`** 갱신(친구 탭 전환 · 메뉴 항목 · 개인 대화 문구 · 대화 기록 화면 → 메신저 대화 · 빈 NPC / 단체방 ref 로 배지 고정) 208/208.
- **2026-09-14 (메신저 · NPC 퀘스트 — 지도 퀘스트 패널 · 퀘스트 토스트, 에이전트 E, docs/DECISIONS.md 「2026-09-14 — 메신저 · NPC 퀘스트 · 단체방」)** —
  - **`map/QuestPanels.ts` (새)** · **`styles/mapquests.css` (새, `mq-`)** — 레이드 지도 좌측 열에 진행 중인 NPC 퀘스트 패널 목록(`ctx.meta.npc.getRaidTracks()`):
    이름 · NPC · 지금 셀 수 있는 레이드 목표 줄 · 패널 안 하단 게이지, 호버 = 상세 툴팁(설명 · 목표 전부 · 행성 · 보상). 서명이 바뀔 때만 다시 짓는다.
  - **`map/MapScreen.ts`** · **`styles/base.css`** — 열 순서 머리 → 퀘스트 → 범례(좌측 하단, 2열) → 발밑 줄, 열 폭 240 → 280 px(캔버스 식 `vw − 340` → `vw − 380`),
    열 높이 = 캔버스 높이로 고정. 목적지 선택 모드는 퀘스트 목록도 숨긴다(`.is-rover`).
  - **`hud/Notifications.ts`** — 옛 기업 퀘스트 완료 토스트 삭제 → NPC 퀘스트 목표 달성 · 보고 가능 · 완료 보상 토스트 (훈련장 제외).
  - 스모크 **`scripts/smoke-map-quests.mjs`** (`ui` · `meta`, 가짜 `NpcQuestRef` — 71 항목).
...
- **2026-09-13 (서재 시리즈 · 비디오게임 — 툴팁 · 칩 띠, 에이전트 C, docs/DECISIONS.md 「2026-09-13 — 서재 시리즈 · 비디오게임」)** —
...
- **2026-09-13 (발전기 전력 — 시설 관리 전력 패널 · 인스펙터, docs/DECISIONS.md 「2026-09-13 — 가구 접근 면 · 발전기 · 암호화폐 채굴」, 전력 에이전트)** — ⚠ 같은 날 폐지됐다 (바로 위). `hud/ShipManage.ts` 만 바뀌었다 (+ `styles/base.css` 의 `.sm-pw-` 절).
  - **전력 패널** `.sm-pw` — 방 목록 열의 발전기 행 **바로 아래**. 머리 `할당 a / 공급 s · 남음 f` → 공급 막대(시설별 할당 조각 = 용도 색, 멈춘 시설은 빨간 빗금, 끝에 남는 전력) →
    ...
    이름을 누르면 `시설 기본` + 가구별 demand · `꺼짐` 이 펼쳐진다(`powerOpen`). 목록은 자기 스크롤(최대 210 px). 방 목록 행도 멈춘 시설이면 `.is-unpowered`(⚡).
    `housing:powerChanged` 에 다시 그린다. 계약 API 가 없으면 패널이 숨는다.
  - **인스펙터** — 전력을 쓰는 가구면 `.sm-pw-ins` `요구 전력 n · 가동 중 (시설 할당 a / 요구 r)` 또는 `요구 전력 n · <사유>`(멈춤은 빨강), 업그레이드 **왼쪽**에
    `.sm-pw-toggle` `비활성화` / `활성화`(`setFurnitureDisabled`). 전력 0 가구에는 둘 다 없다.
  - 「배치 · 강화로 시설이 멈췄다」 토스트는 housing 이 낸다(`parts/Power`) — 하우징 모드의 손 배치도 같은 문장을 받는다.
  - 발전기 최대 Lv.10 — 이 화면은 원래 `getFacility('generator').maxLevel` 을 읽어 숫자를 적지 않았다.

- **2026-09-13 (월드맵 정리 · 탐사 차량 UI — 포크 A)** — 계약(`shared/types.ts` 의 탐사 차량 절 `RoverRef` · `RoverStationDef`, `events.ts` 의 `rover:*` · `fog:discovered 'rover'`)은 읽기만 했다.
  - **월드맵 (사용자 결정)**: 구조물 라벨이 `구조물` · `폐허` 에서 짧은 원래 이름으로(연구소 · 전진기지 · 불시착 함선 · 폐허 전초 · 플랫폼 · 벌레 둥지 · 버섯 군락, 어두운 윤곽 + 겹침 제거 `map/mapIcons.MapLabels`).
    떨어진 아이템 · 설치물 · 지뢰 마커 삭제. 범례에서 활성 탈출 지점 · 핑 3종 · 보급/개봉 상자 · 떨어진 아이템 · 설치물 · 지뢰 · 버려진 구조물 · 폐허 전초 · 거대 버섯 군락 · 벌레 둥지 줄 삭제,
    탐사 차량 · 차량 경로 줄 추가. 플레이어 · 분대원 화살표 1.6배, 범례 견본은 지도와 **같은 그리기 함수**의 캔버스(`base.css` 의 옛 `.sw.*` 모양 규칙 삭제).
    좌하단 `초기화` 버튼 · 안내 줄(`.map-hint`) 삭제 → 우하단 키 가이드가 `… · Tab 또는 Esc 또는 M 닫기`. 스테일 주석(「Escape 는 지도를 닫지 않는다」) 정정.
    지도의 핑 버튼이 하드코딩 `button === 1` 에서 `Keys.PING` 을 따르게(가이드 라벨과 일치), 이동 라벨은 `Keys.FIRE` 가 아니라 실제로 끄는 `LMB`.
  - **탐사 차량**: `map/MapScreen` 흙길 · 정류장 · 차체 레이어와 목적지 선택 모드(1초 홀드 결제), 새 `hud/RoverHud`(탑승 HUD + `rover-view`), `hud/RaidAlerts` 토스트 7종,
    `hud/KeyGuide` 의 `NO_CLOSE_OWNERS`. 스타일은 새 `styles/rover.css`.
  - 스모크: `scripts/smoke-ui-p5.mjs` 의 `.map-hint` 검사(「휠클릭 핑」)를 키 가이드 검사(핑 키 + 닫기에 `또는 M`)로 바꿨다 — 안내 줄이 없어져 그대로면 TypeError 였다.

- **2026-09-13 (탐사 차량 탑승 게이트 — 에이전트 D)** — `hud/CommsWheel`(H) · `hud/Pings`(핑) · `hud/GadgetHandHint` 가 `ctx.player.roverRide` 동안 꺼진다.
  `hud/Nameplates` · `hud/TypingBubbles` 는 `PlayerFlags.IN_ROVER` 인 원격 분대원을 숨긴다 (아바타는 선체 안).

- **2026-09-13 (요리 미니게임 — 에이전트 cook-misc, `docs/DECISIONS.md` 「2026-09-13 — 요리 미니게임」)** — 계약(`shared/cooking` 의 `cookStepsOf` · `COOK_GAME_LABEL_KO` ·
  `mealQualityStars` · `mealQualityBonus` · `normalizeMealQuality` · `CookResult`, `CharBuff.quality` · `cooking`, `housing:cookResult` · `housing:mealServed.quality`,
  `ItemInstance.quality`)은 읽기만 했다.
  - **`hud/mealText.ts`** — 품질 선택 인자(`mealEffects` · `mealEffectLines` · `mealEffectsText`), `mealQualityText`, `cookStepsText`. 보너스 식을 여기 한 곳에 둬서
    툴팁 · 버프 썸네일이 같은 수치를 찍는다.
  - **`hud/ItemTip.ts`** — 요리 `품질` 줄(인스턴스가 있을 때만) · 보너스 반영 능력치 · `조리` 단계 줄.
  - **`hud/BuffStrip.ts` + `styles/buffs.css`** — 식사 썸네일 별 배지(`data-q` + `::after`), 식사 title 의 보너스 반영, `cooking` title 의 단계 줄,
    `BuffCellState.quality`. 버프 종류를 손으로 적은 `Record` · switch 는 이 폴더에 없었다(`paintCell` 의 `itemKind` 판정에 `cooking` 은 일부러 넣지 않았다 —
    만드는 요리의 글리프를 쓰면 식사 버프와 같은 그림이 된다).
  - **`hud/Notifications.ts`** — `housing:cookResult` 토스트(성공 별 + 창고/가방 · 실패 사유), 식탁 토스트의 별, 조리대 요리의 `craft:completed` 토스트 생략.
    같은 순간의 `inventory:itemAdded` 획득 티커는 일반 제작과 똑같이 그대로 뜬다.
  - **`hud/ShipManage.ts`** — `MODEL_GLYPH` 자동 조리 가구 4종 확정 (위 표).
  - ⚠ 품질 줄은 `data-uid` 를 단 요소(기업 · 스테이션 `TradeGrids` 타일, 식탁 등)에서만 뜬다 — 가방 격자 자체의 툴팁은 `inventory/ui/Tooltip` 의 몫이다.
...
- **2026-09-13 (탈출 개편 — 출발 유예 타이머 · 이륙 연출 HUD 페이드, 사용자 결정)** — 흐름은 `src/extraction/README.md`.
  ...
  - **`hud/DangerIndicators`**: 분류 `worm`(`지상이변` · `◎` · 빨강) — `sandworm:warning` 을 분출 시각까지 붙들고 `sandworm:erupted`(놓치면
    분출 + 1.5 s)에 지운다. 게이트 없음(전조는 무리 발밑이다), 1.5 s 안이면 `hot`.
...
- **2026-09-13 (환경 재해 표시 — 「폭풍」 통일 · 지도 포자 합집합 · 맵 밖 클립)** — 사용자 결정.
  ...
  - **`map/MapScreen.ts`** `drawHazard` — 캔버스 클립에 **맵 사각형 클립**을 겹쳤다 (폭풍의 눈이 이제 맵 네 꼭짓점을 품는 원으로 시작해 원 대부분이 맵
    ...
    좌하단 라벨은 `폭풍 · 안전지대 n%`.
- **2026-09-13 (로그 강하 → 레이더 강하 문구 — 행성별 적 난이도, faction-presentation, docs/DECISIONS.md 「2026-09-13 — 행성별 적 팩션」)** — 강하는 이제 레이더만 내리고
  분대장(보스)이 섞이지 않는다 (이벤트 · 필드 `rogueDrop:*` · `boss` 는 계약이라 이름 그대로).
  - **`hud/RaidAlerts.ts`** — 예고 토스트 `적 n명 강하 감지 (분대장 포함) — n초` → `레이더 n명 강하 감지 — n초` (보스 갈래 삭제).
  - **`hud/DangerIndicators.ts`** — 착지 라벨 `로그 n` / `로그 분대장` → `레이더 n`, 분류 이름 `적 강하` → `레이더 강하`, 보스용 진한 빨강
    `DROP_BOSS_COLOR` 삭제(포드는 늘 `DROP_COLOR`). 적 수류탄 라벨 `적 수류탄` 은 그대로(로그 · 레이더 둘 다 던진다).
  - `HudSystem.ts` 주석. `hud/NamedScanWarning` 에는 「로그」 문구가 없어 바뀐 것이 없다.

- **2026-09-13 (요리 재료 티어, 에이전트 D — `docs/DECISIONS.md` 「2026-09-13 — 요리 재료 티어」)** — 계약(`MealDef.effects` · `MEAL_TIER_LABEL_KO` · `SAMPLE_FAMILY_*` ·
  `GROW_SOCKET_*` · `growSocketSlotsFor` · `ANALYSIS_RESULTS` · `analysisTimeMul` · `housing:analysisFound/LevelUp`)은 읽기만 했다.
  - **`hud/ItemTip.ts`** — 요리 티어 + 능력치 줄 전부, 토양 · 배지 최대 내구도 + 소켓 칸(「수확 n 회」 · 「배양 n 회」 삭제), 세포주 스캐폴드 산출 두 줄,
    스캐폴드 한 줄 설명, 소켓 세 줄, 표본 계열, 은퇴 한 줄 (위 표의 `hud/ItemTip.ts` 행). 색은 기존 규약대로 인라인이고 CSS 는 안 바뀌었다.
  - **`hud/mealText.ts`** — `mealEffects` · `mealEffectLines` · `mealEffectsText` · `mealTierLabel`.
  - **`hud/BuffStrip.ts`** — 요리 썸네일 DOM `title` 에 능력치 줄 전부 (`state[].title` 은 그대로).
  - **`hud/Notifications.ts`** — 분석 도감 발견 · 분석 레벨업 토스트 (label `분석`).
  - 식탁 화면(`housing/ui/DiningTable`)의 요리 문구는 housing 폴더의 몫이라 여기서 고치지 않았다.
...
- **2026-09-12 (지상 드론 스캔 — 사용자 결정, 에이전트 D, docs/DECISIONS.md 「2026-09-12 — 전투 소모품」)** — 새 `hud/DroneScanLabels.ts`
  (위 표) · `hud/DroneHud` 에 스캔 홀드 링 `.dsc-ring` · 안내 `.dsc-hint` · 라벨 위젯 소유 + `lateUpdate(ctx)` ·
  `HudSystem.lateUpdate` 한 줄(`this.droneHud.lateUpdate(ctx)`, hud 가 보일 때). 스타일은 `styles/drone.css` 끝 블록 —
  **접두사 `.dsc-` / `.dsl-`** (`.dr-scan` 은 이미 외곽 지지직의 스캔라인이다). 수치(`DRONE_SCAN_RANGE` · `LABEL_HEIGHT` ·
  `LABEL_MAX_DIST`)는 `@/shared`. 검사: `scripts/smoke-drone-scan.mjs`.

- **2026-09-12 (즐겨찾기 칩 — 에이전트 E2)** — 새 `hud/ItemFavoriteMenu.ts`(위 표) · `HudSystem` 에 세 줄(생성 · `bind` · `dispose`,
  `itemTip` 옆) · `styles/base.css` 의 `.item-chip.is-favorite .item-chip-thumb::after`(썸네일 우측 상단 파란 사선 띠 — 색은
  `--c-favorite`, 없으면 `#4a90ff`; 개수 배지는 우측 하단이라 겹치지 않는다, `pointer-events: none`) + 메뉴 `.icm` · `.icm-head` ·
  `.icm-item(.is-on)` · `.icm-mark`(접두사 `rg "\.icm-" src` 로 비어 있음을 확인). 칩 마크업 계약은 `shared/itemChip.ts` 의 새 블록.
  계약 행의 아이템 칩(`meta/ui/CorpView`)과 기업 상점 타일 옵트인은 meta 몫. 검사: `scripts/smoke-favorite-chips.mjs` (50/50).
  ⚠ `hud/ContractPanel` 은 새 목표 `extract_with_items` 를 라벨 `아이템 회수` 로 그대로 그린다(아이템 이름은 없다 — 필요하면 여기서
  `CONTRACT_DEFS` 의 `itemDefId` 를 읽어 한 줄 붙이면 된다).

- **2026-09-12 (임플란트 · 함선 호출 준비 연출 — 사용자 결정, 에이전트 B, docs/DECISIONS.md 「2026-09-12 — 전투 소모품」)** — 계약은 읽기만
  (`implant:ready` · `implant:cooldownRefunded` · `stratagem:ready`, 셋 다 `events.ts` [B]).
  - **`hud/ImplantWidget`** — 준비되는 **순간** 강한 플래시 1회: 썸네일 밖으로 퍼지는 테두리 `.imp-ring` + 안쪽 섬광 `.ib-flash`
    (`.imp-hud.rdy-major`), 대시의 중간 충전은 약하게(`.rdy-minor`). **준비된 동안** 윤곽 글로우 `.is-ready` — 딤드가 아닌 동안(쿨타임형 =
    준비, 충전형 = 충전 1 이상, 배리어 = 잠금 아님, 오버차지 = 에너지 바닥 아님). 순간은 **추측하지 않고** `implant:ready` 만 믿는다
    (implants 가 게임플레이 페이즈에서만 낸다 → 미션 시작 · 리셋 · 장착 · 함선에서는 번쩍이지 않는다). 갈고리 환급
    (`implant:cooldownRefunded`)은 썸네일 오른쪽에 초록 `−N초`(`.imp-refund`, `secs` 규칙) 가 떠오르며 사라지고 차오른 부분이 번쩍인다
    (`.rf-flash` → `.ib-reveal`). 플래시를 따로 요소로 둔 이유: `.pulse` · `.hit` 가 `.imp-thumb` 의 `animation` 을 쓰고 클래스가 남으므로
    같은 요소에 얹으면 둘이 서로의 재생을 막는다. `hub:entered` 에서도 일시 상태를 비운다. 디버그 getter `isReadyGlow` ·
    `lastReadyFlash` · `readyFlashCount`.
  - **`hud/StratagemPanel`** — 같은 연출(`.sc-ring` · `.sb-flash` · `.scall.rdy-major` / `.rdy-minor` · `.is-ready`)을 `stratagem:ready` 로.
    거절 환불로 0 이 된 순간(`refunded`)은 **약한 플래시**로 정했다 — 다시 부를 수 있는 것은 사실이지만 거절 토스트 옆에서 크게
    번쩍이지 않게 (소리는 audio/ 가 내지 않는다).
  - **`styles/implant.css` · `styles/shipCall.css`** — 각자의 「준비 연출」 절. 글로우 절은 `.wielded` · `.armed` **앞**에 두어 손에 든 /
    무장한 테두리가 이긴다. 새 클래스 `rdy-` · `rf-` · `imp-ring` · `imp-refund` · `sc-ring` · `ib-flash` · `sb-flash` 는 `rg` 로 비어 있음을
    확인했다 (`is-ready` 는 hub · housing 도 쓰지만 모두 자기 블록 셀렉터 아래라 겹치지 않는다).
  - 검증: `smoke-tactical` (대시 충전 셋 → `.rdy-minor/.rdy-minor/.rdy-major` + `.is-ready`, 쿨타임 중 `.dim` · 글로우 없음, `refillAll` →
    `.rdy-major`, 갈고리 환급 → `−N초` + `.rf-flash`), `smoke-phase3` (함선 호출 쿨타임 끝 → `.scall.rdy-major` + `.is-ready`, 새 쿨타임 →
    글로우 꺼짐, 거절 환불 → `.rdy-minor`).

- **2026-09-12 (전투 소모품 버프 썸네일 — 에이전트 A1)** — `hud/BuffStrip.paintCell` 이 새 버프 종류 `adrenaline` · `stimulant` 도
  `meal` · `prep` 처럼 **아이템 def 의 글리프 · 색**을 쓴다 (`b.defId` → `ctx.loot.getItemDef`). 시간 게이지 · 남은 초 · `title`
  (= 아이템 이름)은 기존 경로 그대로. CSS 변경 없음. 검사: `scripts/smoke-consumables.mjs`.

- **2026-09-12 (크로스헤어 총구 막힘 경고색 — 사용자 결정)** — `hud/Reticle` 이 `weapon:aimBlocked` 를 받아 `.reticle.blocked` 를
  토글한다 (점 · 틱이 빨강, `background-color` 전이). 총구가 앞 3 m 안의 벽 · 창틀에 걸려 크로스헤어대로 나가지 않는다는 뜻이고,
  빨간 원이 화면 구석에 있어도 알 수 있게 하려는 것. 판정은 weapons/parts/AimLine 하나다.

- **2026-09-12 (캐릭터 버프, 에이전트 ui)** — 계약(`src/shared/charBuffs.ts` · `PlayerRef.buffs` · `RemotePlayerRef.buffs` ·
  `player:buffsChanged` · `net:remoteBuffsChanged`)은 리드가 먼저 적었고 목록은 player(내 것) · net(분대원 것)이 채운다.
  결정 `docs/DECISIONS.md` 「2026-09-12 — 캐릭터 버프」.
  - **`hud/BuffStrip.ts` + `styles/buffs.css`** (새 파일) — 썸네일 줄 하나를 두 곳이 쓴다. 글리프 · 색 원본 · 흐림 · 빨간 테두리 ·
    시간 게이지(임플란트 썸네일과 같은 두 얼굴 기법) · `23h`/`42m`/`35s` · `charBuffTitle` title · 키로 DOM 재사용 · 같은 배열은
    무시 · 타이머가 있을 때만 1초 틱. 접두사 `bfs-`.
  - **`hud/Vitals.ts`** — **PC 체력 블록이 함선에서도 보인다**(사용자 결정). 루트를 소셜 레이어로 옮기는 쪽을 골랐다(2026-09-09
    `HoldGauge` 와 같은 이동): CSS 예외로 게임플레이 레이어에 남기면 그 레이어가 함선에서 통째로 `hidden` 이라 불가능했고, 페이즈마다
    DOM 을 오가게 하는 것은 순서 · z 가 흔들린다. 스태미나 바는 게임플레이 레이어의 **같은 DOM 자리**에 남아 레이드 전용. 함선에서는
    체력 · 실드를 가득으로 그린다. 체력바 바로 아래 버프 줄(`ctx.player.buffs` 참조 폴링 + `player:buffsChanged`).
  - **`hud/Squad.ts`** — 분대원 행 체력바 아래 미니 줄(`ref.buffs`), 내 줄 제외, 행 키에 `buffsRevision`, `net:remoteBuffsChanged`
    구독, `buffStateOf(id)`.
  - **`HudSystem.ts`** — `new Vitals(hudRoot, null)` + `socialRoot.appendChild(vitals.root)`, `vitals.update` 는
    `hudVisible || socialVisible` 일 때. **걷어낸 것:** `hud/MealBadge` · `hud/EnvBadge` · `hud/GymFatigueBadge` 와
    `styles/meal.css` · `styles/env.css`(`.hud-badges` 행 포함 — 다른 소비자 없음) · `styles/gymFatigue.css`, 등록 · update ·
    dispose, 게터 `envBadgeKind` · `isEnvProtected` · `mealBadgeDefId` · `gymFatigueChips` · `isGymFatigueOn`. 새 디버그 훅
    `localBuffs` · `squadBuffs(id)` · `debugLocalBuffs(list | null)`; `debugRemotes` 의 ref 는 `buffs` 를 실어도 된다.
    `hud/mealText.ts` 는 `ItemTip` 이 계속 쓰므로 남았다. `player:envChanged` · `progress:mealChanged` 의 ui 소비자는 이제 없다.
  - **`styles/base.css`** — `.hud.hub .hud-bl { bottom: 32px }` 삭제 (함선에도 체력 블록이 있으니 열은 그 위 122 px). **`styles/drone.css`** —
    `.hud.drone-view .vitals` → `.hud.gameplay.drone-view ~ .hud.social .vitals`.
  - 스모크: 새 **`scripts/smoke-buffs.mjs`**(`verify.mjs` 폴더 `ui` · `player` · `net`). `scripts/smoke-ui-p5.mjs` 의 두 선택자
    `.hud.gameplay .vitals` → `.hud .vitals`. 배지를 검사하던 스모크는 없었다(`rg` 결과 0) — 옮길 검사가 없었다.

- **2026-09-12 (헬스장 A-3a · 서재 매체 A-3e, 에이전트 ui · audio · net · console)** — 계약(`src/shared`)은 리드가 먼저
  커밋했고(`a723a8a`) 이 폴더는 그것을 그린다. 결정 `docs/DECISIONS.md` 「2026-09-12 — 헬스장 · 서재 매체」.
  - **`hud/GymFatigueBadge.ts` + `styles/gymFatigue.css`** (새 파일) — **함선 전용** 운동 디버프 배지. 걸린 디버프마다
    `⚖ 근육통 23:12:05` / `⚖ 심폐 피로 …` 칩(이름은 `GYM_FATIGUE_LABEL_KO`, 글리프 · 색은 헬스장 방의 `ROOM_PURPOSE_GLYPH/COLOR.gym`).
    레이드 배지(`EnvBadge` · `MealBadge`)와 **반대 자리**라 게임플레이 레이어가 아니라 **소셜 레이어**에 살고 `HubDot` 과 같은
    조건으로 스스로 게이트한다(`ctx.phase === 'hub'` · blocker 없음 · 도킹 / 워프 컷씬 아님) — 레이드 · 타이틀 · 메뉴에서는 안 뜬다.
    끝 시각은 `ctx.progression.getGymFatigueUntil(stat)`(optional → `typeof`)에 **이벤트가 올 때만** 다시 묻는다:
    `progress:gymFatigue` · `progress:trainedChanged` · `hub:entered` · `net:profileLoaded` · `game:phaseChanged`. 글자는
    **디버프가 걸려 있고 배지가 보일 때만** 1초마다 다시 쓰고, 0 초에 닿으면 칩을 스스로 내린다(만료는 이벤트가 없다).
    시각은 progression 과 같은 `ctx.net.serverNow() ?? Date.now()`. 자리는 **좌측 하단**(레이드의 체력 자리 — 함선에서는 비어
    있다; 좌측 상단은 튜토리얼 목표 패널 `.tut-panel` 자리라 피했다). 접두사 `gfat-`(`.gym-` 은 housing 의 운동 화면 것).
    `HudSystem.gymFatigueChips` / `isGymFatigueOn` 디버그 훅.
  - **`hud/ItemTip.ts`** — 책 · 디스크 · 레코드(계약의 `shelfItemOf`)가 같은 두 줄을 얻는다: `숙련`(`getSkillDef` 이름) ·
    `꽂는 곳 — 서재 · <보관함>`. 보관함 이름은 **베껴 적지 않고** 서재 가구 중 `SHELF_INTERACTION[medium]` 을 가진 def 에서
    읽는다(housing 이 없으면 `디스크 보관함`). 전에는 이 카드에 책 줄 자체가 없었다(인벤토리 툴팁에만 있었다).
  - **`hud/ShipManage.ts`** — `MODEL_GLYPH` 의 리드 임시 글자 11개 중 헬스장 셋을 바꿨다: 스미스 머신 `╦` → `╫`(벤치 랙 `╤` 와
    카드 크기에서 거의 같았다), 트레드밀 `═` → `▱`(벨트 판), 사이클 `⊘` → `⚯`(두 바퀴 — `⊘` 는 「금지」로 읽혔다). 서재 여덟은
    그대로(`◉` · `⊚` 는 아이템 분류 글자와 같은 원본). 서재 · 헬스장 가구는 전부 실용 가구라 **시설 가구** 탭에 서고
    `maxLevel 1` 이라 인스펙터는 `최대` 다 — 카드 코드는 바뀐 것이 없다.
...
- **2026-09-12 (애니메이션 주기 완화, 사용자 결정)** — `styles/base.css` 두 줄. 둘 다 "너무 빠르게 흔들린다" 는
  ...
  줄이면 아예 안 보인다. ② 원격 `…` 말풍선 `.tbubble i` 0.9s → **1.8s**, 스태거도 **같은 비율**로 0.15/0.3s →
  **0.3/0.6s** — duration 만 늘리면 0.15 s 가 한 바퀴의 8 %밖에 안 돼 세 점이 거의 동시에 뛴다(물결이 깜빡임이 된다).
  두 값의 근거는 `base.css` 안에 주석으로 남겼다.

- **2026-09-12 (하우징 모드 UI 개선, 사용자 결정)** — `hud/ShipManage.ts` + `styles/base.css` 의 시설 관리 블록만.
  이미 지은 용도 숨김 · 발전기 행을 방 목록 아래로 · 시설 가구 / 꾸밈용 가구 하위 탭 · `이미 보유 중` 버튼 (시설 가구
  보유 수 없음) · 인스펙터 `위치 이동` 버튼 · 인스펙터 위 토스트(`.sm-dock` · `.sm-toast`). 입력 절반은
  `hub/HousingMode` 이고 셋이 새 계약 이벤트로 이어진다(`housing:moveRequested` · `moveStateChanged` · `placeRefused`).
  `styles/base.css`: `.sm-subtabs` / `.sm-subtab`, `.fcard-craft { nowrap }` + `.fcard.is-owned`, `.sm-gen` 이 세로
  배치(`.hd` 머리줄) + `.sm-gen-host`, `.sm-dock` 이 `.sm-inspect` 의 절대 위치를 넘겨받았고(카드는 `position: relative`)
  `.sm-toast`, `.sm-ins-acts` 가 양끝 정렬 + `.sm-ins-move`. 이름 변경(사격장 → 시뮬레이션실, 채굴 시설 → 암호화폐
  채굴 시설)은 계약 표시 이름이라 이 폴더의 코드는 그대로 따라온다.

- **2026-09-11 (주방 · 배양조 · 프린터 A-3c · A-14 · A-15, 에이전트 ui)** — 계약(`src/shared`) · `data/*.csv` 는 먼저
  커밋됐고(`61ef305`) 이 폴더는 **고치지 않았다**. 결정은 `docs/DECISIONS.md` 「2026-09-11 — 주방 · 배양조 · 3D 프린터」.
  - **`hud/MealBadge.ts` + `styles/meal.css`** (새 파일) — `progress:mealChanged` 의 유일한 소비자. `EnvBadge` 가
    A-13 에서 그은 선을 그대로 따른다(게임플레이 레이어 = 레이드 전용 · 이벤트로만 사는 무-`update` 위젯 ·
    `.hud.hub` 이중 안전장치). **`active` 만 그린다** — 함선에서 차려 둔 대기분(`meal`)은 식탁 화면의 몫이고 이
    배지의 뜻은 「지금 내 몸에 무엇이 걸려 있나」다. ⚠ 레이드 경계에서 **비우지 않고 다시 묻는** 것이 요점이다:
    출격의 `armPreps()` 와 `game:newMission` 의 선후를 ui 가 보증할 수 없어, `EnvBadge` 처럼 `set(null)` 했다가는
    막 실린 식사를 지운다. 그래서 `getActiveMeal()` 에 물어 다시 그린다(구현이 늦게 붙을 수 있어
    `typeof … === 'function'` — B-13 질의 셋과 같은 처리).
  - **좌상단이 배지 한 줄이 됐다** — `.env-badge` 가 들고 있던 `position:absolute; left:32; top:56` 이 새 flex 행
    `.hud-badges`(`styles/env.css`)로 올라가고 두 배지는 그 안의 칸이 됐다. 행에 **배경 · 테두리 · 여백이 하나도 없는
    것이 「둘 다 없을 때 빈 상자가 남지 않게 한다」의 구현**이다(자식이 둘 다 `display:none` 이면 높이 0). 배지를
    하나 더 붙일 때도 이 행에 넣기만 하면 된다.
  - **`hud/mealText.ts`** (새 파일) — `+15 %` / `+6 kg` / `−20 %` 를 찍는 한 곳. 배지와 툴팁이 **같은 요리를 같은
    말로** 적어야 하는데 둘이 각자 포맷하면 화면마다 달라진다(`hud/stratagemGlyphs` 와 같은 성격의 폴더 공용
    모듈이고, 계약을 건드리지 않고 끝난다). 단위 판단은 하지 않는다 — `shared/labels` 의 `MEAL_BUFF_UNIT` 이
    `'%'` 라고 한 줄만 100 배한다. **`durabilityLossMul` 의 음수 부호를 뒤집지 않는 것이 사용자 요구의 답**이다:
    「장비 손상 −20 %」는 그 자체로 이득으로 읽힌다.
  - **`hud/ItemTip.ts`** — 요리 `사용 — 다음 레이드 1회분` + 버프 이름을 **행 이름으로** 쓴 `<버프> +n`(· tier 2 면
    `구분 — 특선 요리`), 주머니 `주머니 c × r` · `수납`, 세포주 `배양조 n 시간` · `산출물`, 배지 `배양 n 회` ·
    `배양 속도 +n %`. 준비물(`def.prep`) 줄 바로 아래에 같은 결로 붙였고 색은 여전히 **인라인 `style.color`** 다
    (`.it-stats .v` 에 modifier 클래스를 만들지 않는다 — 2026-09-10 `.hold` 사고). 툴팁 3원칙(크기 줄 없음 ·
    무게 좌하단 · 가치 우하단)은 그대로.
  - **`hud/ShipManage.ts`** — `MODEL_GLYPH` 에 새 모델 4종. 작업대 둘(`bench_cook` · `bench_print`)은 원본이
    `shared/WORKBENCH_ICON` 이고(제작 탭 `inventory/ui/labels` 와 같은 글자여야 한다 — 2026-09-10 규약), 스테이션
    둘은 자기 글자를 갖는다: 식탁 `⊞`(자리가 놓인 상판 — 평범한 탁자 `▭` 와 구분된다) · 배양조 `⚗`(증류기 —
    「무언가가 안에서 자란다」). 기존 어떤 글리프와도 겹치지 않는 유니코드 한 글자다. 화면 로직은 안 바뀌었다.
  - **`hud/Notifications.ts`** — `housing:mealServed {defId, by}` → `<이름> 님이 <요리> 을(를) 차렸습니다` 한 줄
    (label `식탁`, success). **토스트의 유일한 주인이 ui 라는 B-12 규약 그대로**이고 housing · net 은 이벤트만 낸다.
  - **`HudSystem.ts`** — 배지 행을 만들고 `EnvBadge` · `MealBadge` 를 그 안에 넣어 `comms` · `hazard` ·
    `raidAlerts` 와 함께 bind / dispose. `update()` 는 없다. Debug getter `mealBadgeDefId`.
  - ⚠ **리드가 알아야 할 것 — 설계안 §4.6 의 「출격 준비 점검에 식사 한 줄」은 이 폴더에서 할 수 없다** (A-13 의
    `noEnvPrep` 과 **글자 그대로 같은 상황**이다). `LaunchWarning` 을 만드는 곳은 `inventory/parts/LaunchCheck.ts`,
    그리는 곳은 `hub/ui/LaunchWarnPanel.ts` 이고 `src/ui/` 에는 그 경로가 하나도 없다. 게다가 `LaunchWarningId` 는
    닫힌 유니온(`noPrimary` … `noEnvPrep`)이라 이번 계약 커밋(`61ef305`)에도 `'noMeal'` 이 **없다**. 필요한 것 셋:
    ① `shared/types.ts` 의 `LaunchWarningId` += `'noMeal'`, ② `inventory/parts/LaunchCheck.ts` 가
    `ctx.progression.getMeal()` 이 null 이면 `{text: '식사를 차리지 않았습니다', detail: '…'}` 를 넣는 것,
    ③ 그러면 `hub/ui/LaunchWarnPanel` 은 **한 줄도 안 고쳐도** 그려지고 막지도 않는다(소프트 게이트가 저절로 지켜진다).

- **2026-09-11 (연구실 A-11 · A-12 · A-13 + B-13, 에이전트 ui)** — 계약(`src/shared`) · `data/*.csv` 는 먼저
  커밋됐고(652aff6) 이 폴더는 **고치지 않았다**. 결정은 `docs/DECISIONS.md` 「2026-09-11 — 연구실」.
  - **`hud/ShipManage.ts`** — ① `MODEL_GLYPH` 에 `analyzer` · `bench_extract` · `bench_mixer`. `Record<FurnitureModelKind,
    string>` 이 전부를 요구하므로 빠진 키는 **typecheck 에러**였다. 작업대 둘은 **`shared/WORKBENCH_ICON` 을 참조한다** —
    같은 글자가 제작 탭(`inventory/ui/labels`)에도 서고 그 표가 원본이라는 2026-09-10 규약 그대로다(`bench_refine` 과 같은
    모양). 분석기는 작업대가 아니라 스테이션이라 자기 글자 `⌬` 를 갖는다(기존 어떤 글리프와도 겹치지 않는다).
    ② **B-13 클릭 인스펙터**(`.sm-inspect`, 사용자 결정) — `housing:furnitureSelected {uid}` 를 받아 글리프 · 이름 ·
    `Lv.n / max` · 다음 강화 비용 칩 · 거절 사유 · `강화` 를 그린다. **모달리스**다: `.sm-confirm` 의 상자 언어는 쓰되
    배경을 덮지 않고 blocker 도 escape 토큰도 잡지 않아, 카드를 띄운 채 다음 가구를 계속 클릭할 수 있고 ESC 는 그대로
    시설 관리를 나간다(2026-09-09 ESC 규칙 — 스택에 올리는 것은 blocker 를 잡는 화면뿐이다). **1초 홀드는 넣지 않았다**:
    강화는 되돌릴 수 없는 확정이 아니다(`housing/ui/GrowStation` 의 강화 줄과 같은 판단). 이것이 이 README 가
    2026-09-11 온실 항목에서 ⚠ 로 남겨 둔 **「가구 강화 UI 가 어디에도 없다」의 답**이다 — `upgradeFurniture` 의 첫
    호출자이고, 작업대 · 재배 스테이션 · 분석기 Lv.2–3 이 비로소 플레이로 도달한다.
    ③ **가구 제작 카드 잠금**(사용자 결정) — `furnitureCraftBlock(def.id)` 의 사유가 있으면 `.fcard.is-locked` 딤드 +
    `title` + **인쇄된 `.fcard-note.is-locked`** + `제작` 비활성이고 목록 **맨 아래**다(안정 정렬이라 같은 등급 안의
    카탈로그 순서는 그대로). 사유를 **인쇄**하는 것은 Phase 12 의 교훈 그대로다 — `title` 에만 두면 포인터 락에서는 영영
    안 보인다. 사유는 `cardsKey` 의 일부여야 한다: 재료가 들어와 사유가 사라지면 카드가 딤드를 벗고 위로 올라와야 한다.
    ④ 계약의 세 질의(`furnitureUpgradeCost` · `furnitureUpgradeBlock` · `furnitureCraftBlock`)는 **구현이 늦게 붙을 수
    있어**(폴더별 병렬 작업) `typeof … === 'function'` 으로 감싸고, 없으면 예전 질의(`canCraftFurniture` ·
    `def.upgradeCost`)로 같은 답을 만든다 — 구현이 붙으면 저절로 그쪽을 쓴다.
  - **`hud/EnvBadge.ts` + `styles/env.css`** (새 파일) — `player:envChanged` 의 유일한 소비자. `.hud.gameplay` 안에
    사는 것이 곧 「레이드에서만 보인다」의 구현이다(레이어가 함선 · 타이틀에서 내려간다); 함선에서 오는 `env: null` 과
    `.hud.hub` 규칙이 이중 안전장치다. 상태가 **둘뿐**인 이유는 준비물이 100 % 상쇄이기 때문이고(사용자 결정 5), 그래서
    게이지가 아니라 배지다. 자리는 좌상단 임무 시계(top 28) 아래 · 계약 패널(top 104) 위의 빈 띠 `top 56`.
  - **`hud/ItemTip.ts`** — 표본 `분석기 해석 n 시간` · `산출물` (· `최초 해석` 보너스), 준비물 `사용 — 다음 레이드
    1회분` · `차단 — <환경> 환경`. 환경 색은 토양 줄과 같은 이유로 **인라인 `style.color`** 다(`.it-stats .v` 에 새
    modifier 클래스를 만들지 않는다 — 2026-09-10 `.hold` 사고). 적는 해석 시간은 **표의 기준 시간**이다: 도감 진척으로
    줄어드는 실제 시간은 분석 화면(`housing/ui/Analyzer`)이 말한다. 툴팁은 도감을 모른다.
  - **`HudSystem.ts`** — `EnvBadge` 를 게임플레이 레이어에 만들고 `comms` · `hazard` · `raidAlerts` 와 함께 bind /
    dispose 한다. `update()` 는 없다(이벤트로만 산다). Debug getters `envBadgeKind` / `isEnvProtected`.
  - ⚠ **리드가 알아야 할 것 — 설계안 §3.3 의 「출격 준비 점검 환경 한 줄」은 이 폴더에서 할 수 없다.** `LaunchWarning`
    을 만드는 곳은 `inventory/parts/LaunchCheck.ts`, 그리는 곳은 `hub/ui/LaunchWarnPanel.ts` 이고 **`src/ui/` 에는 그
    경로가 하나도 없다**. 게다가 `LaunchWarningId` 는 닫힌 유니온이라(`noPrimary` … `noHeal`) 새 줄을 내려면 계약이
    한 칸 늘어야 한다 — 이 사이클에서 폴더 에이전트가 고치지 않기로 한 그 계약이다. 필요한 것 셋: ① `shared/types.ts`
    의 `LaunchWarningId` += `'noEnvPrep'`, ② `inventory/parts/LaunchCheck.ts` 가 `getPlanet(ctx.hub.planet)?.env` 와
    `ctx.progression.hasEnvPrep(env)` 를 보고 `{text: '이 행성은 <환경>입니다', detail: '준비물이 없습니다'}` 를 넣는 것,
    ③ 그러면 `hub/ui/LaunchWarnPanel` 은 **한 줄도 안 고쳐도** 그려진다(목록을 그대로 그리고 막지 않는다 — 소프트 게이트
    요구가 저절로 지켜진다). 목표 행성을 아는 것은 `HubRef.planet` 뿐이라 판정도 inventory 쪽이 맞다.

- **2026-09-11 (온실 개편 — 재배 스테이션, 에이전트 ui/scripts)** — 계약(`src/shared`) 변경 없음. 옛 재배층
  (`furn_grow_rack`)이 은퇴하고 `furn_grow_station`(model/interaction `grow_station`, maxLevel 3)이 들어온 것에 맞춰
  이 폴더가 진 몫은 **셋**이다.
  - **`hud/ShipManage.ts`** — `MODEL_GLYPH` 에 **`grow_station: '✿'`**. `Record<FurnitureModelKind, string>` 이 전부를
    요구하므로 빠진 키는 **typecheck 에러**였다 (장식이 아니다). 글자는 외부 에셋 금지 규약대로 유니코드 한 자이고,
    옛 재배층 `❀` · 화분 `❦` · 작물 분류 `❁` 와 모두 다른 겹꽃을 골라 "층이 쌓인 물건" 을 구분한다. **`grow_rack: '❀'`
    은 지우지 않았다** — `Record` 가 요구하기도 하고, 옛 세이브를 그리는 경로가 `?? '▨'` 로 떨어지면 같은 가구가 화면마다
    달라진다. 화면 자체는 손대지 않았다: `getFurnitureFor` · `getStored()` 를 그대로 그리므로 housing/ 의 `retired`
    필터만으로 은퇴 가구가 목록에서 사라진다.
  - **`hud/ItemTip.ts`** — 토양 `속성` · `수확 n 회`, 씨앗 `맞는 토양`. 속성 값 글자는 `SOIL_TAG_COLOR` 를
    **인라인 `style.color`** 로 칠한다(`rows` 튜플에 색 칸을 하나 더했다) — `.it-stats .v` 에 새 modifier 클래스를 만들면
    HUD 위젯 클래스와 이름이 겹칠 수 있다(2026-09-10 `.hold` 사고). 툴팁 3원칙(크기 줄 없음 · 무게 좌하단 · 가치 우하단)은
    그대로다.
  - **`scripts/smoke-housing.mjs`** — 옛 재배층 대신 재배 스테이션을 검사하고(층 잠금 · 흙 먼저 · 궁합 `readyAt` ·
    수확마다 닳는 흙 · 강화로 열리는 층 · v3 세이브 환불), 끝에 **이 폴더의 툴팁**을 단언한다 — `.item-chip[data-def-id]`
    를 `ctx.uiRoot` 에 잠깐 붙여 pointerover 를 태우고 토양 `속성`(인라인 색) · `수확 n 회` · 씨앗 `맞는 토양`(같은 색)
    과 「크기 줄 없음 · 무게 좌하단 · 가치 우하단」을 본다. 그래서 `verify.mjs` 의 `smoke-housing` 매핑에 `ui` · `items`
    가 붙었다 (이 줄들을 단언하는 스모크가 여기뿐이다).
  - ⚠ **리드가 알아야 할 것 — 가구 강화 UI 가 어디에도 없다.** `HousingRef.upgradeFurniture` / `furnitureUpgradeBlock`
    은 `src/` 안에 **호출자가 하나도 없다**(2026-09-11 확인). 시설 관리의 `.fcard` 는 **def 카드**라 레벨도 강화 비용도
    그리지 않고, 옛 방 메뉴(그 버튼이 있던 곳)는 Phase 8 에 시설 관리로 리다이렉트되며 사라졌다. 그래서 작업대 Lv.2–3 도
    재배 스테이션 Lv.2–3 도 **플레이 중에는 올릴 수 없다** — 스모크는 `state.generatorLevel` 을 직접 올리고
    `upgradeFurniture` 를 직접 불러 층이 열리는 것만 확인한다. 재배 스테이션은 「작업대들과 같은 대접」을 받고 있지만
    그 대접이 곧 「없음」이다. 강화 UI 를 어디에 둘지는 계약 결정이라 이 에이전트가 정하지 않았다 (재배 화면
    `housing/ui/GrowStation` 이 자연스럽다).

- **2026-09-11 (B-11 (1) 말풍선 차단 · B-12 합류 알림 한 줄, 에이전트 ⑤)** — 결정 `docs/DECISIONS.md` 「2026-09-11 — 신뢰 경로의 남은 틈」 (1) · §6.
  계약(`src/shared`) 변경 없음.
  - **`…` 말풍선도 차단을 본다** — `hud/TypingBubbles.place` 가 `!typing` 게이트와 **같은 줄**에서
    `socialSource.isPeerBlocked(ctx, ref.id)` 를 묻는다. 요소를 만들기 **전**이라 차단한 상대는 DOM 을 한 조각도 쓰지 않고,
    `||` 단축 평가라 조회는 실제로 입력 중인 사람에게만 일어난다. `place(ctx, …)` 로 ctx 를 받는 것이 배선 변경의 전부다
    (`bind` 가 ctx 를 보관하지 않는다).
  - **같은 3줄이 두 파일에 있었다 → `menus/social/socialSource.isPeerBlocked` 로 뽑았다** (`hud/ChatLog.isBlockedPeer` 는
    이제 한 줄 위임). `shared` 가 아니라 **`src/ui` 안**인 이유: 두 소비자가 같은 폴더이고, 그 함수가 묻는 것(`socialOf` +
    `getLobbyPlayer`)의 단일 원본이 이미 `socialSource` 다.
  - **건드리지 않은 것**: 이름표(`Nameplates`) · 핑 · 월드 마커 · 분대 패널. 분대원의 **위치**는 게임플레이 정보다 —
    `ChatLog` 가 차단한 상대의 `ping` / `request` 줄을 일부러 남긴 것과 같은 선.
  - **합류 · 이탈 토스트는 한 줄** — `hub/HubSystem` 의 `net:peerJoined` / `net:peerLeft` → `ui:notify {name} 함선 합류 · 이탈`
    두 줄을 지웠다(그 폴더의 주석이 이유를 갖는다). 같은 이벤트에 이 폴더의 `hud/Notifications` 가 `<이름> 합류` · `<이름> 이탈`
    (`'분대'` 라벨)을 띄우는데 `ui:notify` 가 **같은 토스트 스택**이라 함선에서 두 줄이 나란히 떴다. 잃는 것은 `함선` 이라는
    낱말 하나다. ⚠ **반대 방향으로 지우면 안 된다** — `Notifications` 의 `social:inviteResult {accepted}` 가 "합류 줄이 이미
    뜬다" 를 근거로 자기 토스트를 뺐으므로, `:83` 을 지우면 초대 수락 알림이 통째로 사라진다. 그 주석을 그렇게 고쳐 적었다.
  - **스모크 훅**: `TypingBubbles.setDebugRefs` (`Nameplates` 와 같은 모양, `HudSystem.debugRemotes` 가 함께 먹인다) ·
    `TypingBubbles.visibleIds` → `HudSystem.typingBubbleIds`. `scripts/smoke-social.mjs` 에 두 절 추가 —
...
- **2026-09-11 (B-3 통합, 리드)** — `hud/Notifications` 의 `social:inviteResult` 가 `accepted` 에는 토스트를 띄우지 않는다. 받은 사람이 들어오는 순간
  `net:peerJoined` 의 `<이름> 합류` 가 이미 뜨고 hub 의 `<이름> 함선 합류` 까지 세 줄이 겹쳤다 (남은 두 줄 겹침은 **2026-09-11 B-12 에서 해결** —
  아래 항목, `hub/HubSystem` 쪽 두 줄을 지웠다). `smoke-social` 단언 1건 갱신.

- **2026-09-11 (B-3 초대 결과 · B-4 차단 / 전송 확인 / 대화 기록 — 클라이언트, 에이전트 ②)** — 설계안
  `docs/HISTORY.md` 「2026-09-11 (16차: 소셜 · 신뢰 경로 · 연결 배치)」 의 ui 쪽. 규칙과 상태는 `net/SocialSync` 가 갖고 ui 는 그리기만 한다.
  - **초대**: `hud/Community` 카드 × 가 `declineInvite`(보낸 사람에게 거절이 간다). `hud/Notifications` 가 `social:inviteResult`
    를 `SOCIAL_INVITE_OUTCOME_KO` 로 토스트한다 — **거절("…님이 초대를 거절했습니다") ≠ 만료("…님이 초대에 응답하지 않았습니다")**
    (사용자 결정), `superseded` 는 조용. 받은 초대를 서버가 `failed` 로 닫으면 "분대 초대가 취소되었습니다 (사유)" — 사유는 초대용
    문구(`not_found` = 분대가 없어졌습니다 · `full` · `in_mission`), `limit`(서버가 내 스택을 정리)은 조용.
  - **초대 중 배지**(사용자가 권장안보다 더 고름): `menus/social/ProfileCard` 가 `SocialPlayer.inviteAt` 이 있는 카드에
    `.sc-inv 초대 중 · n초`(`inviteAt + SQUAD_INVITE_TTL_S`, 릴레이 시계 `net.serverNow()`), `SocialColumn.tick()` 이 초마다
    카드를 다시 짓지 않고 글자만 고친다(커뮤니티가 열려 있을 때 `Community.update` 가 부른다).
  - **차단**: 카드 우클릭 `차단`(되돌릴 수 있으므로 1초 홀드 없이 확인 카드 한 번 — 친구 · 요청이 양쪽에서 정리된다는 설명) /
    차단된 행은 `대화 기록` · `차단 해제` 만. 커뮤니티 머리 `차단 목록 n` → `SocialPages` 차단 목록(해제 가능).
    **분대 채팅도 가린다**: `hud/ChatLog` 가 `net.getLobbyPlayer(id).code` 가 차단된 분대원의 `text` 줄을 그리지 않는다(핑 · 의사소통 휠 줄은 유지).
  - **전송 확인**: 내 귓속말은 `pending`(흐리게 + `.wst 전송 중…`)으로 그려지고 `social:whisperUpdated` 가 **그 줄을** `sent` /
    `stored` / `failed`(`전송 실패 — 오프라인`)로 바꾼다 — 그려진 줄 + 따로 오류 토스트의 이중 표시가 없어졌다. 입력 한도 120 →
    `SOCIAL_WHISPER_MAX`(릴레이 한도). **`/r <텍스트>`**(`/ㄱ` 도) = 마지막 상대에게, `/r` 만 치면 그 상대로 입력 대상을 바꾼다.
  - **대화 기록**: 카드 우클릭 `대화 기록` → `SocialPages`(줄 목록 + 입력칸). 저장은 `SocialSync` 가 `slotKey(WHISPER_STORAGE_KEY)`.
  - **고친 버그 (기존)**: `hud/Community` 가 초대 스택 재구성 신호로 `inviteKey = ''` 를 썼는데, 그것은 **빈 스택의 키와 같아서**
    마지막 초대가 닫혀도 카드가 화면에 남았다(목 ref 로는 늘 다른 초대가 남아 있어 안 보였다). `STALE_KEY`(`'#'`)로 바꿨다.
  - 새 파일 `menus/social/SocialPages.ts` · `menus/social/whisperText.ts` · `styles/social.css`. `socialSource` 에
    `setDebugSocialRef(ref)`(스모크가 떼어 낸 진짜 `SocialSync` 를 꽂는다) · `setDebugSocial(…, history)`, 목 ref 에 계약 메서드
    (`blocked` · `block` · `declineInvite` · 기록). `HudSystem.debugSocialRef` · `debugSocial(…, history)`.
  - 검사: `scripts/smoke-social.mjs` (198) — 떼어 낸 `SocialSync` 에 서버 프레임을 손으로 먹여 초대 id 경로 · 옛 서버 폴백 ·
    재전송 id · 토스트 문구 · 배지 카운트다운 · 차단 메뉴 → `social:block` · 차단 목록 페이지 · 채팅 줄 숨김 · pending → ack ·
    실패 줄 교체(토스트 없음) · 기록 슬롯 키 · backlog · `/r` · 대화 기록 화면 · 옛 서버 유예.

- **2026-09-11 (B-1 서버 연결 배지 · 전이 토스트 정리, 에이전트 ④)** — 새 `hud/NetBadge.ts` + `styles/netBadge.css`
  (위 표). `HudSystem` 이 타이틀 뒤에 만들고(`#ui-root` 직계 — 타이틀 위에서 버튼을 받아야 한다) 매 프레임 `update`, 스모크용
  `netBadgeState` getter. `서버 설정` 버튼은 기존 설정 오버레이를 `select('network')` 로 연다. **토스트 정리**: `hud/Notifications`
  의 `net:reconnecting`(`서버 재연결 중… (n)`) · `net:resumed`(`재연결됨` · `함선에 복귀했습니다…`) 두 줄을 걷어냈다 — 레이드 중에는
  `game/GameFlowSystem` 이, 함선에서는 `hud/NetBadge` 와 `hub/parts/Transitions.onResumed` 가 같은 순간에 이미 한 줄씩 냈다
  (두 줄씩 뜨던 것). 레이드 HUD 에는 배지가 없다(사용자 결정). 검사: `scripts/smoke-netlink.mjs`.
...
- **2026-09-10 (전술 임플란트 HUD 이전 · 크로스헤어 갈고리 · 위험 인디케이터)** — 새 스타일은 전부 **새 시트
  ...
  (**이름 없음**, 아래에 `Keys.IMPLANT` 키캡). 유형 셋 — 쿨타임형(갈고리 · 정찰) 딤드 + 아래에서 위로
...
## 변경 이력 (2026-09-15 2차): Tab 도 **맨 위 화면부터** 닫는다 — 지도 (사용자 버그 보고, 에이전트 E)

ESC 는 2026-09-09 부터 `ctx.escape`(열린 순서의 역순 LIFO)로 맨 위 하나만 닫는데, **Tab 은 화면마다 자기
`update()` 에서 키를 폴링**하므로 닫히는 순서가 `main.ts` 의 시스템 등록 순서로 정해진다 — 지도 위에 나중에 연
화면(무한 상자 · 인벤토리 창)이 떠 있어도 지도가 먼저 닫혔다. `map/MapScreen.update` 의 Tab 갈래에
`ctx.escape.topKey === BLOCKER` 조건 하나를 더해 **맨 위가 나일 때만** Tab 을 먹고 닫는다.
열린 순서를 아는 곳은 여전히 `ctx.escape` 하나뿐이고, 이 폴더는 그것을 **읽기만** 한다.

- `hud/Community`(메신저)는 이미 `blockers.size === 0 || (size === 1 && 내 토큰)` 으로 같은 일을 하고 있어 무변경.
- `hud/ChatLog` · `menus/messenger/textInput` 은 **가장 안쪽 팝업**(자기 capture 핸들러에서 키를 삼키는 층)이라
  이 규칙의 대상이 아니다. `hud/RescuePicker` 는 열리면 언제나 맨 위라 그대로 뒀다.
- 같은 뿌리의 버그가 **`housing/ui/Panel.ts`** 에 남아 있다 (이 폴더 밖이라 고치지 않았다 — housing 담당에게 넘겼다):
  `HousingPanel.onKeyCapture` 는 window **capture** 리스너라 `stopImmediatePropagation()` 으로 Tab 을 아예 삼킨다.
  그래서 책장 화면을 켠 채 `/items` 로 무한 상자를 열면 Tab 이 **책장**을 닫는다 — `Input` 이 그 키를 기록조차 못 해
  인벤토리는 볼 기회가 없다. 고칠 자리는 그 함수 한 줄(`ctx.escape.topKey` 가 자기 토큰일 때만 가로챈다)이다.
