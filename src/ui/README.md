...
| **menus/messenger/ — 2026-09-14 (메신저 · NPC 퀘스트 · 단체방, docs/plans/messenger-quests.md §5)** | |
...
- **2026-09-14 4차 (함선에서의 `타이틀로` · `게임 종료`, 레인 E, docs/plans/tutorial-polish-2026-09-14d.md §3.E)** — 사용자 결정 하나.
...
- **2026-09-14 3차 (메신저 UI — 목록 · 머리 · 타이핑 연출, 레인 A2, docs/plans/qol-batch-2026-09-14c.md §3.A2)** — 사용자 결정 넷.
...
- **2026-09-14 2차 (튜토리얼 레이드의 화면 · ESC · 캐릭터 생성, 에이전트 ③, docs/plans/tutorial-raid.md)** — 다섯 덩어리.
...
- **2026-09-14 (메신저 대사 선택지, 에이전트 F, docs/plans/tutorial-raid.md §2.5 · §3.F)** — NPC 첫 연락의 **내 대답 버튼 줄**.
...
- **2026-09-14 (피격 잔상 · 인벤토리 키 가이드, 에이전트 B, docs/plans/intel-broker.md §1)** — 사용자 결정 둘.
...
- **2026-09-14 (NPC 개인 신뢰도, 에이전트 C, docs/plans/intel-broker.md §2.7)** — 메신저가 NPC 개인 신뢰도를 **읽고 그린다**(적립은 meta/).
...
- **2026-09-14 (메신저 — 커뮤니티 패널 대체, 에이전트 C, docs/plans/messenger-quests.md §0 · §5)** —
...
- **2026-09-14 (메신저 · NPC 퀘스트 — 지도 퀘스트 패널 · 퀘스트 토스트, 에이전트 E, docs/plans/messenger-quests.md §5)** —
...
- **2026-09-13 (요리 미니게임 — 에이전트 cook-misc, `docs/plans/cooking-minigames.md` §6-5)** — 계약(`shared/cooking` 의 `cookStepsOf` · `COOK_GAME_LABEL_KO` ·
...
- **2026-09-13 (환경 재해 표시 — 「폭풍」 통일 · 지도 포자 합집합 · 맵 밖 클립)** — 사용자 결정.
  ...
  - **`map/MapScreen.ts`** `drawHazard` — 캔버스 클립에 **맵 사각형 클립**을 겹쳤다 (폭풍의 눈이 이제 맵 네 꼭짓점을 품는 원으로 시작해 원 대부분이 맵
    ...
    좌하단 라벨은 `폭풍 · 안전지대 n%`.
- **2026-09-13 (로그 강하 → 레이더 강하 문구 — 행성별 적 난이도, faction-presentation, docs/plans/enemy-factions.md)** — 강하는 이제 레이더만 내리고
...
- **2026-09-13 (요리 재료 티어, 에이전트 D — `docs/plans/food-tiers.md` §6)** — 계약(`MealDef.effects` · `MEAL_TIER_LABEL_KO` · `SAMPLE_FAMILY_*` ·
...
- **2026-09-12 (임플란트 · 함선 호출 준비 연출 — 사용자 결정, 에이전트 B, docs/plans/consumables-keys-favorites.md §2)** — 계약은 읽기만
...
- **2026-09-12 (캐릭터 버프, 에이전트 ui)** — 계약(`src/shared/charBuffs.ts` · `PlayerRef.buffs` · `RemotePlayerRef.buffs` ·
  ...
  설계안 `docs/plans/char-buffs.md` §2 · §6-D.
...
- **2026-09-12 (헬스장 A-3a · 서재 매체 A-3e, 에이전트 ui · audio · net · console)** — 계약(`src/shared`)은 리드가 먼저
  커밋했고(`a723a8a`) 이 폴더는 그것을 그린다. 설계안 `docs/plans/a3a-a3e.md` §6-7.
...
- **2026-09-11 (주방 · 배양조 · 프린터 A-3c · A-14 · A-15, 에이전트 ui)** — 계약(`src/shared`) · `data/*.csv` 는 먼저
  커밋됐고(`61ef305`) 이 폴더는 **고치지 않았다**. 설계안은 `docs/plans/a3c-a14-a15.md` §4.6.
...
- **2026-09-11 (연구실 A-11 · A-12 · A-13 + B-13, 에이전트 ui)** — 계약(`src/shared`) · `data/*.csv` 는 먼저
  커밋됐고(652aff6) 이 폴더는 **고치지 않았다**. 설계안은 `docs/plans/lab-a11-a13.md` §3.3.
...
- **2026-09-11 (B-11 (1) 말풍선 차단 · B-12 합류 알림 한 줄, 에이전트 ⑤)** — 설계안 `docs/plans/net-trust-gaps.md` §5 (1) · §6.
  계약(`src/shared`) 변경 없음.
...
- **2026-09-11 (B-3 초대 결과 · B-4 차단 / 전송 확인 / 대화 기록 — 클라이언트, 에이전트 ②)** — 설계안
  `docs/plans/net-social-trust.md` §2 · §4 의 ui 쪽. 규칙과 상태는 `net/SocialSync` 가 갖고 ui 는 그리기만 한다.
