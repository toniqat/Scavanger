...
| `menus/messenger/Trust.ts` | **(2026-09-14, NPC 개인 신뢰도 — docs/plans/intel-broker.md §2.7)** 기업 신뢰도와 **별개**인 NPC 개인 신뢰도의 읽기 · 그리기. `npcTrustOf(ctx, npcId)` = `ctx.meta.npcTrust` 하나에서 읽어 `{trust, level, next, frac}`(레벨 · 구간은 공용 `REP_TABLE` · `repLevelOf`) — `ctx.meta` 가 없으면 **null** 이라 호출부가 그 자리를 비운다(meta/ 를 import 하지 않는다). `buildNpcTrust(ctx, npcId, name, {compact, color})` = `Lv.n` + `.ms-bar` 게이지 (+ `1,650 / 3,000`, `compact` 면 숫자 없이 짧은 게이지) — 대화 목록 한 줄 · 대화창 머리 · 퀘스트 상세 머리가 같은 조각을 쓴다. `buildTrustChip(name, amount, color)` = 퀘스트 카드 보상 칩: 재화 칩(`buildCurrencyChip`)과 **같은 틀**(`.item-chip.currency-chip` + 글리프 `◈`)이되 `data-currency-id` 를 안 붙인다 — NPC 신뢰도는 `data/currencies.csv` 의 재화가 아니고 가짜 재화 · 가짜 아이템 정의를 만들지 않는다(호버 카드 대신 네이티브 `title`). CSS 는 `styles/messenger.css` 의 `.ms-trust*`. |
| `menus/messenger/format.ts` | DOM 없는 글자 도우미 — `clockText`(말풍선 시각) · `agoText`(목록 방금 · n분 · n시간 · n일) · `clip` · `initialOf` · `roomSystemText`(공용 `roomSystemTextKo` 에 아이디 대체 이름만 채워 넘긴다). |
