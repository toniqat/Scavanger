...
| `styles/env.css` | **행성 상시 환경 배지 스타일시트 + 좌상단 배지 행** (`hud/EnvBadge` 가 import, `hazard.css` 와 같은 규약). **2026-09-11 (A-3c):** `.hud-badges` — 좌상단(`left 32` · `top 56`) flex 행. 식사 배지가 옆에 서면서 `left/top` 이 `.env-badge` 에서 이 행으로 올라왔다. 행에는 **배경도 테두리도 여백도 없어** 두 배지가 모두 `display:none` 이면 높이가 0 이다 (빈 상자가 남지 않는다) — 배지를 하나 더 붙일 자리이기도 하다. `.env-badge(.show)` > `.g` 글리프 · `.nm` 이름 · `.st` 상태, 상태는 둘뿐이다 — `.is-safe`(차분한 dim) · `.is-exposed`(`--ec` 환경색 + 글로우 + `envPulse` 맥박). 준비물은 **100 % 상쇄**라 중간 상태가 없다. `.hud.spectating` 은 숨기고 `.hud.hub` 은 아예 내린다. 색 · 트랜지션은 `base.css` 변수, 환경 색만 인라인 `--ec`. |
| `styles/meal.css` | **식사 배지 스타일시트** (2026-09-11 A-3c, `hud/MealBadge` 가 import — `env.css` 와 같은 규약). `.meal-badge(.show)` 는 `.hud-badges` 행의 **두 번째 칸**이고 상자 언어(여백 · 글자 크기 · 테두리)가 `.env-badge` 와 **같은 값**이라 한 줄에 섰을 때 높이가 맞는다 > `.g` 요리 글리프 · `.nm` 요리 이름 · `.bf` 버프 + 부호 있는 값. 상태는 하나뿐이다 — 실렸거나(`.show`) 없거나; **특선 요리**(`MealDef.tier === 2`)만 `.is-special` 로 테두리에 요리 색이 돌고 은은한 글로우가 붙는다(환경 배지의 `.is-exposed` 와 달리 **경고가 아니므로 맥박이 없다**). 요리 색은 인라인 `--mc`(= `ItemDef.color`, 없으면 `CATEGORY_COLOR.meal`), 나머지는 `base.css` 변수. 끝에 `.hud.spectating` 숨김 · `.hud.hub` 내림 두 줄. |
