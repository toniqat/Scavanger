...
| `styles/env.css` | **행성 상시 환경 배지 스타일시트** (`hud/EnvBadge` 가 import, `hazard.css` 와 같은 규약). `.env-badge(.show)` 좌상단 배지 > `.g` 글리프 · `.nm` 이름 · `.st` 상태, 상태는 둘뿐이다 — `.is-safe`(차분한 dim) · `.is-exposed`(`--ec` 환경색 + 글로우 + `envPulse` 맥박). 준비물은 **100 % 상쇄**라 중간 상태가 없다. `.hud.spectating` 은 숨기고 `.hud.hub` 은 아예 내린다. 색 · 트랜지션은 `base.css` 변수, 환경 색만 인라인 `--ec`. |
