...
| `serverNow()` | **Phase 8.** Relay wall clock in epoch ms: `performance.now() + NetClient.serverTimeOffset` (offset from `welcome.serverTime` / every `pong`). `Date.now()` while disconnected or before the first welcome. `performance.now()` is monotonic, so moving the system clock cannot advance a real-time timer — housing/ stamps `GrowPlot.plantedAt` / `readyAt` with it so 온실 crops agree across devices. |
