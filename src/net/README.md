...
- **전환 1회 폴백 (C-69, 2026-09-11, 사용자 결정)**: 릴레이는 리비전 이전에 쓰인 프로필을 로드하면서 `docsRev[k] = 1` 로
  **시드한다**(`server/Store.ts`). rev 를 한 번도 본 적 없는 클라이언트의 대기 편집은 `baseRev = 0` 이라, 위 ② 규칙을 그대로
  쓰면 새 릴레이에 처음 붙는 순간 **오프라인 편집이 전부 조용히 사라진다**(`net:profileConflict` 를 듣는 UI 는 없다).
  그래서 `applyRecord` 의 ② 에서, **보내지 않은** 쓰기의 **모든** 키가 ① `revs[k]` 를 모름(루프 뒤 ③ 에서야 다시 채우므로
  루프 안의 값은 "직전까지 알던 것") ② 그 항목의 `baseRev === 0` ③ 스탬프가 있다(`at !== null`, `fresh` 는 제외)를
  만족하면 **Phase 9 스탬프로 판정한다** — 모든 키에서 `at >= docsAt[k]` 면 로컬 승리(각 키를 서버 rev 로 rebase 해 큐에
  남긴다), 하나라도 서버가 더 새로우면 서버 승리이되 **조용히** 버린다(경고 · `net:profileConflict` 없음 — Phase 9 에서도
  진 쓰기였다). `setMany` 는 전부 또는 전무: 키가 섞여 있으면 기존 경로(서버 우선 + 경고)로 간다. **① (비행 중) 은 그대로다** —
  리비전을 말하는 릴레이에 이미 보낸 쓰기라 `rev = base + 1` + 같은 문서 가지가 "ack 만 잃은 경우" 를 이미 잡고, 나머지는
  진짜 세션 간 충돌이다(옛 릴레이 모드에서는 `flushLegacy` 가 보낸 즉시 큐에서 빼므로 비행 중 쓰기가 애초에 없다).
- 검증: `smoke-search` 2b 절(스텁 릴레이 14 단언 — C-69 전환 2건 포함) + 합친 디바운스 2 단언, `net:selftest` part 11, 수동 통합(새 릴레이 8880 +
...
```
내 목록 변경 ── player:buffsChanged ─▶ dirty ─(NetSystem.update, 스냅샷 앞)─▶ cbuf state {rev, buffs ≤ WIRE_MAX} → others
받는 쪽   cbuf state ── 로비 멤버만 · rev < 가진 rev 면 버림(단 그 사람의 최신 bfr 와 같으면 받음) · sanitize
                     → entries[id] → ref.buffs / buffsRevision → (목록이 실제로 바뀌었을 때만) net:remoteBuffsChanged
          ps (accepted) ── bfr ≠ ref.buffsRevision 또는 buffsResync → cbufq sync → 그 사람 (피어별 쿨다운 1 회)
보내는 쪽 cbufq sync ── 요청자에게만 지금 목록 (요청자별 같은 쿨다운)
```

- **보내기는 프레임당 한 번이고 스냅샷보다 먼저다.** `player:buffsChanged` 는 `dirty` 만 켜고 `NetSystem.update` 가 스냅샷을 만들기 직전에
  `flush()` 한다. net 은 `main.ts` 에서 player 보다 먼저 돌므로 N 프레임에 바뀐 목록은 N+1 프레임 net update 에서 나가고, 새 `bfr` 를 실은
  스냅샷은 같은 update 에서 **그 뒤에** 나간다 — 받는 쪽이 리비전을 목록보다 먼저 보는 일이 없어 쓸데없는 요청이 안 나간다. 연결이 없거나
  로비가 없으면 그 변경은 그냥 버린다: 다음에 붙었을 때 스냅샷 `bfr` 가 모두에게 요청을 부른다. 권위 검사는 없다(버프에는 효과가 없다).
- **목록은 ref 가 아니라 멤버별 `entries` 에 산다.** 함선 ↔ 레이드 전환마다 ref 가 지워지고 첫 스냅샷에 다시 생기는데
  (`clearRemotes`), 새 ref 는 `getOrCreateRemote` 에서 `mirror` 로 곧바로 물려받는다 — 분대 목록의 썸네일이 깜빡이지 않고 요청도 안 나간다.
  ref 가 없는 멤버(다른 장면)의 `cbuf state` 도 저장된다. 떠난 멤버는 `syncRemoteIdentities` 스윕에서(`prune`), 로비를 떠나면 `dropLobby`
  에서(`clear`) 크루 카드 · 함선 배치와 같은 자리에서 지운다.
- **낮은 rev 가드와 그 예외.** 한 보낸 사람의 메시지는 한 소켓 · 한 릴레이를 지나 순서가 지켜지므로 낮은 rev 는 보통 오지 않는다. 오는 경우는
  **보낸 사람이 새로고침해 리비전이 1 부터 다시 시작한 것**뿐이라, 그 rev 가 그 사람의 최신 스냅샷 `bfr` 와 같으면 받는다.
- **`buffsResync` — 같은 숫자는 증거가 아니다.** `RemotePlayer.resetStream()`(스트림 재시작 감지 · 재접속 복귀 · 고스트 해제)이 켠다.
  새로고침한 보낸 사람의 새 rev 가 우연히 우리가 가진 옛 rev 와 같으면 `bfr` 비교만으로는 낡은 목록이 영영 남기 때문에, 재시작 뒤에는
  한 번 더 묻고 목록을 받으면 끈다(보낸 사람이 버프를 가진 적이 없고 우리도 빈 목록이면 묻지 않고 끈다).
- **내가 재접속하거나 새 로비에 들어갈 때 따로 할 일이 없는 이유.** ① 내가 받는 쪽: 비연속 복귀 · 새 로비는 ref 를 전부 새로 만들고
  (`entries` 도 새 로비면 비어 있다) rev 0 인 새 ref 는 첫 스냅샷의 `bfr` 와 달라 요청한다. 연속 복귀는 `resetStream` 이 `buffsResync` 를
  켠다 — 끊긴 사이에 놓친 `cbuf state` 가 그렇게 돌아온다. ② 내가 보내는 쪽: 끊긴 동안 바뀐 목록은 못 보냈지만, 다시 붙은 뒤 내 스냅샷 `bfr`
  가 남들의 사본과 달라 남들이 묻는다. 둘 다 이미 흐르는 20 Hz 스냅샷이 해결하므로 welcome · `lobby:state` 에 새 규칙이 없다.
- **`furniturePose` 보간.** kind · uid · anchorY · yaw 는 최신 스냅샷에서(이산값, `stance` 처럼 — 그래서 자세 시작 · 끝은 위치보다
  `NET_INTERP_DELAY` 만큼 이르다). **누적 위상**은 위치와 같은 시각(`now − NET_INTERP_DELAY`)에 두 샘플 사이를 선형 보간한다 — 감기지 않는
  값이라 최단 호가 필요 없다. 최신 샘플을 지나면 `stride` 처럼 최신 값을 쥔다(외삽 없음). 두 샘플 사이에서 kind 나 uid 가 바뀌면 끊는다:
  **지금 자세의 첫 샘플 값을 렌더 시각이 거기 닿을 때까지 쥔다** — 막 시작한 자세가 뒤로 돌지 않는다. 자세가 없으면 링을 걷지 않는다.
  `furniturePose` 는 **매 `tick` 제자리에서 고치는 한 객체**다(할당 없음) — 보관하려면 복사한다.
- **메시지 크기 · 빈도.** `cbuf state` ≈ 60–150 B × 항목(최대 `CHAR_BUFF_WIRE_MAX` 12, 지금 규칙으로는 7 이하) → 보통 < 1 KB, 목록이 바뀔 때만
  (자세 시작 · 끝, 식사 · 준비물 · 디버프 변화, 출격 · 복귀). 스냅샷은 `bfr` 숫자 하나(+~8 B)와 자세 중에만 `fp` · `fu`(+~50 B). `cbufq sync`
  는 ~30 B 이고 불일치가 계속되는 동안 피어당 초당 최대 1 회 — 이 기능을 모르는 옛 클라이언트와 섞이면 그 사람에게 초당 1 회씩 계속 묻는다
  (답이 없으니). 서버는 한 줄도 바뀌지 않았다 — 릴레이는 `relay.d.t` 가 문자열인지만 본다(`server/RelayServer.ts`).
- 검사: `scripts/e2e-multiplayer.mjs` 의 두 묶음(아래 변경 이력).

---

## 단체 메신저방 · 개인 대화 읽지 않음 (2026-09-14) — `RoomSync.ts` · `SocialSync`

- **`ctx.net.rooms` = `RoomSync`** (새 파일, `SocialSync` 와 같은 모양): 릴레이가 방의 진실을 들고 이쪽은 거울 + 버스 이벤트다. NetSystem 이 `bus` · `send` ·
  `serverNow` · `me`(= `socialSync.me`) · `isBlocked`(= `socialSync.isBlocked`)를 꽂고, `parts/Messages` 가 `room:state` · `line` · `ack` · `history` · `error` 를
  넘긴다 (`NetClient.SERVER_TYPES` 에 다섯 타입). **`room:get` 을 스스로 보내지 않는다** — 릴레이가 welcome 직후 밀어 주고, 방을 모르는 옛 릴레이는 그 프레임에
  `lobby:error invalid` 로 답해 오류 토스트를 띄우기 때문이다 (옛 릴레이에서는 `available` 이 끝까지 false).
- **요청** — `create(name, invite?)`(이름 정리 · 초대 목록에서 나 · 중복 · 잘못된 코드 제거) · `invite` · `respond`(초대를 곧바로 목록에서 뺀다 — 유일한 낙관적 변경) ·
  `leave` · `kick` · `rename` · `say`(곧바로 `pending` 줄 + `room:line`, ack 로 `sent` / `failed` + `room:lineUpdated`, 10 초 무응답 · 끊김은 `failed`) ·
  `requestHistory(room, before?)`(받은 쪽을 시각 순으로 합치고 겹친 줄은 버린다, `hasMore`). 모르는 방 · 사용 불가면 아무것도 보내지 않는다.
- **읽지 않음** — 방별 `readAt` 을 `slotKey(ROOM_READ_STORAGE_KEY)`(`{v:1, rooms}`)에 둔다. **처음 보는 방은 마지막 줄까지 읽은 것으로 시작**(들어오기 전 기록),
  내 줄 · 시스템 줄 · 차단한 사람의 줄은 세지 않고, 줄을 아직 받지 않은 방은 `lastAt > readAt` 이고 마지막 줄이 남의 대화면 1 로 본다. 내가 쓰면 그 방은 읽음.
  합이 바뀔 때만 `room:unreadChanged {total}`. 목록에서 사라진 방(강퇴 · 나가기)은 줄 캐시와 읽음 표시를 버린다.
- **ui 가 알 것** — 차단한 사람의 줄도 캐시와 `room:line` 에 **남는다** (서버도 저장한다) — 그리는 쪽이 `ctx.net.social.isBlocked(line.code)` 로 숨긴다.
  시스템 줄 문장은 `roomSystemTextKo(line)` (`text` 는 rename 의 새 이름뿐). 수락 직후에는 `room:state` 가 `join` 줄보다 먼저 온다. 끊겨도 줄 캐시는 남는다.
- **개인 대화 읽지 않음 (`SocialSync`)** — 대화 기록의 상대마다 `readAt` 을 함께 저장한다. `whisperUnread(code)` = 그 상대의 받은 줄 중 `readAt` 보다 새것,
  `markWhisperRead(code)` 가 마지막 줄까지 올리고, **내가 보낸 줄도 그 상대를 읽음으로** 만든다. `readAt` 이 없는 옛 기록은 전부 읽음 (업데이트 직후 배지 폭탄 없음).
  합이 바뀔 때만 `social:unreadChanged {total}` (받은 줄 · backlog 한 묶음 · 내 줄 · 읽음 표시). 채팅창과 메신저가 **같은 기록**을 쓰므로 어느 쪽에서 한 대화든 한 줄로 이어진다.
- 검증 (2026-09-14): `scripts/smoke-rooms.mjs` **37/37** (릴레이 없이 새 인스턴스에 프레임을 손으로 먹인다, 개인 vite 5311). 서버 쪽은 `server/selftest.ts`
  part 14 (46, 전체 568/568). `npm run typecheck` · `typecheck:server` 깨끗.
...
- **2026-09-13 (암호화폐 시세 창구, 에이전트 ⑤ — docs/DECISIONS.md 「2026-09-13 — 가구 접근 면 · 발전기 · 암호화폐 채굴」)** — 새 파일 `parts/Crypto.ts`(`CryptoMarketClient`, 위 파일 표).
  ...
  시세는 누군가 `watch()` 를 들고 있을 때만 새로 온다 — 견적 · 매매 화면은 열려 있는 동안 구독을 들고, `available` 과 `pricesAt` 을 본다.
...
- **2026-09-12 (캐릭터 버프 · 가구 자세 동기화, net 에이전트)** — 위 `캐릭터 버프 · 가구 자세` 절이 전부다. 새 파일 `parts/CharBuffs.ts`
  ...
  `snapshotBuffsRev` · `buffsResync`, 배선 다섯 줄(`NetSystem` 필드 · init · update 의 `flush` · dispose, `Messages` 의 `ps` → `onSnapshot`,
  `Remotes` 의 `mirror` · `prune`, `Lobby.dropLobby` 의 `clear`). 계약(`shared/charBuffs.ts` · `shared/net.ts` 끝 · 이벤트 둘 · 상수 둘)은 리드가
  썼고 읽기만 했다. 서버 무변경. `scripts/e2e-multiplayer.mjs` 에 공유 함선 묶음(A 의 운동 디버프 → B 의 `ref.buffs` 같은 리비전 · 타이머 필드 ·
  `net:remoteBuffsChanged`, 위조 목록 정화 → `bfr` 불일치 → `cbufq sync` 로 A 의 낮은 리비전 복귀, 맨 anchor 의 `run` 자세 → B 의
  `furniturePose` kind · uid · yaw · anchorY 와 되돌지 않고 커지는 보간 위상, 일어나면 null), 레이드 첫 스냅샷에서 새 ref 가 목록을 물려받는지,
  끝의 **늦은 합류**(A 가 앉은 채 B 새로고침 → 같은 슬롯 복귀 → `cbufq sync` 로 A 의 목록 · 앉은 자세). player 가 `ctx.player.buffs` /
  `furniturePoseState` 를 아직 안 냈으면 그 게터만 스텁으로 세우고 `note:` 를 찍는다 — 와이어 검사는 그대로 돈다.

- **2026-09-12 (서재 매체 A-3e)** — `model.sanitizeShipVisit` 이 계약의 새 필드 둘을 받는다. 이 함수는 새 객체를 만들어 필드를
  하나씩 옮기므로 **받아 주지 않으면 보낸 쪽이 실어도 방문자에게는 사라진다.** `media`(디스크 전시대 · 레코드랙에 꽂힌 것,
  `PlacedBook` 모양)는 `books` 와 같은 정화 — 그래서 둘이 새 도우미 `sanitizeShelved`(깨진 항목만 버림 · 칸 0 … 99 ·
  `SHIP_VISIT_MAX_BOOKS` 상한)를 함께 쓴다. `toggled`(켜 둔 TV · 레코드 플레이어 uid)는 **같은 문서의 가구 목록에 있는 uid 만**,
  중복 없이, `SHIP_VISIT_MAX_FURNITURE` 까지. 둘 다 비면 생략한다. 매체가 그 보관함과 맞는지는 그리는 쪽(hub 의
  `FurnitureSource`)이 def 로 가린다. ⚠ **보내는 쪽은 이 폴더가 아니다** — 와이어를 만드는 곳은 `hub/parts/Hangar.shipStateWire`
  이고 거기서 `ctx.housing.state.media` · `toggled` 를 실어야 한다.
...
- **2026-09-11 (C-69 리비전 전환 1회 폴백)** — 위 `문서 리비전 (E-6)` 절의 `전환 1회 폴백` 항목. E-6 이전에 쓰인 프로필을
  서버가 `docsRev = 1` 로 시드하므로, rev 를 처음 보는 클라이언트의 오프라인 편집이 새 릴레이에 붙는 순간 전부 버려지고
  있었다(`net:profileConflict` 구독자가 없어 사용자에게는 **조용한 유실**이다). `ProfileSync.applyRecord` 의 ② 루프에
  `virgin` 판정(모든 키가 `revs` 미상 · `baseRev 0` · 스탬프 있음)을 넣어 그런 쓰기만 Phase 9 스탬프(`at >= docsAt[k]`)로
  가리고, 지면 경고 · 이벤트 없이 버린다. `setMany` 는 전부 또는 전무. ① (비행 중) · 옛 릴레이 경로 · 나머지 E-6 규칙은
  한 줄도 바뀌지 않았다. 검사: `scripts/smoke-search.mjs` 2b 절 ⑦ (`seedStampWins` · `seedStampLoses`).
