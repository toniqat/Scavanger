...
| `parts/Fall.ts` | **떨어져서 착지했을 때 얼마나 아픈가** (2026-09-14, 전역 낙하 피해). 높이를 재는 곳은 `PlayerController`(`MoveResult.fallHeight`)이고, 그 높이를 피해로 바꾸는 곳이 여기 하나다 — `fallDamageFor(height)`(순수 식, `FALL_DAMAGE_*`) · `onLanded`(받을 수 있는 상태인가 · 튜토리얼 `fallRule` · `applyDamage` · `player:fell`). 아래 *낙하 피해* 절. |
...
점프대 · 로켓 점프처럼 **스스로 띄운 것은 면제가 아니다** — 안전 높이 5 m 가 평범한 점프(1.3 m)와 함선 데크 ·
2층 바닥 · 낮은 바위에서 뛰어내리는 동선을 전부 덮으므로, 그보다 높이 올라갔다면 그것이 곧 낙하다.
