...
| `parts/IntroWake.ts` | **깨어나는 동안 몸과 카메라는 무엇을 하는가** (2026-09-14, 튜토리얼 오프닝). `PlayerRef.playIntroWake` 의 구현 — 쓰러진 자세(`SoldierPose.downed` 를 1 → 0 으로 되감는다) · 입력 잠금 · 몸을 비추는 고정 카메라 · 끝나면 백뷰로 **하드 컷** + `player:introWakeDone`. 아래 *오프닝 기상 연출* 절. |
