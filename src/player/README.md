...
| `parts/Fall.ts` | **떨어져서 착지했을 때 얼마나 아픈가** (2026-09-14, 전역 낙하 피해). 높이를 재는 곳은 `PlayerController`(`MoveResult.fallHeight`)이고, 그 높이를 피해로 바꾸는 곳이 여기 하나다 — `fallDamageFor(height)`(순수 식, `FALL_DAMAGE_*`) · `onLanded`(받을 수 있는 상태인가 · 튜토리얼 `fallRule` · `applyDamage` · `player:fell`). **2026-09-15 (B-14)**: 같은 자리에서 `camera:shake`(`fallShakeFor`) + 멀티면 `fall` 와이어, 받는 쪽 검사 `receiveRemoteFall`(모양 · 보낸 사람 · 거리 · 요율 → `player:remoteFell`). 아래 *낙하 피해* 절. |
| `parts/IntroWake.ts` | **깨어나는 동안 몸과 카메라는 무엇을 하는가** (2026-09-14, 튜토리얼 오프닝). `PlayerRef.playIntroWake` 의 구현 — 쓰러진 자세(`SoldierPose.downed` 를 1 → 0 으로 되감는다) · 입력 잠금 · 몸을 비추는 고정 카메라 · 일어서는 동안 백뷰 자리로 블렌드 → 끝나면 오버라이드 해제 + `player:introWakeDone`(2026-09-14 — 그 전에는 음수 타이머 버그로 끝나지 않았다). 아래 *오프닝 기상 연출* 절. |
...
| `SoldierRim.ts` | **병사 전용 림(fresnel)** (2026-09-15, TODO D-7). `applySoldierRim(material)` 이 `onBeforeCompile`(모듈 함수 하나) + 상수 `customProgramCacheKey`(`SOLDIER_RIM_PROGRAM_KEY`) + 모듈 공유 유니폼 `SOLDIER_RIM_UNIFORMS` 를 단다 — `lights_fragment_end` 뒤에서 `totalEmissiveRadiance += 림 색 × pow(1 − N·V, SOLDIER_RIM_POWER) × SOLDIER_RIM_STRENGTH × min(1, 간접광 휘도)`. `SoldierModel.mat()`(바이저 제외) · `GearLook` 의 재질이 전부 이것을 지난다. 광원 0 · 프로그램 하나. 아래 *병사 림* 절. |
...
- **회복**: 스스로 재생하지 않는다. ① **함선(허브)에서는 늘 가득** — `syncShield()` 가 `ctx.isHubPhase()` 동안
  ...
  ② 레이드 중에는 **실드 충전기 소모품**(`chargeShield(amount)`, `Infinity` = 가득, 완료음 `shield_charge` — 2026-09-11 C-21 전에는 `stim` 을 pitch 1.25 로 빌려 썼다)뿐이다. 방탄복이 없거나
...
- **`isOvercharged` (2026-09-11, C-3)**: an explicit timer — `setOvercharged(duration)` stores `ctx.time + duration` in `_overchargedUntil` (0 clears) and the getter compares with `ctx.time`. implants' `applyBoost` calls it next to `setSpeedModifier('overcharge', …)`. `Locomotion.speedModifierProduct` no longer infers the flag from a modifier key starting with `overcharge`; `resetTactical` zeroes the timer.
...
- **Air momentum after an impulse (2026-09-14, `PlayerController.airCarry`)**: `applyImpulse` that lifts the body (or lands
  while already airborne) arms `airCarry` until the next landing (`reset` · grapple pull clear it too). While armed and the
  horizontal speed is above the stance / sprint target speed, air control **steers without shrinking** that speed — only
  input pointing against the velocity brakes it (that component × `AIR_ACCEL`). Plain jumps (velocity set directly, no
  impulse) and anything at or under the target speed behave as before. Needed by the bazooka rocket jump's horizontal boost
  (`BAZOOKA_JUMP_FORWARD`), which `AIR_ACCEL` otherwise pulled back to 7.2 m/s within a second. Side effect measured on the
  same spot: jump pad while sprinting 13.4 → 17.4 m (air time 1.08 s unchanged), standing 15 m/s knockback 1.94 → 2.0 m.
...
상태는 `PlayerController` 안에만 있다 (`carrier` · `rideLocal` · `rideWorld` · `rideInertia`). `Obstacle.velocity` ·
`Obstacle.box`(2026-09-09 계약)를 읽고, 수치는 `data/constants.csv` 의
`RIDE_HEADROOM` · `RIDE_FOOT_DROP` · `RIDE_EDGE_MARGIN` · `RIDE_INERTIA_S` · `RIDE_INERTIA_DAMP` 다.
**좌표 변환 세 식은 2026-09-11 (C-18) 부터 `@/shared` 의 `ride.ts`** 다 (`recordRideLocal` · `restoreRideLocal` ·
`rideContains`) — 적 · 시체(`game/Corpses`)가 같은 식으로 전차에 타게 되면서 여기 있던 private 함수를 그대로 옮겼다.
식은 한 줄도 바뀌지 않았고 `rideContains` 의 기본 인자가 위 `RIDE_*` 셋이다.
...
| 무엇 | 어디 |
|---|---|
| 떨어진 **높이**를 잰다 | `PlayerController` — `fallFromY`(접지 중에는 발 높이를 따라가고 공중에서는 최고점만 올라간다) · `fallExempt` · `MoveResult.fallHeight`(착지한 프레임에만 > 0) |
...
| 알린다 | 실제로 깎였을 때만 `player:fell {height, damage, rule}` — 튜토리얼 `drop` 단계 · 착지음(audio `fall_impact`) · 붉은 비네트(ui)가 받는다 (2026-09-15, B-14) |
| 흔들린다 | 같은 자리에서 `camera:shake {min(FALL_SHAKE_MAX, 피해 × FALL_SHAKE_PER_DAMAGE), FALL_SHAKE_S}` (`fallShakeFor`). 받는 곳은 `PlayerSystem.init` 의 `rig.addShake` 하나 — 드론 시점 · 탐사 차량 궤도 카메라에서는 늘 그렇듯 무시된다 |
| 분대원이 듣는다 | `ctx.isMultiplayer && ctx.net` 이면 `{t:'fall', p: 발, d: 실제 피해}` 를 `others` 로 (`FallMessage`, 소리 전용 — 체력은 스냅샷이 싣는다). 받는 쪽 `RemotePlayerSystem.receiveFall` → `receiveRemoteFall` 이 **모양**(유한한 `p` 셋 · 유한한 `d` → `[0, FALL_DAMAGE_MAX]`, 0 이면 버림) · **보낸 사람**(나 아님 · `net.getLobbyPlayer` 멤버 · 이쪽이 게임플레이 페이즈) · **거리**(로컬 카메라에서 `FALL_REMOTE_SOUND_RANGE`) · **요율**(같은 사람 `REMOTE_FALL_MIN_INTERVAL_S` = √(2 · `FALL_DAMAGE_SAFE_M` / `GRAVITY`) ≈ 0.65 s — 피해를 주는 낙하가 그보다 자주 올 수 없다)을 지나면 `player:remoteFell {peerId, position, damage}` 을 낸다. 거절 사유를 돌려준다(스모크용) |
...
**계산하지 않는 경우** — 컨트롤러가 `fallExempt` 로 거르는 것(낙하 중 한 번이라도 서면 착지까지 유지된다):
갈고리 견인 · 가방 부양(자동 catch 포함) · 차량 발판(전차 데크) · 함선 실내 · **사다리에서 놓은 몸** ·
**대시**(`implant:dashed` → `PlayerController.exemptFall()` — 몸을 그 높이 그대로 옮기는 순간이동이라 그 뒤의
낙하까지 대시 전 높이에서 세면 안 된다) · `controller.reset`(스폰 · 부활 · 순간이동).
그리고 `parts/Fall.canTakeFall` 이 한 번 더 거르는 것: 사망 · 전투불능 · **탐사 차량 탑승**(탑승자는 어떤 피해도
받지 않는다) · 드론 조종 · 포드 · 헬포드 강하 · 사다리 · 함선 실내 / 화물칸 · 업힘 · 기상 연출 중 · 게임플레이 페이즈가 아닐 때(함선 · 결과 화면).

점프대 · 로켓 점프 · 넉백처럼 **임펄스로 띄운 몸은 그 착지까지 면제다** (2026-09-14 1차 회귀 수정 — 「남이 띄운 몸은
착지로 죽지 않는다」, `PlayerController.applyImpulse` · `applyKnockback`). 착지 한 번으로 면제가 끝나므로 그 뒤의 평범한 낙하는
다시 센다. 2026-09-15 `smoke-fall-damage` 가 세 경로(임펄스 · 넉백 · 낙하 중 작은 임펄스)를 모두 잰다. ⚠ 이 절의 옛 문장
「스스로 띄운 것은 면제가 아니다」 는 그 회귀 수정 이후 사실이 아니었다.

## 병사 림 (2026-09-15 — TODO D-7, 사용자 결정 「병사 전용 림(fresnel)」)

짙은 청회색 장갑(0x3a4150)이 베르단트 III 같은 이끼 · 독성 녹색 팔레트에서 배경과 같은 밝기로 떨어져 실루엣이 사라졌다.
광원을 더하는 대신(「씬의 광원 개수를 플레이 중에 바꾸지 않는다」) `SoldierRim.ts` 가 조각 셰이더에 가장자리 발광을 얹는다.

- 식: `totalEmissiveRadiance += mix(림 색 0xdce8ff, 간접광 색조, 0.15) × pow(1 − N·V, SOLDIER_RIM_POWER) × SOLDIER_RIM_STRENGTH × min(1, 간접광 휘도)`.
  N·V 는 `geometryNormal` · `geometryViewDir`(뷰 공간). 간접광 = 반구광 · 환경광 조도(`irradiance`)라 **어두운 곳에서는 림도 어둡다** —
  혼자 빛나지 않는다. 색조를 조금만 섞는 이유: 녹색 조도를 그대로 쓰면 녹색 배경과 다시 같은 색이 된다.
- `SOLDIER_RIM_STRENGTH` 0 = 기여 정확히 0. 수치는 `data/constants.csv`. 런타임에 바꾸려면 `SOLDIER_RIM_UNIFORMS.uSoldierRim.value.x`.
- **프로그램 하나**: `onBeforeCompile` 은 모듈 함수 하나, `customProgramCacheKey` 는 상수(`player-soldier-rim`), 유니폼 객체도 모듈에 하나라
  로컬 병사 · 원격 아바타(`SoldierPool`) · `game/Corpses` · 초상화 · `ui/menus/SoldierPreview` · 방탄복 판 · 손에 든 아이템이 같은 프로그램을 쓴다.
  three.js 가 원래 가르는 변형(단면 / 양면 망토)만 남고, 악센트 · 페이드 · 회색 · 과충전 발광은 셰이더를 정의하지 않는 값이라 변형을 늘리지 않는다.
- 재질은 지금처럼 생성자에서 만들어지므로 `ctx.shaders` 선컴파일이 함께 덮는다. 바이저는 발광 모습을 지키려고 림을 달지 않는다(`mat(..., rim = false)`).
...
- **자세**: 새 자세를 만들지 않았다 — 전투불능 자세(`SoldierPose.downed` → `SoldierModel.poseDowned`, 「등을 대고
  ...
  `PlayerSystem` 이 `p.downed = max(downedBlend, IntroWake.wakeBlend(this))` 로 전투불능 블렌드와 큰 쪽을 쓴다.
- **입력**: `moveFrozen` 에 들어가고(이동 · 점프 · 자세 · 구르기 없음) 조준 · 무기(`canUseWeapons`) · 상호작용 ·
  들쳐메기 · 어깨 전환 · **마우스 룩**이 잠긴다 — 드론 조종 게이트를 보는 곳들과 같은 요령이다. 룩까지 잠그는
  이유는 마지막 하드 컷이 연출 시작 시점의 yaw 로 돌아가야 하기 때문이다.
- **카메라**: 몸 옆앞 낮은 곳에서 발을 비추다 일어나는 만큼 눈높이로 함께 올라온다(첫 프레임은 `snap`).
  끝나면 `setCameraOverride(null, undefined, true)` 로 오버라이드를 푼다 — 2026-09-14 부터 진행도 0.55 → 1 동안 이미 리그의 백뷰 자리로 옮겨 와 있으므로(`CAM_RETURN_START`) 해제가 보이지 않는다. 먼 곳에서 **리그 쪽 감쇠**로 블렌드하면 카메라가 지형을 훑으므로 해제 자체는 여전히 스냅이다.
...
- **2026-09-15 (낙하 착지 피드백 B-14 · 병사 림 D-7, 사용자 결정)** — 두 가지.
  ...
  `RemotePlayerSystem` 이 `net.onMessage('fall')` → `receiveFall(msg, from)`(요율 표 `fallHeardAt`, 제거 이벤트에 지운다) → `player:remoteFell`.
  착지음(audio) · 비네트(ui)는 그 폴더들이 같은 두 사건으로 낸다. *낙하 피해* 절의 「낙하 전용 소리 · 흔들림은 없다」를 고쳤다.
  ② 새 `SoldierRim.ts` — `SoldierModel.mat()`(새 인자 `rim`, 바이저만 false) · `GearLook` 의 재질에 공유 fresnel 림. 광원 0 · 프로그램 하나(*병사 림* 절).
  스모크: `smoke-ghost` 끝에 낙하 절(흔들림 세기 · 솔로 무전송 · 멀티 와이어 모양 · 원격 검사 여섯 거절 + 요율 + 상한 클램프).

- **2026-09-14 (튜토리얼 오프닝 수정 — 끝나지 않던 기상 연출 · 카메라 복귀, 사용자 결정)** — `parts/IntroWake` · `PlayerSystem`(주석만) ·
  계약 `PlayerRef.introWaking?`(추가만 — getter 는 원래 있었다).
  ① **연출이 끝나지 않았다.** `introWakeT -= dt` 가 0 을 지나 음수가 된 프레임에 `endIntroWake` 를 불렀는데, 그 함수 첫 줄이
  「연출 중이 아니다」 표식인 `introWakeT < 0` 에서 되돌아갔다 — 오버라이드가 영영 안 풀려 **카메라가 옆자리에 남고**,
  `player:introWakeDone` 이 안 나 튜토리얼이 `wake` 에 갇혔다(`cliff` 체크포인트가 접을 때까지 안내가 없다). 타이머를 `Math.max(0, …)` 로 0 에서 멈춘다.
  ② **카메라 복귀는 하드 컷이 아니라 블렌드다** (사용자 결정 — 「일어나면서 원래 시점으로 돌아와야」): 진행도 `CAM_RETURN_START`(0.55) → 1
  동안 옆 카메라가 리그의 백뷰 자리(`CameraRig.position` + `getLookDir` — 리그는 오버라이드 밑에서도 자기 자리를 계속 계산한다)로
  smoothstep 하고, 1 에서 오버라이드 = 리그라 해제가 안 보인다(스모크 실측: 해제 프레임 이동 < 0.35 m, 해제 뒤 카메라 = 리그).
  ③ `introWaking` 이 계약이 됐다 — `ui/hud/Compass`(연출 중 0 → 끝나면 페이드인)와 inventory Tab(연출 중 무시)이 본다.
  검은 페이드가 한 프레임에 끝나던 것은 이 폴더가 아니라 ui/ 의 CSS 전이(reduced motion) 문제였다. 회귀 `scripts/smoke-intro-wake.mjs`.

- **2026-09-14 (바주카 로켓 점프 — 공중 운동량 보존, 사용자 결정)** — `PlayerController` 한 파일. 새 필드 `airCarry`:
  `applyImpulse` 가 몸을 띄우거나 공중에서 받으면 켜지고, 착지 · `reset` · 갈고리 당김이 끈다. `update` 의 수평 속도 절에서
  켜져 있고 목표 속도보다 빠르면 공중 조작이 **방향만 틀고 크기는 줄이지 않는다**(반대로 누른 성분만 `AIR_ACCEL` 로 감속).
  바주카의 새 수평 가속(`BAZOOKA_JUMP_FORWARD`)이 1 초 안에 달리기 속도로 깎이던 것 — 중력 · 부양은 손대지 않았다
  (사용자 결정: 「더 센 임펄스만」). 낙하 면제(`exemptFall`)는 그대로. 부작용 실측: 달리며 점프대 13.4 → 17.4 m ·
  넉백 1.94 → 2.0 m · 평범한 점프 무변경 (위 Events 절의 *Air momentum after an impulse*).
...
- **2026-09-14 (낮은 통로를 앉아서 지난다 — 리드 마무리)** — 튜토리얼의 「앉아서만 지나갈 수 있는 곳」을 진짜로 만든
  ...
  `PLAYER_PRONE_CLEARANCE_M`). **두 값은 같아야 한다** — 천장 클램프와 밀어내기가 다른 높이를 보면 슬래브 밑에서
  위로는 안 막히는데 옆으로는 밀려나는 모순이 생긴다.
  ② **`parts/Locomotion.canStandHere`** — 머리 위가 막힌 자리에서는 일어서지 않는다. 묻는 곳은 **사람이 누르는
  경로하나**(`updateStanceInput` — C · Z · 달리기 · 점프로 자동 기립)이고, **각본된 기립은 묻지 않는다**
...
- **2026-09-14 (낙하 피해 · 오프닝 기상 연출, 에이전트 B, `docs/DECISIONS.md` 「2026-09-14 — 튜토리얼 개편」)** — 새 `parts/Fall.ts` · `parts/IntroWake.ts`.
  ...
  `parts/Spawn`: `usesHellpod(ctx)` — 훈련장에 이어 **튜토리얼도 헬포드 없음**(`world:ready` · `respawn` 두 경로), 리셋 경로 4곳에서 `cancelIntroWake`.
  `parts/Vitals.die`: 연출 취소. **본편 레이드 동작 변화는 낙하 피해 그 자체뿐이다** — 그 밖의 분기는 전부 `missionMode === 'tutorial'` 안에 있다.

- **2026-09-14 (발사 슬롯 UI 대개편, 에이전트 A)** — `Portraits.ts` 의 카메라 프레이밍만 바꿨다: fov 28 → **30**, 거리 4.3 → **4.4**,
  눈높이 1.15 → **1.05**, 시점 y 1.0 → **0.98**. `hub` 의 발사 슬롯 패널이 화면 세로 70 % 가 되고 초상이 카드 위 55 % 만 쓰게 되면서
  셀 뷰포트가 가로형(~280×200)에서 **세로형**(~280×415)으로 바뀌었다 — `render` 가 `cellWidth / height` 로 aspect 를 잡으므로
  고정된 것은 **세로 FOV** 이고, 몸 높이에서 보이는 세로 폭이 2.14 → 2.36 m 가 되어 발(0 m)과 머리 위 여유가 함께 들어온다.
  카메라를 뒤로 빼는 대신 FOV 를 넓힌 것은 살짝 내려다보는 구도를 지키기 위해서다. **`HUB_READY_PORTRAIT_YAW` 는 `shared` 가
  `data/constants.csv` 에서 읽고 −π/4 → −3π/4 가 됐다** (카메라 쪽 · 화면 오른쪽 3/4 뷰) — 여기서는 값을 쓰기만 한다.
...
- **2026-09-13 (탈출 개편 — `isInShip`)** — `PlayerSystem.isInShip` 이 `shipBounds !== null` **또는** `ctx.extraction.isInShipBay(발)` 이다. extraction 이 착륙한
  ...
  한 줄을 더했다. 이륙 연출 카메라는 기존 `setCameraOverride(pos, look, true)` 를 매 프레임 쓴다 — 이 폴더의 코드는 그 밖에 바뀌지 않았다.
...
- **2026-09-11 (C-42 `Portraits.ts`, 에이전트 5 — 리드 기록)** — 슬롯 색이 바뀌어 `SoldierModel` 을 새로 지을 때 옛 모델은
  ...
  dispose 하면 같은 cacheKey 머티리얼을 쥔 모델이 없어져 셰이더 프로그램이 지워졌다 다시 컴파일됐다. 파일 머리 주석의
  "Nothing here is shared" 는 틀렸다 — 지오메트리는 `SoldierModel` 모듈 캐시를 공유한다(리스너 누적은 페이지당 1벌). 검증:
  `smoke-training` 이 색 변경 전후 보조 렌더러의 **프로그램 id 집합**이 같은지 본다(개수는 재컴파일해도 같다).

- **2026-09-11 (C 항목 배치 — 오버차지 플래그 · 탑승 수학 이전 · 실드 충전음)** — 계약은 읽기만 했다
  (`PlayerRef.setOvercharged?`, `@/shared` 의 `ride.ts`).
  - **C-3** `isOvercharged` 는 `setOvercharged(duration)` 가 세우는 타이머(`_overchargedUntil`)다. `Locomotion.speedModifierProduct`
    의 `key.startsWith('overcharge')` 추론과 `_overcharged` 필드를 지웠다 (림 발광 · weapons 연사 · `PlayerFlags.OVERCHARGED` 는 getter 그대로).
  - **C-18** `PlayerController` 의 private `recordRideLocal` · `restoreRideLocal` · `rideContains` 를 `@/shared` 의 같은 이름 함수로
    바꿨다 — 식 · 기본값 동일(동작 무변경). 적 · 플레이어 시체가 같은 식을 쓴다.
  - **C-21** `chargeShield` 완료음 `stim`(pitch 1.25) → 전용 `shield_charge` (정의는 audio/).
  - `RemoteAvatar` 의 소켓 주석 정정 — `implants/` 가 소켓 동일성으로 붙인다는 문장은 C-43 이 들어오기 전에는 틀렸다.
  - 검증: `smoke-tactical` (`applyBoost` → `isOvercharged` · 만료 · `setOvercharged(0)`), `smoke-raidflow` (전차 데크에 선 플레이어가
    달리는 전차의 로컬 좌표에서 0.3 m 안에 머문다 — 탑승 수학 이전의 회귀), `smoke-ladder` · `smoke-ghost` · `smoke-controls-hub` · `smoke-phase2` 무변경 통과.
