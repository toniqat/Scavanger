import {
  ...
  type ArmorDef, type ArmorPerk, type BackpackDef, type BackpackPerk, type GameContext, type WeightInfo, type WeightState,
} from '@/shared';
...
export class PlayerGear {
  ...
  armorBroken = false;
  backpack: BackpackDef | null = null;
  backpackUid: string | null = null;
  backpackBroken = false;
  ...
  get backpackPerk(): BackpackPerk { return this.backpackBroken ? 'none' : (this.backpack?.perk ?? 'none'); }
  ...
  refresh(ctx: GameContext): void {
    ...
    this.backpack = null; this.backpackUid = null; this.backpackBroken = false;
    ...
    if (inv && typeof inv.getEquipped === 'function') {
      ...
      const bpItem = inv.getEquipped('backpack');
      if (bpItem) {
        this.backpackUid = bpItem.uid;
        const def = inv.getDef?.(bpItem.defId);
        const bpId = def?.backpackId;
        if (bpId && loot && typeof loot.getBackpackDef === 'function') this.backpack = loot.getBackpackDef(bpId) ?? null;
        this.backpackBroken = this.isBroken(ctx, bpItem.uid);
      }
    }
