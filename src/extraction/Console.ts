 ...
 * which is exactly what the fog of war is there to take away: you now have to walk into a pad's reveal radius to
 ...
 * state so the console still reads from close up, which is what makes it discovered in the first place.
 */
export class ExtractionConsole {
  ...
  constructor(position: THREE.Vector3, yaw: number) {
    ...
    this.beacon.visible = false;      // idle: no light shaft (2026-09-09) — `setState('active')` turns it on
  ...
  setState(s: ConsoleState): void {
    ...
    if (s === 'active') {
      ...
      this.beacon.visible = true;     // extraction called = the landmark the whole squad runs toward
    } else if (s === 'idle') {
      ...
      this.beacon.visible = false;    // no light shaft while idle
    } else {
      this.leverTarget = 0.55;
