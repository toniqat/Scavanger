...
| `nav/` | The raid's walkable graph (`WorldRef.nav`, TODO A-18): outdoor grid + per-structure grids, special links (ladder · wall climb · window), A\*, re-measure, gates and flow fields — see [`nav/README.md`](nav/README.md). |
...
- 2026-09-21 — A-18 phase 2, the world side: the nav graph gets wall-climb and window links for small bugs, gates (chokepoints) and flow fields (`nav/parts/Links` · `Gates` · `Flow`). `Structures` feeds it (`navWindows` · `navBuildings`, `windowWhole` · `breakWindow`) and reports every pane that breaks — wire and late-join sync included — through `onGlassBroken`, so the graph's `revision` moves; `GlassSet.isWhole` · `refOf`; `Build.BREACH_W` exported.
