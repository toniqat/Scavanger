...
export class DockingCutscene {
  ...
  private place(progress: number, snap: boolean): void {
    ...
    _dir.normalize();
    this.small.group.position.copy(_pos);
    // nose (−Z) toward travel direction
    _look.copy(_pos).add(_dir);
    this.small.group.lookAt(_look.add(this.root.position));
    this.small.group.rotateY(Math.PI);
    this.small.group.rotation.z = -_dir.z * 0.25;
    const thrust = this.direction === 'dock' ? 1 - p * 0.85 : 0.3 + p * 0.7;
    this.small.setThrust(thrust);
