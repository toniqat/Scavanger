...
export function updateEnemyAI(e: Enemy, dt: number, host: EnemyHost): void {
  ...
  switch (e.state) {
    ...
    case 'attack': {
      ...
      if (!targetAlive && !e.airborne && e.chargePhase !== 2 && !e.spitAtPoint && !e.structAttack) {
...
function attack(e: Enemy, dt: number, host: EnemyHost, t: CombatTarget | null): AttackResult {
  ...
  if (e.attackTimer < windup) {
    ...
    if (!e.attackHitDone) {
      ...
      } else if (!struct && t && !t.isDead && e.distToTarget < reach) {
        host.hitTarget(e, s.attackDamage, e.type === 'warrior' || e.type === 'charger' ? 0.45 : 0.18, t);
