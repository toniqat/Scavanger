...
| `fx/RogueGrenade.ts` | **2026-09-13**: 종류별 몸체 재질, 소이 화염 지대 풀 6개(`ENEMY_INCENDIARY` — 4.5 m · 8 s · 16 dps, 파티클 + 가산 바닥 원판, 광원 0), 권위 지대만 `onFireZoneTick`, 리플리카는 `ee grenadeHit.k` 로 모습만, `explodeNear(p, kind?)`, 디버그 `debugFireZones` · `fireZoneCount`. |
