...
| `Targets.ts` | `CombatTarget { id: PeerId \| 'local'; position; velocity; isDead; present; yaw; eyeHeight; getEyePosition/getForward/getChest/dist2D }` — a stable object per player. `TargetList.refresh(ctx)` rebuilds `all` / `alive` each frame from `ctx.player` plus `ctx.net.getRemotePlayers()` filtered to `connected && !stale && !DROPPING`; departed peers get `present = false` so bugs re-target. Queries: `nearestAlive`, `nearestAliveWithin`, `minDist`, `distToLocal` (listener-relative audio/shake), `randomAlive`, `local`, `anyAlive`. |
