...
- **2026-09-15** **적 구성을 2026-09-13 행성별 팩션에 맞췄다 (TODO E-13).** 스크린샷은 그대로이고 글만 바뀌었다.
  - `11-planets` — 표에 **주적 팩션** 열(낮음 = 안드로이드 · 보통 = 로그 · 레이더 · 높음 = 레이더), 브리핑과 노트의
    「약탈자 거점이 촘촘」 · 「사람(약탈자)이 주적」 삭제, 위협 등급별 적 구성 `<details>`(거점 점거 · 분대장 · 레이더 강하 ·
    네임드 · 지하벌레 · 벌레 체력).
  - `12-structures` — 연구실 · 전진기지는 늘 점거 · 불시착 함선만 빈 곳, **로그 강하 → 레이더 강하**(낮음 행성 없음 ·
    두 파도 표 · 분대장 없음 · 우회조), 열쇠 출처 「로그 시체」 → 「로그 · 레이더 시체」.
  - `16-enemies` — 리드 문단 · 「로그 — 총싸움」 → 「인간형 — 총싸움」(팩션 3종 표 · 로그 분대장 = 그룹장 · 「상자 주변 보스 분대」 삭제),
    새 소절 `#s-named`(로든 · 타길라 · 헤비 — 레이더 팩션 · 25/50 %), 벌레 체력을 `data/enemies.csv` 값으로(스캐빈저 60 → 120 …),
    아틸러리 사거리 140 → 60 m, 지하벌레 행 추가, 위협별 체력 배수 한 줄.
  - `17-extraction` — **탈출 웨이브 삭제**(2026-09-13 탈출 개편): 60초 방어 → 호출 20초 · 착륙 60초 대기 · 10초 출발 유예 ·
    남겨진 사람 재호출, 「왜 120 → 60초」 노트 → 「함선 출발은 레이드 종료가 아니다」.
  - 한 줄씩: `13-rails`(플랫폼 60 % 점거 · 「조사하면 레이더 강하」) · `06-weapons`(충전기 출처 = 안드로이드 · 로그 · 레이더) ·
    `23-corp`(바스티온 계약 「용병 제거 (인간형 적)」) · `24-rep`(「인간형 적 처치」 — 게임 안 라벨) ·
    `28-status`(적 16종 · 팩션 4, 탈출 행) · `09-raid-flow` · `00-intro`(120초 · 60초 버티기 문구).
