...
지금은 적 · 안드로이드 모두 길찾기 없이 조향만 한다 (`allies/parts/Nav.ts` 머리 주석 「This game has no pathfinding」, `enemies/ai/Steering.ts` `seek`).
소유 폴더: `shared`(계약) · `world`(격자 · 건물 그래프) · `allies` · `enemies`.

| ID | 항목 | 근거 |
|---|---|---|
| A-18 | **공용 길찾기 — 벽 너머 목표로 돌아가지 못한다.** 안드로이드는 벽 너머의 핑 상자 · 아이템을 `ALLY_JOB_GIVEUP_S` 뒤 포기하고(2026-09-21 E-14 의 대책 — 길찾기가 생겨도 잠긴 문 뒤 · 닿을 수 없는 지점 때문에 그 포기는 남는다), 적은 건물 안 플레이어를 향해 벽에 붙어 선다. 2026-09-21 사용자 제안 「안드로이드뿐 아니라 적에게도」. 설계 메모: `world` 가 레이드 생성 때 바깥 2D 격자(맵 640 m, 1 m 격자 ≈ 41만 칸)와 구조물의 문 · 계단 · 층 그래프를 굽고 `WorldRef` 에 경로 질의를 연다 → 안드로이드(1~3기)는 기마다 **A\***, 적(~60)은 플레이어마다 반경 ~64 m **흐름장** 하나를 모두가 읽는다. 둘 다 직선이 막혔을 때만 쓰고 트인 곳은 지금 조향 그대로. 권한 쪽(호스트)만 계산해서 와이어는 없다. 넣어야 할 것: 잠긴 문 · 사다리 · 창문 · 움직이는 전차 / 탐사 차량 · 배리케이드 / 돔 실드 같은 동적 장애물. **착수 전 결정 필요**: 적이 문으로 돌아 들어오면 「건물에 숨기」의 난이도가 바뀐다 — `NEST_LEASH_M` · 벽 갉기(`STRUCT_DAMAGE_MUL`) 재조정, 그리고 §5 예산(적 60 · 60 fps) 재측정 | `grep -rniE "astar\|pathfind\|flowfield" src/` → 0 hit (주석 제외) |
