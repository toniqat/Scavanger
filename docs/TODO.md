...
- **묶음 4–5 · 8** = 다음 페이즈 후보. 하나가 한 번의 작업 사이클이다 (소유 폴더를 각 묶음 머리에 적었다).
...
2026-09-21 1단계 완료 — 공용 그래프(`src/world/nav/`, 계약 `shared/nav.ts` · `WorldRef.nav`)와 안드로이드 A\*(`allies/parts/Nav.ts` `route`, 사다리 포함)가 들어갔다.
적은 아직 길찾기 없이 조향만 한다 (`enemies/ai/Steering.ts` `seek`).
소유 폴더: `enemies`(흐름장 · 벽 타기 · 창문) · `world/nav`(`climb` · `window` 간선 굽기) · `shared`(흐름장 계약).

| ID | 항목 | 근거 |
|---|---|---|
| A-18 | **적 길찾기 (2단계) — 적은 건물 안 플레이어를 향해 벽에 붙어 선다.** 1단계(그래프 · 안드로이드)는 끝났다. 남은 것, 2026-09-21 사용자 결정대로: ① 적 **전원**(벌레 · 인간형)이 플레이어마다 반경 ~64 m **흐름장** 하나를 읽고 문으로 돌아 들어온다 — 문 · 계단 같은 좁은 길목은 **한 번에 N마리만** 지난다(입구 속도 제한). 인간형은 사다리도 탄다(`NAV_CAN.LADDER`) ② **소형 벌레만**(중형부터 못 탐) 바깥벽을 타고 **옥상 → 해치 → 사다리로 최상층**에 내려오고, **창문으로도** 기어 들어온다 — 창이 멀쩡하면 먼저 깨려고 한다. 그래프 쪽은 `world/nav` 가 `L_CLIMB`(벽면 → 옥상) · `L_WINDOW`(창 안팎) 간선을 굽고, 어느 벌레가 타는지는 `enemies.csv` 에 열을 새로 둔다 ③ 큰 몸(`NAV_CAN.INDOOR` 없음)은 건물 격자에 들어가지 않는다 ④ 재조정: `NEST_LEASH_M` · 벽 갉기(`STRUCT_DAMAGE_MUL`) ⑤ §5 예산(적 60 · 60 fps)을 흐름장을 켠 채로 다시 잰다. 1단계에서 잰 값: 굽기 CPU 45–150 ms(프레임당 `NAV_BAKE_BUDGET_MS` 로 나눠서), 긴 야외 A\* 1 ms 미만, 닿을 수 없는 목표 ~6 ms (`NAV_SEARCH_MAX_NODES`) | `grep -rniE "flowfield\|flow field" src/enemies` → 0 hit · `world/nav/model.ts` `L_CLIMB` · `L_WINDOW` 는 간선이 하나도 없다 |
