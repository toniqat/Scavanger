...
| A-3c | · 주방 — 요리로 다음 레이드 버프, 공유 함선 테이블. **작물 8종이 이미 있고 설명이 "주방 시설이 열리면 요리 재료가 된다" 고 약속한다** — 지금 그 8종의 소비처는 판매 · 세레스 납품 · 추출기뿐이다. **버프를 담을 그릇은 이미 있다**: A-13 이 만든 `PlayerProfile.prep` / `prepActive`(함선에서 쓰면 다음 레이드 1회분)에 요리를 얹으면 된다 | `data/items.csv` 의 `crop_*`, `src/shared/progression.ts` 의 「준비물」 블록 |

### 묶음 1-G — 온실 · 연구실 로드맵 (2026-09-11, 사용자 6단계 명세)

**1 · 2 · 3 · 4단계는 끝났다** — 1단계(온실 · 재배 스테이션 · 토양)가 2026-09-11 오전,
**2단계(A-11 품종 확장) · 3단계(A-12 연구실 · 분석기 · 해석 도감) · 4단계(A-13 추출기 · 조합대 · 준비물)** 가
같은 날 오후 (→ [HISTORY.md](HISTORY.md), 설계안 `docs/plans/lab-a11-a13.md`). 아래는 남은 **5 · 6단계**다.

| ID | 항목 | 근거 · 선행 |
|---|---|---|
| A-14 | **5단계 — 배양조(온실 배치)**. 온실 산물로 영양 배지를 만들어 채우고, 레이드에서 얻은 세포주 · 균주를 분석기로 해석해 삽입 → 미세조류 · 배양육 · 배양 지방 · 유단백 · 난백 | A-12 · A-13 은 끝났다. 요리 종류가 크게 늘어나므로 **A-3c(주방)가 있어야 뜻이 산다** — 그것이 지금 남은 유일한 선행이다. 해석은 `housing/parts/Lab.ts` 의 분석기에 표본 종류만 더하면 된다 |
| A-15 | **6단계 — 3D 프린터(연구실 배치)**. 배양조 산물을 추출기에 돌려 필라멘트 → **상급 가방**과 **주머니**(특정 재료만 넣는 추가 가방; 아이템 더블클릭 · 우클릭 → 가방 열기 로 인벤토리 좌측 장비칸 자리에 모달리스 팝업 격자) | A-14 선행. 주머니는 「퀵슬롯은 가방 격자가 아니다」(2026-09-09) 가 만든 별도 컨테이너 패턴을 그대로 따르면 된다. 추출기(`bench extract`)는 이미 있다 — 필라멘트 레시피 줄만 늘리면 된다 |
