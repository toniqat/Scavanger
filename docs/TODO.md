...
| B-83 | **주석이 코드와 어긋나거나 죽은 것 8군데** (2026-09-20 영문화 감사, `src/weapons` · `src/net` · `src/stratagems` · `src/console`). `weapons/model.ts:178-181` 은 아무 선언도 문서화하지 않는 떠 있는 doc 이고 내용도 낡았다(「Three weapon slots」— `WEAPON_SLOTS` 는 두 칸, `보조무기` 는 2026-09-10 에 빠졌다) · `weapons/parts/Firing.ts:6` 의 「정밀 사격 정렬(예측 카메라 원점)」은 파일 어디에도 없다(2026-09-12 하이브리드 판정이 걷어낸 잔해) · `weapons/WeaponDefaults.ts` 의 `defaultFor` doc 「both primaries get the rifle, **the secondary the pistol**」은 채워질 수 없는 갈래를 설명한다 — `WEAPON_SLOTS` 는 `['primary','primary2']` 뿐이고 `Loadout.secondary` 는 늘 null 이라(`inventory/model.ts` `slotAccepts` 가 거부한다) 어떤 슬롯도 `DEFAULT_PISTOL` 을 기본값으로 받지 않는다. **죽은 코드는 아니다** — `parts/Slots.resolveDef` 가 `other` 를 만들 때 `defaultFor('secondary')` 를 부르므로 fallback 비교용으로는 매번 돈다 (2026-09-21 정정: 「호출자가 없다」가 아니었다) · `weapons/Grenade.ts` 의 「바깥 띠 60 %」는 `EXPLOSION_OUTER_MUL` 이 2026-09-17 결정으로 **0.5** 가 된 뒤로 틀렸다 · `net/NetSystem.ts:140-141` 은 2026-09-16 에 지워진 `parts/Meal` 의 `meal serve` 를 설명하는 JSDoc 이 살아 있는 `parts/Plates` JSDoc 바로 위에 남아 있다(한 심벌에 JSDoc 두 개) · `net/NetSystem.ts:102-103` 의 `embeddedCache` 는 선언 말고는 트리 어디서도 읽지 않는다(2026-09-15 「빌드에 서버 없음」 잔해, `probeGen:100` 주석도 없어진 shell-route lookup 을 말한다) · `stratagems/parts/Wire.ts:129` 의 「A refusal is silent.」는 같은 함수 본문의 2026-09-11 주석과 정반대다 · `stratagems/parts/Wire.ts:136` 은 없는 `sendDeny` 를 가리킨다(실제는 `sendCallDeny`) | `src/weapons/**` · `src/net/NetSystem.ts` · `src/stratagems/parts/Wire.ts` |
| B-84 | **`구조선` 을 *rescue ship* 이라 부르는 곳 여섯** — 주석 영문화 용어집(`git show bb4eb2c:docs/COMMENT_I18N_PLAN.md` §7)이 금지한 단어다(→ *the rescue drop*). `src/ui` 에 넷(`ui/hud/StratagemPanel.ts:27`·`:159` · `ui/hud/DangerIndicators.ts:97` · `ui/hud/Notifications.ts:155`), `src/shared/types.ts` 에 둘(`:2002`·`:2037`). 같은 폴더의 `ui/hud/RescuePicker.ts` 는 올바르게 적고, **`ui/hud/StratagemWheel.ts` 는 오기가 아니다** — 거기 있는 `구조선 투하` 는 csv 에서 읽는 화면 이름을 규칙대로 그대로 인용한 것이다 (2026-09-21 정정: 이 파일 세 줄은 목록에서 뺐다). 두 폴더 모두 큐를 떠난 뒤라 영문화 커밋에서 고치지 않았다 | `src/ui/hud/**` · `src/shared/types.ts` |
