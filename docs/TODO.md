...
| B-76 | **`src/shared/npc.ts` 가 은퇴한 선택지 라벨을 오기로 적어 두었다 — §3 4b 가 잡는 바로 그 부류인데, 이전 패스가 흘려보냈다.** `:62` · `:75` · `:484` 가 세 번 모두 「생각해볼게」라 적지만 살아 있는 문자열은 `NPC_REPLY_KO.decline = '생각해보지.'`(같은 파일 `:78`)이고, 나머지 전부(`meta/parts/Console.ts:180` · `:182`, `meta/parts/NpcQuests.ts:16` · `:584`, `ui/menus/messenger/QuestCard.ts:11`, `data/npc_quests.csv:11`)가 「생각해보지」다. 「생각해볼게」는 트리 어디에도 문자열로 없다 — 즉 주석의 한글을 실제 문자열에 grep 할 수 있게 남긴다는 규칙 2 의 사슬이 **계약 폴더에서** 끊겨 있다. 4b 는 diff 범위라 영영 못 보고, `check-comment-labels --head` 도 조용하다(비교 대상이 같은 파일 안에 있어 `live` 집합에 들어간다). 2026-09-20 `src/meta` 영문화 때 발견 | `src/shared/npc.ts` |
