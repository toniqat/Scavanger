...
| B-23 | **`src/enemies` 주석 영문화(2026-09-18) 중 드러난 서류 결함 6건** — 번역은 원문을 그대로 옮겼으니 내용 자체를 고쳐야 한다. ① `src/enemies/README.md` 의 「땅굴벌레 등장 판정」 규칙이 `named/Director.ts`(네임드 디렉터)를 가리킨다 — 그 규칙은 `sandworm/Director.ts` 에 있다. ② `Spawner.ts` 의 `AmbientSpawner.tuning` doc 이 「가중치로 표현되지 않는 **둘**」이라 써 놓고 셋을 열거한다(대형 슬롯 확률 · 순찰 베헤모스 · 포병 굴착 확률). ③ `parts/Alerts.ts` 의 구분선이 「총알 추적 · 정찰 x-ray」라고 하고 `EnemyXray` 를 import 하지만 x-ray 는 `parts/Status.ts` 로 옥기고 없다. ④ `parts/Attacks.ts` 머리 주석이 소유 목록에서 `onFireZoneTick`(소이 지대) · `acidBurst`(스퓨어 사망 폭발)를 빼고 있다. ⑤ `RogueDrop.ts:404` 의 「자리가 모자라면 그만큼만 내려온다」 주석이 **0개 가지**(`points.length === 0`) 위에 붙어 있다 — 설명하는 동작은 한 줄 뒤 `begin(...)` 이다. ⑥ `models/BugModel.ts` 의 `statusEmissive` doc 이 「아무것도 빛나지 않으면 false 를 돌려준다」고 하지만 선언이 `: void` 이고 모든 호출부가 무시한다 | `src/enemies/README.md` · `Spawner.ts` · `parts/Alerts.ts` · `parts/Attacks.ts` · `RogueDrop.ts:404` · `models/BugModel.ts` |
