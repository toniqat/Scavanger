...
| `smoke-desktop` | `9340/json/version 이 45 초 안에 응답하지 않았다` | Red in 3 of 3 full runs on 2026-09-16 (before and after `closeBrowser`, which it does not use) and green alone then (50/50). **Since 2026-09-20 it is red alone as well** — 5/6, at the parent commit too, so it is not a code change of ours; the electron process is alive and *is* listening on 9340, it just never answers `/json/version`. Cause still not found — see `docs/TODO.md` E-13. (A second, separate failure — the smoke relay not starting — was this PC's WinNAT reserving **8800–8899**; the relay moved 8823 → 9823 on 2026-09-20, as `smoke-squad-dock` 8894 → 9894 and `smoke-android-lobby` 8896 → 9896 did at the same time.) |
