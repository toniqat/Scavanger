...
| 휠클릭 | 핑 | 홀드 + 드래그. 2026-09-09 부터 **보이는 좌/우 2칸 휠**이 뜬다 (`ui/hud/PingWheel`, 0.15 s 뒤) — 좌 = **여기 조심해**, 우 = **저쪽으로 가자**, **전투불능이면** 좌 = **살려줘**, 우 = **나를 버려**(`PING_HOLD_KINDS`). 아래로 드래그 = 탄약 요청이고 **전투불능일 때는 그 다리가 사라진다**(총을 못 쏘는 사람이 탄약을 부를 이유가 없다 — 그때 아래 드래그는 평범한 핑). 지도 위에서도 놓을 수 있다 |
