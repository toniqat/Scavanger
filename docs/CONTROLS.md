...
| `]` | Tutorial control guide fold / unfold (`GUIDE_TOGGLE`) | Read only in the build tutorial's final raid, while no screen is open (`tutorial/TutorialSystem.pollControlsFold`). Folded = one line `] 조작 가이드 표시` in the same box. Ship management's `[` / `]` furniture cycling is a fixed code on a cursor screen, so scopes do not overlap |
