...
| 폴더 | 시스템 | `ctx` 게시 | 한 줄 책임 |
|---|---|---|---|
| [`src/world/`](src/world/README.md) | `WorldSystem` | `ctx.world` | 절차 지형 · 바이옴 · 소품/장애물 · 상자 · 추출 패드 · 채집 노드 · 시뮬레이션 훈련장 · 충돌/레이캐스트 질의 |
| [`src/enemies/`](src/enemies/README.md) | `EnemySystem` | `ctx.enemies` | 버그 5종 + 휴머노이드 로그 AI · 포병 · 베헤모스 · 팩션 · 시체 루팅 · 상태이상 · 총알 추적 · 호스트/리플리카 동기화 |
| [`src/extraction/`](src/extraction/README.md) | `ExtractionSystem` | — | 추출 콘솔 · 120초 카운트다운 · 함선 착륙/탑승/이륙, 호스트 권한 |
...
| [`src/inventory/`](src/inventory/README.md) | `InventorySystem` | `ctx.inventory` | 디아블로2식 격자 모델 · 가방/장비/퀵슬롯 · 함선 창고 · 컨테이너 감정 · 소켓 · 제작/분해/수리 · Tab 화면(인벤토리/캐릭터/기업/함선) |
| [`src/pickups/`](src/pickups/README.md) | `PickupSystem` | `ctx.pickups` | 월드에 떨어진 아이템 (투척 궤적 · 절차 메시 · 빛기둥 · 호스트 권한 동기화) |
| [`src/meta/`](src/meta/README.md) | `MetaSystem` | `ctx.meta` | 기업 4곳 · 신뢰도 · 크레딧 · 상점/거래대 · 계약 · 퀘스트 · 임플란트 수리 데스크 |
| [`src/progression/`](src/progression/README.md) | `ProgressionSystem` | `ctx.progression` | 레벨/XP · 능력치 5종 · 숙련도 14종 · 파생 수치(`derived`) · 임플란트 장착칸 · 캐릭터 시트 |
...
| 폴더 | 시스템 | `ctx` 게시 | 한 줄 책임 |
|---|---|---|---|
| [`src/hub/`](src/hub/README.md) | `HubSystem` | `ctx.hub` | 개인/공유 함선 내부 · 도킹 & 워프 컷씬 · 발사 포드 · 전체화면 터미널 · 행성 선택 · 작업대 · 시설 관리 모드 |
...
| 폴더 | 시스템 | `ctx` 게시 | 한 줄 책임 |
|---|---|---|---|
| [`src/net/`](src/net/README.md) | `NetSystem` | `ctx.net` | 릴레이 WebSocket 클라이언트 · 세션 토큰 · 로비 · 20 Hz 스냅샷 · 프로필 동기화 · 소셜 · 크루 카드 |
