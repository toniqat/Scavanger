...
| **Frame hitches with many bodies** | [docs/PERF_PLAN.md](docs/PERF_PLAN.md) — **every phase done** (measured 2026-09-19, built and re-measured 2026-09-20). What it settled: the render block is **GPU time**, not draw calls (−31 % draws moved nothing); `ms` on this PC spreads ±1.7 ms on one build, so judge by counters; and a squadmate costs **one more body drawn**, not the wire (Phase D, S5). Harness: `node scripts/perf-measure.mjs` (`--display` splits the render block, `--only s5a·s5b·s5c` runs two clients through the relay, `--alloc` counts garbage by owner) |
...
- **Bodies keep their triangle count down by budget**: every sphere of a bug body goes through `segBudget` → `BUG_MESH_SEGMENTS` (`enemies/models/BugModel`), so the per-part `seg` numbers say what a part wants and one csv row says what it gets. The render block is GPU time on the measured machine — triangles and pixels move it, draw calls do not (`docs/PERF_PLAN.md`).
