...
| `src/player/` | `PlayerSystem` | `ctx.player` | Third-person controller, camera rig (Arc Raiders over-shoulder), procedural soldier model, health, stims, interaction (E), hellpod drop-in |
| `src/weapons/` | `WeaponSystem` | — | Primary/secondary weapons, hitscan/projectiles, tracers, muzzle flash, impacts, recoil, reload, grenades, listens `loadout:changed` |
...
| `src/items/` | (data) | `ctx.loot` | Item & weapon definitions, loot tables, `LootRef` impl |
...
| `src/ui/` | `HudSystem` | — | Arc Raiders-style HUD (health, ammo, compass, objective markers, prompts, notifications, hitmarkers, damage indicators), menus (title, pause, death, mission complete), `styles/base.css` |
| `src/game/` | `GameFlowSystem` | `ctx.phase` | Phase state machine: menu → deploying → playing → extracting → shipLanded → liftoff → complete / dead; stats; restart |
...
`ctx.isGameplayActive()` gates all gameplay input (false when a UI blocker like the inventory is open). Systems that open UI add/remove a token in `ctx.uiBlockers` and call `ctx.input.exitPointerLock()` / player re-locks on click.
