...
- **Docs language: English** for `CLAUDE.md`, every folder `README.md` and `docs/*.md` except `docs/TODO.md` (Korean). **Code comments are English too** (2026-09-18, user's decision — the comments are what an AI reads to work here). **In-game text stays Korean.** A comment that names an on-screen label, a csv name or an identifier keeps it verbatim in backticks (`신호소`, `포인트 투자 확정`, `인내`); the prose around it is English, and a Korean term that is only a concept becomes its English word (`헬스장 단련 보너스` → the gym training bonus). String literals, csv keys and event names are never touched.
- **Talk to the user in Korean.** The rule above is about what an *AI* reads; everything a *person* reads stays Korean — chat replies and work summaries, commit messages (subject and body; generated trailers like `Co-Authored-By:` stay as they are) and `/compact` context summaries. The backtick rule runs in reverse there: labels, paths, identifiers, commands and csv names stay verbatim (`src/allies/parts/Roam.ts`, `npm run verify`, `신호소`) inside Korean prose.
...
- Cross-folder: bus events for messages, `ctx.*Ref` for sync queries. **Never import another feature folder's internals — `@/shared` only.** Accepted library-like exceptions (don't add new kinds): engine helpers `@/core/fx` and `@/core/util/MathUtil`; the `@/items` barrel (item data — inventory, weapons, ui, and the two places that need a **weapon's own damage falloff** to shoot like one: `allies/parts/Combat.ts`, `world/rover/parts/Turret.ts`); the `@/player` barrel for `SoldierModel` (`game/Corpses.ts`, `ui/menus/SoldierPreview.ts`).
...
- **`noUnusedLocals` is on in all three tsconfigs** (2026-09-21, B-97, user's decision). The split above is what made it
  necessary: every `parts/*` file was born with the whole system's import block copied in, and with the flag off `tsc`
  never said so — 2,373 dead declarations across nine folders. A declaration nothing reads is now a **build error**, so a
  contract stub kept 「for later」 is deleted rather than parked (`GameFlowSystem.onRespawnRequest` went that way).
...
- **What a player owns is never the client's word.** `LobbyPlayer.code` · `level` · `shipModel` are filled by the relay in
  `RelayServer.lobbyState` from that member's own profile store record (`Store.card` · `Store.shipModel`, which reads
...
- Messages that affect others are accepted only from the authority (lobby host for `strat call`, `ee`, `crate sync`). The
  **ceiling turret's chosen body** joined them on 2026-09-21 (B-100): `struct turret {id, tg, st}`, sent on a change only,
  ...
  A late joiner is told the armed ones once (`CeilingTurretSet.activeWire`); the shot cadence stays local. Host-bound requests pass shape → sender → distance → rate (`shared/buffRules.ts` `createBuffGuard`); two paths of one ability share a bucket; limits derive from data; flying things may outlive their dead sender.
...
- **A LOD for something *drawn* is a screen size, not a distance.** The enemy animation LOD's `ENEMY_ANIM_LOD_*`
  ...
  which the sniper's glint sprite uses too. A LOD for what a body can **act on** (the AI LOD) stays in metres. And a
  body whose animation **integrates `dt`** rather than reading `anim.time` — every named rogue look file — is exempt
  from a pose LOD outright: a skipped frame is lost there, not caught up (`EnemySystem.poseSkip`).
- **The shadow pass is a budget too.** The sun's box is `SUN_SHADOW_HALF_M` and follows the player, so a caster outside it is wasted work — but a map-wide `InstancedMesh` is never frustum-culled, so scattered props split into a near (casting) and a far mesh around the eye (`PROP_SHADOW_DIST_M`, `world/Props.repackShadowLod`); both halves are sized to the real instance count and `DynamicDrawUsage`, because re-uploading an oversized static buffer costs more than the triangles saved. Waist-high things (crates, containers, debris) cast nothing and only receive.
...
- **Indoors is lit by lifting what is already there, never by a new light** (2026-09-21): a roof within `INDOOR_PROBE_UP_M` above the eye multiplies hemisphere/ambient by `INDOOR_LIGHT_AMBIENT_MUL` and fog by `INDOOR_FOG_MUL` across `INDOOR_LIGHT_FADE_S` — **on the local client only**, nothing on the wire, and it multiplies *on top of* `atmo:override` so a roof softens a storm instead of cancelling it (`core/Atmosphere.stepIndoor`). The probe is one upward ray at `INDOOR_LIGHT_FADE_S / 4`, and the hub · cutscenes · liftoff · non-gameplay phases are excluded.
- The **hub** light pool is aimed at the room the player stands in and, in ship management, the room at the camera's screen centre — the pool's count never changes, only which fixtures own the slots (`hub/interiors/PersonalShip.pickRooms` · `pickFocus`).
...
- **What an explosion *breaks* is one loop**: every player-side blast — grenade · bazooka · mine · ship call · fire zone — hands
  its centre to `shared/explosion.blastDestructibles`, which fades the damage by the same stair and gives it to every
  `Obstacle.destructible` in the radius (2026-09-21, B-98, user's decision). **Player-side only**: the 탐사 차량 counts what
  arrives through `destructible` toward its hostility, so an enemy blast keeps its own `Targets.damageVehicleAt`, which
  never touches `aggro`. A blast is gated exactly where its enemy damage is (`!visualOnly` · `call.local` · the host), and a
...
- Rover: one per raid, host-authoritative; riders take no damage and are not targets; one payer per trip (`rover:<from>:<to>`) — rules in `shared/types.ts` rover section. New combat input gates check both `droneControl` and `roverRide`. **Since 2026-09-21 the player side can damage it and it shoots back**: a shot resolves against a hit zone (4 wheels · front turret · rear turret · hull, `rover/parts/Parts.ts`), part damage also comes off the hull (`ROVER_PART_HULL_MUL`), one dead wheel slows it and two stop it, and past `ROVER_AGGRO_DAMAGE` of cumulative player damage it is **hostile for the rest of the raid** — it keeps its route, refuses boarding and its turrets take the attacker *and their squad* at `ROVER_TURRET_PC_ACCURACY`. The two turrets are a common SMG (front) and a common AR (rear) reading `weapons.csv`' own falloff. Destroyed, it drops `ROVER_WRECK_CRATE_MIN..MAX` cube crates on the **outpost-basement table with the `lockedRoom` epic+ exemption** — a rover raid pays what a basement pays.
- A reload **pauses** for an instant action and **cancels** for a wielded one: the roll and the grapple freeze the timer and resume it (`weapon:reloadPaused` / `Resumed`, the crosshair gauge freezes with it); barrier and overcharge cancel it, as swap · consumable-in-hand · melee · loadout change always did. Drone control and the rover seat do **neither** — they never touched the reload and still do not.
- A 회복 소모품 in hand is **left-click on me, right-click on an ally** (`weapons/parts/AllyHeal.ts`): the target is the smallest angle off the crosshair within `HEAL_ALLY_AIM_CONE_DEG`, human **or** android, startable inside `HEAL_ALLY_RANGE_START` and sustained to `HEAL_ALLY_RANGE_HOLD`. Healing someone else carries **no move-speed penalty** (`CONSUMABLE_SLOW_MUL` is the self use only), a target who cannot use it is never offered, and the item is consumed only once the gift actually lands. A 실드 충전기 goes the same way as `buff shield` (-1 = fill up) and is applied by implants; an android's goes through `AlliesRef.heal` / `chargeShield`. A downed body is nobody's heal target — that is the defibrillator's job, and the defibrillator now reaches a **carried** one too.
...
- **Keys are planet-bound** (2026-09-21): `key_basement_<planet>` · `keycard_lab_<planet>`, one pair per `data/planets.csv` row, and a door is built as `<structures.csv key>_<mission planet>`. A key opens any building of its kind **on its own planet** and nothing anywhere else; the deny text names the planet it wants. Which planet's key drops is rolled **uniformly and independently of where you are** — that is the point, a key found here is an invitation to go there — while the *kind* still follows the source (an outpost rolls a basement key, a lab a keycard). The key list is extracted from the item table, so adding a planet is a csv change (`items/LootTables.PLANET_KEY_VARIANTS`). Old ids resolve through `data/item_aliases.csv`.
- Inside the basement and the lab's locked room hangs an **indestructible ceiling turret** (`world/structures/parts/Turret.ts`): the only way to switch it off is opening that door with the matching key, so the crawl vent is a way in, not a way past. Host-applies damage, warm-up `CEIL_TURRET_WARMUP_S` with an alarm first, and it never follows anyone out of the room.
...
- **One ship model, three places** (2026-09-21): `shared/shipModel.ts` builds the drop-ship, and extraction (`detail: 'full'`), the hangar and the docking cutscene (`'exterior'`, merged, `SHIP_EXTERIOR_SCALE`) all draw from it — the ship parked in the hangar *is* the ship that lands. Which one is `PlayerProfile.shipModel` (through `Profile.migrate`), carried to others as `LobbyPlayer.shipModel` (`lobby:look`) and to a raid as `ex activated.shipModel` — the caller's ship lands, each member's own ship parks. The relay keeps it as a shape-checked string (`sanitizeShipModel`) because `shipModel.ts` pulls three.js; **when ships can be bought the relay must fill it from its profile store** instead of taking the client's word.
- The airlock is its own room with automatic doors: within `SHIP_AIRLOCK_DOOR_RANGE_M` of **anyone** (mine or a squadmate's body) a leaf slides into its wall pocket over `SHIP_AIRLOCK_DOOR_OPEN_S`, and the leaf **is** the collider, so a half-open door half-blocks. The outer hatch is locked in the hub — the ship is in space — and only a raid unlocks it (`PersonalShip.setAirlockOuterLocked`).
...
- **When done, record each fact in exactly one place:**
  ...
  - Write docs in **English** (except `docs/TODO.md`). **Commit messages — and everything said to the user — stay Korean** (§4.1).
