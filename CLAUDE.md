...
| [`src/tutorial/`](src/tutorial/README.md) | `TutorialSystem` | `ctx.tutorial` | **3트랙 계약(2026-09-14, 사용자 결정 — `docs/plans/tutorial-raid.md`): ① `raid`(튜토리얼 레이드) · ② `ship`(레벨업 · 스탯 · 메신저) · ③ `build`(지금의 17단계), 각각 따로 건너뛰기 — 계약과 단계 자리만 들어왔고 구현은 아직 build 하나다)** · 새 캐릭터 안내 **17단계** — 목표 패널 · UI 스포트라이트(**합집합 포커싱 · 0.5초 늦게 · 딤 페이드인**) · 바닥 안내선, 순서 강제 게이트(`blockReason`) + **잠긴 항목 숨김**(`hides`, **함선 창고 아이템 포함**) · 부족한 재료 top-up · **1초 홀드 건너뛰기** |
...
- **퀘스트는 NPC 가 메신저로 준다** (2026-09-14, 사용자 결정 · `docs/plans/messenger-quests.md`). 기업 퀘스트는 전부 지웠고(`QUEST_DEFS` 는 계약이라 빈 배열로 남는다)
