...
- **요리 재료는 티어 사다리이고 분석기가 그 관문이다** (2026-09-13, 사용자 결정 · `docs/plans/food-tiers.md`). T1 채소(온실만) → T2 고기 페이스트(세포주 + 배지 · 소금) →
...
- **인간형 적의 팩션은 행성 threat 가 정한다** (2026-09-13, 사용자 결정 · 설계안 `docs/plans/enemy-factions.md`). threat 1 = 안드로이드(연구소 · 전진기지 집사 로봇,
...
- **가구에는 접근 면이 있다** (2026-09-13, 사용자 결정 · `docs/plans/power-crypto.md`). `data/furniture.csv` 의 `access` — `front`(앞 = 로컬 −Z 한 줄 비움 · 벽 불가 · 앞에서만 E) ·
...
- **함선 시설은 전력으로 돈다** (2026-09-13, 사용자 결정). 시설 요구 = 방 용도 `power` + 활성 가구 `power`(+ 클러스터 코어), 발전기 공급을 **시설마다 수동 할당**하고 모자라면 그 시설 전부 정지 —
  단 요구가 오른 시설은 **할당 안 된 전력에서만** 부족분을 자동으로 가져온다(`POWER_AUTO_TOPUP`, 다른 시설 몫은 안 뺏는다). 조종석은 전력을 쓰지 않는다. 가구를 쓸 수 있나는
  `HousingRef.furnitureOperationalBlock(uid)` 하나다. **현실 시간으로 흐르는 가구는 `serverNow` 가 아니라 `stationNow(uid)` 로 진행도를 잰다** — 멈춘 동안 시계가 서고, 다시 돌면
  `housing:operationalChanged {pausedMs}` 를 받은 자리에서 자기 시각을 그만큼 민다(재배 · 배양 · 해석 · 채굴이 그렇게 한다). 새 시계형 가구도 이 두 줄을 따른다.
