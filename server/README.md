...
- **차단 (B-4)**: `SocialRecord.blocked`. 차단당한 사람이 나에게 하는 것은 **조용히 삼킨다** — 귓속말은 버리고 ack `ok:true`,
  ...
  같이 하기 · 귓속말은 `invalid`(UI 가 먼저 막는다 — 해제가 먼저). 로비 코드로 직접 들어오는 것(`lobby:join`)은 막지 않는다.
