...
| `CryptoMarket.ts` | **2026-09-13 — 암호화폐 시세** (docs/DECISIONS.md 「2026-09-13 — 가구 접근 면 · 발전기 · 암호화폐 채굴」). `CryptoMarket({table, dataDir, seed?, now?})` (표 = `economy.gen.json` 의 `crypto` 절): 코인별 로그 가격이 틱마다 ln(basePrice) 쪽으로 평균 회귀(OU 정확 이산화, 반감기 3일, 틱 σ = volatility × √(tick ÷ 1일)) + 드문 점프 + 기준가 1/5 … 5배 띠. 난수는 **상태 없는 해시**(seed · 코인 · 틱 번호)라 꺼져 있던 틈을 틱 단위로 다시 돌리면 끊기지 않은 실행과 같다. 1분봉 25 h · 1시간봉 32 일, `history(coin, range)` = `CRYPTO_CANDLE_MS` · `CRYPTO_CANDLE_COUNT` 로 묶은 봉(모르는 코인 · 기간 null), `quoteRange(coin, now)` = 최근 `quoteWindowMs + tickMs` 의 최저 · 최고(+ 현재가), `pricesMessage()` (`change24h` = 24시간 전 봉 종가 대비), `tick()` · `start()`(틱 경계 정렬 타이머, unref) → `onTick`, `close()`. `<dataDir>/crypto.json` 디바운스 60 s 비동기 쓰기 + `.bak` + 손상 파일 보존(`Store.ts` 와 같은 춤), 첫 시작 = 31일 백필, 틈 = 최근 24 h 틱 단위 + 그 전 10분 걸음(31일 상한). 아래 `암호화폐 시세` 절. |
| `Rooms.ts` | **2026-09-14 — 단체 메신저방** (docs/DECISIONS.md 「2026-09-14 — 메신저 · NPC 퀘스트 · 단체방」). `RoomStore`: 방 · 멤버 · 초대 · 줄의 규칙과 한도 (방장형 · `ROOM_MEMBER_MAX` · `ROOM_JOINED_MAX` · `ROOM_LINES_MAX` · 초대 TTL · 방 안에서 엄격히 증가하는 줄 시각 · 방장 이관 · 빈 방 삭제 · GC — 친구 · 차단 검사는 relay 가 먼저 한다) 와 `rooms.json` 저장 (`Store.ts` 와 같은 tmp + fsync + `.bak` 춤, 깨진 파일 보존). 소켓을 모르고 연산마다 붙은 줄 + `room:state` 를 받을 아이디 (`RoomOp`) 를 돌려준다. 아래 `단체 메신저방` 절 |
...
| `data/` | Runtime profile store (`profiles.json` + since 2026-09-11 `profiles.json.bak` = the previous generation, and any `profiles.corrupt-<ts>.json` a failed load moved aside), created on the first write. Git-ignored. Delete it to wipe every server profile. **2026-09-13**: `crypto.json` (+ `crypto.json.bak`, any `crypto.corrupt-<ts>.json`) = 암호화폐 시세 이력 (`CryptoMarket.ts`, 약 0.7 MB) — 지우면 다음 시작에 31일을 결정적으로 다시 채운다(프로필과 달리 잃는 것은 지난 차트뿐). |
...
| `room:get` · `room:create {name, invite?, nonce}` · `room:invite {room, code}` · `room:reply {room, accept}` · `room:leave {room}` · `room:kick {room, code}` · `room:rename {room, name}` · `room:say {room, text, nonce}` · `room:history {room, before?}` | **2026-09-14 단체 메신저방** — see `단체 메신저방` below. Profile sockets only (`room:error unavailable`; create · say answer `room:ack`). → `room:state` (requester at once, others coalesced) · `room:line` to connected members (a `say` is not echoed) · `room:ack {nonce, ok, room?, at?, code?}` · `room:history {room, lines, more}` · refusals `room:error {code, message}` (`RoomErrorCode` + `roomErrorMessage`) |
...
사용자 결정 "리비전 전체". 경과는 `docs/HISTORY.md` 「2026-09-11 (16차: 소셜 · 신뢰 경로 · 연결 배치)」. 토큰 = 브라우저 저장소라 **한 프로필의 작성자는
...
`src/shared/credits.ts`(사유 문법 · `EconomyTable` · `CreditLedger` · 창 · 상한), 경과는 `docs/HISTORY.md` 「2026-09-11 (16차: 소셜 · 신뢰 경로 · 연결 배치)」.
...
- **누가 켜나.** `startRelayServer` 가 경제 표에 `crypto` 절이 있으면 늘 `CryptoMarket` 을 만든다 — `index.ts` · `tool.ts`(배포 exe) ·
  데스크톱 셸의 임베디드 릴레이가 모두 이 함수를 지난다. 저장 위치는 프로필 저장소와 **같은 `dataDir`** (`server/data/crypto.json` ·
  exe = `%LOCALAPPDATA%\SCAVANGER\server\crypto.json` · 앱 = `userData/relay-data/crypto.json`), `dataDir: null` 이면 메모리. 시작 로그 한 줄,
  `/health.crypto` = 코인 수(0 = 꺼짐). 옵션 `cryptoSeed` 는 셀프테스트용, `RelayServer.crypto` 로 시장을 들여다본다. `close()` 가 타이머를 멈추고 동기로 쓴다.
- **와이어.** `crypto:watch {on}` — 익명 소켓도 받는다, 소켓별 플래그(`Client.cryptoWatch`), `on` 이면 즉시 `crypto:prices` 한 번. 틱마다
  (`onTick`) 구독한 소켓 전부에 JSON 하나를 보낸다. `crypto:history {coin, range}` — 그 소켓에만 `crypto:history {coin, range, at, candles}`,
  모르는 코인 · 기간은 무응답, 소켓별 토큰 버킷 `CRYPTO_HISTORY_BURST` 16 · `CRYPTO_HISTORY_PER_S` 4(넘친 요청 무응답). 파서는 모양만 본다.
- **매매 검증.** `Economy.ts` 의 `cbuy:<coin>:<units>` · `csell:<coin>:<units>` — `CreditEconomy` 는 순수하게 남고 시세 창은 주입된다
  (`CryptoQuoteSource` · `setCryptoQuotes(market)`). 매수 |delta| ≥ `cryptoTradeCredits('buy', 창 최저가, units)`, 매도 0 < delta ≤
  `cryptoTradeCredits('sell', 창 최고가, units)`, 정수, 1 ≤ units ≤ `maxUnits`, 잠긴 코인은 원장 `quests` 에 해금 퀘스트(= 그 퀘스트의
  `quest:` 크레딧이 **이 서버에서** 지급됐다), 원장 `cryptoAt` 로 시간당 `CREDIT_CRYPTO_MAX_PER_HOUR`(240), `debits` 에 넣지 않는다(환불 짝 없음).
  `Store.applyCreditsTx` 의 「빈 원장 삭제」 가 `cryptoAt` 도 본다. 지갑 소유는 보지 않는다.
- **파일.** `{v:1, seed, savedAt, coins: {<id>: {x, p, at, m: [[t,o,h,l,c]…], h: […], r: [[at, p]…]}}}` — `x` = 로그 가격 원값(다시 읽어도 경로가
  이어진다), `m` 1분봉 · `h` 1시간봉 · `r` 최근 틱. 읽을 때 표에 없는 코인은 버리고, 없는 코인은 31일을 채우고, 시각이 미래인 봉은 버린다(시계 역행).
- **모델 상수**(반감기 3일 · 점프 하루 0.1회 × 크기 2.5 σ · 띠 ×5 · 저장 60 s · 봉 보관 기간)는 csv 가 아니라 `CryptoMarket.ts` 의 이름 있는 상수다 —
  코인별 밸런스(기준가 · 변동성 · 틱 · 창 · 수수료)만 csv 에 있다. 올리려면 `EconomyTable.crypto` 에 필드를 **추가**하고 생성기가 싣는다.

## 단체 메신저방 (2026-09-14)

- **저장소 `Rooms.ts` (`RoomStore`)** — 방은 서버 권위 · 영속이다. `<dataDir>/rooms.json` 을 `Store.ts` 와 같은 춤으로 쓴다 (디바운스 비동기 tmp + fsync →
  `.bak` 세대, 동기 `flush` / `close`, 깨진 파일은 `rooms.corrupt-<시각>.json` 으로 보존한 뒤 `.bak` 복구). 규칙과 한도만 갖고 친구 · 차단 · 접속은 모른다 —
  모든 연산이 소켓 없이 불리고 **붙은 줄 + `room:state` 를 새로 받을 아이디 목록**(`RoomOp`)을 돌려준다. 멤버 · 초대는 `PlayerCode` 로 저장하고
  아이디 → 방 색인 두 개(멤버 · 초대받음)는 메모리다. `SocialRecord.rooms` 는 쓰지 않는다 — 파일 두 개를 한 트랜잭션으로 묶을 수 없어서 방 파일이 유일한 원본이다.
- **규칙** — 누구나 만든다 (만든 사람 = 방장, 한 사람당 `ROOM_JOINED_MAX`). 초대 · 강퇴 · 이름 변경은 방장만(`not_owner`), 초대는 **친구만**(`not_friend`)이고
  영속(`ROOM_INVITE_TTL_MS` 뒤 `expired`). 가득 찬 방(`ROOM_MEMBER_MAX`)의 새 초대 · 수락은 `room_full`, 방이 너무 많은 사람의 수락은 `room_limit` —
  **거절된 수락은 초대를 남긴다**. 방장이 나가면 가장 먼저 들어온 멤버가 방장(`leave` + `owner` 시스템 줄), 마지막 멤버가 나가면 방과 열린 초대가 함께
  사라진다. 줄은 방마다 `ROOM_LINES_MAX`, 시각은 방 안에서 **엄격히 증가**한다 (같은 ms 두 줄이 `room:history {before}` 에서 빠지지 않게).
- **차단** — 소셜과 같은 방향 규칙: 나를 차단한 사람에게 초대 → `not_found`(오타와 구별 안 됨), 내가 차단한 사람에게 → `invalid`. 이미 같은 방인 사람을
  나중에 차단해도 내보내지 않고 줄도 그대로 퍼진다 — **클라이언트가 숨긴다**. 나를 차단한 사람이 보낸 초대는 내 `room:state` 에서 빠진다.
- **흐름** — 토큰 소켓은 `welcome` 직후 `room:state` 를 받는다 (클라이언트는 `room:get` 을 스스로 보내지 않는다). 변경은 `applyRoomOp` 하나로 퍼진다:
  **먼저** `room:state`(요청자는 즉시, 나머지는 소셜과 같은 `PushCoalescer` 창), **그다음** 시스템 · 대화 줄(`room:line`)을 지금 멤버에게 — 수락한 사람이 방을
  알기 전에 자기 `join` 줄을 받지 않는다. `room:say` 는 보낸 사람에게 `room:ack {nonce, ok, at}` 만(에코 없음), 소켓당 토큰 버킷 `ROOM_SAY_BURST` 12 ·
  초당 `ROOM_SAY_PER_S` 2 를 넘으면 `ack {ok:false, code:'limit'}`. 멤버의 접속 · 로비 이동(`pushPresence`) · 끊김도 같은 방 사람들에게 `room:state` 를
  표시한다 (멤버 행의 프리즌스). 익명 소켓은 `room:error unavailable`.
- **GC** — `collectGarbage` 가 프로필 GC 뒤 `rooms.collectGarbage` 를 부른다: 지워진 아이디는 방에서 조용히 빠지고(방장이었으면 `owner` 줄), 비면 방 삭제,
  기한 지난 초대 삭제 → 영향받은 접속자에게 `room:state`.
- 검증: `selftest.ts` part 14 (46 checks — 저장소 규칙 · 한도 · 쪽 넘기기 · GC · 파일 왕복 / 깨진 파일, 실제 소켓의 흐름 · 차단 · 요율 · 방장 이관 · 릴레이 재시작 · 프로필 GC).
...
- **2026-09-14 (단체 메신저방 · 개인 대화, 에이전트 A — docs/DECISIONS.md 「2026-09-14 — 메신저 · NPC 퀘스트 · 단체방」)** — 새 파일 `Rooms.ts` (위 `단체 메신저방` 절). `RelayServer.ts`: `room:*` 파서
  (입력 상한 `MAX_ROOM_NAME_INPUT` · `MAX_ROOM_TEXT_INPUT`, 방 id 모양, nonce), 방 절(`RoomStore` 생성 · `roomInfo` / `buildRoomSnapshot` · `roomPushes` · `applyRoomOp` ·
  `inviteToRoom` · `markRoomPeers` · `sendRoomLine`), 핸들러 9개, welcome 뒤 `room:state` · 끊김 프리즌스 · `pushPresence` 가 방 사람에게도, `collectGarbage` 뒤 방 GC,
  `close()` 에서 `rooms.close()`, `RelayServer.rooms`, `Client.roomTokens`. `selftest.ts` part 14 (46). 주석 명칭 「귓속말」 → 「개인 대화」 (`Store.ts` · `selftest.ts`).
  계약(`shared/social.ts` · `net.ts` 의 2026-09-14 절)에 더한 것: `RoomInfo.lastCode` · `lastSystem`, `ROOM_READ_STORAGE_KEY` · `ROOM_SAY_BURST` · `ROOM_SAY_PER_S`,
  `isValidRoomId` · `sanitizeRoomName` · `sanitizeRoomText` · `roomSystemTextKo`.

- **2026-09-13 (암호화폐 시세 · 매매 검증, 에이전트 ⑤ — docs/DECISIONS.md 「2026-09-13 — 가구 접근 면 · 발전기 · 암호화폐 채굴」)** — 위 `암호화폐 시세` 절이 전부다. 새 파일 `CryptoMarket.ts`.
  `RelayServer.ts`: 시장 생성(`dataDir` 공유 · `cryptoSeed` 옵션) + `economy.setCryptoQuotes(market)`, `crypto:watch` · `crypto:history` 파서와 핸들러
  (`Client.cryptoWatch` · 토큰 버킷 `CRYPTO_HISTORY_BURST` · `CRYPTO_HISTORY_PER_S`), 틱 방송(`onTick`), `close()` 에서 `market.close()`, `RelayServer.crypto`,
  `/health.crypto`. `Economy.ts`: `cbuy` / `csell` 판정 · `commit` 의 `cryptoAt`, `CryptoQuoteSource` · `CreditEconomyOptions.cryptoQuotes` · `setCryptoQuotes`,
  `loadEconomyTable` 의 `crypto` 모양 검사, `sanitizeLedger` · `pruneLedger` 의 `cryptoAt`. `Store.ts`: 빈 원장 삭제가 `cryptoAt` 도 본다(한 줄).
  `scripts/economy-table.mjs`: 표에 `crypto` 절(단위 · 수수료 · 최대 단위 · 창 ms · 틱 ms · 코인별 기준가 · 변동성 · 해금 퀘스트, 코인 한 줄씩) + 검산(csv 와
  일치 · 해금 퀘스트에 크레딧 보상이 있어 원장에 오를 수 있는가 · 가장 긴 사유 왕복) → `economy.gen.json` 재생성(hash `14fcf166fcf63b`, rover 필드 유지).
  `selftest.ts` part 13 **35 checks** (파일 표 `selftest.ts` 행). 탐사 차량(`rover`) 코드는 건드리지 않았다.

- **2026-09-13 (탐사 차량 요금 검증, 포크 F)** — 새 크레딧 사유 `rover:<출발 정류장>:<도착 정류장>` (계약은 `src/shared/credits.ts`:
  `CreditReasonKind 'rover'` · `CreditReason.to` · `EconomyTable.roverFareMin/Max` · `CreditLedger.roverAt?` · `CREDIT_ROVER_MAX_PER_HOUR` 30).
  경로 길이는 미션 시드마다 달라 릴레이가 요금을 계산할 수 없으므로 **범위만** 본다. `Economy.ts` `check` 의 `rover`: delta < 0 · 정수 ·
  |delta| ∈ [`roverFareMin`, `roverFareMax`] (표에 두 값이 없으면 전부 거절) · 같은 정류장 / 잘못된 id 는 `parseCreditReason` 이 이미 null ·
  한 시간에 30 회(원장 `roverAt` — `contractsAt` 과 같은 창). `commit` 은 `roverAt` 에 시각만 적고 **`debits` 에 넣지 않는다** — `refund:` 가
  요금과 짝지어질 길이 없다(파괴 · 사고에 환불 없음, 사용자 결정). `loadEconomyTable` 이 두 값이 있으면 숫자인지 본다. `sanitizeLedger` ·
  `pruneLedger` 가 `roverAt` 을 정리 · 상한, `Store.applyCreditsTx` 의 「빈 원장 삭제」 가 `roverAt` 도 본다(안 보면 요금 한 번마다 상한이
  지워졌다). `scripts/economy-table.mjs` 가 `ROVER_FARE_MIN` · `ROVER_FARE_MAX`(`data/constants.csv`)를 표에 싣고 범위 · 사유 왕복을
  검산 → `economy.gen.json` 재생성 (hash `06ba68307c27ba`). `selftest.ts` part 12 에 **7 checks**(표의 범위 · 문법 왕복 + 같은 정류장 /
  빠진 칸 / 남는 칸 / 잘못된 id / 64자 초과 거절 · 범위 양 끝 수락 · 범위 밖 / 양수 / 0 / 소수 / 같은 정류장 거절 · 범위 없는 표는 전부 거절 ·
  시간당 30 회 상한과 재개방 + 환불 짝 없음 · `sanitizeLedger` 의 `roverAt`).
